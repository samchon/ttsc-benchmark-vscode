...
const shouldSpawnCli = parsedArgs.help || parsedArgs.version || extensionLookupArgs.some(
  a => !!parsedArgs[a],
) || (extensionInstallArgs.some(
  a => !!parsedArgs[a],
...
async function loadCode(nlsConfiguration: INLSConfiguration) {
	...
	if (process.env["VSCODE_DEV"]) {
		...
		process.env["VSCODE_DEV_INJECT_NODE_MODULE_LOOKUP_PATH"] = process.env["VSCODE_DEV_INJECT_NODE_MODULE_LOOKUP_PATH"] || path.join(
      import.meta.dirname,
      "..",
      "remote",
      "node_modules",
