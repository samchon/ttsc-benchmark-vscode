...
class RemoteExtensionHostAgentServer extends Disposable implements IServerAPI {
	...
	constructor(
      ...
      this._productService,
		...
		if (serverBasePath !== undefined && serverBasePath.charCodeAt(
	...
	public async handleRequest(req: http.IncomingMessage, res: http.ServerResponse): Promise<void> {
		...
		if (this._serverBasePath !== undefined && pathname.startsWith(
      this._serverBasePath,
    )) {
			pathname = pathname.substring(this._serverBasePath.length) || "/";
		}
		// for now accept all paths, with or without server product path
		if (pathname.startsWith(this._serverProductPath) && pathname.charCodeAt(
      this._serverProductPath.length,
    ) === CharCode.Slash) {
      ...
      parsedUrl,
		...
		if (pathname === "/vscode-remote-resource") {
          ...
          !platform.isLinux,
			...
			if (requestOrigin && this._webEndpointOriginChecker.matches(
        requestOrigin,
      )) {
	...
	private _updateWithFreeDebugPort(startParams: IRemoteExtensionHostStartParams): Thenable<IRemoteExtensionHostStartParams> {
		if (typeof startParams.port === "number") {
			return findFreePort(startParams.port, 10 /* try 10 ports */, 5000 /* try up to 5 seconds */).then(
        freePort => {
