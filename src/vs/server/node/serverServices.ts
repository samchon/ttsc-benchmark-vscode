...
export async function setupServerServices(connectionToken: ServerConnectionToken, args: ServerParsedArgs, REMOTE_DATA_FOLDER: string, disposables: DisposableStore) {
    ...
    new LoggerChannel(
      loggerService,
      (ctx: RemoteAgentConnectionContext) => getUriTransformer(
        ctx.remoteAuthority,
      ),
    ),
  );
	...
	setTimeout(
    () => cleanupOlderLogs(environmentService.logsHome.with({ scheme: Schemas.file }).fsPath).then(
      null,
      err => logService.error(err),
    ),
    10000,
  );
	disposables.add(
	...
	if (spawnAgentHost) {
		const agentHostStarter = instantiationService.createInstance(
	...
	const hasUsableSpawnPort = Number.isFinite(
    spawnPortNumber,
  ) && spawnPortNumber > 0;
	...
	if (bridgePort || bridgePath) {
      ...
      `[AgentHostChannel] Registered IPC channel '${AgentHostIpcChannels.RemoteProxy}' (upstream: ${bridgePath ?? `${bridgeHost}:${bridgePort}`})`,
		...
		logService.info(
      `[AgentHostChannel] Not registering: no --agent-host-bridge-port / --agent-host-bridge-path set.`,
    );
	}
...
const _uriTransformerCache: { [remoteAuthority: string]: IURITransformer } = Object.create(
  null,
);

function getUriTransformer(remoteAuthority: string): IURITransformer {
	if (!_uriTransformerCache[remoteAuthority]) {
		_uriTransformerCache[remoteAuthority] = createURITransformer(
      remoteAuthority,
    );
	}
