	...
	async call(_: any, command: string, arg?: any): Promise<any> {
		switch (command) {
          ...
          machineInfo: getMachineInfo(),
				...
				const processesPromise: Promise<ProcessItem | void> = options.includeProcesses ? listProcesses(
          process.pid,
        ) : Promise.resolve();
				...
				return Promise.all([processesPromise, ...workspaceMetadataPromises]).then(
          ([processes, _]) => {
	...
	private async _getEnvironmentData(profile?: string): Promise<IRemoteAgentEnvironmentDTO> {
      ...
      p => p.id === profile,
		...
		if (process.platform === "linux") {
			...
			const minorVersion = glibcVersion ? parseInt(
        glibcVersion.split(".")[1],
      ) : 28;
