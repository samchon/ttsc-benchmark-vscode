...
export async function serveFile(filePath: string, cacheControl: CacheControl, logService: ILogService, req: http.IncomingMessage, res: http.ServerResponse, responseHeaders: Record<string, string>): Promise<void> {
	try {
      ...
      filePath,
		...
		responseHeaders["Content-Type"] = textMimeType[extname(
      filePath,
    )] || getMediaMime(filePath) || "text/plain";
...
export class WebClientServer {
	...
	private async _handleStatic(req: http.IncomingMessage, res: http.ServerResponse, resourcePath: string): Promise<void> {
		...
		const normalizedPathname = decodeURIComponent(
      resourcePath,
	...
	private async _handleWebExtensionResource(req: http.IncomingMessage, res: http.ServerResponse, resourcePath: string): Promise<void> {
      ...
      CancellationToken.None,
		...
		const responseHeaders: Record<string, string | string[]> = Object.create(
      null,
    );
	...
	private async _handleRoot(req: http.IncomingMessage, res: http.ServerResponse, parsedUrl: url.UrlWithParsedQuery): Promise<void> {
      ...
      WEB_EXTENSION_PATH,
		...
		const resolveWorkspaceURI = (defaultLocation?: string) => defaultLocation && URI.file(resolve(defaultLocation)).with(
      { scheme: Schemas.vscodeRemote, authority: remoteAuthority },
    );
      ...
      WORKBENCH_NLS_FALLBACK_URL: `${staticRoute}/out/nls.messages.js`,
        ...
        "github-authentication",
          ...
          (await promises.readFile(FileAccess.asFileUri(`${builtinExtensionsPath}/${extensionPath}/package.json`).fsPath)).toString(),
        );
