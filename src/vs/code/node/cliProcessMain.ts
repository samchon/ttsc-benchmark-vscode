...
class CliMain extends Disposable {
	...
	private async doRun(environmentService: INativeEnvironmentService, fileService: IFileService, userDataProfilesService: IUserDataProfilesService, instantiationService: IInstantiationService): Promise<void> {
		...
		if (this.argv["list-extensions"]) {
			return instantiationService.createInstance(ExtensionManagementCLI, [], new ConsoleLogger(LogLevel.Info, false)).listExtensions(
        ...
        profileLocation,
			...
			return instantiationService.createInstance(ExtensionManagementCLI, [], new ConsoleLogger(LogLevel.Info, false)).installExtensions(
        this.asExtensionIdOrVSIX(this.argv["install-extension"] || []),
        this.asExtensionIdOrVSIX(this.argv["install-builtin-extension"] || []),
        installOptions,
        !!this.argv["force"],
      );
		}
		...
		// Uninstall Extension
			...
			return instantiationService.createInstance(ExtensionManagementCLI, [], new ConsoleLogger(LogLevel.Info, false)).uninstallExtensions(
        ...
        profileLocation,
			...
			return instantiationService.createInstance(ExtensionManagementCLI, [], new ConsoleLogger(LogLevel.Info, false)).updateExtensions(
        profileLocation,
      );
		}
		...
		// Locate Extension
			...
			return instantiationService.createInstance(ExtensionManagementCLI, [], new ConsoleLogger(LogLevel.Info, false)).locateExtension(
        this.argv["locate-extension"],
      );
		}

		// Install MCP server
		else if (this.argv["add-mcp"]) {
			return instantiationService.createInstance(McpManagementCli, new ConsoleLogger(LogLevel.Info, false)).addMcpDefinitions(
        this.argv["add-mcp"],
      );
		}
		...
		// Telemetry
			...
			console.log(
        await buildTelemetryMessage(environmentService.appRoot, environmentService.extensionsPath),
      );
		}
	}
