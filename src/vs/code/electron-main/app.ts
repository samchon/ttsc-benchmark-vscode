...
export class CodeApplication extends Disposable {
	...
	private configureSession(): void {
		...
		const isUrlFromWindow = (requestingUrl?: string | undefined) => requestingUrl?.startsWith(
      `${Schemas.vscodeFileResource}://${VSCODE_AUTHORITY}`,
    );
		const isUrlFromWebview = (requestingUrl: string | undefined) => requestingUrl?.startsWith(
      ...
      "deprecated-sync-clipboard-read",
		...
		const invalidateScreenSourceCache = () => {
			...
			if (!isMacintosh || systemPreferences.getMediaAccessStatus(
        "screen",
      ) === "granted") {
				warmUpScreenSources();
			}
	...
	async startup(): Promise<void> {
		...
		try {
			if (isMacintosh && this.configurationService.getValue(
        "window.nativeTabs",
      ) === true && !systemPreferences.getUserDefault(
      ...
      (accessor.get(IMeteredConnectionService) as MeteredConnectionMainService).setTelemetryService(
        accessor.get(ITelemetryService),
      );
    });
	...
	private async setupProtocolUrlHandlers(accessor: ServicesAccessor, mainProcessElectronServer: ElectronIPCServer): Promise<IInitialProtocolUrls | undefined> {
		const windowsMainService = this.windowsMainService = accessor.get(
      IWindowsMainService,
    );
	...
	private async shouldBlockOpenable(openable: IWindowOpenable, windowsMainService: IWindowsMainService, dialogMainService: IDialogMainService): Promise<boolean> {
        ...
        openableUri.scheme === Schemas.file ? getPathLabel(openableUri, { os: OS, tildify: this.environmentMainService }) : openableUri.toString(true),
	...
	private async handleProtocolUrl(windowsMainService: IWindowsMainService, dialogMainService: IDialogMainService, urlService: IURLService, uri: URI, options?: IOpenURLOptions): Promise<boolean> {
      ...
      options,
		...
		const windowOpenableFromProtocolUrl = this.getWindowOpenableFromProtocolUrl(
      uri,
    );
	...
	private async initServices(machineId: string, sqmId: string, devDeviceId: string, sharedProcessReady: Promise<MessagePortClient>): Promise<IInstantiationService> {
      ...
      this.productService,
		...
		services.set(
      ILaunchMainService,
      new SyncDescriptor(LaunchMainService, undefined, false),
    );

		// Diagnostics
		services.set(
      IDiagnosticsMainService,
      new SyncDescriptor(DiagnosticsMainService, undefined, false),
    );
		services.set(
      IDiagnosticsService,
      ...
      new SyncDescriptor(BrowserViewMainService, undefined, false),
    );
		services.set(
      IBrowserViewGroupMainService,
      new SyncDescriptor(BrowserViewGroupMainService, undefined, false),
    );
      ...
      new SyncDescriptor(KeyboardLayoutMainService),
		...
		services.set(
      INativeHostMainService,
      new SyncDescriptor(NativeHostMainService, undefined, false),
    );
      ...
      new SyncDescriptor(NativeWebContentExtractorService, undefined, false),
    );
      ...
      workspacesManagementMainService,
		...
		services.set(
      IWorkspacesService,
      new SyncDescriptor(WorkspacesMainService, undefined, false),
    );
		services.set(
      ...
      new SyncDescriptor(WorkspacesHistoryMainService, undefined, false),
		...
		services.set(
      IURLService,
      new SyncDescriptor(NativeURLService, undefined, false),
    );
	...
	private async openFirstWindow(accessor: ServicesAccessor, initialProtocolUrls: IInitialProtocolUrls | undefined): Promise<ICodeWindow[]> {
      ...
      IAuxiliaryWindowsMainService,
		...
		const context = isLaunchedFromCli(
      process.env,
    ) ? OpenContext.CLI : OpenContext.DESKTOP;
		...
		const waitMarkerFileURI = args.wait && args.waitMarkerFilePath ? URI.file(
      args.waitMarkerFilePath,
    ) : undefined;
