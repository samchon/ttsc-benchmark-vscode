...
export class ExtUri implements IExtUri {
	...
	dirname(resource: URI): URI {
		...
		if (resource.scheme === Schemas.file) {
			...
			if (resource.authority && dirname.length && dirname.charCodeAt(
        0,
      ) !== CharCode.Slash) {
				console.error(
	...
	relativePath(from: URI, to: URI): string | undefined {
		if (from.scheme !== to.scheme || !isEqualAuthority(
      from.authority,
      to.authority,
    )) {
			return undefined;
	...
	isEqualAuthority(a1: string | undefined, a2: string | undefined) {
		return a1 === a2 || (a1 !== undefined && a2 !== undefined && equalsIgnoreCase(
      a1,
      a2,
    ));
	}
	...
	hasTrailingPathSeparator(resource: URI, sep: string = paths.sep): boolean {
		if (resource.scheme === Schemas.file) {
			...
			return fsp.length > extpath.getRoot(
        fsp,
      ).length && fsp[fsp.length - 1] === sep;
		} else {
			const p = resource.path;
			return (p.length > 1 && p.charCodeAt(
        p.length - 1,
      ) === CharCode.Slash) && !(/^[a-zA-Z]:(\/$|\\$)/.test(
        resource.fsPath,
	...
	addTrailingPathSeparator(resource: URI, sep: string = paths.sep): URI {
		...
		if (resource.scheme === Schemas.file) {
			const fsp = originalFSPath(resource);
			isRootSep = ((fsp !== undefined) && (fsp.length === extpath.getRoot(
        fsp,
      ).length) && (fsp[fsp.length - 1] === sep));
		} else {
			sep = "/";
			const p = resource.path;
			isRootSep = p.length === 1 && p.charCodeAt(
        p.length - 1,
      ) === CharCode.Slash;
		}
