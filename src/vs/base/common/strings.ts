...
export function joinStrings(items: (string | undefined | null | false)[], separator: string): string {
	return items.filter(item => item !== undefined && item !== null && item !== false).join(
    separator,
  );
}
...
export function equals(a: string | undefined, b: string | undefined, ignoreCase?: boolean): boolean {
	return a === b || (!!ignoreCase && a !== undefined && b !== undefined && equalsIgnoreCase(
    a,
    b,
  ));
}
...
export function startsWithIgnoreCase(str: string, candidate: string): boolean {
	...
	return len <= str.length && compareSubstringIgnoreCase(
    str,
    candidate,
    0,
    len,
...
export function endsWithIgnoreCase(str: string, candidate: string): boolean {
	...
	return start >= 0 && compareSubstringIgnoreCase(
    str,
    candidate,
    start,
    len,
...
export class InvisibleCharacters {
	...
	public static containsInvisibleCharacter(str: string): boolean {
		for (let i = 0; i < str.length; i++) {
			const codePoint = str.codePointAt(i);
			if (typeof codePoint === "number" && (InvisibleCharacters.isInvisibleCharacter(
        codePoint,
