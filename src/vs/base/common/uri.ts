...
export function uriToFsPath(uri: URI, keepDriveLetterCasing: boolean): string {
	...
	if (uri.authority && uri.path.length > 1 && uri.scheme === "file") {
		...
		&& (uri.path.charCodeAt(1) >= CharCode.A && uri.path.charCodeAt(
      1,
    ) <= CharCode.Z || uri.path.charCodeAt(
      1,
    ) >= CharCode.a && uri.path.charCodeAt(1) <= CharCode.z)
...
function _asFormatted(uri: URI, skipEncoding: boolean): string {
	...
	if (path) {
		...
		if (path.length >= 3 && path.charCodeAt(
      0,
    ) === CharCode.Slash && path.charCodeAt(2) === CharCode.Colon) {
	...
	if (fragment) {
		res += "#";
		res += !skipEncoding ? encodeURIComponentFast(
      fragment,
      false,
      false,
    ) : fragment;
