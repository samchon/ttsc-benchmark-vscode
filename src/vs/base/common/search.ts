...
export function buildReplaceStringWithCasePreserved(matches: string[] | null, pattern: string): string {
	if (matches && (matches[0] !== "")) {
      ...
      "_",
		...
		} else if (strings.containsUppercaseCharacter(
      matches[0][0],
    ) && pattern.length > 0) {
...
function validateSpecificSpecialCharacter(matches: string[], pattern: string, specialCharacter: string): boolean {
	const doesContainSpecialCharacter = matches[0].indexOf(
    specialCharacter,
  ) !== -1 && pattern.indexOf(specialCharacter) !== -1;
	return doesContainSpecialCharacter && matches[0].split(
    specialCharacter,
  ).length === pattern.split(specialCharacter).length;
}
