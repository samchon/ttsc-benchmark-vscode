...
export class InputBox extends Widget {
	...
	constructor(container: HTMLElement, contextViewProvider: IContextViewProvider | undefined, options: IInputOptions) {
      ...
      () => this.element.classList.remove("synthetic-focus"),
        ...
        vertical: ScrollbarVisibility.Auto,
			...
			this._register(
        this.scrollableElement.onScroll(e => this.input.scrollTop = e.scrollTop),
      );
	...
	public get height(): number {
		return typeof this.cachedHeight === "number" ? this.cachedHeight : dom.getTotalHeight(
      this.element,
    );
	}
	...
	private _showMessage(): void {
		...
		let div: HTMLElement;
		const layout = () => div.style.width = dom.getTotalWidth(
      this.element,
    ) + "px";
	...
	constructor(container: HTMLElement, contextViewProvider: IContextViewProvider | undefined, options: IHistoryInputOptions) {
      ...
      new HistoryNavigator<string>(options.history, 100),
        ...
        NLS_PLACEHOLDER_HISTORY_HINT_SUFFIX_IN_PARENS,
				...
				if (options.showPlaceholderOnFocus && !dom.isActiveElement(
          this.input,
        )) {
