...
export class ListView<T> implements IListView<T> {
	...
	private insertItemInDOM(index: number, row?: IRow): void {
		...
		const toAriaState = (value: CheckBoxAccessibleState) => value === "mixed" ? "mixed" : String(
      !!value,
    );
		...
		if (typeof checked === "boolean" || checked === "mixed") {
			...
			const update = (value: CheckBoxAccessibleState) => item.row!.domNode.setAttribute(
        "aria-checked",
        toAriaState(value),
      );
	...
	@memoize get onMouseClick(): Event<IListMouseEvent<T>> { return Event.map(
    this.disposables.add(new DomEmitter(this.domNode, "click")).event,
    e => this.toMouseEvent(e),
    this.disposables,
  ); }
	@memoize get onMouseDblClick(): Event<IListMouseEvent<T>> { return Event.map(
    this.disposables.add(new DomEmitter(this.domNode, "dblclick")).event,
    e => this.toMouseEvent(e),
    this.disposables,
  ); }
	@memoize get onMouseMiddleClick(): Event<IListMouseEvent<T>> { return Event.filter(
    Event.map(
	...
	@memoize get onMouseUp(): Event<IListMouseEvent<T>> { return Event.map(
    this.disposables.add(new DomEmitter(this.domNode, "mouseup")).event,
    e => this.toMouseEvent(e),
    this.disposables,
  ); }
	@memoize get onMouseDown(): Event<IListMouseEvent<T>> { return Event.map(
    this.disposables.add(new DomEmitter(this.domNode, "mousedown")).event,
    e => this.toMouseEvent(e),
    this.disposables,
  ); }
	@memoize get onMouseOver(): Event<IListMouseEvent<T>> { return Event.map(
    this.disposables.add(new DomEmitter(this.domNode, "mouseover")).event,
    e => this.toMouseEvent(e),
    this.disposables,
  ); }
	@memoize get onMouseMove(): Event<IListMouseEvent<T>> { return Event.map(
    this.disposables.add(new DomEmitter(this.domNode, "mousemove")).event,
    e => this.toMouseEvent(e),
    this.disposables,
  ); }
	@memoize get onMouseOut(): Event<IListMouseEvent<T>> { return Event.map(
    this.disposables.add(new DomEmitter(this.domNode, "mouseout")).event,
    e => this.toMouseEvent(e),
    this.disposables,
  ); }
	@memoize get onContextMenu(): Event<IListMouseEvent<T> | IListGestureEvent<T>> { return Event.any<IListMouseEvent<any> | IListGestureEvent<any>>(
    Event.map(
	...
	@memoize get onTouchStart(): Event<IListTouchEvent<T>> { return Event.map(
    this.disposables.add(new DomEmitter(this.domNode, "touchstart")).event,
    e => this.toTouchEvent(e),
    this.disposables,
  ); }
	@memoize get onTap(): Event<IListGestureEvent<T>> { return Event.map(
	...
	private onDragStart(element: T, uri: string, event: DragEvent): void {
		...
		applyDragImage(event, this.domNode, label, [this.domId]);
	...
	private onDragOver(event: IListDragEvent<T>): boolean {
      ...
      (a, b) => a - b,
		...
		if (equalsDragFeedback(
      this.currentDragFeedback,
      feedback,
	...
	private getItemIndexFromEventTarget(target: EventTarget | null): number | undefined {
		...
		while ((isHTMLElement(element) || isSVGElement(
      element,
    )) && element !== this.rowsContainer && scrollableElement.contains(
      element,
    )) {
		...
		while (true) {
          ...
          renderRange,
				...
				if (typeof anchorElementIndex === "number") {
					...
					const newScrollTop = this.elementTop(
            anchorElementIndex,
          ) - anchorElementTopDelta! + deltaScrollTop;
