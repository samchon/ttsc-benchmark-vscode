...
export function setConstant(key: string, value: boolean) {
	if (CONSTANT_VALUES.get(key) !== undefined) { throw illegalArgument(
			...
			case TokenType.Str: {
				...
				if (this._matchOne(TokenType.RegexOp)) {
					...
					if (!this._config.regexParsingWithErrorRecovery) {
						...
						const flags = closingSlashIndex === regexLexeme.length - 1 ? undefined : this._removeFlagsGY(
              regexLexeme.substring(closingSlashIndex + 1),
                ...
                expr.lexeme,
                        ...
                        i,
												...
												parenBalance++;
											} else if (expr.lexeme.charCodeAt(
                        i,
                      ) === CharCode.CloseParen) {
												parenBalance--;
											}
...
export function validateWhenClauses(whenClauses: string[]): any {
  ...
  }); // we run with no recovery to guide users to use correct regexes
...
export class ContextKeyEqualsExpr implements IContextKeyExpression {
	...
	public static create(key: string, value: any, negated: ContextKeyExpression | null = null): ContextKeyExpression {
		if (typeof value === "boolean") {
			return (value ? ContextKeyDefinedExpr.create(
        key,
        negated,
...
export class ContextKeyInExpr implements IContextKeyExpression {
	...
	public evaluate(context: IContext): boolean {
		...
		if (Array.isArray(source)) {
			...
			if (isWindows && typeof item === "string" && item.startsWith(
        "file:///",
      )) {
...
export const IContextKeyService = createDecorator<IContextKeyService>(
  "contextKeyService",
);
