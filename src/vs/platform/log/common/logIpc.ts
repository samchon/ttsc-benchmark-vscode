...
export class LoggerChannelClient extends AbstractLoggerService implements ILoggerService {
	...
	constructor(private readonly windowId: number | undefined, logLevel: LogLevel, logsHome: URI, loggers: ILoggerResource[], private readonly channel: IChannel) {
      ...
      channel.listen<[URI, boolean]>("onDidChangeVisibility", windowId)(
        ([resource, visibility]) => super.setVisibility(
          URI.revive(resource),
          visibility,
        ),
      ),
...
export class LoggerChannel implements IServerChannel {
	...
	listen(context: any, event: string): Event<any> {
		...
		switch (event) {
			...
			case "onDidChangeVisibility": return Event.map<[URI, boolean], [URI, boolean]>(
        this.loggerService.onDidChangeVisibility,
        e => [uriTransformer.transformOutgoingURI(e[0]), e[1]],
      );
			case "onDidChangeLogLevel": return Event.map<LogLevel | [URI, LogLevel], LogLevel | [URI, LogLevel]>(
	...
	async call(context: any, command: string, arg?: any): Promise<any> {
      ...
      context,
        ...
        arg[0][1],
			...
			case "getRegisteredLoggers": return Promise.resolve(
