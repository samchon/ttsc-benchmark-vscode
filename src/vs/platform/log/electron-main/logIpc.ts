...
export class LoggerChannel extends Disposable implements IServerChannel {
	...
	listen(_: unknown, event: string, windowId?: number): Event<any> {
		switch (event) {
			case "onDidChangeLoggers": return windowId ? this.loggerService.getOnDidChangeLoggersEvent(
        windowId,
      ) : this.loggerService.onDidChangeLoggers;
			case "onDidChangeLogLevel": return windowId ? this.loggerService.getOnDidChangeLogLevelEvent(
        windowId,
      ) : this.loggerService.onDidChangeLogLevel;
			case "onDidChangeVisibility": return windowId ? this.loggerService.getOnDidChangeVisibilityEvent(
        windowId,
	...
	async call(_: unknown, command: string, arg?: any): Promise<any> {
		switch (command) {
			case "createLogger": this.createLogger(
        URI.revive(arg[0]),
        arg[1],
        arg[2],
      ); return;
			...
			case "setLogLevel": return isLogLevel(
        arg[0],
      ) ? this.loggerService.setLogLevel(
        arg[0],
      ) : this.loggerService.setLogLevel(URI.revive(arg[0]), arg[1]);
			case "setVisibility": return this.loggerService.setVisibility(
        URI.revive(arg[0]),
        arg[1],
      );
			case "registerLogger": return this.loggerService.registerLogger(
        { ...arg[0], resource: URI.revive(arg[0].resource) },
        arg[1],
      );
			case "deregisterLogger": return this.loggerService.deregisterLogger(
        URI.revive(arg[0]),
      );
		}
