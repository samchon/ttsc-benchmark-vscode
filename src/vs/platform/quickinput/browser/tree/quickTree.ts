...
export class QuickTree<T extends IQuickTreeItem> extends QuickInput implements IQuickTree<T> {
	...
	constructor(ui: QuickInputUI) {
      ...
      this._onDidAcceptEmitter.event,
		...
		this._register(
      ui.tree.onDidChangeCheckedLeafItems(
        e => this._onDidChangeCheckedLeafItems.fire(e as T[]),
      ),
    );
		this._register(
      ui.tree.onDidChangeCheckboxState(
        e => this._onDidChangeCheckboxState.fire(e.item as T),
      ),
    );
	...
	set ariaLabel(ariaLabel: string | undefined) { this._ariaLabel.set(
    ariaLabel,
    undefined,
  ); }

	get placeholder(): string | undefined { return this._placeholder.get(); }
	set placeholder(placeholder: string | undefined) { this._placeholder.set(
    placeholder,
    undefined,
  ); }

	get matchOnDescription(): boolean { return this._matchOnDescription.get(); }
	set matchOnDescription(matchOnDescription: boolean) { this._matchOnDescription.set(
    matchOnDescription,
    undefined,
  ); }

	get matchOnLabel(): boolean { return this._matchOnLabel.get(); }
	set matchOnLabel(matchOnLabel: boolean) { this._matchOnLabel.set(
    matchOnLabel,
    undefined,
  ); }

	get sortByLabel(): boolean { return this._sortByLabel.get(); }
	set sortByLabel(sortByLabel: boolean) { this._sortByLabel.set(
    sortByLabel,
    undefined,
  ); }

	get activeItems(): readonly T[] { return this._activeItems.get(); }
	set activeItems(activeItems: readonly T[]) { this._activeItems.set(
    activeItems,
    undefined,
  ); }
