...
export const applicationMachineSettings: { properties: IStringDictionary<IConfigurationPropertySchema>; patternProperties: IStringDictionary<IConfigurationPropertySchema> } = {
  properties: {},
  patternProperties: {},
};
	...
	private doRegisterDefaultConfigurations(configurationDefaults: IConfigurationDefaults[], bucket: Set<string>) {
		...
		for (const { overrides, source } of configurationDefaults) {
			for (const key in overrides) {
				...
				const configurationDefaultOverridesForKey = this.configurationDefaultsOverrides.get(
          key,
        )
					?? this.configurationDefaultsOverrides.set(key, { configurationDefaultOverrides: [] }).get(
            key,
          )!;
	...
	private mergeDefaultConfigurationsForConfigurationProperty(propertyKey: string, value: unknown, valuesSource: ConfigurationDefaultSource | undefined, existingDefaultOverride: IConfigurationDefaultOverrideValue | undefined): IConfigurationDefaultOverrideValue | undefined {
		...
		const isObjectSetting = types.isObject(value) &&
			(
				...
				property === undefined && (types.isUndefined(
		...
		if (properties) {
          ...
          extensionInfo: configuration.extensionInfo,
				...
				if (OVERRIDE_PROPERTY_REGEX.test(key)) {
					...
					property.scope = types.isUndefinedOrNull(
            property.scope,
            ...
            `Invalid tag 'onExP' found for property '${key}'. Please use 'experiment' property instead.`,
				...
				const excluded = properties[key].hasOwnProperty(
          "included",
...
export function validateProperty(property: string, schema: IRegisteredConfigurationPropertySchema, extensionId?: string): string | null {
	...
	if (configurationRegistry.getConfigurationProperties()[property] !== undefined && (!extensionId || !EXTENSION_UNIFICATION_EXTENSION_IDS.has(
	...
	if (schema.policy?.name && configurationRegistry.getPolicyConfigurations().get(
    schema.policy?.name,
  ) !== undefined) {
		return nls.localize(
...
export const EXTENSION_UNIFICATION_EXTENSION_IDS: Set<string> = new Set(
  product.defaultChatAgent ? [product.defaultChatAgent.extensionId, product.defaultChatAgent.chatExtensionId].map(id => id.toLowerCase()) : [],
);
