...
export class BrowserViewFrameInspector extends Disposable {
	...
	constructor(
      ...
      connection.onClose(() => {
		...
		this._register({
      dispose: () => frame.ipc.removeListener("vscode:browserView:elementPicked", onPicked),
    });
		...
		this._register({
      dispose: () => frame.ipc.removeListener("vscode:browserView:elementPickStopped", onPickStopped),
    });
...
export async function extractNodeData(connection: ICDPConnection, id: { backendNodeId?: number; objectId?: string }): Promise<IElementData> {
	...
	const { node } = await connection.sendCommand(
    "DOM.describeNode",
    id,
  ) as { node: INode };
	if (!node) {
	...
	const { model } = await connection.sendCommand(
    "DOM.getBoxModel",
    { nodeId },
  ) as { model: IBoxModel };
	if (!model) {
	...
	const { rulesText, referencedVars, authorPropertyNames, userAgentPropertyNames } = formatMatchedStyles(
    matched as IMatchedStyles,
  );
	...
	try {
      ...
      { nodeId },
		...
		if (computedStyleArray) {
            ...
            prop.name,
            `${prop.value} /*UA*/`,
          ); // Mark it as coming from User Agent styles.
				}
