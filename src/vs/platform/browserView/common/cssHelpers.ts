...
export function collapseToShorthands(entries: Map<string, string>): string[] {
	...
	const borderVals = borderSideGroups.map(
    g => g.sides.map(s => entries.get(s)),
  );
	const hasAllBorderProps = borderVals.every(
    group => group.every(v => v !== undefined),
  );
	if (hasAllBorderProps) {
		const allUniform = borderVals.every(
	...
	for (const [name, value] of Array.from(entries.entries()).sort(
