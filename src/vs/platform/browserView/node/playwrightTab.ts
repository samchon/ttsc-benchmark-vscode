...
export class PlaywrightTab {
	...
	private async runAndWaitForCompletion<T>(callback: (token: CancellationToken) => Promise<T>, token = CancellationToken.None): Promise<T> {
		...
		const requestListener = (request: playwright.Request) => requests.push(
      request,
    );
		...
		const requestedNavigation = requests.some(
      request => request.isNavigationRequest(),
		...
		for (const request of requests) {
			if (["document", "stylesheet", "script", "xhr", "fetch"].includes(
        request.resourceType(),
      )) { promises.push(
        request.response().then(r => r?.finished()).catch(() => {}),
      ); }
