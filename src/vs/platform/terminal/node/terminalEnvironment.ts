...
export async function getShellIntegrationInjection(
	...
	const shell = process.platform === "win32" ? path.basename(shellLaunchConfig.executable).toLowerCase() : path.basename(
	...
	switch (shell) {
		...
		case "fish": {
          ...
          ShellIntegrationExecutable.FishLogin,
			...
			} else if (originalArgs === shellIntegrationArgs.get(
        ShellIntegrationExecutable.Fish,
      ) || originalArgs === shellIntegrationArgs.get(
        ShellIntegrationExecutable.FishLogin,
      )) {
...
function addEnvMixinPathPrefix(options: ITerminalProcessOptions, envMixin: IProcessEnvironment, shell: string): void {
	if ((isMacintosh || shell === "fish") && options.environmentVariableCollections) {
      ...
      options.environmentVariableCollections,
		...
		const pathEntry = merged.getVariableMap({ workspaceFolder: options.workspaceFolder }).get(
      "PATH",
    );
...
function arePwshLoginArgs(originalArgs: SingleOrMany<string>): boolean {
	if (isString(originalArgs)) {
		...
		return originalArgs.length === 1 && pwshLoginArgs.includes(
...
function arePwshImpliedArgs(originalArgs: SingleOrMany<string>): boolean {
	if (isString(originalArgs)) {
		...
		return originalArgs.length === 0 || originalArgs?.length === 1 && pwshImpliedArgs.includes(
      originalArgs[0].toLowerCase(),
    );
	}
}
...
function areZshBashFishLoginArgs(originalArgs: SingleOrMany<string>): boolean {
	...
	return isString(originalArgs) && shLoginArgs.includes(originalArgs.toLowerCase())
		|| !isString(
      originalArgs,
    ) && originalArgs.length === 1 && shLoginArgs.includes(
      originalArgs[0].toLowerCase(),
    );
}
