...
export class TerminalProcess extends Disposable implements ITerminalChildProcess {
	...
	get shellType(): TerminalShellType | undefined { return isWindows ? this._windowsShellHelper?.shellType : posixShellTypeMap.get(
    this._currentTitle,
      ...
      this._productService,
        ...
        err,
			...
			if (errorMessage?.includes("Cannot launch conpty")) {
				return {
          message: localize("conptyLaunchFailed", "A native exception occurred during launch (Cannot launch conpty). Winpty has been removed, see {0} for more details. You can also try enabling the `{1}` setting.", "https://code.visualstudio.com/updates/v1_109#_removal-of-winpty-support", "terminal.integrated.windowsUseConptyDll"),
        };
			}
			return {
	...
	private async _validateExecutable(): Promise<undefined | ITerminalLaunchError> {
		...
		const envPaths: string[] | undefined = (slc.env && slc.env.PATH) ? slc.env.PATH.split(
      path.delimiter,
    ) : undefined;
	...
	private async setupPtyProcess(
      ...
      ...options,
      env: sanitizeEnvForLogging(options.env as IProcessEnvironment | undefined),
    };
		this._logService.trace(
      "node-pty.IPty#spawn",
	...
	private async _throttleKillSpawn(): Promise<void> {
      ...
      this._ptyOptions,
		...
		while (Date.now() - TerminalProcess._lastKillOrStart < Constants.KillSpawnThrottleInterval) {
			...
			await timeout(
        Constants.KillSpawnThrottleInterval - (Date.now() - TerminalProcess._lastKillOrStart) + Constants.KillSpawnSpacingDuration,
      );
		}
	...
	private _sendProcessTitle(ptyProcess: IPty): void {
      ...
      value: this._currentTitle,
        ...
        value: GeneralShellType.Julia,
			...
			const shellTypeValue = posixShellTypeMap.get(
        sanitizedTitle,
