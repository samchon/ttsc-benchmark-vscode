...
export class ShellIntegrationAddon extends Disposable implements IShellIntegration, ITerminalAddon {
	...
	private _handleVSCodeSequence(data: string): boolean {
		...
		if (!this._hasUpdatedTelemetry && didHandle) {
			this._telemetryService?.publicLog2<{}, { owner: "meganrogge"; comment: "Indicates shell integration was activated" }>(
		...
		switch (command) {
          ...
          args[0] === this._nonce,
			...
			case VSCodeOscPt.Property: {
				const arg0 = args[0];
				const deserialized = arg0 !== undefined ? deserializeVSCodeOscMessage(
          arg0,
        ) : "";
