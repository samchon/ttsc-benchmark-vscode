...
class ActionItemRenderer<T> implements IListRenderer<IActionListItem<T>, IActionMenuTemplateData> {
	...
	renderElement(element: IActionListItem<T>, _index: number, data: IActionMenuTemplateData): void {
		...
		if (element.submenuActions?.length && !element.hover?.content) {
			data.submenuIndicator.className = "action-list-submenu-indicator has-submenu " + ThemeIcon.asClassName(
...
export class ActionListWidget<T> extends Disposable {
	...
	constructor(
      ...
      toDisposable(() => {
		...
		const hasAnySubmenuActions = reserveSubmenuSpace && items.some(
      ...
      this._list.onDidChangeSelection(e => this.onListSelection(e)),
        ...
        dom.$(".action-list-filter-row"),
			...
			if (this._options?.showFilter) {
				...
				this._filterInput.placeholder = this._options?.filterPlaceholder ?? localize(
	...
	private _applyFilter(skipTextFilter = false): void {
		...
		const filterInputHasFocus = this._filterInput && dom.isActiveElement(
      this._filterInput,
    );
	...
	computeMaxWidth(minWidth: number): number {
		...
		const clamp = (w: number) => Math.min(
      Math.max(w, effectiveMinWidth),
      maxWidthCap,
    );
	...
	private onListSelection(e: IListEvent<IActionListItem<T>>): void {
		...
		if (dom.isMouseEvent(e.browserEvent)) {
			...
			if (dom.isHTMLElement(target) && (target.closest(
	...
	private _recomputeGroupTitles(items: readonly IActionListItem<T>[]): void {
		...
		for (let i = 0; i < items.length; i++) {
			const item = items[i];
			if (item.kind === ActionListItemKind.Action && item.group?.title && !seenTitles.has(
        item.group.title,
      )) {
	...
	private _showSubmenuForElement(element: IActionListItem<T>, anchor: HTMLElement): void {
		...
		if (hoverContent) {
			if (dom.isHTMLElement(hoverContent)) {
				...
				const markdown = typeof hoverContent === "string" ? new MarkdownString(
          hoverContent,
        ) : hoverContent;
	...
	private async onListHover(e: IListMouseEvent<IActionListItem<T>>) {
		...
		if (element && element.item && this.focusCondition(element)) {
			...
			const isHoveringToolbar = dom.isHTMLElement(
...
export class ActionList<T> extends Disposable {
	...
	private computeActionWidgetVerticalChromeHeight(): number {
		const widgetContainer = this.domNode.parentElement?.closest(
      ".action-widget",
    );
		if (!widgetContainer) {
			return 0;
		...
		const style = dom.getWindow(widgetContainer).getComputedStyle(
      widgetContainer,
    );
		const toPixels = (value: string): number => Number.parseFloat(value) || 0;
		return toPixels(style.paddingTop) + toPixels(
      style.paddingBottom,
	...
	private computeHeight(): number {
        ...
        (this._showAbove ? spaceAbove : spaceBelow) - this.computeActionWidgetVerticalChromeHeight(),
			...
			const padding = 10;
			const windowHeight = this._layoutService.getContainer(
        targetWindow,
      ).clientHeight;
