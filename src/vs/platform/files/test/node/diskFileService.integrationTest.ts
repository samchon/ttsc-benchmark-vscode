...
export class TestDiskFileSystemProvider extends DiskFileSystemProvider {
	...
	override async stat(resource: URI): Promise<IStat> {
		...
		if (this.invalidStatSize) {
			...
			(res as any).size = String(
        res.size,
      ) as any; // for https://github.com/microsoft/vscode/issues/72909
...
DiskFileSystemProvider.configureFlushOnWrite(
  false,
); // speed up all unit tests by disabling flush on write
