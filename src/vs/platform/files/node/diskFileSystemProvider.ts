...
export class DiskFileSystemProvider extends AbstractDiskFileSystemProvider implements
	...
	async writeFile(resource: URI, content: Uint8Array, opts: IFileWriteOptions): Promise<void> {
		if (opts?.atomic !== false && opts?.atomic?.postfix && await this.canWriteFileAtomic(
      resource,
    )) {
	...
	async open(resource: URI, opts: IFileOpenOptions, disableWriteLock?: boolean): Promise<number> {
		...
		try {
			...
			if (isFileOpenForWriteOptions(opts) && opts.unlock) {
              ...
              error,
            ); // log errors but do not give up writing
					}
				}
			}
	...
	async close(fd: number): Promise<void> {
		...
		try {
            ...
            `[Disk FileSystemProvider]: close() - resource lock removed from handle-lock map ${fd}`,
					...
					this.mapHandleToLock.delete(
            fd,
          ); // only delete from map if this is still our lock!
				}
	...
	private async doWrite(fd: number, pos: number, data: Uint8Array, offset: number, length: number): Promise<number> {
		...
		try {
			bytesWritten = (await Promises.write(
        fd,
        data,
        offset,
        length,
