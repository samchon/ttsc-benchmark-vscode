...
export abstract class BaseWatcher extends Disposable implements IWatcher {
	...
	async watch(requests: IUniversalWatchRequest[]): Promise<void> {
		...
		try {
			...
			for (const [id] of this.suspendedWatchRequests) {
				if (!this.nonCorrelatedWatchRequests.has(
          id,
	...
	private updateWatchers(delayed: boolean): Promise<void> {
      ...
      ...this.correlatedWatchRequests,
		...
		return this.updateWatchersDelayer.trigger(() => this.doWatch(nonSuspendedRequests), delayed ? this.getUpdateWatchersDelay() : 0).catch(
	...
	isSuspended(request: IUniversalWatchRequest): "polling" | boolean {
		...
		return this.suspendedWatchRequestsWithPolling.has(
      id,
    ) ? "polling" : this.suspendedWatchRequests.has(id);
	}
