...
suite("AgentService (node dispatcher)", () => {
  ...
  suite("aggregation", () => {
    ...
    test("listSessions seeds the server-side changeset state from persisted diffs (so chip subscriptions resolve for unopened sessions)", async () => {
      ...
      const snapshot = svc.stateManager.getSnapshot(
        `${sessionUri.toString()}/changeset/session`,
      );
    ...
    test("createSession strips git-only catalogue entries for non-git working directory", async () => {
      ...
      assert.deepStrictEqual(state!.summary.changesets, [
        {
          label: "This Turn",
          uriTemplate: `${session.toString()}/changeset/turn/{turnId}`,
        },
      ]);
    });
  ...
  suite("restoreSession", () => {
    ...
    test("inner assistant messages from subagent do not create extra turns (fixture)", async () => {
      ...
      assert.strictEqual(
        state!.turns.length,
        1,
        `Expected 1 turn but got ${state!.turns.length}: ${state!.turns.map((t) => `"${t.userMessage.text.substring(0, 40)}"`).join(", ")}`,
      );
      assert.strictEqual(
        state!.turns[0].userMessage.text,
