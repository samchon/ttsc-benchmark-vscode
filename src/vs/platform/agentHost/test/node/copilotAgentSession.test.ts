  ...
  suite("sendSteering", () => {
    test("fires steering_consumed after send resolves and the next tool starts", async () => {
      ...
      assert.strictEqual(
        consumed,
        undefined,
        "should keep steering visible after the SDK send resolves",
      );
      ...
      assert.ok(
        consumed,
        "should fire steering_consumed signal after the next tool starts",
      );
    ...
    test("fires steering_consumed after send resolves and the turn ends without a tool", async () => {
      ...
      assert.ok(
        consumed,
        "should fire steering_consumed signal after the turn ends",
      );
      assert.strictEqual((consumed as { id: string }).id, "steer-1");
    });
    ...
    test("fires steering_consumed on abort after send resolves", async () => {
      ...
      assert.ok(
        consumed,
        "should fire steering_consumed signal when the turn is aborted",
      );
      assert.strictEqual((consumed as { id: string }).id, "steer-1");
    });
