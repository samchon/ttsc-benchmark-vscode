...
class FakeCopilotApiService implements ICopilotApiService {
	...
	countTokens(): Promise<Anthropic.MessageTokensCount> { throw new Error(
    "not used in ClaudeAgent tests",
  ); }
}
...
class FakeQuery implements AsyncGenerator<SDKMessage, void> {
	...
	async setModel(model?: string): Promise<void> { this.recordedModels.push(
    model,
  ); }
	setMaxThinkingTokens(): never { throw new Error(
    "FakeQuery: setMaxThinkingTokens not modeled",
  ); }
	async applyFlagSettings(s: Settings): Promise<void> { this.recordedFlagSettings.push(
    s,
  ); }
	initializationResult(): never { throw new Error(
    "FakeQuery: initializationResult not modeled",
  ); }
	supportedCommands(): never { throw new Error(
    "FakeQuery: supportedCommands not modeled",
  ); }
	supportedModels(): never { throw new Error(
    "FakeQuery: supportedModels not modeled",
  ); }
	supportedAgents(): never { throw new Error(
    "FakeQuery: supportedAgents not modeled",
  ); }
	mcpServerStatus(): never { throw new Error(
    "FakeQuery: mcpServerStatus not modeled",
  ); }
	getContextUsage(): never { throw new Error(
    "FakeQuery: getContextUsage not modeled",
  ); }
	reloadPlugins(): never { throw new Error(
    "FakeQuery: reloadPlugins not modeled",
  ); }
	accountInfo(): never { throw new Error(
    "FakeQuery: accountInfo not modeled",
  ); }
	rewindFiles(): never { throw new Error(
    "FakeQuery: rewindFiles not modeled",
  ); }
	readFile(): never { throw new Error("FakeQuery: readFile not modeled"); }
	seedReadState(): never { throw new Error(
    "FakeQuery: seedReadState not modeled",
  ); }
	reconnectMcpServer(): never { throw new Error(
    "FakeQuery: reconnectMcpServer not modeled",
  ); }
	toggleMcpServer(): never { throw new Error(
    "FakeQuery: toggleMcpServer not modeled",
  ); }
	setMcpServers(): never { throw new Error(
    "FakeQuery: setMcpServers not modeled",
  ); }
	streamInput(): never { throw new Error(
    "FakeQuery: streamInput not modeled",
  ); }
...
class RecordingSessionDataService implements ISessionDataService {
	...
	getSessionDataDir(session: URI) { return this._delegate.getSessionDataDir(
    session,
  ); }
	getSessionDataDirById(sessionId: string) { return this._delegate.getSessionDataDirById(
    sessionId,
  ); }
	...
	deleteSessionData(session: URI) { return this._delegate.deleteSessionData(
    session,
  ); }
	get onWillDeleteSessionData() { return this._delegate.onWillDeleteSessionData; }
	cleanupOrphanedData(knownSessionIds: Set<string>) { return this._delegate.cleanupOrphanedData(
    knownSessionIds,
  ); }
