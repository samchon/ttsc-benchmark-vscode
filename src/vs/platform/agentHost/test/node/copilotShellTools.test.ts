...
class TestAgentHostTerminalManager implements IAgentHostTerminalManager {
	...
	onExit(_uri: string, cb: (exitCode: number) => void): IDisposable { return this._onExit.event(
    cb,
  ); }
	onClaimChanged(_uri: string, cb: (claim: TerminalClaim) => void): IDisposable { return this._onClaimChanged.event(
    cb,
  ); }
	...
	hasTerminal(uri: string): boolean { return this.existingTerminalUris.has(
    uri,
  ); }
	...
	fireCommandFinished(event: ICommandFinishedEvent): void { this._onCommandFinished.fire(
    event,
  ); }
	fireExit(exitCode: number): void { this._onExit.fire(exitCode); }
	fireClaimChanged(claim: TerminalClaim): void { this._onClaimChanged.fire(
    claim,
  ); }
