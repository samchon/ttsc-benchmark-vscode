...
class TestAgentPluginManager implements IAgentPluginManager {
  ...
  async syncCustomizations(
    _clientId: string,
    _customizations: CustomizationRef[],
    _progress?: (status: SessionCustomization) => void,
...
class MockCopilotSession {
  ...
  async getMessages(): Promise<SessionEventPayload<SessionEventType>[]> {
    return [];
  }
  ...
  suite("createSession activeClient eager-claim", () => {
    ...
    test("createSession seeds activeClient tools and syncs customizations", async () => {
      ...
      try {
        ...
        const customizations: CustomizationRef[] = [
    ...
    test("setClientCustomizations publishes parsed agents in SessionCustomizationUpdated", async () => {
      ...
      try {
        ...
        await agent.setClientCustomizations(session, "client-1", [
          { uri: pluginDir.toString(), displayName: "Plugin A" },
        ]);
        ...
        assert.strictEqual(
          updatesWithAgents.length > 0,
          true,
          "expected SessionCustomizationUpdated to carry parsed agents",
        );
