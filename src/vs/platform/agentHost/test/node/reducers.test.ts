...
suite("sessionReducer – SessionCustomizationUpdated.agents", () => {
  ...
  const ref: CustomizationRef = {
    uri: "file:///plugin-a",
    displayName: "Plugin A",
  };
  const agentA: CustomizationAgentRef = {
    uri: "file:///plugin-a/agents/helper.md",
    name: "helper",
  };
  const agentB: CustomizationAgentRef = {
  ...
  test("update: replaces previously-set agents when the action carries a new array", () => {
    const seeded = sessionReducer(
      withCustomization(CustomizationStatus.Loading),
      {
        type: ActionType.SessionCustomizationUpdated,
        customization: ref,
        agents: [agentA],
