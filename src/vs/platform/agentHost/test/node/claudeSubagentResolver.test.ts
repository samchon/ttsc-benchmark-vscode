...
class FakeSdkService implements IClaudeAgentSdkService {
	...
	async startup(_p: { options: Options; initializeTimeoutMs?: number }): Promise<WarmQuery> { throw new Error(
    "not used",
  ); }
	...
	async createSdkMcpServer(): Promise<never> { throw new Error(
    "not implemented in test fake",
  ); }
	async tool(): Promise<never> { throw new Error(
    "not implemented in test fake",
  ); }
}
