...
export function isEditTool(toolName: string, command?: string): boolean {
	...
	if (toolName === CopilotToolName.StrReplaceEditor) {
		return command !== undefined && STR_REPLACE_EDITOR_EDIT_COMMANDS.has(
      command,
    );
	}
	return false;
...
function truncate(text: string, maxLength: number): string {
	return text.length > maxLength ? text.substring(
    0,
    maxLength - 3,
  ) + "..." : text;
}
...
export function getToolDisplayName(toolName: string): string {
	switch (toolName) {
		...
		case CopilotToolName.Create: return localize(
      "toolName.create",
      "Create File",
    );
		case CopilotToolName.View: return localize("toolName.read", "Read");
		case CopilotToolName.Bash:
		case CopilotToolName.PowerShell: return localize(
      "toolName.shell",
      "Run Shell Command",
    );
		case CopilotToolName.ReadBash:
		case CopilotToolName.ReadPowerShell: return localize(
      "toolName.readTerminal",
      "Read Terminal",
    );
		case CopilotToolName.WriteBash: return localize(
      "toolName.writeBash",
      "Write to Bash",
    );
		case CopilotToolName.WritePowerShell: return localize(
      ...
      "Write to PowerShell",
		...
		case CopilotToolName.PowerShellShutdown: return localize(
      "toolName.stopShell",
      "Stop Terminal Session",
    );
		case CopilotToolName.ListBash:
		case CopilotToolName.ListPowerShell: return localize(
      ...
      "List Shell Sessions",
		...
		case CopilotToolName.SearchCodeSubagent: return localize(
      "toolName.searchCode",
      "Search Code",
    );
		case CopilotToolName.ApplyPatch: return localize(
      "toolName.applyPatch",
      "Apply Patch",
    );
		case CopilotToolName.GitApplyPatch: return localize(
      "toolName.patch",
      "Patch",
    );
		case CopilotToolName.CodeqlChecker: return localize(
      "toolName.codeqlChecker",
      "CodeQL Security Scan",
    );
		case CopilotToolName.CodeReview: return localize(
      "toolName.codeReview",
      "Code Review",
    );
		case CopilotToolName.ReplyToComment: return localize(
      "toolName.replyToComment",
      "Reply to Comment",
    );
		case CopilotToolName.Think: return localize("toolName.think", "Thinking");
		case CopilotToolName.ReportIntent: return localize(
      "toolName.reportIntent",
      "Report Intent",
    );
		case CopilotToolName.ReportProgress: return localize(
      "toolName.reportProgress",
      "Progress update",
    );
		case CopilotToolName.WebSearch: return localize(
      "toolName.webSearch",
      "Web Search",
    );
		case CopilotToolName.WebFetch: return localize(
      "toolName.fetchWebContent",
      "Fetch Web Content",
    );
		case CopilotToolName.UpdateTodo: return localize(
      "toolName.updateTodo",
      "Update Todo",
    );
		case CopilotToolName.ShowFile: return localize(
      "toolName.showFile",
      "Show File",
    );
		case CopilotToolName.FetchCopilotCliDocumentation: return localize(
      "toolName.fetchCopilotCliDocumentation",
      "Fetch Documentation",
    );
		case CopilotToolName.ProposeWork: return localize(
      "toolName.proposeWork",
      "Propose Work",
    );
		case CopilotToolName.TaskComplete: return localize(
      "toolName.taskComplete",
      "Task Complete",
    );
		case CopilotToolName.AskUser: return localize(
      "toolName.askUser",
      "Ask User",
    );
		case CopilotToolName.Skill: return localize(
      "toolName.invokeSkill",
      "Invoke Skill",
    );
		case CopilotToolName.Task: return localize(
      "toolName.task",
      "Delegate Task",
    );
		case CopilotToolName.ListAgents: return localize(
      "toolName.listAgents",
      "List Agents",
    );
		case CopilotToolName.ReadAgent: return localize(
      "toolName.readAgent",
      "Read Agent",
    );
		case CopilotToolName.ExitPlanMode: return localize(
      "toolName.exitPlanModeFull",
      "Exit Plan Mode",
    );
		case CopilotToolName.Sql: return localize("toolName.sql", "Execute SQL");
		case CopilotToolName.Lsp: return localize(
      "toolName.lsp",
      "Language Server",
    );
		case CopilotToolName.CreatePullRequest: return localize(
      "toolName.createPullRequest",
      "Create Pull Request",
    );
		case CopilotToolName.GhAdvisoryDatabase: return localize(
      "toolName.ghAdvisoryDatabase",
      "Check Dependencies",
    );
		case CopilotToolName.StoreMemory: return localize(
      "toolName.storeMemory",
      "Store Memory",
    );
		case CopilotToolName.ParallelValidation: return localize(
      "toolName.parallelValidation",
      "Validate Changes",
    );
		case CopilotToolName.WriteAgent: return localize(
      "toolName.writeAgent",
      "Write to Agent",
    );
		case CopilotToolName.McpReload: return localize(
      "toolName.mcpReload",
      "Reload MCP Config",
    );
		case CopilotToolName.McpValidate: return localize(
      "toolName.mcpValidate",
      "Validate MCP Config",
    );
		case CopilotToolName.ToolSearchToolRegex: return localize(
...
export function getInvocationMessage(toolName: string, displayName: string, parameters: Record<string, unknown> | undefined): StringOrMarkdown {
	...
	switch (toolName) {
		...
		case CopilotToolName.Sql: {
			...
			return args?.description || localize(
        "toolInvoke.sql",
        "Executing SQL query",
      );
		}
...
export function getToolInputString(toolName: string, parameters: Record<string, unknown> | undefined, rawArguments: string | undefined): string | undefined {
	...
	if (SHELL_TOOL_NAMES.has(toolName) || WRITE_SHELL_TOOL_NAMES.has(toolName)) {
		...
		if (typeof command === "object" && command !== null && hasKey(command, {
      command: true,
    })) {
...
export function getPermissionDisplay(request: ITypedPermissionRequest, workingDirectory?: URI): {
	...
	switch (request.kind) {
		...
		case "custom-tool": {
			...
			if (args && sdkToolName && isShellTool(
        sdkToolName,
