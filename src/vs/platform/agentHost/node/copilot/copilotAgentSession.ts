...
export class CopilotAgentSession extends Disposable {
  ...
  handleClientToolCallComplete(toolCallId: string, result: ToolCallResult) {
    ...
    const binaryResults = result.content
      ...
      .map((c) => ({
        ...
        type: /^image(\/|$)/.test(c.contentType) ? "image" : "resource",
      }));
  ...
  async initializeSession(): Promise<void> {
    this._wrapper = this._register(
      await this._wrapperFactory({
    ...
    this._register(
      wrapper.onUsage((e) => {
        ...
        if (typeof e.data.copilotUsage?.totalNanoAiu === "number") {
          this._turnCopilotUsageTotalNanoAiu +=
            e.data.copilotUsage.totalNanoAiu;
        ...
        this._logService.trace(
          `[Copilot:${sessionId}] Usage: model=${e.data.model}, in=${e.data.inputTokens ?? "?"}, out=${e.data.outputTokens ?? "?"}, cacheRead=${e.data.cacheReadTokens ?? "?"}, cost=${e.data.cost ?? "?"}, totalNanoAiu=${this._turnCopilotUsageTotalNanoAiu || "?"}`,
        );
