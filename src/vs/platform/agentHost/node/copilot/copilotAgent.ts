...
export class CopilotAgent extends Disposable implements IAgent {
  ...
  async getSessionCustomizations(
    session: URI,
  ): Promise<readonly SessionCustomization[]> {
  ...
  async getSessionMetadata(
    ...
    const workingDirectory =
      ...
      (typeof sessionMetadata?.context?.cwd === "string"
        ? URI.file(sessionMetadata.context.cwd)
        : undefined);
    return {
      session,
  ...
  private async _materializeProvisional(
    ...
    const factory: SessionWrapperFactory = async (callbacks) => {
      const resolvedAgentName = provisional.agent
        ? this._resolveAgentName(snapshot, provisional.agent)
        : undefined;
  ...
  async changeAgent(
    ...
    if (entry) {
      ...
      const resolvedAgentName = agent
        ? this._resolveAgentName(entry.appliedSnapshot, agent)
        : undefined;
  ...
  protected async _resumeSession(
    sessionId: string,
  ): Promise<CopilotAgentSession> {
    this._logService.info(
    ...
    const factory: SessionWrapperFactory = async (callbacks) => {
      ...
      const resolvedAgentName = storedMetadata.agent
        ? this._resolveAgentName(snapshot, storedMetadata.agent)
        : undefined;
      try {
        this._logService.info(
    ...
    try {
      ...
      this._resolved = {
        customization: {
          ...
          ...(plugin
            ? { agents: toCustomizationAgentRefs(plugin.agents) }
            : {}),
        },
        pluginDir,
        plugin,
...
class PluginController extends Disposable {
  ...
  public async getAppliedPlugins(
    ...
    const sessionResolved = entry?.currentResolved();
    const sessionPlugins: IParsedPlugin[] =
      sessionResolved?.plugin && this._isEnabled(sessionResolved.customization)
  ...
  public sync(
    ...
    const publishUpdate = (item: IResolvedCustomization) => {
      ...
      if (
        equals(published.get(customization.customization.uri), customization)
      ) {
        return;
      }
  ...
  private _isEnabled(customization: SessionCustomization): boolean {
    return (
      this._enablement.get(customization.customization.uri) ??
      customization.enabled
    );
  }

  private _applyEnablement(
