...
export class ProtocolServerHandler extends Disposable {
  ...
  private _handleInitialize(
    ...
    disposables.add(
      this._clientFileSystemProvider.registerAuthority(params.clientId, {
        resourceList: (uri) =>
          this._sendReverseRequest(params.clientId, "resourceList", {
            uri: uri.toString(),
          }),
        resourceRead: (uri) =>
          this._sendReverseRequest(params.clientId, "resourceRead", {
            uri: uri.toString(),
          }),
        resourceWrite: (params_) =>
          this._sendReverseRequest(params.clientId, "resourceWrite", params_),
        resourceDelete: (params_) =>
          this._sendReverseRequest(params.clientId, "resourceDelete", params_),
        resourceMove: (params_) =>
          this._sendReverseRequest(params.clientId, "resourceMove", params_),
        resourceRequest: (params_) =>
          this._sendReverseRequest(params.clientId, "resourceRequest", params_),
      }),
    );
