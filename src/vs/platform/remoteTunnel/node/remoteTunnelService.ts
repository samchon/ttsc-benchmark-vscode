...
export class RemoteTunnelService extends Disposable implements IRemoteTunnelService {
	...
	constructor(
      ...
      this._logger.onDidChangeLogLevel(
        l => this._logger.info("Log level changed to " + LogLevelToString(l)),
      ),
    );
	...
	private async updateTunnelProcess(): Promise<void> {
		...
		this.telemetryService.publicLog2<RemoteTunnelEnablementEvent, RemoteTunnelEnablementClassification>(
	...
	private _getTunnelName(): string | undefined {
		let name = this.configurationService.getValue<string>(
      CONFIGURATION_KEY_HOST_NAME,
    ) || hostname();
