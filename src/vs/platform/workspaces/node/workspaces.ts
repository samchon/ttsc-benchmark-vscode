...
export function getWorkspaceIdentifier(configPath: URI): IWorkspaceIdentifier {
	...
	function getWorkspaceId(): string {
		let configPathStr = configPath.scheme === Schemas.file ? originalFSPath(
      configPath,
    ) : configPath.toString();
		if (!isLinux) {
		...
		return createHash("md5").update(configPathStr).digest(
      "hex",
...
export function getSingleFolderWorkspaceIdentifier(folderUri: URI, folderStat?: Stats): ISingleFolderWorkspaceIdentifier | undefined {
	...
	function getFolderId(): string | undefined {
		...
		if (folderUri.scheme !== Schemas.file) {
			return createHash("md5").update(folderUri.toString()).digest(
        "hex",
      ); // CodeQL [SM04514] Using MD5 to convert a file path to a fixed length
		}
		...
		return createHash("md5").update(folderUri.fsPath).update(ctime ? String(ctime) : "").digest(
      "hex",
    ); // CodeQL [SM04514] Using MD5 to convert a file path to a fixed length
	}
