...
export function parseArgs<T>(args: string[], options: OptionDescriptions<T>, errorReporter: ErrorReporter = ignoringReporter): T {
    ...
    (a, i) => a.length > 0 && a[0] !== "-" && options.hasOwnProperty(a) && options[a as T].type === "subcommand",
  );
	...
	if (command && firstPossibleCommand) {
		...
		const reporter = errorReporter.getSubcommandReporter ? errorReporter.getSubcommandReporter(
      firstPossibleCommand,
    ) : undefined;
	...
	for (const optionId in options) {
		...
		if (o.deprecates) {
			for (const deprecatedId of o.deprecates) {
                ...
                deprecatedId,
                o.deprecationMessage || localize("deprecated.useInstead", "Use {0} instead.", optionId),
              );
						}
					}
...
function wrapText(text: string, columns: number): string[] {
	const lines: string[] = [];
	while (text.length) {
		let index = text.length < columns ? text.length : text.lastIndexOf(
      " ",
      columns,
    );
		if (index === 0) {
...
export function buildHelpMessage(productName: string, executableName: string, version: string, options: OptionDescriptions<unknown> | Record<string, Option<"boolean"> | Option<"string"> | Option<"string[]"> | Subcommand<Record<string, unknown>>>, capabilities?: { noPipe?: boolean; noInputFiles?: boolean; isChat?: boolean }): string {
	...
	if (subcommands.length) {
		...
		help.push(
      ...formatUsageTexts(subcommands.map(s => [s.command, s.description]), columns),
    );
		help.push("");
	}
