...
suite("TerminalSandboxEngine", () => {
  ...
  function createHost(
    ...
    const host: ITerminalSandboxEngineHost = {
      ...
      > => Promise.resolve({ bubblewrapInstalled: true, socatInstalled: true }),
  ...
  test("wrapCommand uses MXC executable and writes MXC config on Windows", async () => {
    ...
    ok(
      config.filesystem.readonlyPaths.some(
        (path: string) => normalizeWindowsPathForAssert(path) === "c:/app",
      ),
      "App root should be readable for MXC",
    );
    ok(
      config.filesystem.readonlyPaths.some(
        (path: string) =>
    ...
    ok(
      ...
      "MXC actual temp policy should add host temp path to readonly paths",
    );
    ok(
  ...
  test("wrapCommand applies Windows filesystem setting to MXC config", async () => {
    ...
    ok(
      ...
      "Host temp path from Windows policy should be readonly",
    );
    ok(
      config.filesystem.deniedPaths.some(
        (path: string) =>
