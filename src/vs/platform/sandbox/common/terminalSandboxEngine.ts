...
export class TerminalSandboxEngine extends Disposable {
  ...
  private _affectsSandboxConfiguration(
    e: IConfigurationChangeEvent | undefined,
  ): boolean {
    if (!e) {
  ...
  private async _createSandboxConfig(): Promise<string | undefined> {
    ...
    const sandboxSettings =
      this._os === OperatingSystem.Windows
        ? this._windowsMxcRuntime.createConfig({
  ...
  private async _resolveFileSystemPath(path: string): Promise<string> {
    ...
    try {
      ...
      return realpath?.path && realpath.path !== expandedPath
        ? realpath.path
        : expandedPath;
    } catch {
      return expandedPath;
  ...
  private _getSandboxConfiguredEnabledValue():
    ...
    | boolean {
    return (
      this._getSettingValue<AgentSandboxEnabledValue | boolean>(
