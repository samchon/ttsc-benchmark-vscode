...
export const IAccessibilitySignalService = createDecorator<IAccessibilitySignalService>(
...
export class AccessibilitySignalService extends Disposable implements IAccessibilitySignalService {
	...
	constructor(
      ...
      (signal: AccessibilitySignal) => observableConfigValue<{
			sound: EnabledState;
	...
	public async playSignal(signal: AccessibilitySignal, options: IAccessbilitySignalOptions = {}): Promise<void> {
		...
		if (shouldPlayAnnouncement && this.isAnnouncementEnabled(
      signal,
      options.userGesture,
    ) && announcementMessage) {
	...
	public async playSignals(signals: (AccessibilitySignal | { signal: AccessibilitySignal; source: string })[]): Promise<void> {
		...
		const announcements = signalArray.filter(signal => this.isAnnouncementEnabled(signal)).map(
      s => s.announcementMessage,
    );
      ...
      signalArray.filter(signal => this.isSoundEnabled(signal)).map(
        signal => signal.sound.getSound(),
      ),
    );
		await Promise.all(
