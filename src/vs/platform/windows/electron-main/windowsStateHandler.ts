...
export class WindowsStateHandler extends Disposable {
	...
	private saveWindowsState(): void {
      ...
      lastActiveWindow: this.lastClosedState,
		...
		if (!currentWindowsState.lastActiveWindow) {
			...
			if (!activeWindow || activeWindow.isExtensionDevelopmentHost) {
				activeWindow = this.windowsMainService.getWindows().find(
      ...
      window => window.isExtensionDevelopmentHost && !window.isExtensionTestHost,
		...
		if (extensionHostWindow) {
			currentWindowsState.lastPluginDevelopmentHostWindow = this.toWindowState(
        extensionHostWindow,
      );
	...
	private doGetNewWindowState(configuration: INativeWindowConfiguration): INewWindowState {
		...
		if (!configuration.extensionTestsPath) {
			...
			if (isWorkspaceIdentifier(workspace)) {
				const stateForWorkspace = this.state.openedWindows.filter(openedWindow => openedWindow.workspace && openedWindow.workspace.id === workspace.id).map(
          openedWindow => openedWindow.uiState,
        );
			...
			if (isSingleFolderWorkspaceIdentifier(workspace)) {
				const stateForFolder = this.state.openedWindows.filter(openedWindow => openedWindow.folderUri && extUriBiasedIgnorePathCase.isEqual(openedWindow.folderUri, workspace.uri)).map(
          openedWindow => openedWindow.uiState,
        );
			...
			// Empty windows with backups
				...
				const stateForEmptyWindow = this.state.openedWindows.filter(openedWindow => openedWindow.backupPath === configuration.backupPath).map(
          openedWindow => openedWindow.uiState,
        );
      ...
      undefined,
      isWorkspaceIdentifier(configuration.workspace) || isSingleFolderWorkspaceIdentifier(configuration.workspace),
    );
		state.x = Math.round(
	...
	private ensureNoOverlap(state: IWindowUIState): IWindowUIState {
		...
		const existingWindowBounds = this.windowsMainService.getWindows().map(
