...
export abstract class BaseWindow extends Disposable implements IBaseWindow {
	...
	protected setWin(win: electron.BrowserWindow, options?: BrowserWindowConstructorOptions): void {
		...
		this._register(
      Event.fromNodeEventEmitter(win, "closed")(() => {
		...
		this._register(
      Event.fromNodeEventEmitter(win, "focus")(() => {
		...
		this._register(
      Event.fromNodeEventEmitter(this._win, "enter-full-screen")(
        () => this._onDidEnterFullScreen.fire(),
      ),
    );
		this._register(
      Event.fromNodeEventEmitter(this._win, "leave-full-screen")(
        () => this._onDidLeaveFullScreen.fire(),
      ),
    );
		this._register(
      Event.fromNodeEventEmitter(this._win, "always-on-top-changed", (_, alwaysOnTop) => alwaysOnTop)(
		...
		if (useCustomTitleStyle && useWindowControlsOverlay(
      this.configurationService,
		...
		if (isWindows && this.environmentMainService.enableRDPDisplayTracking) {
			...
			this._register(
        Event.fromNodeEventEmitter(screen, "display-added", (event: Electron.Event, display: Display) => ({ event, display }))(
          (e) => {
	...
	private onDisplayAdded(display: Display): void {
		...
		if (state && this._win && WindowStateValidator.validateWindowStateOnDisplay(
      state,
      display,
    )) {
	...
	protected applyState(state: IWindowState, hasMultipleDisplays = electron.screen.getAllDisplays().length > 0): void {
		...
		const windowSettings = this.configurationService.getValue<IWindowSettings | undefined>(
      "window",
    );
	...
	updateWindowControls(options: { height?: number; backgroundColor?: string; foregroundColor?: string; dimmed?: boolean }): void {
		...
		if (!isMacintosh && useWindowControlsOverlay(this.configurationService)) {
			...
			const effectiveBackgroundColor = this.windowControlsDimmed && backgroundColor ? this.dimColor(
        backgroundColor,
      ) : backgroundColor;
			const effectiveForegroundColor = this.windowControlsDimmed && foregroundColor ? this.dimColor(
        foregroundColor,
      ) : foregroundColor;
...
export class CodeWindow extends BaseWindow implements ICodeWindow {
	...
	private registerListeners(): void {
		...
		this._register(
      Event.fromNodeEventEmitter(this._win, "unresponsive")(
        () => this.onWindowError(WindowError.UNRESPONSIVE),
      ),
    );
		this._register(
      Event.fromNodeEventEmitter(this._win, "responsive")(
        () => this.onWindowError(WindowError.RESPONSIVE),
      ),
    );
		this._register(
      Event.fromNodeEventEmitter(this._win.webContents, "render-process-gone", (event, details) => details)(
        details => this.onWindowError(WindowError.PROCESS_GONE, { ...details }),
      ),
    );
		this._register(
      Event.fromNodeEventEmitter(this._win.webContents, "did-fail-load", (event, exitCode, reason) => ({ exitCode, reason }))(
        ({ exitCode, reason }) => this.onWindowError(WindowError.LOAD, {
          reason,
          exitCode,
        }),
      ),
		...
		this._register(
      Event.fromNodeEventEmitter<electron.Event>(this._win.webContents, "will-prevent-unload")(
	...
	private async onWindowError(type: WindowError, details?: { reason?: string; exitCode?: number }): Promise<void> {
		...
		this.telemetryService.publicLog2<WindowErrorEvent, WindowErrorClassification>(
      "windowerror",
      {
        type,
	...
	private async destroyWindow(reopen: boolean, skipRestoreEditors: boolean): Promise<void> {
		...
		if (skipRestoreEditors && workspace) {
			try {
				const workspaceStorage = this.storageMainService.workspaceStorage(
          workspace,
        );
	...
	private onConfigurationUpdated(e?: IConfigurationChangeEvent): void {
		...
		if (isMacintosh && (!e || e.affectsConfiguration(
	...
	load(configuration: INativeWindowConfiguration, options: ILoadOptions = Object.create(
    null,
  )): void {
		this.logService.trace(
      `window#load: attempt to load window (id: ${this._id})`,
		...
		if (process.env.VSCODE_DEV && process.env.VSCODE_DEV_SERVER_URL) {
			...
			windowUrl = FileAccess.asBrowserUri(`vs/sessions/electron-browser/sessions${this.environmentMainService.isBuilt ? "" : "-dev"}.html`).toString(
        true,
      );
		} else {
			windowUrl = FileAccess.asBrowserUri(`vs/code/electron-browser/workbench/workbench${this.environmentMainService.isBuilt ? "" : "-dev"}.html`).toString(
        true,
      );
		}
	...
	private updateConfiguration(configuration: INativeWindowConfiguration, options: ILoadOptions): void {
		...
		if (currentUserEnv) {
			const shouldPreserveLaunchCliEnvironment = isLaunchedFromCli(
        currentUserEnv,
