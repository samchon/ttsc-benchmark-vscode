...
export class McpGatewayChannel<TContext> extends Disposable implements IServerChannel<TContext> {
    ...
    new Emitter<{ gatewayId: string; servers: readonly IMcpGatewayServerInfo[] }>(),
  );
      ...
      `[McpGateway][Channel] IPC call: ${command} from client ${ctx}`,
		...
		switch (command) {
			case "createGateway": {
				...
				let currentServers = await brokerChannel.call<readonly IMcpGatewayServerDescriptor[]>(
          "listServers",
        );
				const onDidChangeServersListener = brokerChannel.listen<readonly IMcpGatewayServerDescriptor[]>(
          "onDidChangeServers",
        );
