...
export class MenuEntryActionViewItem<T extends IMenuEntryActionViewItemOptions = IMenuEntryActionViewItemOptions> extends ActionViewItem {
	...
	private _updateItemClass(item: ICommandAction): void {
		...
		const icon = this._commandAction.checked && isICommandActionToggleInfo(
      item.toggled,
    ) && item.toggled.icon ? item.toggled.icon : item.icon;
...
class SubmenuEntrySelectActionViewItem extends SelectActionViewItem {
	...
	constructor(
      ...
      action,
      action.actions.map(
        a => (a.id === Separator.ID ? SeparatorSelectOption : { text: a.label, isDisabled: !a.enabled, }),
      ),
      0,
      contextViewService,
