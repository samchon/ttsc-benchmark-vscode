...
export class WorkbenchButtonBar extends ButtonBar {
	...
	update(actions: IAction[], secondary: IAction[]): void {
		...
		const configProvider: IButtonConfigProvider = this._options?.buttonConfigProvider ?? (() => ({
      showLabel: true,
    }));
		...
		for (let i = 0; i < actions.length; i++) {
          ...
          small: this._options?.small,
			...
			const composeLabel = (labelValue: string | IMarkdownString): string | IMarkdownString => {
				if (showIcon && action instanceof MenuItemAction && ThemeIcon.isThemeIcon(
          action.item.icon,
        ) && showLabel) {
			...
			const applyLabel = (labelValue: string | IMarkdownString) => {
				...
				const ariaLabelWithKeybinding = this._keybindingService.appendKeybinding(
          labelStringValue,
          action.id,
        );
