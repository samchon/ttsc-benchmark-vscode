...
export class MenuItemAction implements IAction {
	...
	constructor(
		...
		this.enabled = !item.precondition || contextKeyService.contextMatchesRules(
      item.precondition,
    );
		...
		if (item.toggled) {
			const toggled = ((item.toggled as { condition: ContextKeyExpression }).condition ? item.toggled : {
