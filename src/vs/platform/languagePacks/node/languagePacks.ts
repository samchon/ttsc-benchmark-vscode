...
export class NativeLanguagePackService extends LanguagePackBaseService {
	...
	private async postUninstallExtension(extension: ILocalExtension): Promise<void> {
      ...
      language => languagePacks[language] && languagePacks[language].extensions.some(e => areSameExtensions(e.extensionIdentifier, extension.identifier)),
    )) {
			this.logService.info(
...
class LanguagePacksCache extends Disposable {
	...
	private createLanguagePacksFromExtension(languagePacks: { [language: string]: ILanguagePack }, extension: ILocalExtension): void {
		...
		for (const localizationContribution of localizations) {
			if (extension.location.scheme === Schemas.file && isValidLocalization(
        localizationContribution,
          ...
          e => areSameExtensions(e.extensionIdentifier, extensionIdentifier),
        )[0];
	...
	private updateHash(languagePack: ILanguagePack): void {
		if (languagePack) {
          ...
          extension.version,
        ); // CodeQL [SM01510] The extension UUID is not sensitive info and is not manually created by a user
			}
