...
export class SharedWebContentExtractorService implements ISharedWebContentExtractorService {
	...
	async readImage(uri: URI, token: CancellationToken): Promise<VSBuffer | undefined> {
		...
		try {
			...
			if (!response.ok || !contentType?.startsWith(
        "image/",
        ...
        await (response as unknown as { bytes: () => Promise<Uint8Array<ArrayBuffer>> } /* workaround https://github.com/microsoft/TypeScript/issues/61826 */).bytes(),
      );
			return content;
		} catch (err) {
			console.error(err);
