...
export class NativeHostMainService extends Disposable implements INativeHostMainService {
        ...
        Event.map(
          Event.filter(
            Event.fromNodeEventEmitter(
              app,
              "browser-window-blur",
              (event, window: BrowserWindow) => this.auxiliaryWindowsMainService.getWindowByWebContents(
                window.webContents,
              ),
            ),
            window => !!window,
	...
	private async doOpenWindow(windowId: number | undefined, toOpen: IWindowOpenable[], options: IOpenWindowOptions = Object.create(
    null,
  )): Promise<void> {
		if (toOpen.length > 0) {
	...
	async updateWindowAccentColor(windowId: number | undefined, color: "default" | "off" | string, inactiveColor: string | undefined): Promise<void> {
        ...
        window.win.isFocused() ? activeWindowAccentColor : inactiveWindowAccentColor,
      );
		}
	}
	...
	async pickFileFolderAndOpen(windowId: number | undefined, options: INativeOpenDialogOptions): Promise<void> {
        ...
        await Promise.all(paths.map(async path => (await SymlinkSupport.existsDirectory(path)) ? { folderUri: URI.file(path) } : { fileUri: URI.file(path) })),
        options,
        windowId,
      );
		}
	...
	private async openExternalBrowser(windowId: number | undefined, url: string, defaultApplication?: string): Promise<void> {
		const configuredBrowser = defaultApplication ?? this.configurationService.getValue<string>(
	...
	async reload(windowId: number | undefined, options?: { disableExtensions?: boolean }): Promise<void> {
        ...
        window,
        options?.disableExtensions !== undefined ? { _: [], "disable-extensions": options.disableExtensions } : undefined,
      );
		}
	}
	...
	async openDevTools(windowId: number | undefined, options?: Partial<OpenDevToolsOptions> & INativeHostOptions): Promise<void> {
      ...
      options?.mode ? { mode: options.mode, activate: options.activate } : undefined,
    );
	}
