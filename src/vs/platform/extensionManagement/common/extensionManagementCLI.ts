...
const notFound = (id: string) => localize(
  "notFound",
...
export class ExtensionManagementCLI {
	...
	constructor(
		...
		this.extensionsForceVersionByQuality = this.extensionsForceVersionByQuality.map(
      e => e.toLowerCase(),
    );
	}
	...
	public async installExtensions(extensions: (string | URI)[], builtinExtensions: (string | URI)[], installOptions: InstallOptions, force: boolean): Promise<void> {
		...
		try {
			if (extensions.length) {
				this.logger.info(
          this.location ? localize("installingExtensionsOnLocation", "Installing extensions on {0}...", this.location) : localize("installingExtensions", "Installing extensions..."),
        );
			}
			...
			const addInstallExtensionInfo = (id: string, version: string | undefined, isBuiltin: boolean) => {
				if (this.extensionsForceVersionByQuality?.some(
          e => e === id.toLowerCase(),
        )) {
          ...
          installOptions: { ...installOptions, isBuiltin, installPreReleaseVersion: version === "prerelease" || installOptions.installPreReleaseVersion },
        });
			};
			for (const extension of extensions) {
	...
	private async validateVSIX(manifest: IExtensionManifest, force: boolean, profileLocation: URI | undefined, installedExtensions: ILocalExtension[]): Promise<boolean> {
      ...
      id: getGalleryExtensionId(manifest.publisher, manifest.name),
		...
		const existingExtension = installedExtensions.find(
      local => areSameExtensions(extensionIdentifier, local.identifier),
    );

		if (existingExtension) {
			const builtinAutoUpdateMessage = this.validateBuiltinExtensionEnabledWithAutoUpdates(
        existingExtension,
      );
			if (builtinAutoUpdateMessage) {
				this.logger.info(builtinAutoUpdateMessage);
	...
	private validateBuiltinExtensionEnabledWithAutoUpdates(extension: ILocalExtension): string | undefined {
		if (extension.isBuiltin && this.productService.builtInExtensionsEnabledWithAutoUpdates.some(
