...
export class ExtensionManagementChannelClient extends CommontExtensionManagementService implements IExtensionManagementService {
	...
	constructor(
      ...
      this.channel.listen<InstallExtensionEvent>("onInstallExtension")(
		...
		this._register(
      this.channel.listen<readonly InstallExtensionResult[]>("onDidInstallExtensions")(
      ...
      this.channel.listen<UninstallExtensionEvent>("onUninstallExtension")(
		...
		this._register(
      this.channel.listen<DidUninstallExtensionEvent>("onDidUninstallExtension")(
		...
		this._register(
      this.channel.listen<DidUpdateExtensionMetadata>("onDidUpdateExtensionMetadata")(
	...
	getTargetPlatform(): Promise<TargetPlatform> {
		if (!this._targetPlatformPromise) {
			this._targetPlatformPromise = this.channel.call<TargetPlatform>(
        "getTargetPlatform",
      );
		}
	...
	zip(extension: ILocalExtension): Promise<URI> {
      ...
      this.channel.call<UriComponents>("zip", [extension]).then(
        result => URI.revive(result),
      ),
    );
	}

	install(vsix: URI, options?: InstallOptions): Promise<ILocalExtension> {
		return Promise.resolve(this.channel.call<ILocalExtension>("install", [vsix, options])).then(
...
export class ExtensionTipsChannel implements IServerChannel {
	...
	call(context: any, command: string, args?: any): Promise<any> {
		switch (command) {
			case "getConfigBasedTips": return this.service.getConfigBasedTips(
        URI.revive(args[0]),
      );
