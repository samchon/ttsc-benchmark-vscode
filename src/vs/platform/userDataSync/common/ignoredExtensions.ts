...
export class IgnoredExtensionsManagementService implements IIgnoredExtensionsManagementService {
	...
	updateIgnoredExtensions(ignoredExtensionId: string, ignore: boolean): Promise<void> {
		...
		let currentValue = [...this.configurationService.getValue<string[]>("settingsSync.ignoredExtensions")].map(
      id => id.toLowerCase(),
    );
	...
	getIgnoredExtensions(installed: ILocalExtension[]): string[] {
		const defaultIgnoredExtensions = installed.filter(i => i.isMachineScoped).map(
      i => i.identifier.id.toLowerCase(),
    );
	...
	private getConfiguredIgnoredExtensions(): ReadonlyArray<string> {
		return (this.configurationService.getValue<string[]>("settingsSync.ignoredExtensions") || []).map(
      id => id.toLowerCase(),
    );
	}
}
