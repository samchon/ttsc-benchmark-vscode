...
export class UserDataSyncService extends Disposable implements IUserDataSyncService {
	...
	async createManualSyncTask(): Promise<IUserDataManualSyncTask> {
		...
		try {
			latestUserDataOrManifest = await this.userDataSyncStoreService.getLatestData(
        syncHeaders,
      );
		} catch (error) {
			const userDataSyncError = UserDataSyncError.toUserDataSyncError(error);
	...
	private async syncRemoteProfiles(remoteProfiles: ISyncUserDataProfile[], manifest: IUserDataManifest | IUserDataSyncLatestData | null, preview: boolean, executionId: string, token: CancellationToken): Promise<void> {
        ...
        syncProfile,
			...
			this._syncErrors.push(
        ...await this.syncProfile(profileSynchronizer, manifest, preview, executionId, token),
      );
		}
	...
	private async applyManualSync(manifestOrLatestData: IUserDataManifest | IUserDataSyncLatestData | null, executionId: string, token: CancellationToken): Promise<void> {
        ...
        s => s.resource === SyncResource.Profiles,
			...
			const remoteProfiles = (await (userDataProfileManifestSynchronizer as UserDataProfilesManifestSynchroniser).getRemoteSyncedProfiles(
	...
	private async cleanUpStaleStorageData(): Promise<void> {
      ...
      StorageTarget.MACHINE,
        ...
        s => s.resource === SyncResource.Profiles,
			...
			const lastSyncedCollections = lastSyncedProfiles?.map(
        p => p.collection,
      ) ?? [];
	...
	private updateConflicts(): void {
      ...
      this._conflicts,
      conflicts,
      (a, b) => a.profile.id === b.profile.id && a.syncResource === b.syncResource && equals(a.conflicts, b.conflicts, (a, b) => isEqual(a.previewResource, b.previewResource)),
    )) {
...
class ProfileSynchronizer extends Disposable {
	...
	get enabled(): IUserDataSynchroniser[] { return this._enabled.sort((a, b) => a[1] - b[1]).map(
    ([synchronizer]) => synchronizer,
  ); }

	get disabled(): SyncResource[] { return ALL_SYNC_RESOURCES.filter(
	...
	constructor(
      ...
      userDataSyncEnablementService.onDidChangeResourceEnablement(
		...
		this._register(
      toDisposable(
        () => this._enabled.splice(0, this._enabled.length).forEach(
          ([, , disposable]) => disposable.dispose(),
        ),
      ),
    );
	...
	createSynchronizer(syncResource: Exclude<SyncResource, SyncResource.WorkspaceState>): IUserDataSynchroniser & IDisposable {
		switch (syncResource) {
			case SyncResource.Settings: return this.instantiationService.createInstance(
        SettingsSynchroniser,
        this.profile,
        this.collection,
      );
			case SyncResource.Keybindings: return this.instantiationService.createInstance(
        KeybindingsSynchroniser,
        this.profile,
        this.collection,
      );
			case SyncResource.Snippets: return this.instantiationService.createInstance(
        SnippetsSynchroniser,
        this.profile,
        this.collection,
      );
			case SyncResource.Prompts: return this.instantiationService.createInstance(
        PromptsSynchronizer,
        this.profile,
        this.collection,
      );
			case SyncResource.Tasks: return this.instantiationService.createInstance(
        TasksSynchroniser,
        this.profile,
        this.collection,
      );
			case SyncResource.Mcp: return this.instantiationService.createInstance(
        McpSynchroniser,
        this.profile,
        this.collection,
      );
			case SyncResource.GlobalState: return this.instantiationService.createInstance(
        GlobalStateSynchroniser,
        this.profile,
        this.collection,
      );
			case SyncResource.Extensions: return this.instantiationService.createInstance(
        ExtensionsSynchroniser,
        this.profile,
        this.collection,
      );
			case SyncResource.Profiles: return this.instantiationService.createInstance(
        UserDataProfilesManifestSynchroniser,
        this.profile,
        this.collection,
      );
	...
	async sync(manifestOrLatestData: IUserDataManifest | IUserDataSyncLatestData | null, preview: boolean, executionId: string, token: CancellationToken): Promise<[SyncResource, UserDataSyncError][]> {
		...
		try {
			...
			const syncHeaders = createSyncHeaders(executionId);
			const userDataSyncConfiguration = preview ? await this.getUserDataSyncConfiguration(
        manifestOrLatestData,
          ...
          synchroniser.resource,
				...
				try {
					const refOrUserData = getRefOrUserData(
            manifestOrLatestData,
            this.collection,
            synchroniser.resource,
          ) ?? null;
	...
	private updateConflicts(): void {
      ...
      this._conflicts,
      conflicts,
      (a, b) => a.syncResource === b.syncResource && equals(a.conflicts, b.conflicts, (a, b) => isEqual(a.previewResource, b.previewResource)),
    )) {
			this._conflicts = conflicts;
			this._onDidChangeConflicts.fire(conflicts);
		}
