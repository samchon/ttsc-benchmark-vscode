...
export class UserDataSyncServiceChannel implements IServerChannel {
	...
	private async _call(context: any, command: string, args?: any): Promise<any> {
		switch (command) {
			...
			// sync
			case "_getInitialData": return Promise.resolve([
        this.service.status,
        ...
        this.service.lastSyncTime,
			...
			case "resolveContent": return this.service.resolveContent(
        URI.revive(args[0]),
      );
			case "accept": return this.service.accept(
        reviewSyncResource(args[0], this.userDataProfilesService),
        URI.revive(args[1]),
        args[2],
        args[3],
      );
			case "replace": return this.service.replace(
        reviewSyncResourceHandle(args[0]),
      );
			case "cleanUpRemoteData": return this.service.cleanUpRemoteData();
			case "getRemoteActivityData": return this.service.saveRemoteActivityData(
        URI.revive(args[0]),
      );
			case "extractActivityData": return this.service.extractActivityData(
        URI.revive(args[0]),
        URI.revive(args[1]),
      );
		...
		if (command.startsWith("manualSync/")) {
			...
			switch (manualSyncTaskCommand) {
				...
				case "apply": return manualSyncTask.apply().then(
          () => this.manualSyncTasks.delete(this.createKey(manualSyncTask.id)),
        );
				case "stop": return manualSyncTask.stop().finally(
          () => this.manualSyncTasks.delete(this.createKey(manualSyncTask.id)),
        );
			}
		}
...
export class UserDataSyncServiceChannelClient extends Disposable implements IUserDataSyncService {
	...
	get onDidChangeLocal(): Event<SyncResource> { return this.channel.listen<SyncResource>(
    "onDidChangeLocal",
  ); }
	...
	get onDidResetLocal(): Event<void> { return this.channel.listen<void>(
    "onDidResetLocal",
  ); }
	get onDidResetRemote(): Event<void> { return this.channel.listen<void>(
    "onDidResetRemote",
  ); }
