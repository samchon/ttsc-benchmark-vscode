...
export class UserDataSyncClient extends Disposable {
	...
	read(resource: SyncResource, collection?: string): Promise<IUserData> {
		return this.instantiationService.get(IUserDataSyncStoreService).readResource(
      resource,
      null,
      collection,
    );
	...
	async _getResourceManifest(): Promise<IUserDataResourceManifest | null> {
		const manifest = await this.instantiationService.get(IUserDataSyncStoreService).manifest(
      null,
    );
	...
	getSynchronizer(source: SyncResource): IUserDataSynchroniser {
		return (this.instantiationService.get(IUserDataSyncService) as UserDataSyncService).getOrCreateActiveProfileSynchronizer(this.instantiationService.get(IUserDataProfilesService).defaultProfile, undefined).enabled.find(
      s => s.resource === source,
    )!;
	}
...
export class UserDataSyncTestServer implements IRequestService {
	...
	private async doRequest(options: IRequestOptions): Promise<IRequestContext> {
		...
		const relativePath = options.url!.indexOf(
      versionUrl,
    ) === 0 ? options.url!.substring(versionUrl.length) : undefined;
	...
	private async getResourceData(collection: string | undefined, resource: string, ref?: string, headers: IHeaders = {}): Promise<IRequestContext> {
		const collectionData = collection ? this.collections.get(
      collection,
    ) : this.data;
		if (!collectionData) {
			return this.toResponse(501);
