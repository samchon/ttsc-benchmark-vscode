...
export const IFileDialogService = createDecorator<IFileDialogService>(
  "fileDialogService",
);
...
export function getFileNamesMessage(fileNamesOrResources: readonly (string | URI)[]): string {
	...
	message.push(
    ...fileNamesOrResources.slice(0, MAX_CONFIRM_FILES).map(fileNameOrResource => typeof fileNameOrResource === "string" ? fileNameOrResource : basename(fileNameOrResource)),
  );
