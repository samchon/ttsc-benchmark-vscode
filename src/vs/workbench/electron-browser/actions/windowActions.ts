      ...
      includeAuxiliaryWindows: true,
		...
		for (const window of mainWindows) {
          ...
          label: auxiliaryWindows ? localize("windowGroup", "window group") : undefined,
			...
			const resource = window.filename ? URI.file(
        window.filename,
      ) : isSingleFolderWorkspaceIdentifier(
        window.workspace,
      ) ? window.workspace.uri : isWorkspaceIdentifier(
        window.workspace,
      ) ? window.workspace.configPath : undefined;
			const fileKind = window.filename ? FileKind.FILE : isSingleFolderWorkspaceIdentifier(
        window.workspace,
      ) ? FileKind.FOLDER : isWorkspaceIdentifier(
        window.workspace,
      ) ? FileKind.ROOT_FOLDER : FileKind.FILE;
