...
export class GettingStartedPage extends EditorPane {
	...
	private hideCategory(categoryId: string) {
      ...
      category => category.id === categoryId,
		...
		if (!selectedCategory) { throw Error(
	...
	private async selectStep(id: string | undefined, delayFocus = true, preserveFocus?: boolean) {
        ...
        `[data-step-id="${id}"]`,
			...
			if (!stepElement) {
				...
				// eslint-disable-next-line no-restricted-syntax
				stepElement = this.container.querySelector<HTMLDivElement>(
          `[data-step-id]`,
        );
				if (!stepElement) {
	...
	private async buildCategoriesSlide(preserveFocus?: boolean) {
      ...
      "workbench.welcomePage.experimentalOnboarding",
        ...
        StorageScope.APPLICATION,
			...
			const daysSinceFirstSession = ((+new Date()) - (+new Date(
        firstSessionDateString,
      ))) / 1000 / 60 / 60 / 24;
	...
	private buildRecentlyOpenedList(): GettingStartedIndexList<RecentEntry> {
		const renderRecent = (recent: RecentEntry) => {
			...
			if (isRecentFolder(recent)) {
				...
				fullPath = recent.label || this.labelService.getWorkspaceLabel(
          recent.folderUri,
	...
	private buildGettingStartedWalkthroughsList(): GettingStartedIndexList<IResolvedWalkthrough> {
      ...
      this.registerDispatchListeners();
      allWalkthroughsHiddenContext.bindTo(this.contextService).set(
        gettingStartedList.itemCount === 0,
      );
      this.updateCategoryProgress();
    });
	...
	private focusSideEditorGroup() {
		...
		if (!fullSize || fullSize.width <= 700 || this.container.classList.contains(
      "width-constrained",
		...
		if (isCommand) {
          ...
          step => step.id === this.editorInput?.selectedStep,
				...
				if (selectedStepIndex !== undefined &&
					selectedStepIndex > -1 &&
					this.currentWalkthrough?.steps.slice(selectedStepIndex + 1).some(
            step => !step.done,
          )) {
				...
				for (const node of linkedText.nodes) {
                  ...
                  renderFormattedText(
                    ...
                    $("span"),
						...
						const nodeWithTitle: ILink = matchesScheme(
              node.href,
              Schemas.http,
	...
	private buildCategorySlide(categoryID: string, selectedStep?: string, preserveFocus?: boolean) {
      ...
      new DomScrollableElement(stepsContainer, { className: "steps-container" }),
    );
		const stepListComponent = this.detailsScrollbar.getDomNode();

		const categoryFooter = $(".getting-started-footer");
		if (this.editorInput.showTelemetryNotice && getTelemetryLevel(
      this.configurationService,
    ) !== TelemetryLevel.NONE && this.productService.enableTelemetry) {
	...
	private setSlide(toEnable: "details" | "categories", firstLaunch: boolean = false) {
      ...
      this.container.querySelector(".gettingStarted"),
        ...
        ".prev-button.button-link",
			...
			// eslint-disable-next-line no-restricted-syntax
			this.container.querySelector(".gettingStartedSlideDetails")!.querySelectorAll("button").forEach(
        button => button.disabled = true,
      );
			// eslint-disable-next-line no-restricted-syntax
			this.container.querySelector(".gettingStartedSlideCategories")!.querySelectorAll("button").forEach(
        button => button.disabled = false,
      );
			// eslint-disable-next-line no-restricted-syntax
			this.container.querySelector(".gettingStartedSlideCategories")!.querySelectorAll("input").forEach(
        button => button.disabled = false,
      );
		} else {
