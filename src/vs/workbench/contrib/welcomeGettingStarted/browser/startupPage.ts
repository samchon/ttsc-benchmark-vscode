...
export class StartupPageRunnerContribution extends Disposable implements IWorkbenchContribution {
      ...
      this.environmentService,
		...
		if (enabled && this.lifecycleService.startupKind !== StartupKind.ReloadedWindow) {
			...
			if (!this.editorService.activeEditor || this.layoutService.openedDefaultEditors) {
				const startupEditorSetting = this.configurationService.inspect<string>(
          configurationKey,
        );
	...
	private tryOpenWalkthroughForFolder(): boolean {
      ...
      StorageScope.PROFILE,
		...
		if (!toRestore) {
			...
			const restoreData: RestoreWalkthroughsConfigurationValue = JSON.parse(
        toRestore,
      );
	...
	private async openReadme() {
		...
		if (!this.editorService.activeEditor) {
			if (readmes.length) {
				const isMarkDown = (readme: URI) => readme.path.toLowerCase().endsWith(
          ".md",
        );
				await Promise.all([
          this.commandService.executeCommand("markdown.showPreview", null, readmes.filter(isMarkDown), { locked: true }).catch(
            error => {
          ...
          this.editorService.openEditors(
            readmes.filter(readme => !isMarkDown(readme)).map(readme => ({
              resource: readme,
