...
export class WalkthroughsService extends Disposable implements IWalkthroughsService {
	...
	private initCompletionEventListeners() {
      ...
      this.configurationService.onDidChangeConfiguration(e => {
		...
		if (this.userDataSyncEnablementService.isEnabled()) { this.progressByEvent(
      "onEvent:sync-enabled",
    ); }
	...
	private async registerExtensionWalkthroughContributions(extension: IExtensionDescription) {
		const convertExtensionPathToFileURI = (path: string) => path.startsWith(
      "https://",
    )
			? URI.parse(path, true)
      ...
      "workbench.startupEditor",
		...
		if (hadLastFoucs && sectionToOpen && this.configurationService.getValue<string>(
      "workbench.welcomePage.walkthroughs.openOnInstall",
			...
			this.telemetryService.publicLog2<GettingStartedAutoOpenEvent, GettingStartedAutoOpenClassification>(
	...
	getWalkthrough(id: string): IResolvedWalkthrough {
		...
		const walkthrough = this.gettingStartedContributions.get(id);
		if (!walkthrough) { throw Error(
      "Trying to get unknown walkthrough: " + id,
    ); }
	...
	private resolveWalkthrough(category: IWalkthrough): IResolvedWalkthrough {
      ...
      step => this.getStepProgress(step),
		...
		if (!rawCategory) { throw Error(
      "Could not find walkthrough with id " + category.id,
    ); }

		const currentStepIds: string[] = rawCategory.steps.map(s => s.id);

		const hasNewSteps = lastStepIDs && (currentStepIds.length !== lastStepIDs.length || currentStepIds.some(
      (id, index) => id !== lastStepIDs[index],
    ));
	...
	_registerWalkthrough(walkthroughDescriptor: IWalkthrough): void {
      ...
      step.when.keys().forEach(
        key => this.categoryVisibilityContextKeys.add(key),
      );
      this.registerDoneListeners(step);
    });

		walkthroughDescriptor.when.keys().forEach(
      key => this.categoryVisibilityContextKeys.add(key),
    );
	}
...
export const parseDescription = (desc: string): LinkedText[] => desc.split("\n").filter(x => x).map(
  text => parseLinkedText(text),
);

export const convertInternalMediaPathToFileURI = (path: string) => path.startsWith(
  "https://",
)
	? URI.parse(path, true)
...
const convertInternalMediaPathToBrowserURI = (path: string) => path.startsWith(
  "https://",
)
	? URI.parse(path, true)
