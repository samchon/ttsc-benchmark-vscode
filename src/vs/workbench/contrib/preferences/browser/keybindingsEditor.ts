...
export class KeybindingsEditor extends EditorPane<IKeybindingsEditorMemento> implements IKeybindingsEditorPane {
	...
	private _onDefineWhenExpression: Emitter<IKeybindingItemEntry> = this._register(
	...
	private layoutKeybindingsTable(): void {
		...
		const tableHeight = this.dimension.height - (DOM.getDomNodePagePosition(
      this.headerContainer,
    ).height + 12 /*padding*/);
	...
	private selectEntry(keybindingItemEntry: IKeybindingItemEntry | number, focus: boolean = true): void {
		const index = typeof keybindingItemEntry === "number" ? keybindingItemEntry : this.getIndexOf(
      keybindingItemEntry,
    );
	...
	private onKeybindingEditingError(error: unknown): void {
      ...
      typeof error === "string" ? error : localize("error", "Error '{0}' while editing the keybinding. Please open 'keybindings.json' file and check for errors.", `${error}`),
    );
	}
}
...
class WhenInputWidget extends Disposable {
	...
	constructor(
      ...
      keybindingsEditor.onAcceptWhenExpression(
		...
		this._register(
      Event.any(keybindingsEditor.onRejectWhenExpression, this.input.onDidBlur)(
