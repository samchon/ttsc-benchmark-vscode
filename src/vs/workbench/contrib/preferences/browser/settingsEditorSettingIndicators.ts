...
export class SettingsTreeIndicatorsLabel implements IDisposable {
	...
	private createWorkspaceTrustIndicator(): SettingIndicator {
      ...
      new SimpleIconLabel(workspaceTrustElement),
		...
		workspaceTrustLabel.text = "$(shield) " + localize(
	...
	private createSyncIgnoredIndicator(): SettingIndicator {
      ...
      "This setting is ignored during sync",
		...
		disposables.add(
      this.hoverService.setupDelayedHover(syncIgnoredElement, {
        ...this.defaultHoverOptions,
	...
	private createAdvancedIndicator(): SettingIndicator {
      ...
      new SimpleIconLabel(advancedIndicator),
		...
		disposables.add(
      this.hoverService.setupDelayedHover(advancedIndicator, {
        ...this.defaultHoverOptions,
	...
	updatePreviewIndicator(element: SettingsTreeSettingElement) {
      ...
      this.hoverService.setupDelayedHover(this.previewIndicator.element, {
        ...this.defaultHoverOptions,
        content,
      }, {
        setupKeyboardEvents: true,
	...
	updateScopeOverrides(element: SettingsTreeSettingElement, onDidClickOverrideElement: Emitter<ISettingOverrideClickEvent>, onApplyFilter: Emitter<string>) {
		...
		if (element.hasPolicyValue) {
			...
			this.scopeOverridesIndicator.label.text = "$(briefcase) " + localize(
        "policyLabelText",
        ...
        "This setting is managed by your organization and its actual value cannot be changed.",
			...
			this.scopeOverridesIndicator.label.text = "$(lock) " + localize(
          ...
          DOM.addDisposableListener(view, DOM.EventType.CLICK, (e) => {
            ...
            "The following languages have default overrides:",
					...
					for (const language of element.overriddenDefaultsLanguageList) {
						const scopeDisplayText = this.languageService.getLanguageName(
              language,
            );
						contentMarkdownString += "\n- " + createMarkdownLink(
...
export function getIndicatorsLabelAriaLabel(element: SettingsTreeSettingElement, configurationService: IWorkbenchConfigurationService, userDataProfilesService: IUserDataProfilesService, languageService: ILanguageService): string {
	...
	} else if (element.settingsTarget === ConfigurationTarget.USER_LOCAL && configurationService.isSettingAppliedForAllProfiles(
    element.setting.key,
  )) {
		ariaLabelSections.push(
      localize(
