...
export class PreferencesEditor extends EditorPane {
	...
	private readonly editorPanesRegistry = Registry.as<IPreferencesEditorPaneRegistry>(
	...
	constructor(
      ...
      storageService,
		...
		this.searchFocusContextKey = CONTEXT_PREFERENCES_SEARCH_FOCUS.bindTo(
      contextKeyService,
    );
      ...
      DOM.$(".search-container"),
		...
		this._register(
      Event.debounce(this.searchWidget.onDidChange, () => undefined, 300)(
        () => {
	...
	layout(dimension: DOM.Dimension): void {
      ...
      new DOM.Dimension(this.bodyElement.clientWidth, dimension.height - 87),
    );
	}
	...
	private onDidChangePreferencesEditorPane(toAdd: readonly IPreferencesEditorPaneDescriptor[], toRemove: readonly IPreferencesEditorPaneDescriptor[]): void {
		...
		if (toAdd.length > 0) {
			...
			for (const desc of toAdd) {
				...
				if (index !== -1) {
					const action = new PreferenceTabAction(
            desc,
