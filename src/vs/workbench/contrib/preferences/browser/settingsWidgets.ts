...
export class ObjectSettingDropdownWidget extends AbstractListSettingWidget<IObjectDataItem> {
	...
	protected renderItem(item: IObjectDataItem, idx: number): RowElementGroup {
		...
		if (this.propertyNames && item.key.data && !validatePropertyName(
      this.propertyNames,
      item.key.data,
    )) {
	...
	protected addTooltipsToRow(rowElementGroup: RowElementGroup, item: IObjectDataItem): void {
      ...
      accessibleDescription,
		...
		const keyDescription: string | MarkdownString = this.getEnumDescription(
      item.key,
    ) ?? item.keyDescription ?? markdownString;
		this.listDisposables.add(
      this.hoverService.setupDelayedHover(keyElement, {
		...
		const valueDescription: string | MarkdownString = this.getEnumDescription(
      item.value,
    ) ?? markdownString;
		this.listDisposables.add(
