...
export class TaskQuickPick extends Disposable {
	...
	public async show(placeHolder: string, defaultEntry?: ITaskQuickPickEntry, startAtType?: string, name?: string): Promise<Task | undefined | null> {
      ...
      this._quickInputService.createQuickPick<ITaskTwoLevelQuickPickEntry>({
		...
		if (!firstLevelTask) {
			...
			if (topLevelEntriesResult.isSingleConfigured && this._configurationService.getValue<boolean>(
		...
		do {
          ...
          firstLevelTask,
            ...
            disposables,
					...
					disposables.dispose();
					return (selectedEntry?.task && !Types.isString(
            selectedEntry?.task,
          )) ? this._toTask(selectedEntry?.task) : undefined;
				}
	...
	private async _doPickerFirstLevel(picker: IQuickPick<ITaskTwoLevelQuickPickEntry, { useSeparators: true }>, taskQuickPickEntries: QuickPickInput<ITaskTwoLevelQuickPickEntry>[], disposables: DisposableStore): Promise<Task | ConfiguringTask | string | null | undefined> {
      ...
      showWithPinnedItems(this._storageService, runTaskStorageKey, picker, true),
		...
		const firstLevelPickerResult = await new Promise<ITaskTwoLevelQuickPickEntry | undefined | null>(
      resolve => {
        disposables.add(
	...
	public async doPickerSecondLevel(picker: IQuickPick<ITaskTwoLevelQuickPickEntry, { useSeparators: true }>, disposables: DisposableStore, type: string, name?: string) {
		picker.busy = true;
		if (type === SHOW_ALL) {
			const items = (await this._taskService.tasks()).filter(t => !t.configurationProperties.hide).sort((a, b) => this._sorter.compare(a, b)).map(
        task => this._createTaskEntry(task),
      );
			items.push(
		...
		await picker.show();
		picker.busy = false;
		const secondLevelPickerResult = await new Promise<ITaskTwoLevelQuickPickEntry | undefined | null>(
      resolve => {
        disposables.add(
          Event.once(picker.onDidAccept)(async () => {
            resolve(picker.selectedItems ? picker.selectedItems[0] : undefined);
