...
export class TerminalTaskSystem extends Disposable implements ITaskSystem {
	...
	constructor(
      ...
      this._terminalStatusManager = instantiationService.createInstance(TaskTerminalStatus),
		...
		this._register(
      this._taskProblemMonitor = instantiationService.createInstance(TaskProblemMonitor),
    );
	...
	public isTaskVisible(task: Task): boolean {
		...
		const isPanelShowingTerminal = !!this._viewsService.getActiveViewWithId(
      TERMINAL_VIEW_ID,
    );
	...
	public revealTask(task: Task): boolean {
		...
		const isTerminalInPanel: boolean = this._viewDescriptorService.getViewLocationById(
      TERMINAL_VIEW_ID,
	...
	private async _resolveAndFindExecutable(systemInfo: ITaskSystemInfo | undefined, workspaceFolder: IWorkspaceFolder | undefined, task: CustomTask | ContributedTask, cwd: string | undefined, envPath: string | undefined): Promise<string> {
      ...
      envPath.split(delimiter).map(
        p => this._configurationResolverService.resolveAsync(workspaceFolder, p),
      ),
    ) : undefined;
	...
	private _executeCommand(task: CustomTask | ContributedTask, trigger: string, alreadyResolved: Map<string, string>): Promise<ITaskSummary> {
		...
		const systemInfo: ITaskSystemInfo | undefined = this._currentTask.systemInfo = this._taskSystemInfoResolver(
      workspaceFolder,
    );
	...
	private async _createShellLaunchConfig(task: CustomTask | ContributedTask, workspaceFolder: IWorkspaceFolder | undefined, variableResolver: VariableResolver, platform: Platform.Platform, options: CommandOptions, command: CommandString, args: CommandString[], waitOnExit: WaitOnExitValue, presentationOptions: IPresentationOptions): Promise<IShellLaunchConfig | undefined> {
        ...
        waitOnExit,
			...
			const shellArgs = Array.isArray(
        shellLaunchConfig.args,
      ) ? <string[]>shellLaunchConfig.args.slice(0) : [shellLaunchConfig.args];
			const toAdd: string[] = [];
        ...
        args,
			...
			if (platform === Platform.Platform.Windows) {
				...
				if (basename === "cmd.exe" && ((options.cwd && isUNC(
          options.cwd,
        ...
        " ",
			...
			if (task.command.presentation && task.command.presentation.echo) {
				if (needsFolderQualification && workspaceFolder) {
					const folder = cwd && typeof cwd === "object" && Object.hasOwn(
            cwd,
            "path",
          ) ? path.basename(cwd.path) : workspaceFolder.name;
					shellLaunchConfig.initialText = this.taskShellIntegrationStartSequence(
            cwd,
          ...
          trailingNewLine: false,
			...
			const commandExecutable = (task.command.runtime !== RuntimeType.CustomExecution) ? CommandString.value(
        command,
      ) : undefined;
	...
	private _reconnectToTerminals(): void {
      ...
      `Attempting reconnection of ${reconnectedInstances.length} terminals`,
		...
		if (!reconnectedInstances.length) {
          ...
          terminal,
        ) as IReconnectionTaskData | undefined;
				if (data) {
					const terminalData = {
	...
	private _buildShellCommandLine(platform: Platform.Platform, shellExecutable: string, shellOptions: IShellConfiguration | undefined, command: CommandString, originalCommand: CommandString | undefined, args: CommandString[]): string {
      ...
      platform,
		...
		if ((!args || args.length === 0) && Types.isString(
      command,
    ) && (command === originalCommand as string || needsQuotes(
	...
	private _getQuotingOptions(shellBasename: string, shellOptions: IShellConfiguration | undefined, platform: Platform.Platform): IShellQuotingOptions {
		...
		return TerminalTaskSystem._shellQuotes[shellBasename] || TerminalTaskSystem._osShellQuotes[Platform.PlatformToString(
      platform,
    )];
	}
	...
	private _collectTaskVariables(variables: Set<string>, task: CustomTask | ContributedTask): void {
      ...
      task.configurationProperties.problemMatchers,
		...
		if (task.command.runtime === RuntimeType.CustomExecution && (CustomTask.is(
      task,
	...
	private _collectDefinitionVariables(variables: Set<string>, definition: unknown): void {
		if (Types.isString(definition)) {
			...
			definition.forEach(
        (element: unknown) => this._collectDefinitionVariables(
          variables,
          element,
        ),
      );
