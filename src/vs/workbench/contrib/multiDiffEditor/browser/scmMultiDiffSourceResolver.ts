...
export class ScmMultiDiffSourceResolver implements IMultiDiffSourceResolver {
	...
	async resolveDiffSource(uri: URI): Promise<IResolvedMultiDiffSource> {
		const { repositoryUri, groupId } = ScmMultiDiffSourceResolver.parseUri(
      uri,
    )!;
...
export class ScmHistoryItemResolver implements IMultiDiffSourceResolver {
	...
	async resolveDiffSource(uri: URI): Promise<IResolvedMultiDiffSource> {
		const { repositoryId, historyItemId, historyItemParentId, historyItemDisplayId } = ScmHistoryItemResolver.parseUri(
      uri,
    )!;
