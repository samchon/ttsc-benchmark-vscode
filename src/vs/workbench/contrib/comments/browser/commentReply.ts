...
export class CommentReply<T extends IRange | ICellRange> extends Disposable {
	...
	private updateAuthorInfo() {
        ...
        dom.$("img.avatar"),
			...
			img.src = FileAccess.uriToBrowserUri(URI.revive(this._commentThread.canReply.iconPath)).toString(
        true,
      );
		} else {
	...
	setCommentEditorDecorations() {
		...
		const placeholder = hasExistingComments
			...
			: (this._commentOptions?.placeHolder || nls.localize(
          "newComment",
          "Type a new comment",
        ));
	...
	private createReplyButton(commentEditor: ICodeEditor, commentForm: HTMLElement) {
		this._reviewThreadReplyButton = <HTMLButtonElement>dom.append(
      commentForm,
      dom.$(
      ...
      this.hoverService.setupManagedHover(
		...
		this._reviewThreadReplyButton.textContent = this._commentOptions?.prompt || nls.localize(
      "reply",
      "Reply...",
    );
