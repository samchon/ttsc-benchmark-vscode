...
export class CommentNode<T extends IRange | ICellRange> extends Disposable {
	...
	private readonly _reactionsActionBar: MutableDisposable<ActionBar> = this._register(
    new MutableDisposable(),
  );
	...
	constructor(
      ...
      dom.$(".review-comment-contents"),
		...
		if (configurationService.getValue<ICommentsConfiguration | undefined>(
      COMMENTS_SECTION,
    )?.maxHeight !== false) {
	...
	private createReactionsContainer(commentDetailsContainer: HTMLElement): void {
      ...
      this.owner,
		...
		const reactions = this.comment.commentReactions?.filter(
