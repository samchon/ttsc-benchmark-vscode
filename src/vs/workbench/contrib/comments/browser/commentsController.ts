...
class CommentingRangeDecoration implements IModelDeltaDecoration {
	...
	public getActiveRange() {
		return this.id ? this._editor.getModel()!.getDecorationRange(
      this.id,
    ) : undefined;
	}
}
...
class CommentingRangeDecorator {
	...
	public updateSelection(cursorLine: number, range: Range = new Range(
    0,
    0,
    0,
    0,
	...
	public getMatchedCommentAction(commentRange: Range | undefined): MergedCommentRangeActions[] {
		...
		for (const decoration of this.commentingRangeDecorations) {
			...
			if (range && this.areRangesIntersectingOrTouchingByLine(
        range,
        commentRange,
      )) {
...
export function revealCommentThread(commentService: ICommentService, editorService: IEditorService, uriIdentityService: IUriIdentityService,
	...
	for (const editor of currentActiveResources) {
		const model = editor.getModel();
		if ((model instanceof TextModel) && uriIdentityService.extUri.isEqual(
      resource,
      model.uri,
    )) {
...
export class CommentController extends Disposable implements IEditorContribution {
	...
	constructor(
      ...
      contextKeyService,
		...
		this._activeCursorHasComment = CommentContextKeys.activeCursorHasComment.bindTo(
      contextKeyService,
    );
		this._activeEditorHasCommentingRange = CommentContextKeys.activeEditorHasCommentingRange.bindTo(
      contextKeyService,
    );
		this._commentWidgetVisible = CommentContextKeys.commentWidgetVisible.bindTo(
      contextKeyService,
    );
      ...
      reverse,
		...
		if (this.accessibilityService.isScreenReaderOptimized()) {
			...
			if (commentRangeStart && commentRangeEnd) {
				...
				oneLine ? status(
          nls.localize("commentRange", "Line {0}", commentRangeStart),
        ) : status(
          nls.localize(
            "commentRangeStart",
            "Lines {0} to {1}",
	...
	private async handleCommentAdded(editorId: string | undefined, uniqueOwner: string, thread: languages.AddedCommentThread): Promise<void> {
      ...
      zoneWidget => zoneWidget.uniqueOwner === uniqueOwner && zoneWidget.commentThread.commentThreadHandle === -1 && Range.equalsRange(zoneWidget.commentThread.range, thread.range),
		...
		if ((continueOnCommentIndex !== undefined) && continueOnCommentIndex >= 0) {
			continueOnCommentText = this._inProcessContinueOnComments.get(uniqueOwner)?.splice(
      ...
      pendingEdits,
		...
		this._commentInfos.filter(info => info.uniqueOwner === uniqueOwner)[0].threads.push(
      thread,
    );
		this.tryUpdateReservedSpace();
	}
	...
	private async resumePendingComment(editorURI: URI, thread: languages.PendingCommentThread) {
      ...
      zoneWidget => zoneWidget.uniqueOwner === thread.uniqueOwner && Range.lift(zoneWidget.commentThread.range)?.equalsRange(thread.range),
        ...
        isReply: false,
			...
			if (!existingPendingComment || thread.comment.body.includes(
        existingPendingComment.body,
      )) {
	...
	private async openCommentsView(thread: languages.CommentThread) {
		if (thread.comments && (thread.comments.length > 0) && threadHasMeaningfulComments(
      thread,
    )) {
			const openViewState = this.configurationService.getValue<ICommentsConfiguration>(
        COMMENTS_SECTION,
      ).openView;
			if (openViewState === "file") {
				...
				const hasShownView = this.viewsService.getViewWithId<CommentsPanel>(
          COMMENTS_VIEW_ID,
        )?.hasRendered;
				if (!hasShownView) {
					return this.viewsService.openView(COMMENTS_VIEW_ID);
	...
	private onEditorMouseDown(e: IEditorMouseEvent): void {
		this.mouseDownInfo = (e.target.element?.className.indexOf(
      "comment-range-glyph",
    ) ?? -1) >= 0 ? parseMouseDownInfoFromEvent(e) : null;
	}
	...
	private onEditorMouseUp(e: IEditorMouseEvent): void {
      ...
      e,
		...
		const mouseUpIsOnDecorator = (e.target.element.className.indexOf(
      "comment-range-glyph",
    ) >= 0);
	...
	public addCommentAtLine(range: Range | undefined, e: IEditorMouseEvent | undefined): Promise<void> {
		const newCommentInfos = this._commentingRangeDecorator.getMatchedCommentAction(
      range,
    );
	...
	private tryUpdateReservedSpace(uri?: URI) {
      ...
      return hasRanges || (info.threads.length > 0);
		...
		const resourceHasCommentingRanges = uri ? this.commentService.resourceHasCommentingRanges(
      uri,
    ) : false;
		...
		if (hasCommentsOrRanges && this.commentService.isCommentingEnabled) {
			if (!this._commentingRangeSpaceReserved) {
				...
				const { lineDecorationsWidth, extraEditorClassName } = this.getExistingCommentEditorOptions(
          this.editor,
        );
