...
export class CommentsPanel extends FilterViewPane implements ICommentsView {
	...
	constructor(
      ...
      StorageTarget.MACHINE,
		...
		this.hasCommentsContextKey = CONTEXT_KEY_HAS_COMMENTS.bindTo(
      contextKeyService,
    );
		this.someCommentsExpandedContextKey = CONTEXT_KEY_SOME_COMMENTS_EXPANDED.bindTo(
      contextKeyService,
    );
		this.commentsFocusedContextKey = CONTEXT_KEY_COMMENT_FOCUSED.bindTo(
      contextKeyService,
    );
	...
	private updateFilter() {
      ...
      total === filtered || total === 0 ? undefined : nls.localize("showing filtered results", "Showing {0} of {1}", filtered, total),
    );
	...
	private getReplyCountAsString(node: CommentNode, forAriaLabel?: boolean): string {
		return node.replies.length && !forAriaLabel ? nls.localize(
      "replyCount",
      " {0} replies,",
      node.replies.length,
    ) : "";
