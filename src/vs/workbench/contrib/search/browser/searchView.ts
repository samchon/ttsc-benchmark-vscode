...
export class SearchView extends ViewPane {
	...
	constructor(
      ...
      this.contextKeyService.createScoped(this.container),
		...
		Constants.SearchContext.SearchViewFocusedKey.bindTo(this.contextKeyService).set(
      true,
    );
	...
	private async refreshHasAISetting(): Promise<void> {
		...
		if (shouldShowAI && !this.tree.hasNode(
      this.searchResult.aiTextSearchResult,
    )) {
			if (this.model.searchResult.getCachedSearchComplete(false)) {
				return this.refreshAndUpdateCount();
			}
		} else if (!shouldShowAI && this.tree.hasNode(
      this.searchResult.aiTextSearchResult,
    )) {
			return this.refreshAndUpdateCount();
		}
	...
	protected override renderBody(parent: HTMLElement): void {
      ...
      [...this.scmService.repositories].some(
        repo => repo.provider.groups.some(group => group.resources.length > 0),
      ),
    );
	...
	private async onConfigurationUpdated(event?: IConfigurationChangeEvent): Promise<void> {
		if (event && (event.affectsConfiguration(
	...
	async selectPreviousMatch(): Promise<void> {
        ...
        nextItem,
      ); // recreate navigator because modifying the tree can invalidate it
	...
	private updateTextFromSelection({ allowUnselectedWord = true, allowSearchOnType = true }, editor?: IEditor): boolean {
		const seedSearchStringFromSelection = this.configurationService.getValue<IEditorOptions>(
      "editor",
	...
	toggleQueryDetails(moveFocus = true, show?: boolean, skipLayout?: boolean, reverse?: boolean): void {
		show = typeof show === "undefined" ? !this.queryDetails.classList.contains(
      "more",
    ) : Boolean(show);
	...
	private _searchWithIncludeOrExclude(include: boolean, folderPaths: string[]) {
		if (!folderPaths.length || folderPaths.some(
	...
	private async onSearchComplete(
      ...
      this.shouldShowAIResults() && !!aiResults,
		...
		if (completed && this.tree.hasNode(
      this.searchResult.aiTextSearchResult,
		...
		if (!allResults) {
          ...
          new SearchLinkButton(
			...
			this.viewModel.searchResult.toggleHighlights(
        this.isVisible(),
      ); // show highlights
	...
	private doSearch(query: ITextQuery, excludePatternText: string, includePatternText: string, triggeredOnType: boolean, shouldKeepAIResults: boolean, shouldUpdateAISearch: boolean): Thenable<void> {
      ...
      2000,
		...
		if (this.configurationService.getValue<ISearchConfigurationProperties>(
      "search",
	...
	private updateSearchResultCount(disregardExcludesAndIgnores?: boolean, onlyOpenEditors?: boolean, clear: boolean = false): void {
      ...
      this.viewModel.searchResult.aiTextSearchResult.hidden,
		...
		const messageEl = this.clearMessage();
		const resultMsg = clear ? "" : this.buildResultCountMessage(
      resultCount,
      fileCount,
    );
	...
	private handleKeywordClick(keyword: string, index: number) {
      ...
      shouldKeepAIResults: false,
		...
		this.telemetryService.publicLog2<KeywordClickEvent, KeywordClickClassification>(
      "searchKeywordClick",
      {
        index,
	...
	private showSearchWithoutFolderMessage(): void {
      ...
      new SearchLinkButton(
        ...
        () => {
          this.commandService.executeCommand(OpenFolderAction.ID).catch(
            err => errors.onUnexpectedError(err),
          );
        },
        this.hoverService,
      ),
	...
	private shouldOpenInNotebookEditor(match: ISearchTreeMatch, uri: URI): boolean {
		...
		return isIMatchInNotebook(match) || (uri.scheme !== network.Schemas.untitled && this.notebookService.getContributedNotebookTypes(
      uri,
    ).length > 0);
	}
	...
	private onFocus(lineMatch: ISearchTreeMatch, preserveFocus?: boolean, sideBySide?: boolean, pinned?: boolean): Promise<any> {
		...
		const resource = isSearchTreeMatch(
      lineMatch,
    ) ? lineMatch.parent().resource : (<ISearchTreeFileMatch>lineMatch).resource;
		return (useReplacePreview && this.viewModel.isReplaceActive() && !!this.viewModel.replaceString && !(this.shouldOpenInNotebookEditor(
      lineMatch,
      resource,
    ))) ?
	...
	async open(element: FileMatchOrMatch, preserveFocus?: boolean, sideBySide?: boolean, pinned?: boolean, resourceInput?: URI): Promise<void> {
		...
		const oldParentMatches = isSearchTreeMatch(
      element,
    ) ? element.parent().matches() : [];
		const resource = resourceInput ?? (isSearchTreeMatch(
      element,
    ) ? element.parent().resource : (<ISearchTreeFileMatch>element).resource);
			...
			if (isSearchTreeMatch(element)) {
              ...
              e => e.id() === element.id(),
						...
						if (isIMatchInNotebook(match)) {
							...
							if (!this.tree.getFocus().includes(
                match,
...
export function getSelectionTextFromEditor(allowUnselectedWord: boolean, activeEditor: IEditor): string | null {
	...
	if (range.isEmpty()) {
		if (allowUnselectedWord) {
			const wordAtPosition = editor.getModel().getWordAtPosition(
...
class RefreshTreeController extends Disposable {
	...
	private async retrieveFileStats(): Promise<void> {
		const files = this.searchView.model.searchResult.matches().filter(f => !f.fileStat).map(
      f => f.resolveFileStat(this.fileService),
    );
		await Promise.all(files);
	}
