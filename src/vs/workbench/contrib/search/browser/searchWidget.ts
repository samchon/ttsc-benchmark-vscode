...
export class SearchWidget extends Widget {
	...
	private _updateSearchInputAriaLabel(includeHint: boolean): void {
      ...
      "Search: Type Search Term and press Enter to search",
		...
		if (includeHint && !this._accessibilityHelpHintAnnounced && this.configurationService.getValue(
	...
	isReplaceShown(): boolean {
		return this.replaceContainer ? !this.replaceContainer.classList.contains(
      "disabled",
    ) : false;
	}
	...
	private renderSearchInput(parent: HTMLElement, options: ISearchWidgetOptions): void {
      ...
      this.searchInput.onKeyDown(
        (keyboardEvent: IKeyboardEvent) => this.onSearchInputKeyDown(
          keyboardEvent,
        ),
      ),
    );
      ...
      this.searchInput.onCaseSensitiveKeyDown(
        (keyboardEvent: IKeyboardEvent) => this.onCaseSensitiveKeyDown(
          keyboardEvent,
        ),
      ),
    );
      ...
      this.searchInput.onRegexKeyDown(
        (keyboardEvent: IKeyboardEvent) => this.onRegexKeyDown(keyboardEvent),
      ),
    );
		this._register(
      ...
      this.showContextToggle.onChange(() => this.onContextLinesChanged()),
        ...
        { type: "number", inputBoxStyles: defaultInputBoxStyles },
			...
			this.contextLinesInput.value = "" + (this.configurationService.getValue<ISearchConfigurationProperties>(
        "search",
	...
	private renderReplaceInput(parent: HTMLElement, options: ISearchWidgetOptions): void {
      ...
      this.replaceInput.onPreserveCaseKeyDown(
        (keyboardEvent: IKeyboardEvent) => this.onPreserveCaseKeyDown(
          keyboardEvent,
        ),
      ),
    );
	...
	setReplaceAllActionState(enabled: boolean): void {
		if (this.replaceAllAction && (this.replaceAllAction.enabled !== enabled)) {
			...
			this.replaceAllAction.label = enabled ? SearchWidget.REPLACE_ALL_ENABLED_LABEL(
