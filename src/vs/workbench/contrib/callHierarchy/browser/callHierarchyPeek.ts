...
export class CallHierarchyTreePeekWidget extends peekView.PeekViewWidget {
	...
	private _applyTheme(theme: IColorTheme) {
		const borderColor = theme.getColor(
      peekView.peekViewBorder,
    ) || Color.transparent;
		this.style({
	...
	private async _updatePreview() {
		...
		for (const loc of locations) {
			if (loc.uri.toString() === previewUri.toString()) {
				...
				fullRange = !fullRange ? loc.range : Range.plusRange(
          loc.range,
          fullRange,
        );
			}
	...
	async showModel(model: CallHierarchyModel): Promise<void> {
		...
		const root = <ITreeNode<callHTree.Call, FuzzyScore>>this._tree.getNode(
      model,
    ).children[0];
