...
export class McpServerIconWidget extends McpServerWidget {
	...
	render(): void {
        ...
        type,
      ) ? this.mcpServer.icon.dark : this.mcpServer.icon.light;
          ...
          { once: true },
			...
			this.iconElement.style.display = "none";
			this.iconElement.src = "";
			this.codiconIconElement.className = this.mcpServer.codicon ? `codicon ${this.mcpServer.codicon}` : ThemeIcon.asClassName(
        mcpServerIcon,
      );
			this.codiconIconElement.style.display = "inherit";
			this.iconLoadingDisposable.clear();
...
export class StarredWidget extends McpServerWidget {
	...
	render(): void {
		...
		const parent = this.small ? this.container : dom.append(
      this.container,
      dom.$("span.rating", { tabIndex: 0 }),
    );
