...
export class McpAddConfigurationCommand {
	...
	private async getServerType(): Promise<AddConfigurationType | undefined> {
      ...
      },
      {
        ...
        label: localize("mcp.serverType.http", "HTTP (HTTP or Server-Sent Events)"),
		...
		const discovery = this._configurationService.getValue<{ [K in DiscoverySource]: boolean }>(
      mcpDiscoverySection,
    );
		if (discovery && typeof discovery === "object" && allDiscoverySources.some(
      d => !discovery[d],
    )) {
			items.push({
        kind: "discovery",
	...
	private async getConfigurationTarget(): Promise<ConfigurationTarget | IWorkspaceFolder | undefined> {
		const options: (IQuickPickItem & { target?: ConfigurationTarget | IWorkspaceFolder })[] = [
      {
		...
		const raLabel = this._environmentService.remoteAuthority && this._label.getHostLabel(
      ...
      this._environmentService.remoteAuthority,
        ...
        description: localize("mcp.target..remote.description", "Available on this remote machine, runs on {0}", raLabel),
      });
		}
		...
		if (workbenchState !== WorkbenchState.EMPTY) {
          ...
          description: localize("mcp.target.workspace.description.remote", "Available in this workspace, runs on {0}", raLabel),
        });
			} else {
				options.push({
          target,
          label: localize("mcp.target.workspace", "Workspace"),
          description: localize("mcp.target.workspace.description", "Available in this workspace, runs locally"),
        });
			}
		}
	...
	private async getAssistedConfig(type: AssistedConfigurationType): Promise<{ name?: string; server: Omit<IMcpStdioServerConfiguration, "type">; inputs?: IMcpServerVariable[]; inputValues?: Record<string, string> } | undefined> {
      ...
      this._quickInputService.createQuickPick<IQuickPickItem & { id: LoadAction; helpUri?: URI }>(),
    );
      ...
      "mcp.addserver",
      {
		...
		switch (loadingAction?.id) {
			...
			case LoadAction.OpenUri:
				if (loadingAction.helpUri) { this._openerService.open(
          loadingAction.helpUri,
        ); }
				return undefined;
	...
	public async run(): Promise<void> {
      ...
      target,
		...
		if (packageType) {
			this._telemetryService.publicLog2<AddServerCompletedData, AddServerCompletedClassification>(
		...
		switch (pick?.id) {
			...
			case "install":
				...
				try {
					...
					const { inputs, ...config }: IMcpServerConfiguration & { inputs?: IMcpServerVariable[] } = parseJsonc(
...
export class McpInstallFromManifestCommand {
	...
	async run(): Promise<void> {
      ...
      galleryManifest.remotes,
		...
		let inputs: IMcpServerVariable[] | undefined;
		try {
			const { mcpServerConfiguration, notices } = this._mcpManagementService.getMcpServerConfigurationFromManifest(
        galleryManifest,
        packageType,
      );
