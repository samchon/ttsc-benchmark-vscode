...
export class McpElicitationService implements IMcpElicitationService {
	...
	private async _doElicitUrl(elicitation: MCP.ElicitRequestURLParams, token: CancellationToken): Promise<MCP.ElicitResult> {
		...
		try {
			if (await this._openerService.open(elicitation.url, {
        allowCommands: false,
      })) {
	...
	private async _doElicitForm(elicitation: MCP.ElicitRequestFormParams | Pre20251125ElicitationParams, token: CancellationToken): Promise<MCP.ElicitResult> {
        ...
        elicitation.requestedSchema.required || [],
            ...
            quickPick,
            {
              enum: schema.enum.map((v, i) => ({ const: v, title: schema.enumNames[i] })),
	...
	private _getFieldPlaceholder(schema: MCP.PrimitiveSchemaDefinition, required: boolean): string {
		...
		if (!required) {
			placeholder = placeholder ? `${placeholder} (${localize("optional", "Optional")})` : localize(
        "optional",
        "Optional",
      );
		}
	...
	private _validateString(value: string, schema: MCP.StringSchema): { isValid: boolean; parsedValue?: string; message?: string } {
		if (schema.minLength && value.length < schema.minLength) {
			return {
        isValid: false,
        message: localize("mcp.elicit.validation.minLength", "Minimum length is {0}", schema.minLength),
      };
		}
		if (schema.maxLength && value.length > schema.maxLength) {
			return {
        isValid: false,
        message: localize("mcp.elicit.validation.maxLength", "Maximum length is {0}", schema.maxLength),
      };
		}
		if (schema.format) {
		...
		switch (format) {
			case "email":
				return value.includes("@")
					...
					: {
              isValid: false,
              message: localize("mcp.elicit.validation.email", "Please enter a valid email address"),
            };
			case "uri":
				if (URL.canParse(value)) {
					...
					return {
            isValid: false,
            message: localize("mcp.elicit.validation.uri", "Please enter a valid URI"),
          };
				}
			case "date": {
				...
				if (!dateRegex.test(value)) {
					return {
            isValid: false,
            message: localize("mcp.elicit.validation.date", "Please enter a valid date (YYYY-MM-DD)"),
          };
				}
			...
			case "date-time": {
				...
				return !isNaN(dateTime.getTime())
					...
					: {
              isValid: false,
              message: localize("mcp.elicit.validation.dateTime", "Please enter a valid date-time"),
            };
			}
			default:
				return { isValid: true };
	...
	private _validateNumber(value: string, schema: MCP.NumberSchema): { isValid: boolean; parsedValue?: number; message?: string } {
		...
		if (isNaN(parsed)) {
			return {
        isValid: false,
        message: localize("mcp.elicit.validation.number", "Please enter a valid number"),
      };
		}
		if (schema.type === "integer" && !Number.isInteger(parsed)) {
			return {
        isValid: false,
        message: localize("mcp.elicit.validation.integer", "Please enter a valid integer"),
      };
		}
		if (schema.minimum !== undefined && parsed < schema.minimum) {
			return {
        isValid: false,
        message: localize("mcp.elicit.validation.minimum", "Minimum value is {0}", schema.minimum),
      };
		}
		if (schema.maximum !== undefined && parsed > schema.maximum) {
			return {
        isValid: false,
        message: localize("mcp.elicit.validation.maximum", "Maximum value is {0}", schema.maximum),
      };
		}
