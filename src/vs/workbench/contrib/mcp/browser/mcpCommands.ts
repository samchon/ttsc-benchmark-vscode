...
export class McpServerOptionsCommand extends Action2 {
	...
	override async run(accessor: ServicesAccessor, id: string): Promise<void> {
      ...
      IAuthenticationQueryService,
		...
		const collection = mcpRegistry.collections.get().find(
      c => c.id === server.collection.id,
    );
		const serverDefinition = collection?.serverDefinitions.get().find(
      s => s.id === server.definition.id,
    );
	...
	private _getAuthActions(
		...
		for (const [providerId, accountName] of authenticationQueryService.mcpServer(serverId).getAllAccountPreferences()) {
			...
			const accountQuery = authenticationQueryService.provider(providerId).account(
        accountName,
      );
	...
	private async _handleAuth(
		...
		accountQuery.mcpServer(definition.id).setAccessAllowed(
      false,
      definition.label,
    );
		if (signOut) {
...
export class AddConfigurationAction extends Action2 {
	...
	async run(accessor: ServicesAccessor, configUri?: string): Promise<void> {
		...
		const target = configUri ? workspaceService.getWorkspaceFolder(
      URI.parse(configUri),
    ) : undefined;
...
export class EditStoredInput extends Action2 {
	...
	run(accessor: ServicesAccessor, inputId: string, uri: URI | undefined, configSection: string, target: ConfigurationTarget): void {
		const workspaceFolder = uri && accessor.get(IWorkspaceContextService).getWorkspaceFolder(
      uri,
    );
...
export class ShowConfiguration extends Action2 {
	...
	run(accessor: ServicesAccessor, collectionId: string, serverId: string): void {
		const collection = accessor.get(IMcpRegistry).collections.get().find(
      c => c.id === collectionId,
    );
		if (!collection) {
			return;
		...
		const server = collection?.serverDefinitions.get().find(
      s => s.id === serverId,
    );
		const editorService = accessor.get(IEditorService);
...
export class RestartServer extends Action2 {
	...
	async run(accessor: ServicesAccessor, serverId: string, opts?: IMcpServerStartOpts) {
		const s = accessor.get(IMcpService).servers.get().find(
      s => s.definition.id === serverId,
    );
		s?.showOutput();
		await s?.stop();
...
export class OpenWorkspaceFolderMcpResourceCommand extends Action2 {
	...
	async run(accessor: ServicesAccessor) {
		...
		const workspaceFolder = workspaceFolders.length === 1 ? workspaceFolders[0] : await commandService.executeCommand<IWorkspaceFolder>(
      PICK_WORKSPACE_FOLDER_COMMAND_ID,
    );
		if (workspaceFolder) {
			await editorService.openEditor({
        resource: workspaceFolder.toResource(WORKSPACE_STANDALONE_CONFIGURATIONS[MCP_CONFIGURATION_KEY]),
      });
		}
	}
}
...
export class McpConfigureSamplingModels extends Action2 {
	...
	async run(accessor: ServicesAccessor, server: IMcpServer): Promise<number> {
		...
		allItems.sort(
      (a, b) => (b.picked ? 1 : 0) - (a.picked ? 1 : 0) || a.label.localeCompare(b.label),
    );
