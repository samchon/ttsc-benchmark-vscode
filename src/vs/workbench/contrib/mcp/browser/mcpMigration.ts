...
export class McpConfigMigrationContribution extends Disposable implements IWorkbenchContribution {
	...
	private async migrateMcpConfig(): Promise<void> {
        ...
        this.userDataProfileService.currentProfile.settingsResource,
			...
			if (userMcpConfig && userMcpConfig.servers && Object.keys(
        userMcpConfig.servers,
      ).length > 0) {
				await Promise.all(
          Object.entries(userMcpConfig.servers).map(
            ([name, config], index) => this.mcpManagementService.install({
              name,
              config,
      ...
      false,
		...
		if (remoteEnvironment) {
          ...
          remoteEnvironment.settingsPath,
				...
				if (userRemoteMcpConfig && userRemoteMcpConfig.servers && Object.keys(
          userRemoteMcpConfig.servers,
            ...
            Object.entries(userRemoteMcpConfig.servers).map(
              ([name, config], index) => this.mcpManagementService.install({
                name,
                config,
                inputs: index === 0 ? userRemoteMcpConfig.inputs : undefined,
              }, {
	...
	private async checkForMcpConfigInFile(settingsFile: URI, isRemote: boolean): Promise<void> {
		try {
			...
			if (mcpConfig && mcpConfig.servers && Object.keys(
        mcpConfig.servers,
      ).length > 0) {
