...
class McpWorkbenchServer implements IWorkbenchMcpServer {
	...
	get readmeUrl(): URI | undefined {
		return this.local?.readmeUrl ?? (this.gallery?.readmeUrl ? URI.parse(
      this.gallery.readmeUrl,
    ) : undefined);
	}
...
export class McpWorkbenchService extends Disposable implements IMcpWorkbenchService {
	...
	constructor(
		...
		super();
		this._register(
      this.mcpManagementService.onDidInstallMcpServersInCurrentProfile(
        e => this.onDidInstallMcpServers(e),
      ),
    );
		this._register(
      this.mcpManagementService.onDidUpdateMcpServersInCurrentProfile(
        e => this.onDidUpdateMcpServers(e),
      ),
    );
		this._register(
      this.mcpManagementService.onDidUninstallMcpServerInCurrentProfile(
        e => this.onDidUninstallMcpServer(e),
      ),
    );
		this._register(
      this.mcpManagementService.onDidChangeProfile(
        e => this.onDidChangeProfile(),
      ),
    );
	...
	private onDidUninstallMcpServer(e: DidUninstallWorkbenchMcpServerEvent) {
		...
		const uninstalled = this._local.find(
      server => this.areSameMcpServers(server.local, e),
    );
		if (uninstalled) {
	...
	private onDidInstallMcpServers(e: readonly IWorkbenchMcpServerInstallResult[]) {
		...
		for (const { local, source, name } of e) {
			let server = this.installing.find(
        server => server.local && local ? this.areSameMcpServers(server.local, local) : server.name === name,
      );
			this.installing = server ? this.installing.filter(
        e => e !== server,
      ) : this.installing;
			if (local) {
				if (server) {
	...
	private onDidUpdateMcpServers(e: readonly IWorkbenchMcpServerInstallResult[]) {
		for (const result of e) {
			...
			const serverIndex = this._local.findIndex(
        server => this.areSameMcpServers(server.local, result.local),
      );
	...
	private async syncInstalledMcpServers(): Promise<void> {
		...
		if (infos.length) {
			const galleryServers = await this.mcpGalleryService.getMcpServersFromGallery(
        infos,
      );
	...
	async install(server: IWorkbenchMcpServer, installOptions?: IWorkbencMcpServerInstallOptions): Promise<IWorkbenchMcpServer> {
		...
		if (server.gallery) {
			...
			return this.doInstall(
        server,
        () => this.mcpManagementService.installFromGallery(
          gallery,
          installOptions,
        ),
      );
	...
	private async doInstall(server: McpWorkbenchServer, installTask: () => Promise<IWorkbenchLocalMcpServer>): Promise<IWorkbenchMcpServer> {
		...
		try {
			...
			this.telemetryService.publicLog2<McpServerInstallData, McpServerInstallClassification>(
        "mcp/serverInstall",
        {
          serverName,
          source,
	...
	private getWorkspaceMcpConfigPath(mcpResource: URI): IMcpConfigPath | undefined {
		...
		if (workspace.configuration && this.uriIdentityService.extUri.isEqual(
	...
	private async handleMcpServerUrl(url: string): Promise<boolean> {
		try {
			...
			const local = this.local.find(
        e => e.name === gallery.name,
      ) ?? this.instantiationService.createInstance(
        McpWorkbenchServer,
        e => this.getInstallState(e),
        e => this.getRuntimeStatus(e),
	...
	private getRuntimeStatus(mcpServer: McpWorkbenchServer): McpServerEnablementStatus | undefined {
		...
		const server = this.mcpService.servers.get().find(
      s => s.definition.id === mcpServer.id,
    );
		if (!server) {
	...
	private getEnablementStatus(mcpServer: McpWorkbenchServer): McpServerEnablementStatus | undefined {
		...
		if (accessValue === McpAccessValue.Registry) {
			...
			if (remoteUrl && !mcpServer.gallery.configuration.remotes?.some(
...
export class MCPContextsInitialisation extends Disposable implements IWorkbenchContribution {
	...
	constructor(
		...
		const mcpServersGalleryStatus = McpServersGalleryStatusContext.bindTo(
      contextKeyService,
    );
      ...
      mcpGalleryManifestService.mcpGalleryManifestStatus,
		...
		this._register(
      mcpGalleryManifestService.onDidChangeMcpGalleryManifestStatus(
        status => mcpServersGalleryStatus.set(status),
      ),
    );

		const hasInstalledMcpServersContextKey = HasInstalledMcpServersContext.bindTo(
      contextKeyService,
    );
