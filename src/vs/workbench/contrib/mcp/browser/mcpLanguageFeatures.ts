...
export class McpLanguageFeatures extends Disposable implements IWorkbenchContribution {
    ...
    new MutableDisposable<{ model: ITextModel; inConfig: ConfigDescriptor; tree: Node } & IDisposable>(),
  );
	...
	private async _provideInlayHints(model: ITextModel, range: Range): Promise<InlayHintList | undefined> {
		...
		const { tree, inConfig } = parsed;
		const mcpSection = inConfig.section ? findNodeAtLocation(tree, [
      ...inConfig.section,
    ]) : tree;
		if (!mcpSection) {
			return undefined;
...
function forEachPropertyWithReplacement(node: Node, callback: (node: Node) => void) {
	if (node.type === "string" && typeof node.value === "string" && node.value.includes(
    ConfigurationResolverExpression.VARIABLE_LHS,
  )) {
		callback(node);
	} else if (node.type === "property") {
		// skip the property name
		node.children?.slice(1).forEach(
