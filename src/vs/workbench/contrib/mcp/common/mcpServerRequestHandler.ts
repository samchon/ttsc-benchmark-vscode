...
export class McpServerRequestHandler extends Disposable {
	...
	private readonly _onDidReceiveElicitationCompleteNotification = this._register(
    new Emitter<MCP.ElicitationCompleteNotification>(),
  );
	...
	protected constructor({
      ...
      new JsonRpcProtocol(message => this.send(message as MCP.JSONRPCMessage), {
		...
		try {
			switch (request.method) {
				...
				case "notifications/tasks/status":
					this._taskManager.getClientTask(request.params.taskId)?.onDidUpdateState(
            request.params,
          );
					return;
				default:
	...
	listResourcesIterable(params?: MCP.ListResourcesRequest["params"], token?: CancellationToken): AsyncIterable<MCP.Resource[]> {
		return this.sendRequestPaginated<MCP.ListResourcesRequest, MCP.ListResourcesResult, MCP.Resource>(
      "resources/list",
	...
	listResourceTemplates(params?: MCP.ListResourceTemplatesRequest["params"], token?: CancellationToken): Promise<MCP.ResourceTemplate[]> {
      ...
      this.sendRequestPaginated<MCP.ListResourceTemplatesRequest, MCP.ListResourceTemplatesResult, MCP.ResourceTemplate>(
	...
	async callTool(params: MCP.CallToolRequest["params"] & MCP.Request["params"], token?: CancellationToken, onStatusMessage?: (message: string) => void): Promise<MCP.CallToolResult> {
		const response = await this.sendRequest<MCP.CallToolRequest, MCP.CallToolResult | MCP.CreateTaskResult>(
	...
	getTaskResult(params: { taskId: string }, token?: CancellationToken): Promise<MCP.GetTaskPayloadResult> {
		return this.sendRequest<MCP.GetTaskPayloadRequest, MCP.GetTaskPayloadResult>(
...
export class McpTask<T extends MCP.Result> extends Disposable implements IMcpTaskInternal {
	...
	constructor(
		...
		const inputRequiredLookup = observableValue<ObservablePromise<MCP.Task> | undefined>(
      "activeResultLookup",
      undefined,
    );
