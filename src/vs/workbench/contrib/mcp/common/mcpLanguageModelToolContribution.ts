...
class McpToolImplementation implements IToolImpl {
	...
	async prepareToolInvocation(context: IToolInvocationPreparationContext): Promise<IPreparedToolInvocation> {
      ...
      this._productService.nameShort,
		...
		const mcpUiEnabled = this._configurationService.getValue<boolean>(
      ...
      isError: callResult.isError === true,
		...
		for (const item of callResult.content) {
          ...
          underlyingMimeType: mimeType,
			...
			const isForModel = !audience || audience.includes(
        LanguageModelPartAudience.Assistant,
          ...
          item.resource.uri,
				...
				if (item.resource.mimeType && getAttachableImageExtension(
