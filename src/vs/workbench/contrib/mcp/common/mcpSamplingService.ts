...
export class McpSamplingService extends Disposable implements IMcpSamplingService {
	...
	private async _getMatchingModelInner(server: IMcpServer, isDuringToolCall: boolean, preferences: MCP.ModelPreferences | undefined): Promise<ModelMatch | string> {
		...
		if (isDuringToolCall && !config.allowedDuringChat && !this._sessionSets.allowedDuringChat.has(
      server.definition.id,
    )) {
			return config.allowedDuringChat === undefined ? ModelMatch.UnsureAllowedDuringChat : ModelMatch.NotAllowed;
		} else if (!isDuringToolCall && !config.allowedOutsideChat && !this._sessionSets.allowedOutsideChat.has(
      server.definition.id,
    )) {
		...
		const foundModelIds = config.allowedModels?.filter(
      m => !!this._languageModelsService.lookupLanguageModel(m),
		...
		if (preferences?.hints) {
			const found = mapFindFirst(
        preferences.hints,
        hint => foundModelIds.find(
          model => model.toLowerCase().includes(hint.name!.toLowerCase()),
        ),
      );
			if (found) {
				return found;
	...
	private _getDefaultModels() {
		...
		const someDefault = candidates.findIndex(
      c => Object.values(c.model.isDefaultForLocation).some(Boolean),
    );
		if (someDefault !== -1) {
	...
	private _getConfig(server: IMcpServer) {
		...
		const configValue = this._configurationService.inspect<Record<string, IMcpServerSamplingConfiguration>>(
