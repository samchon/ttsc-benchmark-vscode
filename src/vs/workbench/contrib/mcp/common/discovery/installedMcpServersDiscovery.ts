		...
		try {
          ...
          sandbox: server.rootSandbox,
			...
			for (const [id, [mcpConfigPath, serverDefinitions]] of collections) {
				const newServerDefinitions = observableValue<readonly McpServerDefinition[]>(
          this,
          serverDefinitions,
        );
				...
				const collectionDefinitionsChanged = existingCollection ? !McpCollectionDefinition.equals(
          existingCollection.definition,
          newCollection,
        ) : true;
				if (!collectionDefinitionsChanged) {
					const serverDefinitionsChanged = existingCollection ? !equals(
