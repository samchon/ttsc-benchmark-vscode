...
export class OpenBrowserTool implements IToolImpl {
	...
	async invoke(invocation: IToolInvocation, _countTokens: CountTokensCallback, _progress: ToolProgress, token: CancellationToken): Promise<IToolResult> {
        ...
        ...this.browserViewService.getKnownBrowserViews().values(),
			...
			if (allPages.length === 0) {
				return {
          content: [{ kind: "text", value: "No browser pages are currently open." }],
        };
			}
        ...
        token,
			...
			if (shareResult) {
				...
				return {
          content: [{ kind: "text", value: "The user opted not to share an existing page." }],
        };
			}
		}
        ...
        this.browserViewService,
        params.url,
        { includeBlank: false, sharingState: BrowserViewSharingState.NotShared },
      );
	...
	private async _promptForUnsharedPages(invocation: IToolInvocation, candidateEditors: BrowserEditorInput[], params: IOpenBrowserToolParams, token: CancellationToken): Promise<IToolResult | undefined> {
      ...
      invocation.chatStreamToolCallId ?? invocation.callId,
		...
		const editor = this.browserViewService.getKnownBrowserViews().get(
      selectedOptionId,
    );
		if (!editor) {
	...
	private _buildShareCarousel(editors: BrowserEditorInput[], url: string | undefined, resolveId: string): ChatQuestionCarouselData {
        ...
        "\u00A0-\u00A0",
			...
			const truncatedUrl = editorUrl.length > 40 ? editorUrl.substring(
        0,
        40,
      ) + "\u2026" : editorUrl;
			options.push({
	...
	private async _shareExistingPage(sessionId: string, editor: BrowserEditorInput): Promise<IToolResult> {
		...
		if (model.sharingState !== BrowserViewSharingState.Shared) {
			if (!(await model.setSharedWithAgent(true))) {
				return {
          content: [{ kind: "text", value: "The user declined to share the page." }],
        };
			}
		}
