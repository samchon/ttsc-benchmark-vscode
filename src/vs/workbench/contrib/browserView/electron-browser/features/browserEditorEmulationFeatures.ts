...
class BrowserEmulationToolbar extends Disposable {
  ...
  private _writeInputs(
    device: IBrowserDeviceProfile | undefined,
    screen: IBrowserScreenProfile | undefined,
  ): void {
    this._suppressChange = true;
    try {
...
export class BrowserEditorEmulationSupport extends BrowserEditorContribution {
  ...
  private _computeLayout(
    ...
    const viewportWidth =
      screen?.width ?? Math.max(1, Math.round(paneWidth / scale));
    const viewportHeight =
      screen?.height ?? Math.max(1, Math.round(paneHeight / scale));
    return {
  ...
  get screen(): IBrowserScreenProfile | undefined {
    return this._screen;
  }
  ...
  private _syncContextKeys(
    device: IBrowserDeviceProfile | undefined,
    _screen: IBrowserScreenProfile | undefined,
  ): void {
  ...
  private _resetAxis(axis: "x" | "y"): void {
    ...
    const screen = this._screen ?? {};
    const next: IBrowserScreenProfile =
      axis === "x"
