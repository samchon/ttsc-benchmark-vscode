...
const CONTEXT_BROWSER_EDITOR_OPEN = new RawContextKey<boolean>(
  "browserEditorOpen",
  false,
  localize("browser.editorOpen", "Whether any browser editor is currently open"),
);
...
class BrowserTabQuickPick extends Disposable {
	...
	private _buildItems(): void {
      ...
      GroupsOrder.GRID_APPEARANCE,
		...
		const backgroundEditors = [...this._browserViewService.getKnownBrowserViews().values()].filter(
      e => !viewsInGroups.has(e.id),
    );
      ...
      this._browserViewService.onDidChangeBrowserViews(() => this._buildItems()),
    );
...
class OpenFileInIntegratedBrowserAction extends Action2 {
	...
	async run(accessor: ServicesAccessor, resource?: URI): Promise<void> {
		...
		const fileUri = resource ?? EditorResourceAccessor.getOriginalUri(
...
class NewTabAction extends Action2 {
	...
	async run(accessor: ServicesAccessor, _browserEditor = accessor.get(
    IEditorService,
...
class CloseAllBrowserTabsAction extends Action2 {
	...
	async run(accessor: ServicesAccessor): Promise<void> {
      ...
      GroupsOrder.GRID_APPEARANCE,
			...
			const browserEditors = group.getEditors(EditorsOrder.SEQUENTIAL).filter(
...
class CloseAllBrowserTabsInGroupAction extends Action2 {
	...
	async run(accessor: ServicesAccessor): Promise<void> {
      ...
      editorService.activeEditorPane?.group?.id ?? editorGroupsService.activeGroup.id,
    );
		if (!group) {
			return;
		}
...
MenuRegistry.appendMenuItem(MenuId.EditorTitleContext, {
  command: { id: BrowserViewCommandId.CloseAllInGroup, title: localize("browser.closeAllInGroupShort", "Close All Browser Tabs") },
  group: "1_close",
  order: 55,
  when: BROWSER_EDITOR_ACTIVE,
});
