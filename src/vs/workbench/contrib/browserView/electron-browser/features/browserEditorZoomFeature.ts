...
class BrowserZoomPill extends Disposable {
	...
	show(zoomLabel: string, isAtOrAboveDefault: boolean): void {
      ...
      750,
    ); // Chrome shows the zoom level for 1.5 seconds, but we show it for less because ours is non-interactive
	}
}
...
export class BrowserEditorZoomSupport extends BrowserEditorContribution {
	...
	constructor(
		...
		super(editor);
		this._canZoomInContext = CONTEXT_BROWSER_CAN_ZOOM_IN.bindTo(
      contextKeyService,
    );
		this._canZoomOutContext = CONTEXT_BROWSER_CAN_ZOOM_OUT.bindTo(
      contextKeyService,
    );
