...
export class UserDataSyncDataViews extends Disposable {
	...
	private registerMachinesView(container: ViewContainer): void {
      ...
      treeView,
		...
		this._register(
      Event.any(this.userDataSyncMachinesService.onDidChange, this.userDataSyncService.onDidResetRemote)(
        () => treeView.refresh(),
      ),
    );
		const viewsRegistry = Registry.as<IViewsRegistry>(Extensions.ViewsRegistry);
...
abstract class UserDataSyncActivityViewDataProvider<T = Profile> implements ITreeViewDataProvider {
	...
	async getChildren(element?: ITreeItem): Promise<ITreeItem[]> {
		try {
            ...
            element.handle,
            promise = this.getSyncResourceHandles(<T>(<ProfileTreeItem>element).profile),
          );
				}
	...
	private async getSyncResourceHandles(profile?: T): Promise<SyncResourceHandleTreeItem[]> {
      ...
      ALL_SYNC_RESOURCES.map(async syncResource => {
		...
		const syncResourceHandles = result.flat().sort(
	...
	override async getChildren(element?: ITreeItem): Promise<ITreeItem[]> {
		if (!element) {
          ...
          "remoteActivity",
				...
				try { await this.fileService.del(this.activityDataLocation, {
          recursive: true,
        }); } catch (e) {/* ignore */ }
...
class UserDataSyncMachinesViewDataProvider implements ITreeViewDataProvider {
	...
	async getChildren(element?: ITreeItem): Promise<ITreeItem[]> {
		...
		try {
			...
			machines = machines.filter(m => !m.disabled).sort(
        (m1, m2) => m1.isCurrent ? -1 : 1,
      );
