...
export class SwitchRemoteViewItem extends Disposable {
	...
	private completedRemotes: DisposableMap<string, IRemoteSelectItem> = this._register(
    new DisposableMap(),
  );
	...
	constructor(
		...
		super();
		this.selectedRemoteContext = SELECTED_REMOTE_IN_EXPLORER.bindTo(
      contextKeyService,
    );
	...
	public setSelectionForConnection(): boolean {
        ...
        remoteAuthority.split("+")[0],
				...
				: (virtualWorkspace ? [virtualWorkspace]
					: (this.storageService.get(REMOTE_EXPLORER_TYPE_KEY, StorageScope.WORKSPACE)?.split(
              ",",
            ) ?? this.storageService.get(REMOTE_EXPLORER_TYPE_KEY, StorageScope.PROFILE)?.split(
              ",",
            )));
	...
	public removeOptionItems(views: IViewDescriptor[]) {
		for (const view of views) {
			if (view.group && view.group.startsWith("targets") && view.remoteAuthority && (!view.when || this.contextKeyService.contextMatchesRules(
        view.when,
      ))) {
				const authority = isStringArray(
          view.remoteAuthority,
        ) ? view.remoteAuthority : [view.remoteAuthority];
