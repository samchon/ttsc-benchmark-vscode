...
class TunnelTreeVirtualDelegate implements ITableVirtualDelegate<ITunnelItem> {
	...
	getHeight(row: ITunnelItem): number {
		return (row.tunnelType === TunnelType.Add && !this.remoteExplorerService.getEditableData(
      undefined,
    )) ? 30 : 22;
	}
}
...
export class TunnelViewModel implements ITunnelViewModel {
	...
	get all(): TunnelItem[] {
      ...
      this._candidates.set(
		...
		if ((this.model.forwarded.size > 0) || this.remoteExplorerService.getEditableData(
      undefined,
    )) {
	...
	isEmpty(): boolean {
		return (this.detected.length === 0) &&
			((this.forwarded.length === 0) || (this.forwarded.length === 1 &&
				(this.forwarded[0].tunnelType === TunnelType.Add) && !this.remoteExplorerService.getEditableData(
          undefined,
        )));
	}
}
...
class LocalAddressColumn implements ITableColumn<ITunnelItem, ActionBarCell> {
	...
	private static getHoverText(localAddress: string) {
		return function (configurationService: IConfigurationService) {
			const editorConf = configurationService.getValue<{ multiCursorModifier: "ctrlCmd" | "alt" }>(
        "editor",
      );
			...
			const uri = localAddress.startsWith(
        "http",
      ) ? localAddress : `http://${localAddress}`;
			return markdown.appendLink(uri, "Follow link").appendMarkdown(
        ` (${clickLabel})`,
      );
		};
	}
...
class ActionBarRenderer implements ITableRenderer<ActionBarCell, IActionBarTemplateData> {
	...
	renderElement(element: ActionBarCell, index: number, templateData: IActionBarTemplateData): void {
		...
		if (element.editId === TunnelEditId.New && (editableData = this.remoteExplorerService.getEditableData(
      undefined,
    ))) {
...
class TunnelItem implements ITunnelItem {
	...
	get processDescription(): string | undefined {
		...
		if (this.runningProcess) {
			if (this.pid && this.remoteExplorerService?.namedProcesses.has(
        this.pid,
      )) {
...
const TunnelPrivacyContextKey = new RawContextKey<TunnelPrivacyId | string | undefined>(
  "tunnelPrivacy",
  undefined,
  true,
);
...
// host:port[]
const TunnelViewMultiSelectionContextKey = new RawContextKey<string[] | undefined>(
...
export class TunnelPanel extends ViewPane {
	...
	constructor(
      ...
      hoverService,
		...
		this.tunnelCloseableContext = TunnelCloseableContextKey.bindTo(
      contextKeyService,
    );
		this.tunnelPrivacyContext = TunnelPrivacyContextKey.bindTo(
      contextKeyService,
    );
		this.tunnelPrivacyEnabledContext = TunnelPrivacyEnabledContextKey.bindTo(
      contextKeyService,
    );
		this.tunnelPrivacyEnabledContext.set(tunnelService.canChangePrivacy);
		this.protocolChangableContextKey = ProtocolChangeableContextKey.bindTo(
      contextKeyService,
    );
		this.protocolChangableContextKey.set(tunnelService.canChangeProtocol);
		this.tunnelProtocolContext = TunnelProtocolContextKey.bindTo(
      contextKeyService,
    );
		this.tunnelViewFocusContext = TunnelViewFocusContextKey.bindTo(
      contextKeyService,
    );
		this.tunnelViewSelectionContext = TunnelViewSelectionContextKey.bindTo(
      contextKeyService,
    );
		this.tunnelViewMultiSelectionContext = TunnelViewMultiSelectionContextKey.bindTo(
      contextKeyService,
    );
		this.portChangableContextKey = PortChangableContextKey.bindTo(
      contextKeyService,
    );
	...
	private createTable(): void {
      ...
      this.table.onDidBlur(() => this.tunnelViewFocusContext.set(false)),
		...
		const rerender = () => this.table?.splice(
      0,
	...
	private hasOpenLinkModifier(e: MouseEvent): boolean {
		const editorConf = this.configurationService.getValue<{ multiCursorModifier: "ctrlCmd" | "alt" }>(
      "editor",
    );
...
export class TunnelPanelDescriptor implements IViewDescriptor {
	...
	constructor(viewModel: ITunnelViewModel, environmentService: IWorkbenchEnvironmentService) {
		...
		this.remoteAuthority = environmentService.remoteAuthority ? environmentService.remoteAuthority.split(
      "+",
    )[0] : undefined;
	}
}
	...
	export function handler(): ICommandHandler {
		return async (accessor, arg): Promise<{ port: number; label: string } | undefined> => {
			...
			if (isITunnelItem(arg)) {
				...
				const context = accessor.get(IContextKeyService).getContextKeyValue<string | undefined>(
          TunnelViewSelectionKeyName,
        );
				const tunnel = context ? remoteExplorerService.tunnelModel.forwarded.get(
          context,
        ) : undefined;
				if (tunnel) {
...
namespace ClosePortAction {
	...
	export function inlineHandler(): ICommandHandler {
		return async (accessor, arg) => {
			...
			const multiSelectContext = contextKeyService.getContextKeyValue<string[] | undefined>(
        TunnelViewMultiSelectionKeyName,
      );
			if (multiSelectContext) {
				...
				const context = contextKeyService.getContextKeyValue<string | undefined>(
          TunnelViewSelectionKeyName,
        );
				const tunnel = context ? remoteExplorerService.tunnelModel.forwarded.get(
          context,
...
export namespace OpenPortInPreviewAction {
	...
	export async function run(model: TunnelModel, openerService: IOpenerService, externalOpenerService: IExternalUriOpenerService, key: string) {
        ...
        ":",
      ) ? `[${tunnel.remoteHost}]` : tunnel.remoteHost;
...
namespace CopyAddressAction {
	...
	export function inlineHandler(): ICommandHandler {
		return async (accessor, arg) => {
          ...
          TunnelViewSelectionKeyName,
				...
				tunnelItem = context ? remoteExplorerService.tunnelModel.forwarded.get(
          context,
        ) : undefined;
			}
			if (tunnelItem) {
	...
	export function commandPaletteHandler(): ICommandHandler {
		return async (accessor, arg) => {
			...
			const tunnels = Array.from(remoteExplorerService.tunnelModel.forwarded.values()).concat(
...
const isNotMultiSelectionExpr = TunnelViewMultiSelectionContextKey.isEqualTo(
  undefined,
);
