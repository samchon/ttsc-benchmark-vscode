...
export class AccessibleView extends Disposable {
	...
	constructor(
      ...
      this._contextKeyService,
		...
		this._accessibleViewSupportsNavigation = accessibleViewSupportsNavigation.bindTo(
      this._contextKeyService,
    );
		this._accessibleViewVerbosityEnabled = accessibleViewVerbosityEnabled.bindTo(
      this._contextKeyService,
    );
		this._accessibleViewGoToSymbolSupported = accessibleViewGoToSymbolSupported.bindTo(
      this._contextKeyService,
    );
		this._accessibleViewCurrentProviderId = accessibleViewCurrentProviderId.bindTo(
      this._contextKeyService,
    );
		this._accessibleViewInCodeBlock = accessibleViewInCodeBlock.bindTo(
      this._contextKeyService,
    );
		this._accessibleViewContainsCodeBlocks = accessibleViewContainsCodeBlocks.bindTo(
      this._contextKeyService,
    );
	...
	setPosition(position: Position, reveal?: boolean, select?: boolean): void {
		...
		if (select) {
			const lineLength = this._editorWidget.getModel()?.getLineLength(
        position.lineNumber,
      ) ?? 0;
			if (lineLength) {
	...
	navigateToCodeBlock(type: "next" | "previous"): void {
		...
		if (type === "previous") {
			codeBlock = codeBlocks.reverse().find(
        c => c.endLine < position.lineNumber,
      );
		} else {
	...
	private _verbosityEnabled(): boolean {
		...
		return isIAccessibleViewContentProvider(
      this._currentProvider,
	...
	goToSymbol(): void {
		...
		this._instantiationService.createInstance(AccessibleViewSymbolQuickPick, this).show(
      this._currentProvider,
    );
	}
	...
	private _updateContextKeys(provider: AccesibleViewContentProvider, shown: boolean): void {
      ...
      provider.provideNextContent !== undefined || provider.providePreviousContent !== undefined,
    );
      ...
      this._menuService.createMenu(
		...
		if (providedActions) {
			for (const providedAction of providedActions) {
				providedAction.class = providedAction.class || ThemeIcon.asClassName(
...
export class AccessibleViewService extends Disposable implements IAccessibleViewService {
	...
	getLastPosition(): Position | undefined {
		...
		return lastLine !== undefined && lastLine > 0 ? new Position(
      lastLine,
      1,
    ) : undefined;
	}
