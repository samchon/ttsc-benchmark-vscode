...
export class ShowSignalSoundHelp extends Action2 {
	...
	override async run(accessor: ServicesAccessor): Promise<void> {
      ...
      quickInputService.createQuickPick<IQuickPickItem & { signal: AccessibilitySignal }>(),
    );
		qp.items = items;
		qp.selectedItems = items.filter(
      i => accessibilitySignalService.isSoundEnabled(i.signal) || userGestureSignals.includes(i.signal) && configurationService.getValue(i.signal.settingsKey + ".sound") !== "never",
    );
...
export class ShowAccessibilityAnnouncementHelp extends Action2 {
	...
	override async run(accessor: ServicesAccessor): Promise<void> {
      ...
      quickInputService.createQuickPick<IQuickPickItem & { signal: AccessibilitySignal }>(),
		...
		qp.items = items;
		qp.selectedItems = items.filter(
      i => accessibilitySignalService.isAnnouncementEnabled(i.signal) || userGestureSignals.includes(i.signal) && configurationService.getValue(i.signal.settingsKey + ".announcement") !== "never",
    );
