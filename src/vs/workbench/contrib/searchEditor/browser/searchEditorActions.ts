	...
	async (accessor: ServicesAccessor, _args: OpenSearchEditorArgs = {}, toSide = false) => {
      ...
      activeWorkspaceRootUri,
		...
		if (activeEditorControl) {
			...
			selected = (selection && activeModel?.getModel()?.getValueInRange(
        selection,
      )) ?? "";

			if (selection?.isEmpty() && configurationService.getValue<ISearchConfigurationProperties>(
        "search",
      ).seedWithNearestWord) {
				const wordAtPosition = activeModel.getModel()?.getWordAtPosition(
		...
		const seedSearchStringFromSelection = _args.location === "new" || configurationService.getValue<IEditorOptions>(
      "editor",
      ...
      query: seedSearchStringFromSelection ? selected : undefined,
		...
		for (const entry of Object.entries(_args)) {
			...
			if (value !== undefined) {
				...
				(args as any)[name as any] = (typeof value === "string") ? await configurationResolverService.resolveAsync(
          lastActiveWorkspaceRoot,
          value,
        ) : value;
			}
		}
		const existing = editorService.getEditors(EditorsOrder.MOST_RECENTLY_ACTIVE).find(
      id => id.editor.typeId === SearchEditorInput.ID,
    );
		...
		const searchOnType = configurationService.getValue<ISearchConfigurationProperties>(
      "search",
    ).searchOnType;
		if (
...
export const createEditorFromSearchResult =
	async (accessor: ServicesAccessor, searchResult: ISearchResult, rawIncludePattern: string, rawExcludePattern: string, onlySearchInOpenEditors: boolean) => {
		...
		const configurationService = accessor.get(IConfigurationService);
		const sortOrder = configurationService.getValue<ISearchConfigurationProperties>(
      "search",
    ).sortOrder;
		...
		const labelFormatter = (uri: URI): string => labelService.getUriLabel(uri, {
      relative: true,
    });
      ...
      sortOrder,
		...
		const contextLines = configurationService.getValue<ISearchConfigurationProperties>(
      "search",
