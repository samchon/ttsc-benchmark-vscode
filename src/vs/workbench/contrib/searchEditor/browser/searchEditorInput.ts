...
export class SearchEditorInput extends EditorInput {
	...
	override matches(other: EditorInput | IUntypedEditorInput): boolean {
		...
		if (other instanceof SearchEditorInput) {
			return !!(other.modelUri.fragment && other.modelUri.fragment === this.modelUri.fragment) || !!(other.backingUri && isEqual(
        other.backingUri,
        this.backingUri,
      ));
		}
	...
	private async suggestFileName(): Promise<URI> {
		...
		const searchFileName = (query.replace(
      /[^\w \-_]+/g,
      "_",
    ) || "Search") + SEARCH_EDITOR_EXT;
		return joinPath(
...
export const getOrMakeSearchEditorInput = (
	...
	if (!searchEditorModelFactory.models.has(modelUri)) {
        ...
        accessor => searchEditorModelFactory.initializeModelFromExistingFile(
          accessor,
          modelUri,
          existingData.fileUri,
			...
			const searchEditorSettings = configurationService.getValue<ISearchConfigurationProperties>(
        "search",
      ).searchEditor;
			...
			const priorConfig = reuseOldSettings ? new Memento<{ searchConfig?: SearchConfiguration }>(SearchEditorInput.ID, storageService).getMemento(
        ...
        ...existingData.config,
			...
			if (existingData.from === "rawData") {
				...
				instantiationService.invokeFunction(
          accessor => searchEditorModelFactory.initializeModelFromRawData(
            accessor,
            modelUri,
            config,
            existingData.resultsContents,
				...
				instantiationService.invokeFunction(
          accessor => searchEditorModelFactory.initializeModelFromExistingModel(
            accessor,
            modelUri,
            config,
          ),
