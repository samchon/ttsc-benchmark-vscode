...
export class EditTrackingFeature extends Disposable {
	...
	constructor(
      ...
      reader => new Set(extensions.read(reader).map(e => e.id?.toLowerCase())),
    );
      ...
      reader => editSourceDetailsEnabled.read(reader) || !!copilotInstalled.read(reader) || !!copilotChatInstalled.read(reader),
    );

		const instantiationServiceWithInterceptedTelemetry = this._instantiationService.createChild(
