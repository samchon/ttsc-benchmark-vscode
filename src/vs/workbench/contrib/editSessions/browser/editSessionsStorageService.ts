...
export class EditSessionsWorkbenchService extends Disposable implements IEditSessionsStorageService {
	...
	constructor(
      ...
      this.authenticationService.onDidChangeSessions(
		...
		this._register(
      this.storageService.onDidChangeValue(StorageScope.APPLICATION, EditSessionsWorkbenchService.CACHED_SESSION_STORAGE_KEY, this._store)(
	...
	private async getAccountPreference(reason: "read" | "write"): Promise<AuthenticationSession & { providerId: string } | undefined> {
      ...
      this.quickInputService.createQuickPick<ExistingSession | AuthenticationProviderOption | IQuickPickItem>(
        { useSeparators: true },
      ),
    );
		quickpick.ok = false;
