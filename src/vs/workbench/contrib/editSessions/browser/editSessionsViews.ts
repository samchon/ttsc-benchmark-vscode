...
class EditSessionDataViewDataProvider implements ITreeViewDataProvider {
	...
	private async getAllEditSessions(): Promise<ITreeItem[]> {
      ...
      "editSessions",
        ...
        session.ref,
			...
			const label = content.folders.map((folder) => folder.name).join(
        ", ",
      ) ?? session.ref;
        ...
        machineId,
			...
			const description = machineName === undefined ? fromNow(
        session.created,
        true,
	...
	private async getEditSessionFolderContents(ref: string, folderName: string): Promise<ITreeItem[]> {
      ...
      ref,
		...
		const currentWorkspaceFolder = this.workspaceContextService.getWorkspace().folders.find(
