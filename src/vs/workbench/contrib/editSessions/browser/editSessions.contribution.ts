...
export class EditSessionsContribution extends Disposable implements IWorkbenchContribution {
	...
	private async autoResumeEditSession() {
      ...
      "workbench.cloudChanges.autoResume",
        ...
        async (progress) => await this.resumeEditSession(this.environmentService.editSessionId, undefined, undefined, undefined, progress).finally(() => this.environmentService.editSessionId = undefined),
      );
        ...
        "Resuming cloud changes, reason: cloud changes enabled...",
			...
			await this.progressService.withProgress(
        resumeProgressOptions,
        async (progress) => await this.resumeEditSession(undefined, true, undefined, undefined, progress),
      );
	...
	async resumeEditSession(ref?: string, silent?: boolean, forceApplyUnrelatedChange?: boolean, applyPartialMatch?: boolean, progress?: IProgress<IProgressStep>, serializedData?: string): Promise<void> {
		...
		this.logService.info(
      ref !== undefined ? `Resuming changes from cloud with ref ${ref}...` : "Checking for pending cloud changes...",
    );

		if (silent && !(await this.editSessionsStorageService.initialize(
      "read",
      true,
    ))) {
			return;
      ...
      ref: "",
        ...
        applyPartialMatch,
            ...
            decodeEditSessionFileContent(editSession.version, contents!),
				...
				} else if (type === ChangeType.Deletion && await this.fileService.exists(
          uri,
        )) {
	...
	async storeEditSession(fromStoreCommand: boolean, cancellationToken: CancellationToken): Promise<string | undefined> {
		...
		for (const repository of this.scmService.repositories) {
			...
			const trackedUris = this.getChangedResources(
        repository,
			...
			const workspaceFolder = rootUri ? this.contextService.getWorkspaceFolder(
        rootUri,
      ) : undefined;
			...
			if (ex instanceof UserDataSyncStoreError) {
				switch (ex.code) {
					case UserDataSyncErrorCode.TooLarge:
						...
						this.telemetryService.publicLog2<UploadFailedEvent, UploadFailedClassification>(
	...
	private async shouldContinueOnWithEditSession(): Promise<boolean> {
      ...
      useEditSessionsWithContinueOn,
			...
			this.telemetryService.publicLog2<EditSessionsAuthCheckEvent, EditSessionsAuthCheckClassification>(
	...
	private registerContinueInLocalFolderAction(): void {
		...
		if (getVirtualWorkspaceLocation(
	...
	private async resolveDestination(command: string): Promise<URI | "noDestinationUri" | undefined> {
		...
		try {
			...
			if (uri === undefined) {
				this.telemetryService.publicLog2<EvaluateContinueOnDestinationEvent, EvaluateContinueOnDestinationClassification>(
          "continueOn.openDestination.outcome",
          { selection: command, outcome: "noDestinationUri" },
        );
			...
			if (URI.isUri(uri)) {
				this.telemetryService.publicLog2<EvaluateContinueOnDestinationEvent, EvaluateContinueOnDestinationClassification>(
          "continueOn.openDestination.outcome",
          { selection: command, outcome: "resolvedUri" },
        );
				return uri;
			}
			...
			this.telemetryService.publicLog2<EvaluateContinueOnDestinationEvent, EvaluateContinueOnDestinationClassification>(
        "continueOn.openDestination.outcome",
        { selection: command, outcome: "invalidDestination" },
      );
			return undefined;
		} catch (ex) {
			if (ex instanceof CancellationError) {
				this.telemetryService.publicLog2<EvaluateContinueOnDestinationEvent, EvaluateContinueOnDestinationClassification>(
          "continueOn.openDestination.outcome",
          { selection: command, outcome: "cancelled" },
        );
			} else {
				this.telemetryService.publicLog2<EvaluateContinueOnDestinationEvent, EvaluateContinueOnDestinationClassification>(
          "continueOn.openDestination.outcome",
          { selection: command, outcome: "unknownError" },
        );
			}
			return undefined;
		}
	...
	private createPickItems(): (ContinueEditSessionItem | IQuickPickSeparator)[] {
      ...
      this.contextService.getWorkspace(),
		...
		const sortedItems: (ContinueEditSessionItem | IQuickPickSeparator)[] = items.sort(
