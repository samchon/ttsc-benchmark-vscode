      ...
      report: _ => this._progress.report({ increment: 100 / this._edits.length }),
		...
		for (const range of ranges) {
          ...
          await this._performFileEdits(<ResourceFileEdit[]>group, this._undoRedoGroup, this._undoRedoSource, this._confirmBeforeUndo, progress),
				...
				resources.push(
          await this._performTextEdits(<ResourceTextEdit[]>group, this._undoRedoGroup, this._undoRedoSource, progress, reason),
        );
          ...
          await this._performCellEdits(<ResourceNotebookCellEdit[]>group, this._undoRedoGroup, this._undoRedoSource, progress),
				...
				resources.push(
          await this._performOpaqueEdits(<ResourceAttachmentEdit[]>group, this._undoRedoGroup, this._undoRedoSource, progress),
        );
			} else {
...
export class BulkEditService implements IBulkEditService {
	...
	async apply(editsIn: ResourceEdit[] | WorkspaceEdit, options?: IBulkEditOptions): Promise<IBulkEditResult> {
		...
		if (this._previewHandler && (options?.showPreview || edits.some(
      value => value.metadata?.needsConfirmation,
    ))) {
      ...
      !!options?.confirmBeforeUndo,
        ...
        e => e.veto(this._shouldVeto(label, e.reason), "veto.blukEditService"),
      );
			...
			if (options?.respectAutoSaveConfig && this._configService.getValue(
        autoSaveSetting,
      ) === true && resources.length > 1) {
