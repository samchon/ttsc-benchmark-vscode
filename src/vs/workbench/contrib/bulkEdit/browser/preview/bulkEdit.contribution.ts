...
class BulkEditPreviewContribution {
	...
	constructor(
		...
		this._ctxEnabled = BulkEditPreviewContribution.ctxEnabled.bindTo(
      contextKeyService,
    );
	}
	...
	private async _previewEdit(edits: ResourceEdit[]): Promise<ResourceEdit[]> {
		...
		const uxState = this._activeSession?.uxState ?? new UXState(
...
const refactorPreviewViewIcon = registerIcon(
  ...
  Codicon.lightbulb,
  localize("refactorPreviewViewIcon", "View icon of the refactor preview view."),
);
