...
export class BulkEditPane extends ViewPane {
	...
	constructor(
      ...
      hoverService,
		...
		this._ctxHasCategories = BulkEditPane.ctxHasCategories.bindTo(
      contextKeyService,
    );
		this._ctxGroupByFile = BulkEditPane.ctxGroupByFile.bindTo(
      contextKeyService,
    );
		this._ctxHasCheckedChanges = BulkEditPane.ctxHasCheckedChanges.bindTo(
      contextKeyService,
    );
	}
	...
	protected override renderBody(parent: HTMLElement): void {
      ...
      onDidChangeVisibility: this.onDidChangeBodyVisibility,
		...
		this._treeDataSource = this._instaService.createInstance(
      BulkEditDataSource,
    );
	...
	private async _openElementInMultiDiffEditor(e: IOpenEvent<BulkEditElement | undefined>): Promise<void> {
		...
		const result = await this._computeResourceDiffEditorInputs.get(
      fileOperations,
    );
