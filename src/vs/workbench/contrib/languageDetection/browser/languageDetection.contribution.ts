...
class LanguageDetectionStatusContribution implements IWorkbenchContribution {
	...
	private async _doUpdate(): Promise<void> {
      ...
      this._renderDisposables,
		...
		const enablementConfig = this._configurationService.getValue<LanguageDetectionHintConfig>(
      "workbench.editor.languageDetectionHints",
		...
		if (disableLightbulb || !editorUri) {
			...
			const lang = await this._languageDetectionService.detectLanguage(
        editorUri,
      );
			...
			if (lang && lang !== existing && skip[existing] !== lang) {
				const detectedName = this._languageService.getLanguageName(
          lang,
        ) || lang;
