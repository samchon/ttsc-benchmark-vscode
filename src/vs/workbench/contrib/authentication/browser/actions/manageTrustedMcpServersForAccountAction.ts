...
export class ManageTrustedMcpServersForAccountAction extends Action2 {
	...
	override run(accessor: ServicesAccessor, options?: { providerId: string; accountLabel: string }): Promise<void> {
		...
		return instantiationService.createInstance(ManageTrustedMcpServersForAccountActionImpl).run(
      options,
    );
	}
}
...
class ManageTrustedMcpServersForAccountActionImpl {
	...
	async run(options?: { providerId: string; accountLabel: string }) {
      ...
      (i): i is TrustedMcpServersQuickPickItem => i.type !== "separator" && !!i.picked,
    );
		picker.show();
	}
	...
	private async _resolveAccountQuery(providerId: string | undefined, accountLabel: string | undefined): Promise<IAccountQuery | undefined> {
		if (providerId && accountLabel) {
			return this._authenticationQueryService.provider(providerId).account(
        accountLabel,
      );
		}
      ...
      matchOnDescription: true,
		...
		return pick ? this._authenticationQueryService.provider(pick.providerId).account(
      pick.label,
    ) : undefined;
	}
	...
	private async _getAllAvailableAccounts() {
		...
		for (const providerId of this._mcpServerAuthenticationService.getProviderIds()) {
			const provider = this._mcpServerAuthenticationService.getProvider(
        providerId,
      );
			const sessions = await this._mcpServerAuthenticationService.getSessions(
        providerId,
      );
	...
	private async _getItems(accountQuery: IAccountQuery) {
      ...
      this._mcpServerService.servers.get().map(s => [
        s.definition.id,
        s.definition.label,
      ]),
    );
