...
export class ManageAccountPreferencesForMcpServerAction extends Action2 {
	...
	override run(accessor: ServicesAccessor, mcpServerId?: string, providerId?: string): Promise<void> {
		return accessor.get(IInstantiationService).createInstance(ManageAccountPreferenceForMcpServerActionImpl).run(
      mcpServerId,
      providerId,
    );
	}
...
class ManageAccountPreferenceForMcpServerActionImpl {
	...
	async run(mcpServerId?: string, providerId?: string) {
		...
		const mcpServer = this._mcpService.servers.get().find(
      s => s.definition.id === mcpServerId,
    );
		if (!mcpServer) {
		...
		if (!providerId) {
			...
			const mcpServerQuery = this._authenticationQueryService.mcpServer(
        mcpServerId,
      );
	...
	private _handleNoAccounts(picker: IQuickPick<IQuickPickItem, { useSeparators: true }>): IDisposable {
      ...
      "No accounts are currently used by this MCP server.",
		...
		picker.show();
		return Event.filter(picker.onDidTriggerButton, (e) => e === this._quickInputService.backButton)(
      () => this.run(),
    );
	}
	...
	private async _accept(mcpServerId: string, selectedItems: ReadonlyArray<AccountPreferenceQuickPickItem>) {
		for (const item of selectedItems) {
			...
			const mcpQuery = this._authenticationQueryService.provider(providerId).mcpServer(
        mcpServerId,
      );
