...
export class SnippetCompletionProvider implements CompletionItemProvider {
	...
	async provideCompletionItems(model: ITextModel, position: Position, context: CompletionContext): Promise<CompletionList> {
      ...
      await this._snippets.getSnippets(languageId, model.uri),
		...
		for (const snippet of snippets) {
			...
			if (context.triggerKind === CompletionTriggerKind.TriggerCharacter && !snippet.prefixLow.startsWith(
        triggerCharacterLow,
      )) {
    ...
    ) /*end in whitespace */ || !lineContentLow /*empty line*/)) {
			for (const snippet of snippets) {
				const insert = Range.fromPositions(position);
				const replace = lineContentLow.indexOf(
          snippet.prefixLow,
          columnOffset,
        ) === columnOffset ? insert.setEndPosition(
          position.lineNumber,
