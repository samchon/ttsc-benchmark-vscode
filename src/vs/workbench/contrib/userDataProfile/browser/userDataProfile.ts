...
export class UserDataProfilesWorkbenchContribution extends Disposable implements IWorkbenchContribution {
	...
	constructor(
		...
		this.currentProfileContext = CURRENT_PROFILE_CONTEXT.bindTo(
      contextKeyService,
    );
	...
	private async reportWorkspaceProfileInfo(): Promise<void> {
		...
		const count = this.userDataProfilesService.profiles.filter(
      p => !p.isInternal,
    ).length - 1;
		if (count > 0) {
			this.telemetryService.publicLog2<UserProfilesCountEvent, UserProfilesCountClassification>(
        "profiles:count",
        { count },
      );
		}
      ...
      this.workspaceContextService.getWorkbenchState(),
		...
		this.telemetryService.publicLog2<WorkspaceProfileInfoEvent, WorkspaceProfileInfoClassification>(
