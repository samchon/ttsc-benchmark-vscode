...
export class WebviewEditor extends EditorPane {
	...
	private claimWebview(input: WebviewInput): void {
		...
		const isModal = isHTMLElement(
      modalEditorContainer,
    ) && this._element && modalEditorContainer.contains(this._element);
		this._clippingContainer = isModal ? undefined : this._workbenchLayoutService.getContainer(
      this.window,
      Parts.EDITOR_PART,
    );
