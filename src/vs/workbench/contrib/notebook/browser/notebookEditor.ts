...
export class NotebookEditor extends EditorPane implements INotebookEditorPane, IEditorPaneWithScrolling {
	...
	override hasFocus(): boolean {
      ...
      value.getDomNode() || DOM.isAncestorOfActiveElement(value.getOverflowContainerDomNode()),
    ));
	}
	...
	override async setInput(input: NotebookEditorInput, options: INotebookEditorOptions | undefined, context: IEditorOpenContext, token: CancellationToken, noRetry?: boolean): Promise<void> {
        ...
        () => this._onDidChangeInputCapabilities(input),
			...
			this._widget = <IBorrowValue<NotebookEditorWidget>>this._instantiationService.invokeFunction(
        ...
        model.notebook.onDidChangeContent(
          () => this._onDidChangeSelection.fire({
			...
			const viewState = options?.viewState ?? this._loadNotebookEditorViewState(
        input,
      );
	...
	private _loadNotebookEditorViewState(input: NotebookEditorInput): INotebookEditorViewState | undefined {
      ...
      GroupsOrder.MOST_RECENTLY_ACTIVE,
			...
			if (group.activeEditorPane !== this && group.activeEditorPane instanceof NotebookEditor && group.activeEditor?.matches(
        input,
      )) {
