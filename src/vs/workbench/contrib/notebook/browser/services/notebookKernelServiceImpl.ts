...
export class NotebookKernelService extends Disposable implements INotebookKernelService {
	...
	constructor(
		...
		this._register(
      _notebookService.onDidAddNotebookDocument(this._tryAutoBindNotebook, this),
    );
	...
	private _tryAutoBindNotebook(notebook: INotebookTextModel, onlyThisKernel?: INotebookKernel): void {
      ...
      NotebookTextModelLikeId.str(notebook),
		...
		if (!existingKernel || !NotebookKernelService._score(
	...
	getMatchingKernel(notebook: INotebookTextModelLike): INotebookKernelMatchResult {
      ...
      NotebookTextModelLikeId.str(notebook),
		...
		const selected = selectedId ? this._kernels.get(
      selectedId,
    )?.kernel : undefined;
		const suggestions = kernels.filter(item => item.instanceAffinity > 1).map(
      item => item.kernel,
    );
		const hidden = kernels.filter(item => item.instanceAffinity < 0).map(
      item => item.kernel,
    );
	...
	getRunningSourceActions(notebook: INotebookTextModelLike) {
		...
		if (existingInfo) {
			return existingInfo.actions.filter(action => action[0].execution).map(
        action => action[0],
      );
		}
