...
export class NotebookKernelHistoryService extends Disposable implements INotebookKernelHistoryService {
	...
	constructor(@IStorageService private readonly _storageService: IStorageService,
      ...
      this._storageService.onWillSaveState(() => this._saveState()),
		...
		this._register(
      this._storageService.onDidChangeValue(StorageScope.WORKSPACE, NotebookKernelHistoryService.STORAGE_KEY, this._store)(
        () => {
          this._loadState();
        },
      ),
	...
	getKernels(notebook: INotebookTextModelLike): { selected: INotebookKernel | undefined; all: INotebookKernel[] } {
		const allAvailableKernels = this._notebookKernelService.getMatchingKernel(
      notebook,
    );
      ...
      ...this._mostRecentKernelsMap[notebook.notebookType].values(),
		...
		const all = mostRecentKernelIds.map(kernelId => allKernels.find(kernel => kernel.id === kernelId)).filter(
      kernel => !!kernel,
    ) as INotebookKernel[];
		this._notebookLoggingService.debug(
      "History",
