...
export class DiffEditorHeightCalculatorService {
	...
	public async diffAndComputeHeight(original: URI, modified: URI): Promise<number> {
      ...
      this.textModelResolverService.createModelReference(modified),
        ...
        0,
			...
			return (visibleLineCount * this.lineHeight) + getEditorPadding(
        visibleLineCount,
      ).top + getEditorPadding(visibleLineCount).bottom + unchangeRegionsHeight;
		} finally {
	...
	public computeHeightFromLines(lineCount: number): number {
		return lineCount * this.lineHeight + getEditorPadding(
      lineCount,
