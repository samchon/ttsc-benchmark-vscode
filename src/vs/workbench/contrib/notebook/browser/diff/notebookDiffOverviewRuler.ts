...
export class NotebookDiffOverviewRuler extends Themable {
	...
	private _layoutNow() {
		...
		const contentHeight = this._diffElementViewModels.map(view => view.totalHeight).reduce(
      (a, b) => a + b,
      0,
    );
