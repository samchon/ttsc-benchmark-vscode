...
export class NotebookDocumentMetadataViewModel extends DiffElementViewModelBase {
	...
	getHeight(lineHeight: number) {
		if (this._layoutInfo.layoutState === CellLayoutState.Uninitialized) {
			const editorHeight = this.cellFoldingState === PropertyFoldingState.Collapsed ? 0 : this.computeInputEditorHeight(
        lineHeight,
      );
...
export abstract class DiffElementCellViewModelBase extends DiffElementViewModelBase {
			...
			case "insert":
				{
					...
					const editorHeight = lineCount * lineHeight + getEditorPadding(
            lineCount,
          ).top + getEditorPadding(lineCount).bottom;
					return editorHeight;
				}
			case "delete":
			case "modified":
	...
	protected _layout(delta: ILayoutInfoDelta) {
		...
		const outputHeight = this.ignoreOutputs ? 0 : (delta.recomputeOutput || delta.rawOutputHeight !== undefined || delta.outputMetadataHeight !== undefined) ? this._getOutputTotalHeight(
      rawOutputHeight,
