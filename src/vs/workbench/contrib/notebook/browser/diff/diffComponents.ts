...
class PropertyHeader extends Disposable {
	...
	buildHeader(): void {
      ...
      this.menuService.createMenu(this.accessor.menuId, scopedContextKeyService),
    );
...
export class NotebookDocumentMetadataElement extends Disposable {
	...
	updateSourceEditor(): void {
		...
		const updateSourceEditor = () => {
			...
			const editorHeight = this.viewModel.layoutInfo.editorHeight !== 0 ? this.viewModel.layoutInfo.editorHeight : this.viewModel.computeInputEditorHeight(
        lineHeight,
      );
	...
	private updateEditorOptionsForWhitespace() {
		...
		const ignoreTrimWhitespace = this.textConfigurationService.getValue<boolean>(
      uri,
...
abstract class AbstractElementRenderer extends Disposable {
	...
	buildBody(): void {
      ...
      "notebook.diff.ignoreMetadata",
		...
		this._ignoreOutputs = this.configurationService.getValue<boolean>(
...
export class ModifiedElement extends AbstractElementRenderer {
	...
	updateSourceEditor(): void {
		...
		const renderSourceEditor = () => {
			...
			const lineHeight = this.notebookEditor.getLayoutInfo().fontInfo.lineHeight || 17;
			const editorHeight = this.cell.layoutInfo.editorHeight !== 0 ? this.cell.layoutInfo.editorHeight : this.cell.computeInputEditorHeight(
        lineHeight,
      );
