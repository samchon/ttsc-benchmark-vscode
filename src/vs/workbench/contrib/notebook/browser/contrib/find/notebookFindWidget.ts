...
class NotebookFindWidget extends SimpleFindReplaceWidget implements INotebookEditorContribution {
	...
	constructor(
		...
		const findSearchHistory = FindWidgetSearchHistory.getOrCreate(
      storageService,
    );
	...
	protected override _updateMatchesCount(): void {
		...
		if (this._state.matchesCount > 0) {
			...
			const matchesPosition: string = this._findModel.currentMatch < 0 ? "?" : String(
        (this._findModel.currentMatch + 1),
      );
