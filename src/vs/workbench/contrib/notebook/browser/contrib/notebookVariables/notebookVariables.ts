...
export class NotebookVariables extends Disposable implements IWorkbenchContribution {
	...
	private handleInitEvent(notebook?: URI) {
        ...
        notebook,
      ) && !this.initialized && this.initializeView()) {
	...
	private hasVariableProvider(notebookUri?: URI) {
		...
		return notebook && this.notebookKernelService.getMatchingKernel(
      notebook,
    ).selected?.hasVariableProvider;
	}

	private initializeView() {
		const debugViewContainer = this.viewDescriptorService.getViewContainerById(
      debugContainerId,
    );
