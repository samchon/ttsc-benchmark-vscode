...
async function runCell(editorGroupsService: IEditorGroupsService, context: INotebookActionContext, editorService?: IEditorService): Promise<void> {
	...
	if (context.ui && context.cell) {
      ...
      Iterable.single(context.cell),
		...
		const selectedCells = context.selectedCells?.length ? context.selectedCells : [
      context.cell!,
    ];
...
function handleAutoReveal(cell: ICellViewModel, notebookEditor: IActiveNotebookEditor): void {
	...
	const cellOutputScrollBottom = notebookEditor.getAbsoluteBottomOfElement(
    cell,
  );
	...
	const revealWithTopPadding = (position: number) => { notebookEditor.setScrollTop(
    position - SMART_VIEWPORT_TOP_REVEAL_PADDING,
  ); };
	const revealWithNoPadding = (position: number) => { notebookEditor.setScrollTop(
    position,
  ); };
	const revealWithBottomPadding = (position: number) => { notebookEditor.setScrollTop(
