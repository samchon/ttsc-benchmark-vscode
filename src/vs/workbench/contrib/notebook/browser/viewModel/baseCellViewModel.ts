...
export abstract class BaseCellViewModel extends Disposable {
	...
	constructor(
      ...
      model.onDidChangeMetadata(() => {
		...
		this._commentOptions = this._configurationService.getValue<IEditorCommentsOptions>(
      "editor.comments",
	...
	getCellDecorationRange(decorationId: string): Range | null {
		if (this._textEditor) {
			...
			return this._textEditor.getModel()?.getDecorationRange(
        decorationId,
      ) ?? null;
		}
	...
	protected cellStartFind(value: string, options: INotebookFindOptions): model.FindMatch[] | null {
      ...
      new Range(1, 1, lineCount, this.textBuffer.getLineLength(lineCount) + 1),
        ...
        cellMatches.push(
          ...this.textBuffer.findMatchesLineByLine(new Range(range.startLineNumber, range.startColumn, range.endLineNumber, range.endColumn), searchData, options.regex || false, 1000),
        );
      });
		}
	...
	override dispose() {
		...
		if (this._undoRedoService.getUriComparisonKey(
      this.uri,
    ) === this.uri.toString()) {
