...
class NotebookDiffEditorSerializer implements IEditorSerializer {
	...
	deserialize(instantiationService: IInstantiationService, raw: string) {
		...
		if (!data || !URI.isUri(resource) || !URI.isUri(
      originalResource,
    ) || typeof name !== "string" || typeof viewType !== "string") {
			return undefined;
		}
	...
	async provideTextContent(resource: URI): Promise<ITextModel | null> {
		...
		for (const cell of ref.object.notebook.cells) {
          ...
          cell.language,
				...
				const languageSelection = languageId ? this._languageService.createById(
          languageId,
        ) : (cell.cellKind === CellKind.Markup ? this._languageService.createById(
          "markdown",
...
class CellInfoContentProvider {
	...
	async provideOutputTextContent(resource: URI): Promise<ITextModel | null> {
		...
		const ref = await this._notebookModelResolverService.resolve(data.notebook);
		const cell = ref.object.notebook.cells.find(
      cell => !!cell.outputs.find(op => op.outputId === data.outputId || op.alternativeOutputId === data.outputId),
    );
...
class SimpleNotebookWorkingCopyEditorHandler extends Disposable implements IWorkbenchContribution, IWorkingCopyEditorHandler {
	...
	isOpen(workingCopy: IWorkingCopyIdentifier, editor: EditorInput): boolean {
		...
		return editor instanceof NotebookEditorInput && editor.viewType === this._getViewType(
      workingCopy,
