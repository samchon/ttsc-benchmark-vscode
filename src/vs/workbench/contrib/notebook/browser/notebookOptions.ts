...
export class NotebookOptions extends Disposable {
	...
	readonly previousModelToCompare = observableValue<NotebookTextModel | undefined>(
    "previousModelToCompare",
    undefined,
  );
	...
	constructor(
		...
		super();
		const showCellStatusBar = this.configurationService.getValue<ShowCellStatusBarType>(
      NotebookSetting.showCellStatusBar,
    );
		const globalToolbar = overrides?.globalToolbar ?? this.configurationService.getValue<boolean | undefined>(
      NotebookSetting.globalToolbar,
    ) ?? true;
		const stickyScrollEnabled = overrides?.stickyScrollEnabled ?? this.configurationService.getValue<boolean | undefined>(
      NotebookSetting.stickyScrollEnabled,
    ) ?? false;
		const stickyScrollMode = this._computeStickyScrollModeOption();
		const consolidatedOutputButton = this.configurationService.getValue<boolean | undefined>(
      NotebookSetting.consolidatedOutputButton,
    ) ?? true;
		const consolidatedRunButton = this.configurationService.getValue<boolean | undefined>(
      NotebookSetting.consolidatedRunButton,
    ) ?? false;
		const dragAndDropEnabled = overrides?.dragAndDropEnabled ?? this.configurationService.getValue<boolean | undefined>(
      NotebookSetting.dragAndDropEnabled,
    ) ?? true;
		const cellToolbarLocation = this.configurationService.getValue<string | { [key: string]: string }>(
      NotebookSetting.cellToolbarLocation,
    ) ?? {
      "default": "right",
    };
		const cellToolbarInteraction = overrides?.cellToolbarInteraction ?? this.configurationService.getValue<string>(
      NotebookSetting.cellToolbarVisibility,
    );
		const compactView = this.configurationService.getValue<boolean | undefined>(
      ...
      this.isReadonly,
		...
		const fontSize = this.configurationService.getValue<number>(
      "editor.fontSize",
    );
      ...
      NotebookSetting.markdownLineHeight,
		...
		editorOptionsCustomizations = isObject(
      editorOptionsCustomizations,
    ) ? editorOptionsCustomizations : {};
		const interactiveWindowCollapseCodeCells: InteractiveWindowCollapseCodeCells = this.configurationService.getValue(
      NotebookSetting.interactiveWindowCollapseCodeCells,
		...
		const deprecatedOutputLineHeightSetting = this.configurationService.getValue<number>(
      NotebookSetting.outputLineHeightDeprecated,
		...
		const deprecatedOutputFontSizeSetting = this.configurationService.getValue<number>(
      NotebookSetting.outputFontSizeDeprecated,
        ...
        NotebookSetting.outputFontSize,
			...
			outputFontSize = this.configurationService.getValue<number>(
        NotebookSetting.outputFontSize,
      ) || fontSize;
		}

		let outputFontFamily: string;
		const deprecatedOutputFontFamilySetting = this.configurationService.getValue<string>(
      NotebookSetting.outputFontFamilyDeprecated,
		...
		const deprecatedOutputScrollingSetting = this.configurationService.getValue<boolean>(
      ...
      NotebookSetting.outputWordWrap,
		...
		const outputLineLimit = this.configurationService.getValue<number>(
      NotebookSetting.textOutputLineLimit,
    ) ?? 30;
		const linkifyFilePaths = this.configurationService.getValue<boolean>(
	...
	private _computeEditorTopPadding(): number {
          ...
          true,
				...
				if (options.afterContentClassName || options.beforeContentClassName) {
                  ...
                  "::after",
								...
								const editorOptions = this.configurationService.getValue<IEditorOptions>(
                  "editor",
                );
								updateEditorTopPadding(
                  createBareFontInfoFromRawSettings(editorOptions, PixelRatio.getInstance(this.targetWindow).value).lineHeight + 2,
                );
      ...
      this.codeEditorService.onDecorationTypeRegistered(onDidAddDecorationType),
		...
		this.codeEditorService.listDecorationTypes().forEach(
	...
	private _computeOutputLineHeight(lineHeight: number, outputFontSize: number): number {
        ...
        createBareFontInfoFromRawSettings(
          ...
          PixelRatio.getInstance(this.targetWindow).value,
			...
			if (fontSize === 0) {
				fontSize = this.configurationService.getValue<number>(
          "editor.fontSize",
        );
			}
	...
	private _updateConfiguration(e: IConfigurationChangeEvent) {
      ...
      NotebookSetting.markupFontFamily,
		...
		if (cellStatusBarVisibility) {
			configuration.showCellStatusBar = this.configurationService.getValue<ShowCellStatusBarType>(
        NotebookSetting.showCellStatusBar,
      );
		}

		if (cellToolbarLocation) {
			configuration.cellToolbarLocation = this.configurationService.getValue<string | { [key: string]: string }>(
        NotebookSetting.cellToolbarLocation,
      ) ?? {
        "default": "right",
      };
		...
		if (cellToolbarInteraction && !this.overrides?.cellToolbarInteraction) {
			configuration.cellToolbarInteraction = this.configurationService.getValue<string>(
        NotebookSetting.cellToolbarVisibility,
      );
		}
		...
		if (compactView) {
			const compactViewValue = this.configurationService.getValue<boolean | undefined>(
        NotebookSetting.compactView,
      ) ?? true;
		...
		if (globalToolbar && this.overrides?.globalToolbar === undefined) {
			configuration.globalToolbar = this.configurationService.getValue<boolean>(
        NotebookSetting.globalToolbar,
      ) ?? true;
		}

		if (stickyScrollEnabled && this.overrides?.stickyScrollEnabled === undefined) {
			configuration.stickyScrollEnabled = this.configurationService.getValue<boolean>(
        NotebookSetting.stickyScrollEnabled,
      ) ?? false;
		}

		if (stickyScrollMode) {
			configuration.stickyScrollMode = this.configurationService.getValue<"flat" | "indented">(
        NotebookSetting.stickyScrollMode,
      ) ?? "flat";
		}

		if (consolidatedOutputButton) {
			configuration.consolidatedOutputButton = this.configurationService.getValue<boolean>(
        NotebookSetting.consolidatedOutputButton,
      ) ?? true;
		}

		if (consolidatedRunButton) {
			configuration.consolidatedRunButton = this.configurationService.getValue<boolean>(
        NotebookSetting.consolidatedRunButton,
      ) ?? true;
		}
		...
		if (dragAndDropEnabled) {
			configuration.dragAndDropEnabled = this.configurationService.getValue<boolean>(
        NotebookSetting.dragAndDropEnabled,
      ) ?? true;
		}

		if (fontSize) {
			configuration.fontSize = this.configurationService.getValue<number>(
        "editor.fontSize",
      );
		}

		if (outputFontSize || fontSize) {
			configuration.outputFontSize = this.configurationService.getValue<number>(
        NotebookSetting.outputFontSize,
		...
		if (markdownLineHeight) {
			configuration.markdownLineHeight = this.configurationService.getValue<number>(
        NotebookSetting.markdownLineHeight,
      );
		}

		if (outputFontFamily) {
			configuration.outputFontFamily = this.configurationService.getValue<string>(
        NotebookSetting.outputFontFamily,
      );
		}
		...
		if (outputWordWrap) {
			configuration.outputWordWrap = this.configurationService.getValue<boolean>(
        NotebookSetting.outputWordWrap,
      );
		}

		if (outputScrolling) {
			configuration.outputScrolling = this.configurationService.getValue<boolean>(
        NotebookSetting.outputScrolling,
      );
		}

		if (outputLinkifyFilePaths) {
			configuration.outputLinkifyFilePaths = this.configurationService.getValue<boolean>(
        NotebookSetting.LinkifyOutputFilePaths,
      );
		}

		if (minimalError) {
			configuration.outputMinimalError = this.configurationService.getValue<boolean>(
        NotebookSetting.minimalErrorRendering,
      );
		}

		if (markupFontFamily) {
			configuration.markupFontFamily = this.configurationService.getValue<string>(
        NotebookSetting.markupFontFamily,
      );
		}
	...
	private _computeInsertToolbarPositionOption(isReadOnly: boolean) {
		return isReadOnly ? "hidden" : this.configurationService.getValue<"betweenCells" | "notebookToolbar" | "both" | "hidden">(
      NotebookSetting.insertToolbarLocation,
    ) ?? "both";
	}

	private _computeInsertToolbarAlignmentOption() {
		return this.configurationService.getValue<"left" | "center">(
      NotebookSetting.experimentalInsertToolbarAlignment,
    ) ?? "center";
	}

	private _computeShowFoldingControlsOption() {
		return this.configurationService.getValue<"always" | "never" | "mouseover">(
      NotebookSetting.showFoldingControls,
    ) ?? "mouseover";
	}

	private _computeFocusIndicatorOption() {
		return this.configurationService.getValue<"border" | "gutter">(
      NotebookSetting.focusIndicator,
    ) ?? "gutter";
	}

	private _computeStickyScrollModeOption() {
		return this.configurationService.getValue<"flat" | "indented">(
      NotebookSetting.stickyScrollMode,
    ) ?? "flat";
	}
	...
	computeBottomToolbarOffset(totalHeight: number, viewType: string) {
		const { bottomToolbarGap, bottomToolbarHeight } = this.computeBottomToolbarDimensions(
      viewType,
    );
