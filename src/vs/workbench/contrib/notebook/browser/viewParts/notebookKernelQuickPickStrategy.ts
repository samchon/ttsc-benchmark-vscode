...
abstract class KernelPickerStrategyBase implements IKernelPickerStrategy {
	...
	async showQuickPick(editor: IActiveNotebookEditor, wantedId?: string, skipAutoRun?: boolean): Promise<boolean> {
      ...
      scopedContextKeyService,
		...
		if (quickPickItems.length === 1 && supportAutoRun(
      quickPickItems[0],
    ) && !skipAutoRun) {
	...
	protected async _showKernelExtension(
        ...
        e => areSameExtensions(e.identifier, extension.identifier),
				...
				const canInstall = await extensionWorkbenchService.canInstall(
          extension,
        );
				if (canInstall === true) {
	...
	protected async _getKernelRecommendationsQuickPickItems(
		...
		const suggestedExtension: INotebookExtensionRecommendation | undefined = language ? this.getSuggestedKernelFromLanguage(
	...
	private getSuggestedLanguage(notebookTextModel: NotebookTextModel): string | undefined {
		...
		if (!suggestedKernelLanguage) {
			const cellLanguages = notebookTextModel.cells.map(cell => cell.language).filter(
...
export class KernelPickerMRUStrategy extends KernelPickerStrategyBase {
	...
	protected override _getMatchingResult(notebook: NotebookTextModel): INotebookKernelMatchResult {
		const { selected, all } = this._notebookKernelHistoryService.getKernels(
      notebook,
    );
		const matchingResult = this._notebookKernelService.getMatchingKernel(
      notebook,
    );
		return {
      selected: selected,
      ...
      this._quickInputService.createQuickPick<KernelQuickPickItem>({
		...
		if (quickPickItem) {
            ...
            selectedKernelPickItem.command,
              ...
              kernel => kernel.id === `ms-toolsai.jupyter/${selectedKernelId}`,
            );
						if (kernel) {
	...
	private async _calculdateKernelSources(editor: IActiveNotebookEditor) {
      ...
      editor.scopedContextKeyService,
		...
		const actions = await this._notebookKernelService.getKernelSourceActions2(
      notebook,
    );
		const matchResult = this._getMatchingResult(notebook);
	...
	private async _executeCommand<T>(notebook: NotebookTextModel, command: string | Command): Promise<T | undefined | void> {
		...
		if (typeof command === "string" || !command.arguments || !Array.isArray(
      command.arguments,
	...
	static updateKernelStatusAction(notebook: NotebookTextModel, action: IAction, notebookKernelService: INotebookKernelService, notebookKernelHistoryService: INotebookKernelHistoryService) {
		const detectionTasks = notebookKernelService.getKernelDetectionTasks(
      notebook,
    );
		...
		const runningActions = notebookKernelService.getRunningSourceActions(
      notebook,
    );
