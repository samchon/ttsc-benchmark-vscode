...
export class NotebookEditorWidget extends Disposable implements INotebookEditorDelegate, INotebookEditor {
	...
	constructor(
      ...
      "editor.scrollBeyondLastLine",
		...
		const container = creationOptions.codeWindow ? this.layoutService.getContainer(
      creationOptions.codeWindow,
      ...
      this.scopedContextKeyService,
		...
		new RawContextKey<boolean>(PreventDefaultContextMenuItemsContextKeyName, false).bindTo(this.scopedContextKeyService).set(
      true,
    );
	...
	private _createCellList(): void {
      ...
      new CellDragAndDropController(this, this._body),
		...
		const getScopedContextKeyService = (container: HTMLElement) => this._list.contextKeyService.createScoped(
      container,
    );
	...
	async setOptions(options: INotebookEditorOptions | undefined) {
		...
		const cellOptions = options?.cellOptions ?? this._parseIndexedCellOptions(
      options,
    );
		if (cellOptions) {
	...
	private async _warmupViewportMarkdownCells(viewModel: NotebookViewModel, viewState: INotebookEditorViewState | undefined) {
		if (viewState && viewState.cellTotalHeights) {
			...
			const scrollBottom = scrollTop + Math.max(
        ...
        requests.map(
          ([model, offset]) => this.createMarkupCellInitialization(
            model,
            offset,
          ),
        ),
	...
	private getBodyHeight(dimensionHeight: number) {
		return Math.max(
      dimensionHeight - (this._notebookTopToolbar?.useGlobalToolbar ? /** Toolbar height */ 26 : 0),
      0,
    );
	}
	...
	private layoutNotebook(dimension: DOM.Dimension, shadowElement?: HTMLElement) {
		...
		this._dimension = dimension;
		const newBodyHeight = this.getBodyHeight(
      dimension.height,
	...
	private layoutContainerOverShadowElement(anchorElement: HTMLElement | undefined): void {
		...
		const isModal = DOM.isHTMLElement(
      modalEditorContainer,
    ) && modalEditorContainer.contains(anchorElement);
		const clippingContainer = isModal ? undefined : this.layoutService.getContainer(
      DOM.getWindow(this.getDomNode()),
      Parts.EDITOR_PART,
    );
	...
	hasOutputTextSelection() {
		...
		while (container
			...
			if ((container as HTMLElement).classList && (container as HTMLElement).classList.contains(
        "output",
      )) {
				return true;
			}
	...
	async focusNotebookCell(cell: ICellViewModel, focusItem: "editor" | "container" | "output", options?: IFocusNotebookCellOptions) {
        ...
        options?.outputWebviewFocused || this._webviewFocused,
			...
			if (itemDOM && itemDOM.ownerDocument.activeElement && itemDOM.contains(
        itemDOM.ownerDocument.activeElement,
      )) {
	...
	private async _warmupAll(includeOutput: boolean) {
		...
		for (let i = 0; i < cells.length; i++) {
			if (cells[i].cellKind === CellKind.Markup && !this._webview!.markupPreviewMapping.has(
        cells[i].id,
      )) {
	...
	async find(query: string, options: INotebookFindOptions, token: CancellationToken, skipWarmup: boolean = false, shouldGetSearchPreviewInfo = false, ownerID?: string): Promise<CellFindMatchWithIndex[]> {
		...
		const findMatches = this._notebookViewModel.find(query, options).filter(
