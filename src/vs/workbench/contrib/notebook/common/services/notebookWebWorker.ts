...
export class NotebookWorker implements IWebWorkerServerRequestHandler, IDisposable {
	...
	async $computeDiff(originalUrl: string, modifiedUrl: string): Promise<INotebookDiffResult> {
      ...
      key => !modified.transientDocumentMetadata[key],
		...
		const originalDiff = new LcsDiff(CellSequence.create(original), CellSequence.create(modified)).ComputeDiff(
      false,
    );
      ...
      metadataChanged: false,
		...
		if (cellMapping.every(
      c => c.type === "modified" || c.type === "unchanged",
    )) {
			return {
        metadataChanged,
        cellsDiff: originalDiff,
		...
		const cellsInsertedOrDeletedDiff = new LcsDiff(CellSequence.createWithCellId(original.cells), CellSequence.createWithCellId(modified.cells)).ComputeDiff(
      false,
    );
	...
	canComputeDiffWithCellIds(original: MirrorNotebookDocument, modified: MirrorNotebookDocument): boolean {
		return this.canComputeDiffWithCellInternalIds(
      original,
      modified,
	...
	isOriginalCellMatchedWithModifiedCell(originalCell: MirrorCell) {
		return (originalCell.internalMetadata?.internalId as string || "").startsWith(
      PREFIX_FOR_UNMATCHED_ORIGINAL_CELLS,
    );
	}
