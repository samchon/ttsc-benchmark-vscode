...
export class TestingPeekOpener extends Disposable implements ITestingPeekOpener {
	...
	public peekUri(uri: URI, options: IShowResultOptions = {}) {
		...
		const message = result.getStateById(
      parsed.testExtId,
    )?.tasks[parsed.taskIndex].messages[parsed.messageIndex];
		if (!message?.location) {
			return false;
		}
      ...
      selection: message.location.range,
      ...options.options,
    });
		return true;
	}
	...
	public openCurrentInEditor(): void {
        ...
        options,
			...
			this.commandService.executeCommand("markdown.showPreview", current.messageUri).catch(
        err => {
	...
	private getActiveControl(): InspectSubject | undefined {
		...
		return controller?.subject.get() ?? this.viewsService.getActiveViewWithId<TestResultsView>(
	...
	private openPeekOnFailure(evt: TestResultItemChange) {
      ...
      TestingConfigKeys.AutoOpenPeekView,
		...
		switch (cfg) {
          ...
          visibleEditors.filter(isCodeEditor).map(
            e => e.getModel()?.uri.toString(),
          ),
        );
				if (!Iterable.some(
...
export class TestingOutputPeekController extends Disposable implements IEditorContribution {
	 ...
	 */
	public readonly subject = derived(
	...
	private closePeekOnCertainResultEvents(evt: ResultChangeEvent) {
        ...
        undefined,
        undefined,
      ); // close the peek if results are cleared
		}
	}
...
class TestResultsPeek extends PeekViewWidget {
	...
	private applyTheme() {
		...
		const borderColor = (isError ? theme.getColor(
      testingPeekBorder,
    ) : theme.getColor(testingMessagePeekBorder)) || Color.transparent;
		const headerBg = (isError ? theme.getColor(
      testingPeekHeaderBackground,
    ) : theme.getColor(
      testingPeekMessageHeaderBackground,
    )) || Color.transparent;
	...
	protected override _fillContainer(container: HTMLElement): void {
        ...
        this.contextKeyService.createScoped(container),
			...
			TestingContextKeys.isInPeek.bindTo(this.scopedContextKeyService).set(
        true,
      );
	...
	public setModel(subject: InspectSubject): Promise<void> {
      ...
      revealLocation,
      10,
    ); // 10 is just a random number, we resize once content is available
