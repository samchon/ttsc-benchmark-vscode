...
export const percent = (cc: ICoverageCount) => clamp(
  cc.total === 0 ? 1 : cc.covered / cc.total,
  0,
  1,
);
...
export const calculateDisplayedStat = (coverage: CoverageBarSource, method: TestingDisplayedCoveragePercent) => {
	switch (method) {
		...
		case TestingDisplayedCoveragePercent.Minimum: {
			...
			if (coverage.branch) { value = Math.min(
        value,
        percent(coverage.branch),
      ); }
			if (coverage.declaration) { value = Math.min(
        value,
        percent(coverage.declaration),
      ); }
			return value;
		}
...
export namespace labels {
	export const showingFilterFor = (label: string) => localize(
	...
	export const percentCoverage = (percent: number, precision?: number) => localize(
