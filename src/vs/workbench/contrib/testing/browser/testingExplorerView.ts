...
export class TestingExplorerView extends ViewPane {
	...
	constructor(
      ...
      new RunOnceScheduler(() => this.layoutBody(), 1),
		...
		this._register(
      Event.any(crService.onDidChange, testProfileService.onDidChange)(() => {
        this.updateActions();
      }),
    );
	...
	public getTreeIncludeExclude(profileOrBitset: ITestRunProfile | TestRunProfileBitset, withinItems?: InternalTestItem[], filterToType: "visible" | "selected" = "visible") {
		...
		const attempt = (element: TestExplorerTreeElement, alreadyIncluded: boolean) => {
			...
			if (!(element instanceof TestItemTreeElement) || !this.viewModel.tree.hasElement(
        element,
      )) {
				return;
			}
		...
		for (const root of withinItems || this.testService.collection.rootItems) {
			...
			if (typeof profileOrBitset === "object" && !canUseProfileWithTest(
        profileOrBitset,
        root,
      )) {
				continue;
...
class ResultSummaryView extends Disposable {
	...
	private render() {
        ...
        icons.testingStatesToIcons.get(dominantState ?? TestResultState.Unset)!,
			...
			duration.textContent = last instanceof LiveTestResult ? formatDuration(
	...
	private renderActivityBadge(countSummary: CountSummary, isRunning: boolean) {
		...
		this.badgeDisposable.value = this.lastBadge && this.activityService.showViewActivity(
...
class TestingExplorerViewModel extends Disposable {
	...
	constructor(
      ...
      new StoredValue<ISerializedTestTreeCollapseState>(
		...
		this._viewSorting = TestingContextKeys.viewSorting.bindTo(
      contextKeyService,
    );
		this._viewMode.set(
      this.storageService.get("testing.viewMode", StorageScope.WORKSPACE, TestExplorerViewMode.Tree) as TestExplorerViewMode,
    );
		this._viewSorting.set(
      this.storageService.get("testing.viewSorting", StorageScope.WORKSPACE, TestExplorerViewSorting.ByLocation) as TestExplorerViewSorting,
    );
      ...
      () => new Set(
        editorGroupsService.groups.flatMap(g => g.editors).map(e => e.resource).filter(
          isDefined,
        ),
      ),
    );
	...
	private tryPeekError(item: TestItemTreeElement) {
		const lookup = item.test && this.testResults.getStateById(
      item.test.item.extId,
    );
	...
	private reevaluateWelcomeState() {
		const shouldShowWelcome = this.testService.collection.busyProviders === 0 && testCollectionIsEmpty(
      this.testService.collection,
        ...
        ) ? WelcomeExperience.ForDocument : WelcomeExperience.ForWorkspace)
...
const hasNodeInOrParentOfUri = (collection: IMainThreadTestCollection, ident: IUriIdentityService, testUri: URI, fromNode?: string) => {
	...
	while (queue.length) {
		for (const id of queue.pop()!) {
			...
			if (!node.item.uri || !ident.extUri.isEqualOrParent(
        testUri,
        node.item.uri,
      )) {
				continue;
...
class TestsFilter implements ITreeFilter<TestExplorerTreeElement> {
	...
	private testState(element: TestItemTreeElement): FilterResult {
        ...
        element.state,
      ) ? FilterResult.Include : FilterResult.Inherit;
		}
...
class TestItemRenderer extends Disposable
	...
	private fillActionBar(element: TestItemTreeElement, data: ITestElementTemplateData) {
      ...
      TestingContextKeys.isContinuousModeOn.key,
		...
		const crChild = !crSelf && this.crService.isEnabledForAChildOf(
      element.test.item.extId,
    );
	...
	public _renderElement(node: ITreeNode<TestItemTreeElement, FuzzyScore>, data: ITestElementTemplateData): void {
		...
		data.icon.className = "computed-state " + (icon ? ThemeIcon.asClassName(
      icon,
    ) : "");
