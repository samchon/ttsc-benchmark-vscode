...
export class TestService extends Disposable implements ITestService {
	...
	constructor(
      ...
      MutableObservableValue.stored(
		...
		this.isRefreshingTests = TestingContextKeys.isRefreshingTests.bindTo(
      contextKeyService,
    );
		this.activeEditorHasTests = TestingContextKeys.activeEditorHasTests.bindTo(
      contextKeyService,
    );
	...
	public async expandTest(id: string, levels: number) {
		await this.testControllers.get().get(TestId.fromString(id).controllerId)?.expandTest(
      id,
      levels,
    );
	}
	...
	public async runTests(req: AmbiguousRunTestsRequest, token = CancellationToken.None): Promise<ITestResult> {
        ...
        (a, b) => a.controllerId === b.controllerId ? 0 : 1,
          ...
          test,
				...
				for (const byProfile of groupBy(
          withControllers,
	...
	public async getTestsRelatedToCode(uri: URI, position: Position, token: CancellationToken = CancellationToken.None): Promise<InternalTestItem[]> {
      ...
      [...this.testExtHosts.values()].map(
		...
		return testIds.flatMap(ids => ids.map(id => this.collection.getNodeById(id))).filter(
      isDefined,
    );
	}
	...
	public async getCodeRelatedToTest(test: InternalTestItem, token: CancellationToken = CancellationToken.None): Promise<Location[]> {
		return (await this.testControllers.get().get(test.controllerId)?.getRelatedCode(
      test.item.extId,
      token,
    )) || [];
	}
