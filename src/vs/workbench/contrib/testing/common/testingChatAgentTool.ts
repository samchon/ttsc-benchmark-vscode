...
export class TestingChatAgentToolContribution extends Disposable implements IWorkbenchContribution {
	...
	constructor(
      ...
      toolsService.registerTool(RunTestTool.DEFINITION, runTestsTool),
		...
		const testFailureTool = instantiationService.createInstance(
      TestFailureTool,
    );
		this._register(
...
export class RunTestTool implements IToolImpl {
	...
	async invoke(invocation: IToolInvocation, countTokens: CountTokensCallback, progress: ToolProgress, token: CancellationToken): Promise<IToolResult> {
      ...
      message: localize("runTestTool.invoke.progress", "Starting test run..."),
		...
		if (group === TestRunProfileBitset.Coverage) {
			if (!testCases.some(
        tc => this._testProfileService.capabilitiesForTest(tc.item) & TestRunProfileBitset.Coverage,
      )) {
	...
	private async _getTestCasesToRun(params: IRunTestToolParams, tests: IncrementalTestCollectionItem[], progress: ToolProgress): Promise<IncrementalTestCollectionItem[]> {
		...
		progress.report({
      message: localize("runTestTool.invoke.filterProgress", "Filtering tests..."),
    });
	...
	private async _getFileTestsToRun(params: IRunTestToolParams, progress: ToolProgress): Promise<IncrementalTestCollectionItem[]> {
		...
		progress.report({
      message: localize("runTestTool.invoke.filesProgress", "Discovering tests..."),
    });

		const firstWorkspaceFolder = this._workspaceContextService.getWorkspace().folders.at(
      0,
    )?.uri;
...
export async function getFileCoverageDetails(file: FileCoverage, path: string): Promise<string> {
	const pct = getTotalCoveragePercent(
    file.statement,
    file.branch,
    file.declaration,
  ) * 100;
	...
	try {
		...
		for (const detail of details) {
			if (detail.type === DetailType.Declaration) {
				if (!detail.count) {
					const line = Position.isIPosition(
            detail.location,
          ) ? detail.location.lineNumber : detail.location.startLineNumber;
					uncoveredDeclarations.push({ name: detail.name, line });
				}
			} else {
				if (!detail.count) {
					const startLine = Position.isIPosition(
            detail.location,
          ) ? detail.location.lineNumber : detail.location.startLineNumber;
					const endLine = Position.isIPosition(
            detail.location,
          ) ? detail.location.lineNumber : detail.location.endLineNumber;
				...
				if (detail.branches) {
					for (const branch of detail.branches) {
						if (!branch.count) {
							let line: number;
							if (branch.location) {
								line = Position.isIPosition(
                  branch.location,
                ) ? branch.location.lineNumber : branch.location.startLineNumber;
							} else {
								line = Position.isIPosition(
                  detail.location,
                ) ? detail.location.lineNumber : detail.location.startLineNumber;
