...
export function getExpandedBodySize(model: IDebugModel, sessionId: string | undefined, countLimit: number): number {
	const length = model.getBreakpoints().length + model.getExceptionBreakpointsForSession(
    sessionId,
  ).length + model.getFunctionBreakpoints().length + model.getDataBreakpoints().length + model.getInstructionBreakpoints().length;
...
export class BreakpointsView extends ViewPane {
	...
	constructor(
      ...
      contextKeyService,
		...
		this.breakpointItemType = CONTEXT_BREAKPOINT_ITEM_TYPE.bindTo(
      contextKeyService,
    );
		this.breakpointIsDataBytes = CONTEXT_BREAKPOINT_ITEM_IS_DATA_BYTES.bindTo(
      contextKeyService,
    );
		this.breakpointHasMultipleModes = CONTEXT_BREAKPOINT_HAS_MODES.bindTo(
      contextKeyService,
    );
		this.breakpointSupportsCondition = CONTEXT_BREAKPOINT_SUPPORTS_CONDITION.bindTo(
      contextKeyService,
    );
		this.breakpointInputFocused = CONTEXT_BREAKPOINT_INPUT_FOCUSED.bindTo(
      contextKeyService,
    );
		this._register(
	...
	private updateSize(): void {
      ...
      this.viewDescriptorService.getViewContainerByViewId(this.id)!,
		...
		this.minimumBodySize = this.orientation === Orientation.VERTICAL ? Math.min(
      MAX_VISIBLE_BREAKPOINTS * rowHeight,
	...
	private updateBreakpointsHint(delayed = false): void {
		...
		const dbg = currentType ? this.debugService.getAdapterManager().getDebugger(
      currentType,
    ) : undefined;
		...
		if (message && debuggerHasUnverifiedBps?.length && this.debugService.getModel().areBreakpointsActivated()) {
			if (delayed) {
				const mdown = new MarkdownString(undefined, { isTrusted: true }).appendMarkdown(
          message,
        );
		...
		if (this.isBodyVisible()) {
          ...
          return typeof id === "number" && hitBreakpointIds.indexOf(id) !== -1;
			...
			if (!found) {
				...
				if (this.autoFocusedElement && equals(
          focus,
          selection,
	...
	private getTreeElements(): ICompressedTreeElement<BreakpointTreeElement>[] {
		...
		if (showAsTree && sourceBreakpoints.length > 0) {
			...
			for (const [uriStr, breakpoints] of breakpointsByUri) {
				...
				const children: ICompressedTreeElement<BreakpointTreeElement>[] = breakpoints.map(
          bp => ({
            element: bp,
	...
	private get flatElements(): BreakpointItem[] {
		const model = this.debugService.getModel();
		const sessionId = this.debugService.getViewModel().focusedSession?.getId();
		const elements = (<ReadonlyArray<IEnablement>>model.getExceptionBreakpointsForSession(sessionId)).concat(model.getFunctionBreakpoints()).concat(model.getDataBreakpoints()).concat(model.getBreakpoints()).concat(
	...
	private renderDataBreakpoint(dataBreakpoint: DataBreakpoint, data: IDataBreakpointTemplateData): void {
      ...
      (session && !session.capabilities.supportsDataBreakpoints) || !this.debugService.getModel().areBreakpointsActivated(),
		...
		if (dataBreakpoint.accessType) {
			const accessType = dataBreakpoint.accessType === "read" ? localize(
        "read",
        "Read",
      ) : dataBreakpoint.accessType === "write" ? localize(
        "write",
        "Write",
...
export function getBreakpointMessageAndIcon(state: State, breakpointsActivated: boolean, breakpoint: BreakpointItem, labelService: ILabelService, debugModel: IDebugModel): { message?: string; icon: ThemeIcon; showAdapterUnverifiedMessage?: boolean } {
	...
	if (breakpoint instanceof Breakpoint && breakpoint.triggeredBy) {
		triggeringBreakpoint = debugModel.getBreakpoints().find(
      bp => bp.getId() === breakpoint.triggeredBy,
    );
	}
	...
	const message = breakpoint.message ? breakpoint.message : breakpoint instanceof Breakpoint && labelService ? labelService.getUriLabel(
    breakpoint.uri,
...
abstract class MemoryBreakpointAction extends Action2 {
	...
	private parseAddress(range: string, isFinal: boolean): { error: string } | { address: string; bytes: number } | undefined {
		...
		const isNum = (e: string) => isFinal ? /^0x[0-9a-f]*|[0-9]*$/i.test(
      e,
