...
const onceFilter = (event: Event<ITaskEvent>, filter: (e: ITaskEvent) => boolean) => Event.once(
...
export class DebugTaskRunner implements IDisposable {
	...
	async runTaskAndCheckErrors(
        ...
        take: 2,
			...
			const onTaskErrors = this.configurationService.getValue<IDebugConfiguration>(
        "debug",
      ).onTaskErrors;
