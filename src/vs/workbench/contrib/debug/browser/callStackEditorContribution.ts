...
export class CallStackEditorContribution extends Disposable implements IEditorContribution {
	...
	constructor(
		...
		const setDecorations = () => this.decorations.set(
      this.createCallStackDecorations(),
    );
		this._register(
      Event.any(this.debugService.getViewModel().onDidFocusStackFrame, this.debugService.getModel().onDidChangeCallStack)(
        () => {
          setDecorations();
        },
      ),
