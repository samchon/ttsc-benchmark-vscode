...
export class DebugEditorContribution implements IDebugEditorContribution {
	...
	constructor(
      ...
      this.displayedStore,
		...
		this.exceptionWidgetVisible = CONTEXT_EXCEPTION_WIDGET_VISIBLE.bindTo(
      contextKeyService,
    );
	...
	private registerListeners(): void {
      ...
      this.editor.onMouseDown(
        (e: IEditorMouseEvent) => this.onEditorMouseDown(e),
      ),
    );
		this.toDispose.push(this.editor.onMouseUp(() => this.mouseDown = false));
		this.toDispose.push(
      this.editor.onMouseMove(
        (e: IEditorMouseEvent) => this.onEditorMouseMove(e),
      ),
    );
	...
	private updateHoverConfiguration(): void {
		const model = this.editor.getModel();
		if (model) {
			this.editorHoverOptions = this.configurationService.getValue<IEditorHoverOptions>(
        "editor.hover",
        {
          resource: model.uri,
	...
	async showHover(position: Position, focus: boolean, mouseEvent?: IMouseEvent): Promise<void> {
		...
		const model = this.editor.getModel();
		if (sf && model && this.uriIdentityService.extUri.isEqual(
      sf.source.uri,
      model.uri,
    )) {
	...
	private async onFocusStackFrame(sf: IStackFrame | undefined): Promise<void> {
		...
		if (model) {
			...
			if (sf && this.uriIdentityService.extUri.isEqual(
        sf.source.uri,
        model.uri,
      )) {
	...
	async addLaunchConfiguration(): Promise<void> {
		...
		if (!configurationsArrayPosition) {
          ...
          eol,
				...
				setProperty(model.getValue(), ["launch"], { "configurations": [] }, {
          tabSize,
          insertSpaces,
          eol,
        })[0];
		...
		const insertLine = (position: Position): Promise<any> => {
			...
			if (model.getLineLastNonWhitespaceColumn(
        position.lineNumber,
      ) > position.column) {
	...
	private async updateInlineValueDecorations(stackFrame: IStackFrame | undefined): Promise<void> {
		...
		const model = this.editor.getModel();
		const inlineValuesSetting = this.configurationService.getValue<IDebugConfiguration>(
      "debug",
    ).inlineValues;
		const inlineValuesTurnedOn = inlineValuesSetting === true || inlineValuesSetting === "on" || (inlineValuesSetting === "auto" && model && this.languageFeaturesService.inlineValuesProvider.has(
      model,
    ));
        ...
        Date.now() - startTime,
          ...
          stackFrame.range.startColumn,
            ...
            scope.range.startColumn,
				...
				const ownRanges = viewRanges.map(r => r.intersectRanges(scopeRange)).filter(
          isDefined,
        );
		...
		if (this.editor.getOption(EditorOption.wordWrap) !== "off") {
			...
			if (position && this.editor.getVisibleRanges().some(
        r => r.containsPosition(position),
      )) {
				preservePosition = {
          position,
