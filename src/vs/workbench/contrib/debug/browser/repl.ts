...
export class Repl extends FilterViewPane implements IHistoryNavigationWidget {
	...
	constructor(
      ...
      new HistoryNavigator(
		...
		this.multiSessionRepl = CONTEXT_MULTI_SESSION_REPL.bindTo(
      contextKeyService,
    );
	...
	async selectSession(session?: IDebugSession): Promise<void> {
		...
		if (!session) {
			...
			if (focusedSession) {
				...
				session = this.debugService.getModel().getSessions(true).find(
          s => !sessionsToIgnore.has(s),
        );
			}
		}
		if (session) {
	...
	override async focus(): Promise<void> {
		...
		await timeout(
      0,
    ); // wait a task for the repl to get attached to the DOM, #83387
		this.replInput.focus();
	}
	...
	private get isMultiSessionView(): boolean {
		return this.debugService.getModel().getSessions(true).filter(
      s => s.hasSeparateRepl() && !sessionsToIgnore.has(s),
    ).length > 1;
	}
	...
	private createReplTree(): void {
      ...
      this.replOptions,
		...
		const wordWrap = this.configurationService.getValue<IDebugConfiguration>(
      "debug",
    ).console.wordWrap;
	...
	private createReplInput(container: HTMLElement): void {
      ...
      this.instantiationService.createChild(
		...
		const config = this.configurationService.getValue<IDebugConfiguration>(
      "debug",
    );
...
class ReplOptions extends Disposable implements IReplOptions {
	...
	private update() {
		const debugConsole = this.configurationService.getValue<IDebugConfiguration>(
      "debug",
    ).console;
		this._replConfig = {
