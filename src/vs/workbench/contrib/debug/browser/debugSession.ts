...
export class DebugSession implements IDebugSession {
	...
	get autoExpandLazyVariables(): boolean {
		...
		const value = this.configurationService.getValue<IDebugConfiguration>(
      "debug",
	...
	rawUpdate(data: IRawModelUpdate): void {
		...
		if (stoppedDetails) {
          ...
          thread.clearCallStack();
				...
				const thread = typeof stoppedDetails.threadId === "number" ? this.threads.get(
          stoppedDetails.threadId,
        ) : undefined;
				if (thread) {
	...
	private async enableDependentBreakpoints(hitBreakpointIdsOrThread: Thread | number[]) {
        ...
        bp => hitBreakpointIdsOrThread.includes(bp.getIdFromAdapter(this.id)!),
      );
		} else {
