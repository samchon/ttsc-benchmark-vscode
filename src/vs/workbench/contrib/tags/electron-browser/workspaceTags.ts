...
export class WorkspaceTags implements IWorkbenchContribution {
	...
	private async reportWindowsEdition(): Promise<void> {
      ...
      "EditionID",
		...
		this.telemetryService.publicLog2<{ edition: string }, { owner: "sbatten"; comment: "Information about the Windows edition."; edition: { classification: "SystemMetaData"; purpose: "BusinessInsight"; comment: "The Windows edition." } }>(
      "windowsEdition",
      { edition: value },
    );
	}
	...
	*/
	...
	private reportCloudStats(): void {
		const uris = this.contextService.getWorkspace().folders.map(
      folder => folder.uri,
    );
