...
export class BaseIssueReporterService extends Disposable {
	...
	constructor(
      ...
      extension => extension.id.toLocaleLowerCase() === data.extensionId?.toLocaleLowerCase(),
		...
		if (issueTitle) {
			...
			const issueTitleElement = this.getElementById<HTMLInputElement>(
        "issue-title",
      );
		...
		if (issueBody) {
			// eslint-disable-next-line no-restricted-syntax
			const description = this.getElementById<HTMLTextAreaElement>(
        "description",
      );
			if (description) {
	...
	private handleExtensionData(extensions: IssueReporterExtensionData[]) {
      ...
      allExtensions: installedExtensions,
		...
		if (this.disableExtensions || installedExtensions.length === 0) {
			// eslint-disable-next-line no-restricted-syntax
			(<HTMLButtonElement>this.getElementById(
        "disableExtensions",
      )).disabled = true;
		}
	...
	private updateExtensionSelector(extensions: IssueReporterExtensionData[]): void {
      ...
      return {
		...
		// eslint-disable-next-line no-restricted-syntax
		const extensionsSelector = this.getElementById<HTMLSelectElement>(
      "extension-selector",
    );
	...
	public setEventHandlers(): void {
		(["includeSystemInfo", "includeProcessInfo", "includeWorkspaceInfo", "includeExtensions", "includeExperiments", "includeExtensionData"] as const).forEach(
      elementId => {
      ...
      this.updateAcknowledgementState();
		...
		// eslint-disable-next-line no-restricted-syntax
		const showInfoElements = this.window.document.getElementsByClassName(
      "showInfo",
    );
      ...
      this.openLink(url);
		...
		this.window.document.onkeydown = async (e: KeyboardEvent) => {
			...
			if (cmdOrCtrlKey && e.key === "w") {
				...
				// eslint-disable-next-line no-restricted-syntax
				const issueTitle = (<HTMLInputElement>this.getElementById(
          "issue-title",
        ))!.value;
	...
	public async renderBlocks(): Promise<void> {
		...
		// eslint-disable-next-line no-restricted-syntax
		const workspaceBlock = this.window.document.querySelector(
      ".block-workspace",
    );
		// eslint-disable-next-line no-restricted-syntax
		const extensionsBlock = this.window.document.querySelector(
      ".block-extensions",
    );
		// eslint-disable-next-line no-restricted-syntax
		const experimentsBlock = this.window.document.querySelector(
      ".block-experiments",
    );
		// eslint-disable-next-line no-restricted-syntax
      ...
      "issue-description-subtitle",
		...
		// eslint-disable-next-line no-restricted-syntax
		const downloadExtensionDataLink = <HTMLAnchorElement>this.getElementById(
      "extension-data-download",
    )!;
		...
		if (extensionData && extensionData.length > MAX_EXTENSION_DATA_LENGTH) {
			...
			const formattedTime = date.toTimeString().split(" ")[0].replace(
        /:/g,
        "-",
      ); // HH-MM-SS
	...
	public async createIssue(shouldCreate?: boolean, privateUri?: boolean): Promise<boolean> {
		...
		if (!this.validateInputs()) {
			...
			// eslint-disable-next-line no-restricted-syntax
			const invalidInput = this.window.document.getElementsByClassName(
        "invalid-input",
      );
	...
	private getExtensionGitHubUrl(): string {
      ...
      /^https?:\/\/github\.com\/([^\/]*)\/([^\/]*)\/?(\/issues)?$/,
		...
		} else if (extensionUrl && extensionUrl.match(
      /^https?:\/\/github\.com\/([^\/]*)\/([^\/]*)$/,
    )) {
	...
	public setLoading(element: HTMLElement) {
      ...
      extensionDataCaption2 => hide(extensionDataCaption2),
    );
	...
	public removeLoading(element: HTMLElement, fromReporter: boolean = false) {
      ...
      this.window.document.querySelectorAll(".ext-parens"),
		...
		extensionDataCaption2.forEach(
      extensionDataCaption2 => show(extensionDataCaption2),
    );
