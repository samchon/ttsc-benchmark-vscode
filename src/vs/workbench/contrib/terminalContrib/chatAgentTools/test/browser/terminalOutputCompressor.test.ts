...
suite("gitDiffFilter - regression", () => {
  ...
  test("does not match `git difftool` (only diff/show)", () => {
    ok(
      !gitDiffFilter.matches("run_in_terminal", { command: "git difftool HEAD~1" }),
    );
    ok(
      !gitDiffFilter.matches("run_in_terminal", { command: "git difftool --tool=vscode" }),
    );
  });

  test("does not match `git diff-tree` or `git diff-files`", () => {
    ok(
      !gitDiffFilter.matches("run_in_terminal", { command: "git diff-tree HEAD" }),
    );
...
suite("gitStatusFilter", () => {
  ...
  test("does not match git stash", () => {
    ok(
      !gitStatusFilter.matches("run_in_terminal", { command: "git stash list" }),
    );
  });
});
...
suite("testRunnerFilter", () => {
  ...
  test("matches common test runners", () => {
    ...
    ok(
      testRunnerFilter.matches("run_in_terminal", { command: "npx vitest run" }),
    );
  });
});
