...
export async function createRunInTerminalToolData(
	...
	const allowToRunUnsandboxedCommands = configurationService.getValue<boolean>(
...
export class RunInTerminalTool extends Disposable implements IToolImpl {
	...
	private get _autoApproveUnsandboxedCommands(): boolean {
		return this._allowUnsandboxedCommands && this._configurationService.getValue<boolean>(
	...
	constructor(
      ...
      lifecycleService.onWillShutdown(() => {
		...
		this._osBackend = this._remoteAgentService.getEnvironment().then(
      remoteEnv => remoteEnv?.os ?? OS,
    );

		this._terminalToolCreator = this._instantiationService.createInstance(
      ToolTerminalCreator,
    );
      ...
      this._register(
        this._instantiationService.createInstance(
          ...
          this._treeSitterCommandParser,
          (message, args) => this._logService.info(
	...
	async handleToolStream(context: IToolInvocationStreamContext, _token: CancellationToken): Promise<IStreamedToolInvocation | undefined> {
		...
		return {
      invocationMessage: localize("runInTerminal.streaming.default", "Running command"),
    };
	}
	...
	async prepareToolInvocation(context: IToolInvocationPreparationContext, token: CancellationToken): Promise<IPreparedToolInvocation | undefined> {
		...
		const allowUnsandboxedCommands = this._getAllowToRunUnsandboxedCommands(
      args,
    );
		...
		if (this._shouldRejectUnsandboxedExecutionRequest(
      isSandboxEnabled,
      ...
      commandLineAnalyzers.map(e => e.analyze(commandLineAnalyzerOptions)),
		...
		const disclaimersRaw = commandLineAnalyzerResults.map(e => e.disclaimers).filter(e => !!e).flatMap(
      e => e,
    );
		let disclaimer: IMarkdownString | undefined;
		if (disclaimersRaw.length > 0) {
			const disclaimerTexts = disclaimersRaw.map(
        d => typeof d === "string" ? d : d.value,
      );
        ...
        d => typeof d !== "string",
            ...
            isTrusted: { enabledCommands: [TerminalChatCommandId.OpenTerminalSettingsLink] },
          }
				: { supportThemeIcons: true };
      ...
      e => e.isAutoApproveAllowed,
		...
		const isFinalAutoApproved = isUnsandboxedAutoApproved || isSandboxAutoApproved || isAutoApprovedByRules || commandLineAnalyzerResults.some(
      e => e.forceAutoApproval,
    );
		...
		if (isFinalAutoApproved || (isAutoApproveEnabled && commandLineAnalyzerResults.some(
      e => e.autoApproveInfo,
    ))) {
	...
	private async _confirmAutomaticUnsandboxRetry(sessionResource: URI | undefined, command: string, shell: string, blockedDomains: string[] | undefined, riskAssessment: { toolId: string; parameters: unknown } | undefined, token: CancellationToken): Promise<boolean> {
		...
		const chatModel = sessionResource && this._chatService.getSession(
      sessionResource,
    );
	...
	async invoke(invocation: IToolInvocation, _countTokens: CountTokensCallback, _progress: ToolProgress, token: CancellationToken): Promise<IToolResult> {
      ...
      `RunInTerminalTool: Invoking with options ${JSON.stringify(args)}`,
		...
		if (args.timeout !== undefined && (Number.isNaN(
      args.timeout,
    ) || args.timeout < 0)) {
			return {
				content: [{
      ...
      "The sandboxed execution output indicated the sandbox blocked the command.",
		...
		const isNewSession = !executionOptions.persistentSession && !this._sessionTerminalAssociations.has(
      chatSessionResource,
    );
      ...
      TerminalCapability.CommandDetection,
		...
		const timeoutValue = args.timeout !== undefined ? clamp(
      args.timeout,
      0,
      Number.MAX_SAFE_INTEGER,
    ) : undefined;
		if (timeoutValue !== undefined && timeoutValue > 0) {
			const shouldEnforceTimeout = executionOptions.waitStrategy === "idle" || this._configurationService.getValue(
        ...
        commandLineForMetadata,
          ...
          commandId,
              ...
              didSandboxWrapCommand,
				...
				const idleSilenceMs = this._configurationService.getValue<number>(
          TerminalChatAgentToolsSettingId.IdleSilenceTimeoutMs,
            ...
            new RunOnceScheduler(
              () => idleSilenceDeferred.complete({
                type: "idleSilence" as const,
              }),
              idleSilenceMs,
            ),
          );
            ...
            `RunInTerminalTool: Continue in background triggered, returning output collected so far`,
					...
					outputLineCount = backgroundOutput ? count(
            backgroundOutput.trim(),
            "\n",
          ) + 1 : 0;
            ...
            true,
					...
					outputLineCount = timeoutOutput ? count(
            timeoutOutput.trim(),
            "\n",
          ) + 1 : 0;
            ...
            true,
					...
					outputLineCount = idleSilenceOutput ? count(
            idleSilenceOutput.trim(),
            "\n",
          ) + 1 : 0;
            ...
            );
						{
							...
							if (executeResult.exitCode !== undefined) {
								...
								if (state.timestamp !== undefined) {
									state.duration = state.duration ?? Math.max(
                    0,
              ...
              `RunInTerminalTool: Finished \`${execution.strategy.type}\` execute strategy with exitCode \`${executeResult.exitCode}\`, result.length \`${executeResult.output?.length}\`, error \`${executeResult.error}\``,
						...
						outputLineCount = executeResult.output === undefined ? 0 : count(
	...
	private async _extractImagesFromOutput(output: string, cwd: URI | undefined): Promise<IToolResult["content"]> {
		...
		for (const filePath of matches) {
			try {
				...
				const stat = await this._fileService.stat(fileUri).catch(
          () => undefined,
        );
	...
	private async _initTerminal(chatSessionResource: URI, termId: string, terminalToolSessionId: string | undefined, isBackground: boolean, token: CancellationToken): Promise<IToolTerminal> {
		...
		if (!isBackground) {
			const cachedTerminal = this._sessionTerminalAssociations.get(
        chatSessionResource,
      );
	...
	private _restoreTerminalAssociations(): void {
      ...
      "{}",
		...
		try {
			const associations: Record<number, IStoredTerminalAssociation> = JSON.parse(
        storedAssociations,
      );
	...
	private _cleanupSessionTerminals(chatSessionResource: URI): void {
		const sessionTerminals = this._sessionTerminalInstances.get(
      chatSessionResource,
    );
		const toolTerminal = this._sessionTerminalAssociations.get(
      chatSessionResource,
    );
		const terminalsToDispose = sessionTerminals ?? (toolTerminal ? new Set([
      toolTerminal.instance,
    ]) : undefined);
	...
	private _addSessionTerminalAssociation(chatSessionResource: URI, toolTerminal: IToolTerminal): void {
		...
		let sessionTerminals = this._sessionTerminalInstances.get(
      chatSessionResource,
    );
		if (!sessionTerminals) {
	...
	private _registerCompletionNotification(terminalInstance: ITerminalInstance, termId: string, chatSessionResource: URI, commandName: string, toolSpecificData: IChatTerminalToolInvocationData, outputMonitor?: OutputMonitor, alreadyNotifiedInputNeededOutput?: string): void {
      ...
      notificationKey,
		...
		// _activeExecutions.
		const executionForDisposal = RunInTerminalTool._activeExecutions.get(
      termId,
    );
...
class RestoredTerminalExecution extends Disposable implements IActiveTerminalExecution {
	readonly completionPromise: Promise<ITerminalExecuteStrategyResult> = Promise.resolve(
    { output: undefined, error: "restoredTerminalExecutionNotAwaitable" },
  );
...
export class TerminalProfileFetcher {
	...
	constructor(
		...
		this.osBackend = this._remoteAgentService.getEnvironment().then(
      remoteEnv => remoteEnv?.os ?? OS,
    );
	}
