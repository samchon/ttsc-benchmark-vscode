...
export class GetTerminalOutputTool extends Disposable implements IToolImpl {
	...
	constructor(
      ...
      terminalService.onDidDisposeInstance(
        instance => this._forgetTerminalInstance(instance.instanceId),
      ),
    );
	}
	...
	private _formatOutput(id: string, terminalInstanceId: number, output: string): string {
      ...
      TerminalChatAgentToolsSettingId.OutputDeltas,
		...
		const previousOutputSnapshot = this._lastOutputSnapshotByExecutionId.get(
      id,
    );
      ...
      output,
		...
		if (output.length > previousOutputSnapshot.length && this._hashOutput(
      output,
      previousOutputSnapshot.length,
