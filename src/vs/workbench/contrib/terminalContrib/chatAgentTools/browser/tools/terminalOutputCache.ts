...
function classifySingleHead(head: { head: string; sub: string | undefined }): IClassification {
	switch (head.head) {
		case "git": {
			...
			if (head.sub && /^(add|commit|push|pull|fetch|merge|rebase|reset|checkout|switch|restore|cherry-pick|revert|stash|tag|branch|am|apply|clean|rm|mv)$/.test(
        head.sub,
      )) {
		...
		case "find":
			return {
        cls: head.head === "find" || head.head === "tree" ? CacheClass.Slow : CacheClass.Fast,
        invalidates: [],
      };
		case "npm":
		case "pnpm":
		...
		case "vitest":
		case "cargo":
			if (head.head === "cargo" && head.sub && /^(test|nextest|check|build)$/.test(
        head.sub,
      )) {
				return { cls: CacheClass.Medium, invalidates: [] };
			}
...
export class TerminalOutputCache implements IToolResultCache {
	...
	private _invalidateByProgram(cwd: string, programs: readonly string[]): void {
        ...
        parseCommand(e.command)?.segments[0] ?? { raw: "", tokens: [], rawTokens: [], envPrefixes: [], wrappers: [], trailingSeparator: undefined },
      );
