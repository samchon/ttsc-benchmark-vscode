...
export class CreateAndRunTaskTool implements IToolImpl {
	...
	async invoke(invocation: IToolInvocation, _countTokens: CountTokensCallback, _progress: ToolProgress, token: CancellationToken): Promise<IToolResult> {
      ...
      "\t",
		...
		_progress.report({
      message: new MarkdownString(localize("copilotChat.fetchingTask", "Resolving the task")),
    });
		...
		if (!task) {
			return {
        content: [{ kind: "text", value: `Task not found: ${args.task.label}` }],
        toolResultMessage: new MarkdownString(localize("copilotChat.taskNotFound", "Task not found: `{0}`", args.task.label)),
      };
		}
        ...
        this._tasksService,
			...
			_progress.report({
        message: new MarkdownString(localize("copilotChat.runningTask", "Running task `{0}`", args.task.label)),
      });
        ...
        Boolean,
			...
			if (!terminals || terminals.length === 0) {
				return {
          content: [{ kind: "text", value: `Task started but no terminal was found for: ${args.task.label}` }],
          toolResultMessage: new MarkdownString(localize("copilotChat.noTerminal", "Task started but no terminal was found for: `{0}`", args.task.label)),
        };
			}
