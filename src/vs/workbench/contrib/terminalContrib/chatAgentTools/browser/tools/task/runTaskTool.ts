...
export class RunTaskTool implements IToolImpl {
	...
	async invoke(invocation: IToolInvocation, _countTokens: CountTokensCallback, _progress: ToolProgress, token: CancellationToken): Promise<IToolResult> {
      ...
      true,
        ...
        toolResultMessage: new MarkdownString(localize("chat.taskNotFound", "Task not found: \`{0}\`", args.id)),
      };
		}
		...
		if (activeTasks.includes(task)) {
			return {
        content: [{ kind: "text", value: `The task ${taskLabel} is already running.` }],
        toolResultMessage: new MarkdownString(localize("chat.taskAlreadyRunning", "The task \`{0}\` is already running.", taskLabel)),
      };
		}
      ...
      this._tasksService,
        ...
        dependencyTasks ?? task,
			...
			if (!resources || resources.length === 0) {
				return {
          content: [{ kind: "text", value: `Task started but no terminal was found for: ${taskLabel}` }],
          toolResultMessage: new MarkdownString(localize("chat.noTerminal", "Task started but no terminal was found for: \`{0}\`", taskLabel)),
        };
			}
			const terminals = this._terminalService.instances.filter(
        t => resources.some(
	...
	async prepareToolInvocation(context: IToolInvocationPreparationContext, token: CancellationToken): Promise<IPreparedToolInvocation | undefined> {
      ...
      true,
		...
		if (!task) {
			return {
        invocationMessage: new MarkdownString(localize("chat.taskNotFound", "Task not found: \`{0}\`", args.id)),
      };
		}
		const taskLabel = task._label;
		const activeTasks = await this._tasksService.getActiveTasks();
		if (task && activeTasks.includes(task)) {
			return {
        invocationMessage: new MarkdownString(localize("chat.taskAlreadyActive", "The task is already running.")),
      };
		}
