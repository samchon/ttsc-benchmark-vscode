...
export class TerminalChatService extends Disposable implements ITerminalChatService {
	...
	private readonly _onDidRegisterTerminalInstanceForToolSession = this._register(
	...
	constructor(
		...
		this._hasToolTerminalContext = TerminalChatContextKeys.hasChatTerminals.bindTo(
      this._contextKeyService,
    );
		this._hasHiddenToolTerminalContext = TerminalChatContextKeys.hasHiddenChatTerminals.bindTo(
      this._contextKeyService,
    );
	...
	registerTerminalInstanceWithToolSession(terminalToolSessionId: string | undefined, instance: ITerminalInstance): void {
		...
		const existingToolSessionId = this._toolSessionIdByTerminalInstance.get(
      instance,
    );
	...
	registerTerminalInstanceWithChatSession(chatSessionResource: URI, instance: ITerminalInstance): void {
		...
		const existingResource = this._chatSessionResourceByTerminalInstance.get(
      instance,
    );
	...
	isBackgroundTerminal(terminalToolSessionId?: string): boolean {
      ...
      terminalToolSessionId,
		...
		return this._terminalService.instances.includes(
      instance,
	...
	getMostRecentProgressPart(): IChatTerminalToolProgressPart | undefined {
		if (!this._mostRecentProgressPart || !this._activeProgressParts.has(
	...
	private _updateHasToolTerminalContextKeys(): void {
		...
		const hiddenTerminalCount = this.getToolSessionTerminalInstances(
      true,
    ).length;
