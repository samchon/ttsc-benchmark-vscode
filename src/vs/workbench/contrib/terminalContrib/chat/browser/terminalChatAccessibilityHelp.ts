...
export function getAccessibilityHelpText(accessor: ServicesAccessor): string {
	...
	content.push(
    makeRequestKeybinding ? localize("inlineChat.input", "The input box is where the user can type a request and can make the request ({0}). The widget will be closed and all content will be discarded when the Escape key is pressed and the terminal will regain focus.", makeRequestKeybinding) : localize("inlineChat.inputNoKb", "The input box is where the user can type a request and can make the request by tabbing to the Make Request button, which is not currently triggerable via keybindings. The widget will be closed and all content will be discarded when the Escape key is pressed and the terminal will regain focus."),
  );
	content.push(
    openAccessibleViewKeybinding ? localize("inlineChat.inspectResponseMessage", "The response can be inspected in the accessible view ({0}).", openAccessibleViewKeybinding) : localize("inlineChat.inspectResponseNoKb", "With the input box focused, inspect the response in the accessible view via the Open Accessible View command, which is currently not triggerable by a keybinding."),
  );
	content.push(
    focusResponseKeybinding ? localize("inlineChat.focusResponse", "Reach the response from the input box ({0}).", focusResponseKeybinding) : localize("inlineChat.focusResponseNoKb", "Reach the response from the input box by tabbing or assigning a keybinding for the command: Focus Terminal Response."),
  );
	content.push(
    focusInputKeybinding ? localize("inlineChat.focusInput", "Reach the input box from the response ({0}).", focusInputKeybinding) : localize("inlineChat.focusInputNoKb", "Reach the response from the input box by shift+tabbing or assigning a keybinding for the command: Focus Terminal Input."),
  );
	content.push(
    runCommandKeybinding ? localize("inlineChat.runCommand", "With focus in the input box or command editor, the Terminal: Run Chat Command ({0}) action.", runCommandKeybinding) : localize("inlineChat.runCommandNoKb", "Run a command by tabbing to the button as the action is currently not triggerable by a keybinding."),
  );
	content.push(
    insertCommandKeybinding ? localize("inlineChat.insertCommand", "With focus in the input box command editor, the Terminal: Insert Chat Command ({0}) action.", insertCommandKeybinding) : localize("inlineChat.insertCommandNoKb", "Insert a command by tabbing to the button as the action is currently not triggerable by a keybinding."),
  );
	content.push(
    localize(
      "inlineChat.toolbar",
