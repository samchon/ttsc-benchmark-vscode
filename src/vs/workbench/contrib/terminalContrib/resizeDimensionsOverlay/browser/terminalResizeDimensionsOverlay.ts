...
export class TerminalResizeDimensionsOverlay extends Disposable {
	...
	private readonly _resizeOverlayHideTimeout: MutableDisposable<IDisposable> = this._register(
	...
	constructor(
		...
		this._register(
      (xterm as XtermTerminal).raw.onResize(
        dims => this._handleDimensionsChanged(dims),
      ),
    );
		this._register(
      toDisposable(() => {
