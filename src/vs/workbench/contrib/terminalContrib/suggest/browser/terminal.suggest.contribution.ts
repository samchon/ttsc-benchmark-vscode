...
class TerminalSuggestContribution extends DisposableStore implements ITerminalContribution {
	...
	private readonly _lspAddons: DisposableMap<string, LspCompletionProviderAddon> = this.add(
    new DisposableMap(),
  );
	...
	get lspAddons(): LspCompletionProviderAddon[] { return Array.from(
    this._lspAddons.values(),
  ); }
	...
	private async _loadLspCompletionAddon(xterm: RawXtermTerminal): Promise<void> {
		...
		if (!this._ctx.instance.shellType || !(lspTerminalObj = getTerminalLspSupportedLanguageObj(
      this._ctx.instance.shellType,
    ))) {
			this._lspAddons.clearAndDisposeAll();
			return;
	...
	private async _prepareAddonLayout(xterm: RawXtermTerminal): Promise<void> {
		...
		const xtermElement = xterm.element ?? await this._waitForXtermElement(
      xterm,
    );
...
class TerminalSuggestProvidersConfigurationManager extends Disposable {
	...
	private _updateConfiguration(): void {
		...
		this._terminalContributionService.terminalCompletionProviders.forEach(
      o => providers.set(o.extensionIdentifier, {
        ...o,
        id: o.extensionIdentifier,
      }),
    );
