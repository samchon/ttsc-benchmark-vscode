...
export class TerminalQuickFixAddon extends Disposable implements ITerminalAddon, ITerminalQuickFixAddon {
	...
	private readonly _decorationDisposables: MutableDisposable<IDisposable> = this._register(
    new MutableDisposable(),
  );
	...
	private _disposeQuickFix(command: ITerminalCommand, id: string): void {
		...
		this._telemetryService?.publicLog2<QuickFixResultTelemetryEvent, QuickFixClassification>(
      "terminal/quick-fix",
      {
        quickFixId: id,
...
export async function getQuickFixesForCommand(
	...
	for (const options of quickFixOptions.values()) {
		for (const option of options) {
          ...
          option,
          option.outputMatcher ? getLinesForCommand(terminal.buffer.active, terminalCommand, terminal.cols, option.outputMatcher) : undefined,
        );
				...
				for (const quickFix of asArray(quickFixes)) {
                  ...
                  fix.terminalCommand,
							...
							case TerminalQuickFixType.Opener: {
								...
								const uriLabel = isUrl ? encodeURI(
                  fix.uri.toString(true),
...
function toActionWidgetItems(inputQuickFixes: readonly TerminalQuickFixItem[], showHeaders: boolean): IActionListItem<TerminalQuickFixItem>[] {
	...
	for (const quickFix of showHeaders ? inputQuickFixes : inputQuickFixes.filter(
    i => !!i.action,
  )) {
