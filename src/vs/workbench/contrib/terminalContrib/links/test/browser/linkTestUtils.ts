...
export async function assertLinkHelper(
	...
	const lastLineIndex = textSplit.filter((e, i) => i !== textSplit.length - 1).reduce(
    (p, c) => {
	...
	const actualLinks = (await detector.detect(lines, lastLineIndex, detector.xterm.buffer.active.cursorY)).map(
    e => {
      return {
