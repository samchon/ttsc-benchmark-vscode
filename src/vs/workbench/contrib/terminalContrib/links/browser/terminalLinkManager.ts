...
export class TerminalLinkManager extends DisposableStore {
	...
	constructor(
		...
		const enableFileLinksConfig = this._configurationService.getValue<ITerminalConfiguration>(
	...
	protected async _getLinksForType(y: number, type: "word" | "url" | "localFile" | "localFolder"): Promise<ILink[] | undefined> {
		switch (type) {
          ...
          r => this._standardLinkProviders.get(TerminalWordLinkDetector.id)?.provideLinks(
            y,
            r,
          ),
        ));
			case "url":
				return (await new Promise<ILink[] | undefined>(
          r => this._standardLinkProviders.get(TerminalUriLinkDetector.id)?.provideLinks(
            y,
            r,
          ),
        ));
          ...
          r => this._standardLinkProviders.get(TerminalLocalLinkDetector.id)?.provideLinks(
            y,
            r,
          ),
        ));
				return links?.filter(
          link => (link as TerminalLink).type === TerminalBuiltinLinkType.LocalFile,
        );
			}
			case "localFolder": {
				const links = (await new Promise<ILink[] | undefined>(
          r => this._standardLinkProviders.get(TerminalLocalLinkDetector.id)?.provideLinks(
            ...
            r,
				...
				return links?.filter(
          link => (link as TerminalLink).type === TerminalBuiltinLinkType.LocalFolderInWorkspace,
        );
			}
		}
	}
	...
	protected _isLinkActivationModifierDown(event: MouseEvent): boolean {
		const editorConf = this._configurationService.getValue<{ multiCursorModifier: "ctrlCmd" | "alt" }>(
      "editor",
    );
