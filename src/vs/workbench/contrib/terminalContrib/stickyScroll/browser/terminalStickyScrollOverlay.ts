...
export class TerminalStickyScrollOverlay extends Disposable {
	...
	private readonly _webglAddon: MutableDisposable<WebglAddonType> = this._register(
	...
	private _updateContent(command: ITerminalCommand | ICurrentPartialCommand, startMarker: IMarker) {
		...
		const rowOffset = !isPartialCommand && command.endMarker ? Math.max(
      buffer.viewportY - command.endMarker.line + 1,
      0,
    ) : 0;
      ...
      Math.floor(xterm.rows * Constants.StickyScrollPercentageCap),
		...
		const stickyScrollLineCount = Math.min(
      promptRowCount + commandRowCount - 1,
      maxLineCount,
    ) - rowOffset;
	...
	private _ensureElement() {
      ...
      addStandardDisposableListener(
        hoverOverlay,
        "wheel",
        e => this._xterm?.raw.element?.dispatchEvent(new WheelEvent(e.type, e)),
      ),
    );
