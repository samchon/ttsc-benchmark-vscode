...
export class TerminalInstance extends Disposable implements ITerminalInstance {
	...
	private readonly _initialDataEventsListener: MutableDisposable<IDisposable> = this._register(
	...
	private readonly _messageTitleDisposable: MutableDisposable<IDisposable> = this._register(
    new MutableDisposable(),
  );
	private _widgetManager: TerminalWidgetManager;
	private readonly _dndObserver: MutableDisposable<IDisposable> = this._register(
    new MutableDisposable(),
  );
	...
	private _targetRef: ImmortalReference<TerminalLocation | undefined> = new ImmortalReference(
    undefined,
  );
	...
	get hasFocus(): boolean { return dom.isAncestorOfActiveElement(
	...
	constructor(
      ...
      this.title,
        ...
        path: this._shellLaunchConfig.cwd,
			...
			if (cwdUri) {
				this._workspaceFolder = this._workspaceContextService.getWorkspaceFolder(
          cwdUri,
        ) ?? undefined;
			}
		}
      ...
      this._contextKeyService,
		...
		this._terminalAltBufferActiveContextKey = TerminalContextKeys.altBufferActive.bindTo(
      scopedContextKeyService,
    );
      ...
      this._shellLaunchConfig,
		...
		this._register(
      this.capabilities.onDidAddCapability(
        e => this._logService.debug("terminalInstance added capability", e.id),
      ),
    );
		this._register(
      this.capabilities.onDidRemoveCapability(
        e => this._logService.debug("terminalInstance removed capability", e.id),
      ),
    );
	...
	private _initDimensions(): void {
		...
		const computedStyle = dom.getWindow(this._container).getComputedStyle(
      this._container,
    );
	...
	private _getDimension(width: number, height: number): ICanvasDimensions | undefined {
      ...
      dom.getWindow(this.domElement),
		...
		const computedStyle = dom.getWindow(this.xterm.raw.element).getComputedStyle(
	...
	get shouldPersist(): boolean { return this._processManager.shouldPersist && !this.shellLaunchConfig.isTransient && (!this.reconnectionProperties || this._configurationService.getValue(
    "task.reconnection",
  ) === true); }
	...
	protected async _createXterm(): Promise<XtermTerminal | undefined> {
      ...
      this._contextKeyService,
		...
		const disableShellIntegrationReporting = (this.shellLaunchConfig.executable === undefined || this.shellType === undefined) || !shellIntegrationSupportedShellTypes.includes(
      this.shellType,
    );
      ...
      this.xterm.onDidRequestRunCommand(e => {
		...
		const initialTextWrittenPromise = this._shellLaunchConfig.initialText ? new Promise<void>(
	...
	private async _createProcess(): Promise<void> {
		...
		} else if (this._workspaceContextService.getWorkspace().folders.length === 0 && this._cwd && this._userHome && normalizeDriveLetter(
      this._cwd,
    ) !== normalizeDriveLetter(this._userHome)) {
	...
	public addBufferMarker(properties: IMarkProperties): void {
		this.capabilities.get(TerminalCapability.BufferMarkDetection)?.addMark(
      properties,
    );
	}
	...
	private _onProcessData(ev: IProcessDataEvent): void {
      ...
      /(?<seq>\x1b\][16]33;(?:C|D(?:;\d+)?)\x07)/g,
		...
		if (ev.trackCommit) {
			ev.writePromise = new Promise<void>(
	...
	private _relaunchWithShellIntegrationDisabled(exitMessage: string | undefined): void {
		...
		this._telemetryService.publicLog2<{}, { owner: "meganrogge"; comment: "Indicates the process exited when created with shell integration args" }>(
	...
	private async _updatePtyDimensions(rawXterm: XTermTerminal): Promise<void> {
		...
		const roundedPixelHeight = pixelHeight ? Math.round(
      pixelHeight,
    ) : undefined;
	...
	private async _refreshEnvironmentVariableInfoWidgetState(info?: IEnvironmentVariableInfo): Promise<void> {
		...
		if (
			...
			(!this._shellLaunchConfig.isFeatureTerminal || (this.reconnectionProperties && this._configurationService.getValue(
        "task.reconnection",
      ) === true)) &&
			// Not a custom pty
	...
	async changeIcon(icon?: TerminalIcon): Promise<TerminalIcon | undefined> {
		...
		const iconPicker = this._scopedInstantiationService.createInstance(
      TerminalIconPicker,
    );
	...
	async handleMouseEvent(event: MouseEvent, contextMenu: IMenu): Promise<{ cancelContextMenu: boolean } | void> {
		...
		if (dom.isHTMLElement(event.target) && (event.target.classList.contains(
      "scrollbar",
...
class TerminalInstanceDragAndDropController extends Disposable implements dom.IDragAndDropObserverCallbacks {
	...
	async onDrop(e: DragEvent) {
		...
		if (!path && e.dataTransfer.files.length > 0 && getPathForFile(
	...
	private _getViewOrientation(): Orientation {
		...
		const terminalLocation = this._viewDescriptorService.getViewLocationById(
      TERMINAL_VIEW_ID,
    );
		return terminalLocation === ViewContainerLocation.Panel && isHorizontal(
      panelPosition,
    )
...
export class TerminalLabelComputer extends Disposable {
	...
	refreshLabel(instance: Pick<ITerminalInstance, "shellLaunchConfig" | "shellType" | "cwd" | "fixedCols" | "fixedRows" | "initialCwd" | "processName" | "sequence" | "userHome" | "workspaceFolder" | "staticTitle" | "capabilities" | "title" | "description">, reset?: boolean): void {
		...
		const useAgentCliTitle = tabs.allowAgentCliTitle && TerminalLabelComputer.agentCliShellTypes.has(
...
export function parseExitResult(
	...
	const code = isNumber(
    exitCodeOrError,
  ) ? exitCodeOrError : exitCodeOrError.code;
	...
	switch (typeof exitCodeOrError) {
		...
		case "object": {
			...
			if (exitCodeOrError.message.toString().includes(
        "Could not find pty with id",
      )) {
				break;
			}
        ...
        /.*error code:\s*(\d+).*$/,
			...
			if (conptyError) {
				const errorCode = conptyError.length > 1 ? parseInt(
          conptyError[1],
        ) : undefined;
				switch (errorCode) {
					case 5:
...
export class TerminalInstanceColorProvider implements IXtermColorProvider {
	...
	getBackgroundColor(theme: IColorTheme) {
		...
		const location = this._viewDescriptorService.getViewLocationById(
      TERMINAL_VIEW_ID,
    )!;
