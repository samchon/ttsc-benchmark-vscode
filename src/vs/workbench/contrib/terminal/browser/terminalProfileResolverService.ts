...
export abstract class BaseTerminalProfileResolverService extends Disposable implements ITerminalProfileResolverService {
	...
	constructor(
        ...
        env => this._primaryBackendOs = env?.os || OS,
      );
		} else {
	...
	resolveIcon(shellLaunchConfig: IShellLaunchConfig, os: OperatingSystem): void {
		if (shellLaunchConfig.icon) {
			shellLaunchConfig.icon = this._getCustomIcon(
	...
	async resolveShellLaunchConfig(shellLaunchConfig: IShellLaunchConfig, options: IShellLaunchConfigResolveOptions): Promise<void> {
		...
		// an issue
		const resource = shellLaunchConfig === undefined || isString(
      shellLaunchConfig.cwd,
    ) ? undefined : shellLaunchConfig.cwd;
	...
	private async _getUnresolvedDefaultProfile(options: IShellLaunchConfigResolveOptions): Promise<ITerminalProfile> {
        ...
        `terminal.integrated.agentHostProfile.${this._getOsKey(options.os)}`,
			...
			const agentHostShellProfile = this._getUnresolvedAgentHostShellProfile(
        options,
      );
		...
		if (options.allowAutomationShell) {
			const automationShellProfile = this._getUnresolvedAutomationShellProfile(
        options,
      );
	...
	private async _getUnresolvedFallbackDefaultProfile(options: IShellLaunchConfigResolveOptions): Promise<ITerminalProfile> {
      ...
      options.os,
		...
		if (options.os === OperatingSystem.Macintosh && path.parse(executable).name.match(
      /(zsh|bash)/,
    )) {
	...
	private _getUnresolvedAutomationShellProfile(options: IShellLaunchConfigResolveOptions): ITerminalProfile | undefined {
      ...
      `terminal.integrated.automationProfile.${this._getOsKey(options.os)}`,
		...
		if (this._isValidAutomationProfile(automationProfile, options.os)) {
			automationProfile.icon = this._getCustomIcon(
	...
	private _getUnresolvedAgentHostShellProfile(options: IShellLaunchConfigResolveOptions): ITerminalProfile | undefined {
      ...
      `terminal.integrated.agentHostProfile.${this._getOsKey(options.os)}`,
		...
		if (this._isValidAutomationProfile(agentHostProfile, options.os)) {
			agentHostProfile.icon = this._getCustomIcon(
	...
	private async _resolveProfile(profile: ITerminalProfile, options: IShellLaunchConfigResolveOptions): Promise<ITerminalProfile> {
		...
		if (options.os === OperatingSystem.Windows) {
			...
			if (!isWoW64 && windir) {
				...
				if (profile.path && profile.path.toLowerCase().indexOf(
          sysnativePath,
        ) === 0) {
      ...
      lastActiveWorkspace,
		...
		if (profile.args) {
          ...
          profile.args.map(
            arg => this._resolveVariables(arg, env, lastActiveWorkspace),
          ),
        );
			}
		}
