...
class RemoteTerminalBackend extends BaseTerminalBackend implements ITerminalBackend {
	...
	async persistTerminalState(): Promise<void> {
		...
		const serialized = await this._remoteTerminalChannel.serializeTerminalState(
      ids,
    );
	...
	async createProcess(
		...
		const terminalConfig = this._configurationService.getValue<ITerminalConfiguration>(
      TERMINAL_CONFIG_SECTION,
    );
	...
	async attachToRevivedProcess(id: number): Promise<ITerminalChildProcess | undefined> {
		...
		try {
			const newId = await this._remoteTerminalChannel.getRevivedPtyNewId(
        id,
      ) ?? id;
	...
	async getWslPath(original: string, direction: "unix-to-win" | "win-to-unix"): Promise<string> {
		...
		return this._remoteTerminalChannel.getWslPath(
      original,
      direction,
    ) || original;
	}
