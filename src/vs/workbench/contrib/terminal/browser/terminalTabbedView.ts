...
export class TerminalTabbedView extends Disposable {
	...
	constructor(
      ...
      this._instantiationService.createInstance(
		...
		this._terminalIsTabsNarrowContextKey = TerminalContextKeys.tabsNarrow.bindTo(
      contextKeyService,
    );
		this._terminalTabsFocusContextKey = TerminalContextKeys.tabsFocus.bindTo(
      contextKeyService,
    );
		this._terminalTabsMouseContextKey = TerminalContextKeys.tabsMouse.bindTo(
      contextKeyService,
    );
		...
		this._register(
      Event.any(this._terminalGroupService.onDidChangeInstances, this._terminalGroupService.onDidChangeGroups)(
        () => {
          this._refreshShowTabs();
		...
		this._register(
      Event.any(this._terminalChatService.onDidRegisterTerminalInstanceWithToolSession, this._terminalService.onDidChangeInstances, this._terminalService.onDidDisposeInstance)(
        () => {
          this._refreshShowTabs();
          this._updateChatTerminalsEntry();
        },
	...
	private _shouldShowTabs(): boolean {
		...
		const hiddenChatTerminals = this._terminalChatService.getToolSessionTerminalInstances(
      true,
    );
		if (!enabled) {
			return false;
	...
	private _handleOnDidSashReset(): void {
		...
		if (ctx) {
			const style = dom.getWindow(this._tabListElement).getComputedStyle(
        this._tabListElement,
      );
	...
	private _getAdditionalWidth(instance: ITerminalInstance): number {
		...
		const splitAnnotationWidth = (this._terminalGroupService.getGroupForInstance(
      instance,
    )?.terminalInstances.length || 0) > 1 ? WidthConstants.SplitAnnotation : 0;
	...
	private _shouldHandleEmptyAreaDrop(event: DragEvent): boolean {
		...
		if (targetNode && (this._tabListDomElement.contains(
      targetNode,
