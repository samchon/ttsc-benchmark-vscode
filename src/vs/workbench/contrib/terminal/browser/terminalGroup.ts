...
export class TerminalGroup extends Disposable implements ITerminalGroup {
	...
	private readonly _onDidDisposeInstance: Emitter<ITerminalInstance> = this._register(
    new Emitter<ITerminalInstance>(),
  );
	readonly onDidDisposeInstance = this._onDidDisposeInstance.event;
	private readonly _onDidFocusInstance: Emitter<ITerminalInstance> = this._register(
    new Emitter<ITerminalInstance>(),
  );
	...
	getLayoutInfo(isActive: boolean): ITerminalTabLayoutInfoById {
      ...
      instance => isNumber(instance.persistentProcessId) && instance.shouldPersist,
		...
		const totalSize = instances.map(t => this._splitPaneContainer?.getPaneSize(t) || 0).reduce(
	...
	moveInstance(instances: SingleOrMany<ITerminalInstance>, index: number, position: "before" | "after"): void {
      ...
      instance => !this.terminalInstances.includes(instance),
		...
		for (const item of instances) {
			const originSourceGroupIndex = position === "after" ? this._terminalInstances.indexOf(
        item,
	...
	attachToElement(element: HTMLElement): void {
        ...
        TERMINAL_VIEW_ID,
			...
			const orientation = this._terminalLocation === ViewContainerLocation.Panel && isHorizontal(
        this._panelPosition,
	...
	get title(): string {
		...
		let title = this.terminalInstances[0].title + this._getBellTitle(
      this.terminalInstances[0],
    );
	...
	layout(width: number, height: number): void {
        ...
        TERMINAL_VIEW_ID,
			...
			if (terminalPositionChanged) {
				const newOrientation = newTerminalLocation === ViewContainerLocation.Panel && isHorizontal(
          newPanelPosition,
        ) ? Orientation.HORIZONTAL : Orientation.VERTICAL;
