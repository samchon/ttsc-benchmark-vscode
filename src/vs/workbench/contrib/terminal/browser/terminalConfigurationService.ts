...
export class TerminalConfigurationService extends Disposable implements ITerminalConfigurationService {
	...
	setPanelContainer(panelContainer: HTMLElement): void { return this._fontMetrics.setPanelContainer(
    panelContainer,
  ); }
	configFontIsMonospace(): boolean { return this._fontMetrics.configFontIsMonospace(); }
	getFont(w: Window, xtermCore?: IXtermCore, excludeDimensions?: boolean): ITerminalFont { return this._fontMetrics.getFont(
    w,
    xtermCore,
    excludeDimensions,
  ); }
	shouldCommandSkipShell(commandId: string): boolean { return this._skipTerminalCommands.has(
    commandId,
  ); }
...
export class TerminalFontMetrics extends Disposable {
	...
	configFontIsMonospace(): boolean {
		const fontSize = 15;
		const fontFamily = this._terminalConfigurationService.config.fontFamily || this._configurationService.getValue<IEditorOptions>(
      "editor",
	...
	getFont(w: Window, xtermCore?: IXtermCore, excludeDimensions?: boolean): ITerminalFont {
		const editorConfig = this._configurationService.getValue<IEditorOptions>(
      "editor",
    );
      ...
      MINIMUM_LETTER_SPACING,
		...
		const lineHeight = this._terminalConfigurationService.config.lineHeight ? Math.max(
      ...
      charHeight: 0,
		...
		if (rect && rect.width && rect.height) {
			...
			if (this._terminalConfigurationService.config.gpuAcceleration === "off") {
				...
				this._lastFontMeasurement.charWidth = cssCellWidth - Math.round(
          letterSpacing,
        ) / w.devicePixelRatio;
			}
		}
