...
export class TerminalProfileQuickpick {
	...
	async showAndGetResult(type: "setDefault" | "createInstance"): Promise<IQuickPickTerminalObject | DefaultProfileName | undefined> {
		...
		if (type === "setDefault") {
          ...
          profilesKey,
            ...
            path: result.profile.path,
					...
					(profilesConfig as { [key: string]: ITerminalProfileObject })[result.profile.profileName] = this._createNewProfileConfig(
            result.profile,
          );
					await this._configurationService.updateValue(
            profilesKey,
      ...
      result.profile,
      { profileName: true },
    ) ? result.profile.profileName : result.profile.title;
	}
	...
	private async _createAndShow(type: "setDefault" | "createInstance"): Promise<IProfileQuickPickItem | undefined> {
        ...
        label: nls.localize("terminalProfiles", "profiles"),
			...
			quickPickItems.push(
        ...this._sortProfileQuickPickItems(configProfiles.map(e => this._createProfileQuickPickItem(e)), defaultProfileName!),
      );
		}
      ...
      label: nls.localize("ICreateContributedTerminalProfileOptions", "contributed"),
		...
		if (contributedProfiles.length > 0) {
			quickPickItems.push(
        ...this._sortProfileQuickPickItems(contributedProfiles, defaultProfileName!),
      );
		}
        ...
        label: nls.localize("terminalProfiles.detected", "detected"),
			...
			quickPickItems.push(
        ...this._sortProfileQuickPickItems(autoDetectedProfiles.map(e => this._createProfileQuickPickItem(e)), defaultProfileName!),
      );
		}
	...
	private async _isProfileSafe(profile: ITerminalProfile | IExtensionTerminalProfile): Promise<boolean> {
		const isUnsafePath = hasKey(profile, {
      profileName: true,
    }) && profile.isUnsafePath;
		const requiresUnsafePath = hasKey(profile, {
      profileName: true,
    }) && profile.requiresUnsafePath;
	...
	private _createProfileQuickPickItem(profile: ITerminalProfile): IProfileQuickPickItem {
      ...
      profile.icon,
    )) ? profile.icon : Codicon.terminal;
		const label = `$(${icon.id}) ${profile.profileName}`;
		const friendlyPath = profile.isFromPath ? basename(
      profile.path,
    ) : profile.path;
