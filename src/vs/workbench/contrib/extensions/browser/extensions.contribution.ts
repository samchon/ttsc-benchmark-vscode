...
const CONTEXT_GALLERY_ALL_PRIVATE_REPOSITORY_SIGNED = new RawContextKey<boolean>(
  "galleryAllPrivateRepositorySigned",
  false,
);
...
class ExtensionsContributions extends Disposable implements IWorkbenchContribution {
	...
	constructor(
		...
		super();
		const hasLocalServerContext = CONTEXT_HAS_LOCAL_SERVER.bindTo(
      contextKeyService,
    );
		...
		const hasRemoteServerContext = CONTEXT_HAS_REMOTE_SERVER.bindTo(
      contextKeyService,
    );
		...
		const hasWebServerContext = CONTEXT_HAS_WEB_SERVER.bindTo(
      contextKeyService,
    );
	...
	private async updateGalleryCapabilitiesContexts(extensionGalleryManifest: IExtensionGalleryManifest | null): Promise<void> {
      ...
      !!(extensionGalleryManifest && getExtensionGalleryManifestResourceUri(extensionGalleryManifest, ExtensionGalleryResourceType.ExtensionDetailsViewUri)),
    );
	}
	...
	private registerExtensionAction(extensionActionOptions: IExtensionActionOptions): IDisposable {
		const menus = extensionActionOptions.menu ? Array.isArray(
      extensionActionOptions.menu,
    ) ? extensionActionOptions.menu : [extensionActionOptions.menu] : [];
...
class ExtensionToolsContribution extends Disposable implements IWorkbenchContribution {
	...
	constructor(
		...
		super();
		const searchExtensionsTool = instantiationService.createInstance(
