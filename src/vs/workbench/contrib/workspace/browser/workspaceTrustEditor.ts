	...
	renderElement(item: ITrustedUriItem, index: number, templateData: ITrustedUriPathColumnTemplateData): void {
      ...
      addDisposableListener(
		...
		const accept = () => {
			...
			const uri = hasDriveLetter(pathToUse) ? item.uri.with({
        path: posix.sep + toSlashes(pathToUse),
...
function getHostLabel(labelService: ILabelService, item: ITrustedUriItem): string {
	return item.uri.authority ? labelService.getHostLabel(
    item.uri.scheme,
    item.uri.authority,
...
export class WorkspaceTrustEditor extends EditorPane {
	...
	private registerListeners(): void {
      ...
      this.configurationService.onDidChangeRestrictedSettings(
		...
		this._register(
      this.workspaceTrustManagementService.onDidChangeTrust(() => this.render()),
    );
		this._register(
      this.workspaceTrustManagementService.onDidChangeTrustedFolders(
        () => this.render(),
	...
	private async render() {
		...
		// Header Section
		this.headerTitleText.innerText = this.getHeaderTitleText(
      isWorkspaceTrusted,
    );
      ...
      `command:workbench.trust.configure`,
		...
		this.headerContainer.className = this.getHeaderContainerClass(
      isWorkspaceTrusted,
    );
	...
	private getExtensionCount(): number {
      ...
      this.workspaceService.getWorkspace(),
		...
		const localExtensions = this.extensionWorkbenchService.local.filter(ext => ext.local).map(
      ext => ext.local!,
    );
        ...
        ext => this.extensionManifestPropertiesService.getExtensionUntrustedWorkspaceSupportType(ext.manifest) === false,
      )) {
				set.add(extension.identifier.id);
			}
		}
	...
	private async renderAffectedFeatures(numSettings: number, numExtensions: number): Promise<void> {
      ...
      ThemeIcon.asClassNameArray(checkListIcon),
		...
		const [untrustedTitle, untrustedSubTitle] = this.getFeaturesHeaderText(
      false,
    );
