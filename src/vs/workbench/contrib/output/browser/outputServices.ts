...
export class OutputService extends Disposable implements IOutputService, ITextModelContentProvider {
	...
	constructor(
      ...
      "",
		...
		this.activeOutputChannelContext = ACTIVE_OUTPUT_CHANNEL_CONTEXT.bindTo(
      contextKeyService,
    );
      ...
      this.onActiveOutputChannel(
		...
		this.activeFileOutputChannelContext = CONTEXT_ACTIVE_FILE_OUTPUT.bindTo(
      contextKeyService,
    );
		this.activeLogOutputChannelContext = CONTEXT_ACTIVE_LOG_FILE_OUTPUT.bindTo(
      contextKeyService,
    );
		this.activeOutputChannelLevelSettableContext = CONTEXT_ACTIVE_OUTPUT_LEVEL_SETTABLE.bindTo(
      contextKeyService,
    );
		this.activeOutputChannelLevelContext = CONTEXT_ACTIVE_OUTPUT_LEVEL.bindTo(
      contextKeyService,
    );
		this.activeOutputChannelLevelIsDefaultContext = CONTEXT_ACTIVE_OUTPUT_LEVEL_IS_DEFAULT.bindTo(
      contextKeyService,
    );
      ...
      registry.onDidRemoveChannel(channel => this.onDidRemoveChannel(channel)),
        ...
        channels && channels.length > 0 ? this.getChannel(channels[0].id) : undefined,
      );
		}
	...
	getChannelDescriptor(id: string): IOutputChannelDescriptor | undefined {
		return Registry.as<IOutputChannelRegistry>(Extensions.OutputChannels).getChannel(
      id,
    );
	}
	...
	getLogLevel(channel: IOutputChannelDescriptor): LogLevel | undefined {
      ...
      channel.source,
    ] : isMultiSourceOutputChannelDescriptor(channel) ? channel.source : [];
		if (sources.length === 0) {
			return undefined;
		}
	...
	async saveOutputAs(outputPath?: URI, ...channels: IOutputChannelDescriptor[]): Promise<void> {
        ...
        channel.uri,
          ...
          VSBuffer.fromString(modelRef.object.textEditorModel.getValue()),
			...
			if (channels.length > 1) {
				Registry.as<IOutputChannelRegistry>(Extensions.OutputChannels).removeChannel(
          channel.id,
        );
			}
		}
	...
	private async onDidRegisterChannel(channelId: string): Promise<void> {
		...
		if (!this.activeChannel || this.activeChannelIdInStorage === channelId) {
			...
			const outputView = this.viewsService.getActiveViewWithId<OutputViewPane>(
        OUTPUT_VIEW_ID,
      );
	...
	private instantiateChannel(id: string): OutputChannel {
		const channelData = Registry.as<IOutputChannelRegistry>(Extensions.OutputChannels).getChannel(
      id,
    );
		if (!channelData) {
		...
		if (!this.outputFolderCreationPromise) {
			this.outputFolderCreationPromise = this.fileService.createFolder(this.outputLocation).then(
        () => undefined,
      );
		}
		return this.instantiationService.createInstance(
	...
	private setLevelContext(): void {
		...
		const channelLogLevel = descriptor ? this.getLogLevel(
      descriptor,
    ) : undefined;
	...
	private async setLevelIsDefaultContext(): Promise<void> {
      ...
      descriptor,
		...
		if (channelLogLevel !== undefined) {
			const channelDefaultLogLevel = this.defaultLogLevelsService.getDefaultLogLevel(
