...
export class SimpleSettingRenderer {
	...
	getActions(uri: URI) {
      ...
      uri.path.substring(1),
		...
		const currentSettingValue = this._configurationService.inspect(
      settingId,
    ).userValue;

		if ((newSettingValue !== undefined) && newSettingValue === currentSettingValue && this._updatedSettings.has(
      settingId,
    )) {
			...
			const setting = this.getSetting(settingId);
			const trySettingMessage = setting ? this.getSettingMessage(
        setting,
        newSettingValue,
      ) : undefined;
