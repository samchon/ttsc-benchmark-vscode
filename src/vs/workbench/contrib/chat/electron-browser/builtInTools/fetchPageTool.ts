...
export class FetchWebPageTool implements IToolImpl {
	...
	async invoke(invocation: IToolInvocation, _countTokens: CountTokensCallback, _progress: ToolProgress, token: CancellationToken): Promise<IToolResult> {
      ...
      urls,
		...
		if (webContents.every(
      e => e.status === "error" || e.status === "redirect",
    )) {
	...
	async prepareToolInvocation(context: IToolInvocationPreparationContext, token: CancellationToken): Promise<IPreparedToolInvocation | undefined> {
      ...
      ...fileUrisOutsideWorkspace,
		...
		if (context.chatSessionResource) {
			...
			const userMessages = model?.getRequests().map(
        r => r.message.text.toLowerCase(),
      );
      ...
      pastTenseMessage,
		...
		const allDomainsTrusted = Iterable.every(
      urlsNeedingConfirmation,
      u => this._trustedDomainService.isValid(u),
    );
