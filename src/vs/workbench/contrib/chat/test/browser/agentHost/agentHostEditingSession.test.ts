...
function createSession(
  ...
  opts?: { computeDiffResult?: IDocumentDiff | null },
  ...
  const mockTextModelService = new (class extends mock<ITextModelService>() {
    override async createModelReference(uri: URI) {
      ...
      return {
        object: {
          textEditorModel: { uri, getValue: () => content },
        } as IResolvedTextEditorModel,
        dispose: () => {},
      };
    }
  })();
  const mockEditorWorkerService =
...
suite("AgentHostEditingSession", () => {
  ...
  test("getEntry finds entry by URI", () => {
    ...
    assert.strictEqual(
      session.getEntry(URI.parse("file:///nonexistent")),
      undefined,
    );
  });
  ...
  test("getDiffForSession aggregates diff stats", async () => {
    ...
    const stats = await waitForObservable(
      session.getDiffForSession(),
      (s) => s.added === 10,
    );
  ...
  suite("undo/redo", () => {
    test("undo writes before-content to file and updates state", async () => {
      const beforeContentUri = toAgentHostUri(
        URI.parse("content://before-1"),
        "local",
      );
    ...
    test("redo writes after-content to file", async () => {
      ...
      const afterContentUri = toAgentHostUri(
        URI.parse("content://after-1"),
        "local",
      );
      const fileUri = toAgentHostUri(URI.file("/workspace/file.ts"), "local");
      const contentMap = new Map<string, string>();
    ...
    test("undo after multiple checkpoints removes entries correctly", async () => {
      const contentMap = new Map<string, string>();
      contentMap.set(
        toAgentHostUri(URI.parse("content://before-a"), "local").toString(),
        "a-before",
      );
      contentMap.set(
        toAgentHostUri(URI.parse("content://before-b"), "local").toString(),
        "b-before",
      );
      contentMap.set(
        toAgentHostUri(URI.file("/workspace/a.ts"), "local").toString(),
        "a-current",
      );
      contentMap.set(
        toAgentHostUri(URI.file("/workspace/b.ts"), "local").toString(),
        "b-current",
      );
      const session = createSession(store, contentMap);
  ...
  suite("getDiffsForFilesInSession", () => {
    test("returns diffs for all files", async () => {
      ...
      const diffs = await waitForObservable(
        session.getDiffsForFilesInSession(),
        (d) => d.length > 0 && d[0].added > 0,
      );
  ...
  suite("getDiffsForFilesInRequest", () => {
    test("returns diffs scoped to a request", async () => {
      ...
      const req1Diffs = await waitForObservable(
        session.getDiffsForFilesInRequest("req-1"),
        (d) => d.length > 0 && d[0].added > 0,
      );
      assert.strictEqual(req1Diffs.length, 1);
      assert.strictEqual(req1Diffs[0].added, 5);

      const req2Diffs = await waitForObservable(
        session.getDiffsForFilesInRequest("req-2"),
        (d) => d.length > 0 && d[0].added > 0,
      );
  ...
  suite("snapshots", () => {
    test("getSnapshotUri returns URI for valid checkpoint", () => {
      ...
      const entry = session.entries.get()[0];
      const snapshotUri = session.getSnapshotUri(
        "req-1",
        entry.modifiedURI,
        undefined,
      );
    ...
    test("getSnapshotUri returns undefined for unknown request", () => {
      ...
      const result = session.getSnapshotUri(
        "nonexistent",
        URI.file("/x"),
        undefined,
      );
    ...
    test("getSnapshotContents fetches content from connection", async () => {
      ...
      const wrappedAfterUri = toAgentHostUri(
        URI.parse("content://after"),
        "local",
      );
      contentMap.set(wrappedAfterUri.toString(), "file content here");

      const entry = session.entries.get()[0];
      const content = await session.getSnapshotContents(
        "req-1",
        entry.modifiedURI,
        undefined,
      );
  ...
  suite("getEntryDiffBetweenStops", () => {
    test("returns undefined for unknown requestId", () => {
      const session = createSession(store, new Map());
      const result = session.getEntryDiffBetweenStops(
        URI.file("/f"),
        "unknown-req",
        undefined,
      );
    ...
    test("computes diff for a known stop", async () => {
      const contentMap = new Map<string, string>();
      const beforeUri = toAgentHostUri(
        URI.parse("content://before-1"),
        "local",
      );
      ...
      const entry = session.entries.get()[0];
      const diffObs = session.getEntryDiffBetweenStops(
        entry.modifiedURI,
        "req-1",
        "tc-1",
      );
  ...
  suite("getEntryDiffBetweenRequests", () => {
    test("returns undefined for unknown requests", () => {
      ...
      const diffObs = session.getEntryDiffBetweenRequests(
        URI.file("/f"),
        "unknown",
        "also-unknown",
      );
  ...
  suite("requestDisablement", () => {
    ...
    test("disables requests after undo", async () => {
      const contentMap = new Map<string, string>();
      contentMap.set(
        toAgentHostUri(URI.file("/workspace/file.ts"), "local").toString(),
        "before",
      );
      const session = createSession(store, contentMap);
    ...
    test("clears disablement after redo", async () => {
      ...
      contentMap.set(
        toAgentHostUri(URI.parse("content://after-2"), "local").toString(),
        "after-2",
      );
      const session = createSession(store, contentMap);
  ...
  suite("restoreSnapshot", () => {
    test("restoreSnapshot with undefined stopId navigates before the request", async () => {
      ...
      contentMap.set(
        toAgentHostUri(URI.parse("content://after-1"), "local").toString(),
        "after-1",
      );
      const session = createSession(store, contentMap);
      ...
      assert.strictEqual(session.entries.get().length, 1);
      assert.strictEqual(
        session.entries.get()[0].lastModifyingRequestId,
        "req-1",
      );
      // req-2 should be disabled
      assert.strictEqual(session.requestDisablement.get().length, 1);
      assert.strictEqual(
        session.requestDisablement.get()[0].requestId,
        "req-2",
      );
    });
