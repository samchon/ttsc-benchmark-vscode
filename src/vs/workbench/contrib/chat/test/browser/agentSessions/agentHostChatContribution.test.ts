  ...
  suite("session ID resolution", () => {
    test("requests backend session for provider-owned new resource", () =>
      runWithFakedTimers({ useFakeTimers: true }, async () => {
        ...
        assert.strictEqual(
          (agentHostService.turnActions[0].action as ITurnStartedAction)
            .userMessage.text,
          "Hello",
        );
      ...
      runWithFakedTimers({ useFakeTimers: true }, async () => {
        ...
        assert.deepStrictEqual(
          agentHostService.turnActions.map(
            (d) => (d.action as ITurnStartedAction).userMessage.text,
          ),
          ["Recovered"],
        );
      }));
  ...
  suite("server-initiated turns", () => {
    ...
    test("syncs text updates for existing queued pending messages", async () => {
      ...
      agentHostService.sessionStates.set(backendSession.toString(), {
        ...
        lifecycle: SessionLifecycle.Ready,
        queuedMessages: [
          { id: "queued-request-1", userMessage: { text: "old queued text" } },
        ],
      });
      ...
      runWithFakedTimers({ useFakeTimers: true }, async () => {
        ...
        agentHostService.fireAction({
          ...
          action: {
            ...
            kind: "queued",
            id: "q-1",
            userMessage: { text: "will be consumed" },
          } as SessionAction,
          serverSeq: 3,
          origin: undefined,
        });
      ...
      runWithFakedTimers({ useFakeTimers: true }, async () => {
        ...
        agentHostService.fireAction({
          ...
          action: {
            ...
            kind: "steering",
            id: "steer-1",
            userMessage: { text: "be more careful" },
          } as SessionAction,
          serverSeq: 3,
          origin: undefined,
        });
  ...
  suite("customizations", () => {
    ...
    test("re-dispatches activeClientChanged when customizations observable changes", async () => {
      ...
      const ac = activeClientAction!.action as {
        activeClient: { customizations?: CustomizationRef[] };
      };
    ...
    test("dispatches activeClientChanged when restoring a session where current client customizations are stale", async () => {
      ...
      agentHostService.sessionStates.set(sessionResource.toString(), {
        ...
        activeClient: {
          ...
          tools: [],
          customizations: [
            { uri: "file:///plugin-old", displayName: "Plugin Old" },
          ],
        },
      });
