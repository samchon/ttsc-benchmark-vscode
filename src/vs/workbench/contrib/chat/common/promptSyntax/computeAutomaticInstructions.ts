...
export class ComputeAutomaticInstructions {
	...
	public async collect(variables: ChatRequestVariableSet, token: CancellationToken): Promise<void> {
		...
		const instructionFiles = await this._promptsService.getInstructionFiles(
      token,
    );
	...
	private sendTelemetry(telemetryEvent: InstructionsCollectionEvent): void {
		...
		this._telemetryService.publicLog2<InstructionsCollectionEvent, InstructionsCollectionClassification>(
	...
	private async _logSkillLoadedTelemetry(skills: readonly IAgentSkill[]): Promise<void> {
		...
		try {
			...
			for (const skill of skills) {
				const skillPlugin = skill.pluginUri ? pluginByUri.get(
          skill.pluginUri,
        ) : undefined;
				this._telemetryService.publicLog2<SkillLoadedIntoContextEvent, SkillLoadedIntoContextClassification>(
	...
	public async addApplyingInstructions(instructionFiles: readonly IInstructionFile[], context: { files: ResourceSet; instructions: ResourceSet }, variables: ChatRequestVariableSet, telemetryEvent: InstructionsCollectionEvent, debugInfo: InstructionsCollectionDebugInfo, token: CancellationToken): Promise<void> {
      ...
      PromptsConfig.INCLUDE_APPLYING_INSTRUCTIONS,
        ...
        currentSessionType,
          ...
          name: basename(uri).toString(),
          uri,
          reason: localize("debugDetail.noMatch", "applyTo '{0}' did not match any attached files", pattern),
        });
			}
		}
	}
	...
	private async _getCustomizationsIndex(instructionFiles: readonly IInstructionFile[], _existingVariables: ChatRequestVariableSet, telemetryEvent: InstructionsCollectionEvent, debugInfo: InstructionsCollectionDebugInfo, token: CancellationToken): Promise<IPromptTextVariableEntry | undefined> {
        ...
        PromptsConfig.USE_NESTED_AGENT_MD,
			...
			const agentsMdPromise = searchNestedAgentMd ? this._promptsService.listNestedAgentMDs(
        token,
      ) : Promise.resolve([]);
		...
		if (runSubagentTool) {
			const generalPurposeAgentEnabled = !!this._configurationService.getValue<boolean>(
        ...
        a => a.enabled,
          ...
          `Use the ${runSubagentTool.variable} tool with the agent name to run the subagent.`,
            ...
            `<description>Full-capability agent for complex multi-step tasks requiring high-quality reasoning. Has access to the same tools and capabilities as the current agent and inherits the parent agent's model and system prompt. Use for tasks that don't fit a more specialized agent.</description>`,
              ...
              name: agent.name,
              uri: agent.uri,
              reason: localize("debugDetail.agentNotInvocable", "not invocable by model"),
            });
					}
				}
		...
		while (next) {
			...
			if (result && result.body) {
				...
				for (const ref of result.body.fileReferences) {
					...
					if (url && !seen.has(url) && (isPromptOrInstructionsFile(
            url,
