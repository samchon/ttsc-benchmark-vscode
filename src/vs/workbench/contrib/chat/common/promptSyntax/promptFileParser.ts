	...
	private get _parsedHeader(): ParsedHeader {
        ...
        "",
			...
			const asRange = ({ startOffset, endOffset }: { startOffset: number; endOffset: number }): Range => {
				const startPos = transformer.getPosition(
          startOffset,
        ...
        0,
			...
			for (let i = this.range.startLineNumber - 1, lineStartOffset = bodyOffset; i < this.range.endLineNumber - 1; i++) {
				...
				if (fenceMatch) {
					...
					if (fencedCodeBlockFenceChar === fenceChar && fenceLength >= fencedCodeBlockFenceLength && /^\s*$/.test(
            restOfLine,
          )) {
            ...
            match.index + fullMatch.length + 1,
					...
					if (markdownLinkRanges.some(
            mdRange => Range.areIntersectingOrTouching(mdRange, fullRange),
          )) {
						continue;
					}
					if (isInsideInlineCode(match.index)) {
...
export function parseCommaSeparatedList(stringValue: IScalarValue): ISequenceValue {
	...
	while (pos < input.length) {
      ...
      type: "scalar",
      value: value,
      range: new Range(positionOffset.lineNumber, positionOffset.column + startPos, positionOffset.lineNumber, positionOffset.column + endPos),
      format: quoteStyle,
    });
