...
export class PromptBodyAutocompletion implements CompletionItemProvider {
	...
	private async collectFilePathCompletions(model: ITextModel, position: Position, pathRange: Range, suggestions: CompletionItem[]): Promise<void> {
      ...
      pathRange.setEndPosition(position.lineNumber, position.column),
		...
		const pathSeparator = pathUntilPosition.includes(
      "/",
    ) || !pathUntilPosition.includes("\\") ? "/" : "\\";
		...
		if (pathUntilPosition.match(/[^\/]\.\.$/i)) { // ends with `..`
			...
			while (i >= 0 && ![CharCode.Slash, CharCode.Backslash].includes(
        pathUntilPosition.charCodeAt(i),
      )) {
				i--;
			}
			parentFolderPath = pathUntilPosition.substring(
        0,
        i + 1,
      ); // the segment up to the `/` or `\` before the position
		}
      ...
      varWord.endColumn,
		...
		if (nameMatch) {
			...
			if (nameMatch[1] === "file:") {
				return {
          type: "file",
          contentRange: new Range(position.lineNumber, contentCol, position.lineNumber, varWord.endColumn),
          range,
        };
