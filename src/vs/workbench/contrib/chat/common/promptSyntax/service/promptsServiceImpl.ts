...
export class PromptsService extends Disposable implements IPromptsService {
	...
	private async computeListPromptFiles(type: PromptsType, token: CancellationToken): Promise<readonly IPromptPath[]> {
      ...
      this.fileLocator.listFiles(type, PromptsStorage.user, token).then(
        uris => uris.map(
          uri => ({ uri, storage: PromptsStorage.user, type } satisfies IUserPromptPath),
        ),
      ),
      this.fileLocator.listFiles(type, PromptsStorage.local, token).then(
        uris => uris.map(
          uri => ({ uri, storage: PromptsStorage.local, type } satisfies ILocalPromptPath),
        ),
      ),
	...
	private async _collectSourceFolderDiagnostics(type: PromptsType): Promise<IPromptSourceFolderResult[]> {
		const resolvedFolders = await this.fileLocator.getSourceFoldersInDiscoveryOrder(
      type,
    );
	...
	public async listPromptFilesForStorage(type: PromptsType, storage: PromptsStorage, token: CancellationToken): Promise<readonly IPromptPath[]> {
		...
		switch (storage) {
			...
			case PromptsStorage.local:
				promptPaths = await this.fileLocator.listFiles(type, PromptsStorage.local, token).then(
          uris => uris.map(
            uri => ({ uri, storage: PromptsStorage.local, type } satisfies ILocalPromptPath),
          ),
        );
			...
			case PromptsStorage.user:
				promptPaths = await this.fileLocator.listFiles(type, PromptsStorage.user, token).then(
          uris => uris.map(
            uri => ({ uri, storage: PromptsStorage.user, type } satisfies IUserPromptPath),
          ),
        );
	...
	private async computeSlashCommandDiscoveryInfo(token: CancellationToken): Promise<ISlashCommandDiscoveryInfo> {
      ...
      PromptsConfig.USE_AGENT_SKILLS,
		...
		const skills = useAgentSkills ? await this.listPromptFiles(
      PromptsType.skill,
      token,
    ) : [];
	...
	private slashCommandsFromDiscoveryInfo(discoveryInfo: ISlashCommandDiscoveryInfo): readonly IChatPromptSlashCommand[] {
		...
		for (const model of this.modelService.getModels()) {
			if (model.getLanguageId() === PROMPT_LANGUAGE_ID && model.uri.scheme === Schemas.untitled && !seen.has(
        model.uri,
      )) {
				const parsedPromptFile = this.getParsedPromptFile(model);
				const name = parsedPromptFile?.header?.name ?? getCleanPromptName(
          model.uri,
        );
	...
	getPromptLocationLabel(promptPath: IPromptPath): string {
		switch (promptPath.storage) {
			case PromptsStorage.local: return this.labelService.getUriLabel(
        dirname(promptPath.uri),
        { relative: true },
      );
        ...
        "User Data",
          ...
          promptPath.extension.displayName ?? promptPath.extension.id,
			...
			case PromptsStorage.plugin: return localize(
        "plugin.capitalized",
        "Plugin",
      );
	...
	public async listAgentInstructions(token: CancellationToken, logger: Logger | undefined): Promise<IAgentInstructionFile[]> {
		...
		const includeParents = this.configurationService.getValue(
      ...
      this.fileLocator.findFilesInRoots(
		...
		return result.sort(
      (a, b) => a.uri.toString().localeCompare(b.uri.toString()),
    );
	}
	...
	private async computeSkillDiscovery(token: CancellationToken): Promise<IPromptDiscoveryInfo> {
      ...
      f => f.status === "loaded" && f.promptPath.name,
		...
		this.telemetryService.publicLog2<AgentSkillsFoundEvent, AgentSkillsFoundClassification>(
      "agentSkillsFound",
      {
        totalSkillsFound,
	...
	private async computeSkillDiscoveryInfo(token: CancellationToken): Promise<IPromptFileDiscoveryResult[]> {
      ...
      token,
		...
		const pluginSkills = this._pluginPromptFilesByType.get(
      PromptsType.skill,
    ) ?? [];
...
class ModelChangeTracker extends Disposable {
	...
	constructor(modelService: IModelService) {
      ...
      modelService.onModelRemoved(
        model => onRemove(model.getLanguageId(), model.uri),
      ),
    );
	}
