...
export class RunSubagentTool extends Disposable implements IToolImpl {
	...
	getToolData(): IToolData {
		...
		const generalPurposeAgentEnabled = this.configurationService.getValue<boolean>(
	...
	async invoke(invocation: IToolInvocation, _countTokens: CountTokensCallback, _progress: ToolProgress, token: CancellationToken): Promise<IToolResult> {
      ...
      invocation.context.sessionResource,
        ...
        ChatConfiguration.GeneralPurposeAgentEnabled,
			...
			if (subAgentName && !isGeneralPurpose) {
              ...
              args.model,
					...
					if (modeCustomTools) {
						...
						const enablementMap = this.languageModelToolsService.toToolAndToolSetEnablementMap(
              modeCustomTools,
              undefined,
            );
			...
			const allowInvocationsFromSubagents = this.configurationService.getValue<boolean>(
	...
	private checkMultiplierConstraint(modelId: string, mainModelId: string | undefined): { exceeds: false } | { exceeds: true; reason: string } {
		...
		const mainModelMetadata = this.languageModelsService.lookupLanguageModel(
      mainModelId,
    );
		const modelMetadata = this.languageModelsService.lookupLanguageModel(
      modelId,
    );
	...
	private getAvailableModelsInfo(mainModelId: string | undefined): string {
		...
		for (const { id, metadata } of models) {
			const qualifiedName = ILanguageModelChatMetadata.asQualifiedName(
        metadata,
      );
	...
	private resolveSubagentModel(subagent: ICustomAgent | undefined, mainModelId: string | undefined, explicitModelQualifiedName?: string): { modeModelId: string | undefined; resolvedModelName: string | undefined } {
		...
		if (subagent && !explicitModelResolved) {
			...
			if (modeModelQualifiedNames) {
				...
				for (const qualifiedName of modeModelQualifiedNames) {
					const lmByQualifiedName = this.languageModelsService.lookupLanguageModelByQualifiedName(
            qualifiedName,
          );
		...
		const resolvedModelMetadata = modeModelId ? this.languageModelsService.lookupLanguageModel(
      modeModelId,
    ) : undefined;
	...
	async prepareToolInvocation(context: IToolInvocationPreparationContext, _token: CancellationToken): Promise<IPreparedToolInvocation | undefined> {
      ...
      ChatConfiguration.GeneralPurposeAgentEnabled,
		...
		const subagent = (args.agentName && !isGeneralPurpose) ? await this.getSubAgentByName(
      args.agentName,
    ) : undefined;
