...
export class ChatRequestParser {
	...
	parseChatRequest(sessionResource: URI, message: string, location: ChatAgentLocation = ChatAgentLocation.Chat, context: IChatParserContext = {}): IParsedChatRequest {
		const references = this.variableService.getDynamicVariables(
      sessionResource,
    ); // must access this list before any async calls
		const selectedToolAndToolSets = this.variableService.getSelectedToolAndToolSets(
      sessionResource,
    );
	...
	private tryToParseVariable(message: string, offset: number, position: IPosition, parts: ReadonlyArray<IParsedChatRequestPart>, toolsByName: ReadonlyMap<string, IToolData>, toolSetsByName: ReadonlyMap<string, IToolSet>): ChatRequestToolPart | ChatRequestToolSetPart | undefined {
      ...
      position.column + full.length,
		...
		if (toolset) {
			const value = Array.from(toolset.getTools()).map(
        t => new ChatRequestToolPart(varRange, varEditorRange, t.toolReferenceName ?? t.displayName, t.id, t.displayName, t.icon).toVariableEntry(),
      );
      ...
      c => c.command === command && matchesSessionType(c.sessionTypes, context?.sessionType),
		...
		if (!usedAgent || slashCommand?.silent || capabilities?.supportsPromptAttachments) {
          ...
          c => c.name === command,
			...
			const isPromptCommand = this.promptsService.isValidSlashCommandName(
        command,
      );
			if (isPromptCommand) {
