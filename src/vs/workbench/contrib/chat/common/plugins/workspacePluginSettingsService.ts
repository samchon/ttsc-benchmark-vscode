...
export const IWorkspacePluginSettingsService = createDecorator<IWorkspacePluginSettingsService>(
...
function parseExtraMarketplaces(json: unknown, logPrefix: string, logService: ILogService): readonly IWorkspaceMarketplaceEntry[] {
	...
	for (const [name, value] of Object.entries(obj)) {
		...
		const reference = marketplaceEntryToReference(
      value as IExtraMarketplaceJson,
    );
		if (!reference) {
			logService.debug(
...
class WorkspaceSettingsReader extends Disposable {
	...
	constructor(
      ...
      () => workspaceContextService.getWorkspace().folders.map(
        f => f.uri.path ? joinPath(f.uri, configFolder) : joinPath(f.uri.with({ path: "/" }), configFolder),
      ),
    );
