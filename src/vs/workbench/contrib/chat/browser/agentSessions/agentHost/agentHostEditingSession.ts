...
class AgentHostModifiedFileEntry implements IModifiedFileEntry {
  ...
  readonly isCurrentlyBeingModifiedBy = constObservable<
    | { responseModel: IChatResponseModel; undoStopId: string | undefined }
    | undefined
  >(undefined);
  readonly lastModifyingResponse = constObservable<
  ...
  readonly autoAcceptController = constObservable<
  ...
  getEditorIntegration(
    ...
    return {
      ...
      async acceptNearestChange(
        _change?: IModifiedFileEntryChangeHunk,
      ): Promise<void> {
        /* no-op */
      },
...
export class AgentHostEditingSession
  extends Disposable
  implements IChatEditingSession
{
  ...
  private readonly _entriesObs = observableValue<
    readonly AgentHostModifiedFileEntry[]
  >(this, []);
    ...
    derivedOpts(
      {
        equalsFn: (a, b) =>
          ...
          a.every(
            (v, i) =>
              v.requestId === b[i].requestId &&
              v.afterUndoStop === b[i].afterUndoStop,
          ),
      },
      (reader) => {
  ...
  addToolCallEdits(requestId: string, tc: ToolCallState): IChatProgress[] {
    ...
    const edits: IToolCallFileEdit[] = fileEdits.map(
      (edit: IToolCallFileEdit) => ({
        kind: edit.kind,
    ...
    for (const edit of edits) {
      ...
      if (
        edit.kind === FileEditKind.Create ||
        edit.kind === FileEditKind.Delete ||
        edit.kind === FileEditKind.Rename
      ) {
        progressParts.push({
          kind: "workspaceEdit",
          edits: [
            {
              oldResource:
                ...
                (edit.kind === FileEditKind.Delete ? edit.resource : undefined),
              newResource:
                edit.kind === FileEditKind.Delete ? undefined : edit.resource,
            },
          ],
        });
      }
      ...
      requestId !== undefined
        ? this._checkpoints.findIndex(
            (cp) =>
              ...
              (stopId === undefined || cp.undoStopId === stopId),
          )
        : -1;
   ...
   */
  private _getFileDiffObservable(
    uri: URI,
    fromIdx: number,
    toIdx: number,
  ): IObservable<IEditSessionEntryDiff | undefined> {
    ...
    for (
      ...
      i <= toIdx && i < this._checkpoints.length;
      i++
    ) {
      for (const edit of this._checkpoints[i].edits) {
        if (edit.resource.toString() === uriStr) {
   ...
   */
  private _computeFileDiffObservable(
    beforeUri: URI,
    afterUri: URI,
    fileUri: URI,
  ): IObservable<IEditSessionEntryDiff | undefined> {
   ...
   */
  private async _computeFileDiff(
    beforeUri: URI,
    afterUri: URI,
    fileUri: URI,
    ...
    try {
      const beforeRef =
        await this._textModelService.createModelReference(beforeUri);
      refs.add(beforeRef);
      const afterRef =
        await this._textModelService.createModelReference(afterUri);
      refs.add(afterRef);
  ...
  getDiffsForFilesInRequest(
    ...
    return derived(this, (r) => {
      ...
      return this._readDiffsFromCheckpoints(
        filteredCheckpoints[0] - 1,
        filteredCheckpoints[filteredCheckpoints.length - 1],
      );
    }).map((diffs, r) => diffs?.read(r) || []);
  }
  ...
  private async _redoInteractionImpl(): Promise<void> {
    ...
    let nextIdx = idx + 1;
    while (
      nextIdx < this._checkpoints.length &&
      this._checkpoints[nextIdx].undoStopId === undefined
    ) {
      nextIdx++;
    }
  ...
  private async _writeCheckpointContent(
    ...
    const ops = checkpoint.edits.map(async (edit) => {
      try {
        ...
        this._logService.warn(
          `[AgentHostEditingSession] Failed to ${direction === "before" ? "undo" : "redo"} ${edit.kind} for ${edit.resource.toString()}`,
          err,
        );
      }
    });
  ...
  private _readDiffsFromCheckpoints(
    ...
    const observables = uris.map((uri) =>
      this._getFileDiffObservable(uri, fromIdx, toIdx),
    );

    return derived((reader) =>
      observables.flatMap((o) => o.read(reader)).filter(isDefined),
    );
  }
}
