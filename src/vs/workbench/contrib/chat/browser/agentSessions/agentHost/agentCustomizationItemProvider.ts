  ...
  implements ICustomizationItemProvider
{
  ...
  constructor(
    ...
    this._agentCustomizations =
      this._readRootCustomizations(rootStateSubscription.value) ??
      this._agentInfo.customizations ??
      [];

    this._register(
      rootStateSubscription.onDidChange((rootState) => {
        const next =
          this._readRootCustomizations(rootState) ??
          this._readAgentCustomizations(rootState) ??
          this._agentCustomizations;
    ...
    this._register(
      this._connection.onDidAction((envelope) => {
        if (
  ...
  private _readRootCustomizations(
    rootState: RootState | Error | undefined,
  ): readonly CustomizationRef[] | undefined {
  ...
  private _readAgentCustomizations(
    rootState: RootState | Error | undefined,
  ): readonly CustomizationRef[] | undefined {
...
function toStatusString(
  status: CustomizationStatus | undefined,
): "loading" | "loaded" | "degraded" | "error" | undefined {
  switch (status) {
    case CustomizationStatus.Loading:
      return "loading";
    case CustomizationStatus.Loaded:
      return "loaded";
    case CustomizationStatus.Degraded:
      return "degraded";
    case CustomizationStatus.Error:
      return "error";
    default:
      return undefined;
  }
