  ...
  implements IChatSessionContentProvider
{
        ...
        if (sessionState) {
          ...
          if (sessionState.activeTurn) {
            ...
            history.push({
              ...
              variableData: userMessageToVariableData(
                sessionState.activeTurn.userMessage,
                this._config.connectionAuthority,
              ),
            });
            history.push({
    ...
    if (!isNewSession) {
      ...
      if (this._pendingHistoryTurns.has(sessionResource)) {
        session.registerDisposable(
          Event.once(this._chatService.onDidCreateModel)((model) => {
  ...
  private _syncPendingMessages(
    ...
    if (currentSteering) {
      if (
        ...
        currentSteering.text !== prevSteering.userMessage.text
      ) {
        this._dispatchAction(backendSession, {
          ...
          userMessage: {
            text: currentSteering.text,
            attachments: currentSteering.attachments,
          },
        });
      }
  ...
  private _getCurrentActiveClient(
    customizations: CustomizationRef[] = this._config.customizations?.get() ??
      [],
  ): SessionActiveClient {
    return {
              ...
              for (const rp of turn.responseParts) {
                if (rp.kind === ResponsePartKind.ToolCall) {
                  ...
                  if (
                    ...
                    const fileEditParts =
                      completedToolCallToEditParts(completedTc);
   ...
   */
  private _ensureEditingSession(
    sessionResource: URI,
  ): AgentHostEditingSession | undefined {
    const chatModel = this._chatService.getSession(sessionResource);
    if (!chatModel) {
      return undefined;
    }
