...
} from "../../widget/input/chatInputPickerActionItem.js";
...
export class WorkbenchAgentHostAgentPickerActionItem extends ChatInputPickerActionViewItem {
  private readonly _currentAgent = observableValue<
    CustomizationAgentRef | undefined
  >("agentHostCurrentAgent", undefined);
  ...
  constructor(
    ...
    > = {
      ...
      reporter: {
        id: "ChatAgentHostAgentPicker",
        name: "ChatAgentHostAgentPicker",
        includeOptions: true,
      },
    };
    ...
    this._register(
      this._provisional.onDidChange((changed) => {
      ...
      const toolbarActions: IAction[] = [
        {
          ...
          enabled: true,
          run: async () => {
            void this._openerService.open(agentUri);
          },
        },
      ];
      rows.push({
  ...
  private _resolveBackend(sessionResource: URI): URI | undefined {
    return (
      ...
      toBackendSessionUri(sessionResource)
    );
  }

  private _reattach(): void {
    const resource = this._sessionResource();
    const provisionalBackend = resource
      ? this._provisional.get(resource)
      : undefined;
    const fallbackBackend = resource
      ? toBackendSessionUri(resource)
      : undefined;
    ...
    const readyToSubscribe =
      !!backend &&
      (!resource || !isUntitledChatSession(resource) || !!provisionalBackend);
    const targetBackend = readyToSubscribe ? backend : undefined;

    if (
      this._subRef.value?.backendSession.toString() ===
