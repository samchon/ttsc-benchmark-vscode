...
export async function resolveTokenForResource(
	...
	for (const server of authorizationServers) {
		...
		const providerId = await authenticationService.getOrActivateProviderIdForServer(
      serverUri,
      resourceServer,
    );
		if (!providerId) {
	...
	for (const agent of agents) {
        ...
        options.logPrefix,
          ...
          `${options.logPrefix} No token resolved for resource: ${resource.resource}`,
			...
			if (options.authTokenCache && !options.authTokenCache.updateAndIsChanged(
        resource.resource,
        token,
      )) {
	...
	for (const resource of protectedResources) {
      ...
      options.logPrefix,
		...
		for (const server of resource.authorization_servers ?? []) {
			const serverUri = URI.parse(server);
			const providerId = await options.authenticationService.getOrActivateProviderIdForServer(
        serverUri,
        resourceUri,
      );
			if (!providerId) {
