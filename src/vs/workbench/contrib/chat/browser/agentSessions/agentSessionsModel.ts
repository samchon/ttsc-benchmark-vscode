...
export function isAgentSessionSection(obj: unknown): obj is IAgentSessionSection {
	...
	return typeof candidate.section === "string" && Array.isArray(
    candidate.sessions,
  );
}
        ...
        `    Last Request Ended: ${session.timing.lastRequestEnded ? new Date(session.timing.lastRequestEnded).toISOString() : "N/A"}`,
			...
			if (session.metadata && Object.keys(session.metadata).length > 0) {
				...
				for (const [key, value] of Object.entries(session.metadata)) {
					const renderedValue = typeof value === "string" ? value : safeStringify(
            value,
          );
...
export class AgentSessionsModel extends Disposable implements IAgentSessionsModel {
	...
	get sessions(): IAgentSession[] { return Array.from(
	...
	async resolve(provider: string | string[] | undefined): Promise<void> {
		...
		await Promise.all(
      providers.map(
        provider => this.resolveProvider(provider, { refreshProvider: true }),
      ),
    );
	}
	...
	private async doResolveProvider(provider: string, options: { refreshProvider: boolean }, token: CancellationToken): Promise<void> {
		...
		for await (const { chatSessionType, items: providerSessions } of this.chatSessionsService.getChatSessionItems(
      [provider],
      token,
    )) {
			...
			for (const session of providerSessions) {
				...
				if (agentSessionProvider !== undefined) {
					...
					providerLabel = mapSessionContributionToType.get(
            chatSessionType,
	...
	private isMarkedUnread(session: IInternalAgentSessionData): boolean {
		return this.sessionStates.get(
      session.resource,
    )?.read === AgentSessionsModel.UNREAD_MARKER;
	}
...
class AgentSessionsCache {
	...
	saveSessionStates(states: ResourceMap<IAgentSessionState>): void {
		const serialized: ISerializedAgentSessionState[] = Array.from(states.entries()).map(
      ([resource, state]) => ({
      ...
      StorageScope.WORKSPACE,
		...
		try {
			...
			for (const entry of cached) {
				states.set(
          typeof entry.resource === "string" ? URI.parse(entry.resource) : URI.revive(entry.resource),
          {
