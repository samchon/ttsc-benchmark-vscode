...
export class AgentSessionRenderer extends Disposable implements ICompressibleTreeRenderer<IAgentSession, FuzzyScore, IAgentSessionItemTemplate> {
	...
	renderElement(session: ITreeNode<IAgentSession, FuzzyScore>, index: number, template: IAgentSessionItemTemplate, details?: ITreeElementRenderDetails): void {
		...
		if (this.options.useStatusOnlyIcons) {
			...
			template.icon.style.color = statusIcon.color ? asCssVariable(
        statusIcon.color.id,
      ) : "";
			...
			template.icon.style.color = icon.color ? asCssVariable(
        icon.color.id,
      ) : "";
      ...
      matches: createMatches(session.filterData),
		...
		ChatContextKeys.isArchivedAgentSession.bindTo(template.contextKeyService).set(
      session.element.isArchived(),
    );
		ChatContextKeys.isPinnedAgentSession.bindTo(template.contextKeyService).set(
      session.element.isPinned(),
    );
		ChatContextKeys.isReadAgentSession.bindTo(template.contextKeyService).set(
      session.element.isRead(),
    );
		ChatContextKeys.agentSessionType.bindTo(template.contextKeyService).set(
      session.element.providerType,
		...
		template.pinnedIndicator.className = "agent-session-pinned-indicator " + (ThemeIcon.asClassName(
      Codicon.pinned,
    ));
		...
		if (!isSessionInProgressStatus(
      session.element.status,
    ) && diff && hasValidDiff(diff)) {
		...
		ChatContextKeys.hasAgentSessionChanges.bindTo(template.contextKeyService).set(
      hasAgentSessionChanges,
		...
		const hideDetails = hasDescription && isSessionInProgressStatus(
      session.element.status,
    );
...
export class AgentSessionSectionRenderer implements ICompressibleTreeRenderer<IAgentSessionSection, FuzzyScore, IAgentSessionSectionTemplate> {
	...
	renderElement(element: ITreeNode<IAgentSessionSection, FuzzyScore>, index: number, template: IAgentSessionSectionTemplate, details?: ITreeElementRenderDetails): void {
		...
		// Toolbar
		ChatContextKeys.agentSessionSection.bindTo(template.contextKeyService).set(
...
export class AgentSessionsDataSource extends Disposable implements IAsyncDataSource<IAgentSessionsModel, AgentSessionListItem> {
	...
	private groupSessionsIntoSections(sessions: IAgentSession[]): AgentSessionListItem[] {
		...
		const sortedSessions = sorter instanceof AgentSessionsSorter
			? sessions.sort((a, b) => sorter.compare(a, b, true))
	...
	private groupSessionsCapped(sortedSessions: IAgentSession[]): AgentSessionListItem[] {
		...
		const firstArchivedIndex = sortedSessions.findIndex(
      session => session.isArchived(),
		...
		const pinnedSessions = nonArchivedSessions.filter(
	...
	private groupSessionsByRepository(sortedSessions: IAgentSession[]): AgentSessionListItem[] {
		...
		const sortedRepoGroups = [...repoMap.values()].sort(
      (a, b) => compareIgnoreCase(a.label, b.label),
    );
...
export class AgentSessionsDragAndDrop extends Disposable implements ITreeDragAndDrop<AgentSessionListItem> {
	...
	onDragStart(data: IDragAndDropData, originalEvent: DragEvent): void {
		const elements = (data.getData() as AgentSessionListItem[]).filter(
      e => isAgentSession(e),
    );
		const uris = coalesce(elements.map(e => e.resource));
		this.instantiationService.invokeFunction(
      accessor => fillEditorsDragData(accessor, uris, originalEvent),
    );
	}
