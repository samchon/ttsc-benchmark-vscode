...
export class PlanReviewFeedbackEditorContribution extends Disposable implements IEditorContribution {
	...
	constructor(
		...
		this._hasFeedbackContextKey = hasPlanReviewFeedback.bindTo(
      contextKeyService,
    );
	...
	private _isWidgetTarget(target: EventTarget | Element | null): boolean {
		return !!this._widget && !!target && this._widget.getDomNode().contains(
      target as Node,
    );
	}
	...
	private _onModelChanged(): void {
		...
		this._isActivePlan = !!model && this._planReviewFeedbackService.isActivePlanReview(
      model.uri,
    );
		this._updateDecorations();
	}
	...
	private _updatePosition(): void {
		...
		const scrolledPosition = this._editor.getScrolledVisiblePosition(
      cursorPosition,
    );
		if (!scrolledPosition) {
			this._widget.setPosition(null);
