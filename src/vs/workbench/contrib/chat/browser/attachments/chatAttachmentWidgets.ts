...
export class TerminalCommandAttachmentWidget extends AbstractChatAttachmentWidget {
	...
	constructor(
      ...
      attachment.command,
		...
		const clickHandler = () => this.openResource(
      attachment.resource,
...
export class ImageAttachmentWidget extends AbstractChatAttachmentWidget {
	...
	constructor(
      ...
      configurationService,
		...
		const clickHandler = async () => {
			if ((resource || imageData) && configurationService.getValue<boolean>(
    ...
    )?.name ?? this.currentLanguageModel.identifier : "Current model";

		const fullName = resource ? this.labelService.getUriLabel(
      resource,
      ...
      createImageElements(
		...
		const canOpenCarousel = !!imageData && configurationService.getValue<boolean>(
      ChatConfiguration.ImageCarouselEnabled,
    );
...
function createImageElements(resource: URI | undefined, name: string, fullName: string,
	...
	if (previewFeaturesDisabled) {
      ...
      hoverImage,
        ...
        {},
        omittedState === OmittedState.Partial ? localize("chat.imageAttachmentWarning", "This GIF was partially omitted - current frame will be sent.") : fullName,
      );
...
export class DefaultChatAttachmentWidget extends AbstractChatAttachmentWidget {
	...
	private readonly _tooltipHover: MutableDisposable<IDisposable> = this._register(
	...
	constructor(
      ...
      configurationService,
		...
		if (isStringVariableEntry(
      attachment,
    ) && attachment.icon && (ThemeIcon.isFile(
      attachment.icon,
    ) || ThemeIcon.isFolder(attachment.icon)) && attachment.resourceUri) {
      ...
      localize("chat.attachment", "Attached context, {0}", attachment.name),
		...
		if (attachment.kind === "diagnostic") {
			if (attachment.filterUri) {
				resource = attachment.filterUri ? URI.revive(
          attachment.filterUri,
        ) : undefined;
		...
		if ((isStringVariableEntry(
      attachment,
    ) || attachment.kind === "generic") && attachment.tooltip) {
...
export class PromptFileAttachmentWidget extends AbstractChatAttachmentWidget {
	...
	private updateLabel(attachment: IPromptFileVariableEntry) {
      ...
      PromptFileVariableKind.PromptFile,
		...
		const title = this.labelService.getUriLabel(
      resource,
...
export class NotebookCellOutputChatAttachmentWidget extends AbstractChatAttachmentWidget {
	...
	private renderErrorOutput(resource: URI, attachment: INotebookOutputVariableEntry) {
		...
		const withIcon = attachment.icon?.id ? `$(${attachment.icon.id})\u00A0${attachmentLabel}` : attachmentLabel;
		const buffer = this.getOutputItem(
      resource,
      attachment,
	...
	private renderImageOutput(resource: URI, attachment: INotebookOutputVariableEntry) {
		...
		const clickHandler = async () => await this.openResource(
      resource,
      { editorOptions: { preserveFocus: true } },
      false,
      undefined,
		...
		const currentLanguageModelName = this.currentLanguageModel ? this.languageModelsService.lookupLanguageModel(
      this.currentLanguageModel.identifier,
    )?.name ?? this.currentLanguageModel.identifier : undefined;
		const buffer = this.getOutputItem(
      resource,
      attachment,
    )?.data.buffer ?? new Uint8Array();
...
export class ElementChatAttachmentWidget extends AbstractChatAttachmentWidget {
	constructor(
      ...
      title: localize("chat.clickToViewContents", "Click to view the contents of: {0}", attachmentLabel),
    });
	...
	private getHoverContent(attachment: IElementVariableEntry): IDelayedHoverOptions {
      ...
      attachment.computedStyles,
		...
		// ATTRIBUTES section
		if (attachment.attributes && Object.keys(
	...
	private shouldRenderRichElementHover(attachment: IElementVariableEntry): boolean {
      ...
      attachment.attributes,
		...
		if (attachment.computedStyles && Object.keys(
      attachment.computedStyles,
    ).length > 0) {
			return true;
		}
...
export class BrowserViewAttachmentWidget extends AbstractChatAttachmentWidget {
	...
	private _resolveInput(): void {
		const input = this._browserViewService.getKnownBrowserViews().get(
      this._attachment.browserId,
    );
...
export function hookUpResourceAttachmentDragAndContextMenu(accessor: ServicesAccessor, widget: HTMLElement, resource: URI): IDisposable {
    ...
    dom.addDisposableListener(widget, "dragstart", e => {
      instantiationService.invokeFunction(
        accessor => fillEditorsDragData(accessor, [resource], e),
      );
...
export function hookUpSymbolAttachmentDragAndContextMenu(accessor: ServicesAccessor, widget: HTMLElement, parentContextKeyService: IContextKeyService, attachment: { name: string; value: Location; kind: SymbolKind }, contextMenuId: MenuId): IDisposable {
	...
	const ensureContextKeyService = () => {
        ...
        parentContextKeyService.createScoped(widget),
			...
			chatAttachmentResourceContextKey.bindTo(scopedContextKeyService).set(
        attachment.value.uri.toString(),
      );
			setResourceContext(
        accessor,
        scopedContextKeyService,
