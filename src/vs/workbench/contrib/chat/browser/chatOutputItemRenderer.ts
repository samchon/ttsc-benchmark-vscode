...
export const IChatOutputRendererService = createDecorator<IChatOutputRendererService>(
...
export class ChatOutputRendererService extends Disposable implements IChatOutputRendererService {
	...
	hasCodeBlockRenderer(languageIdentifier: string): boolean {
		return Array.from(this._contributions.values()).some(
      value => value.codeBlockLanguageIdentifiers.some(
        identifier => equalsIgnoreCase(identifier, languageIdentifier),
      ),
    );
	}
	...
	private getOrigin(rendererData: RendererEntry): string | undefined {
		return rendererData.options.extension ? this._originStore.getOrigin(
