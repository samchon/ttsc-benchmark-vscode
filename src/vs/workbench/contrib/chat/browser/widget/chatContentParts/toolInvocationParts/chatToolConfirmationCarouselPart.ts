...
export class ChatToolConfirmationCarouselPart extends Disposable {
	...
	private onKeydown(e: KeyboardEvent): void {
		...
		const focusContentAfterNavigation = dom.isHTMLElement(
      e.target,
	...
	private updateMaxHeightStyle(): void {
		...
		const maxHeight = expanded ? Math.max(
      MIN_CAROUSEL_MAX_HEIGHT,
      this.maxHeight,
	...
	private hasInnerContentLargerThanCollapsedLimit(element: HTMLElement): boolean {
		if (this.isExpandableContentElement(element) && this.getElementHeight(
      element,
	...
	private getCollapsedMaxHeight(): number {
		const configuredMaxHeight = this.maxHeight === undefined ? Number.POSITIVE_INFINITY : Math.max(
      MIN_CAROUSEL_MAX_HEIGHT,
      this.maxHeight,
    );
		return Math.min(
