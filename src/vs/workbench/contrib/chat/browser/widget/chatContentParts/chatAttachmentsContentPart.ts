...
export class ChatAttachmentsContentPart extends Disposable {
	...
	private renderAttachment(attachment: IChatRequestVariableEntry, container: HTMLElement) {
		const resource = URI.isUri(attachment.value) ? attachment.value : attachment.value && typeof attachment.value === "object" && "uri" in attachment.value && URI.isUri(
      attachment.value.uri,
    ) ? attachment.value.uri : undefined;
		const range = attachment.value && typeof attachment.value === "object" && "range" in attachment.value && Range.isIRange(
      ...
      (ref) => (typeof ref.reference === "object" && "variableName" in ref.reference && ref.reference.variableName === attachment.name) || (URI.isUri(ref.reference) && basename(ref.reference.path) === attachment.name),
        ...
        this._contextResourceLabels,
			...
			widget = this.chatAttachmentWidgetRegistry.createWidget(
        attachment,
        { shouldFocusClearButton: false, supportsDeletion: false },
        container,
      )
