...
export class ChatPlanReviewPart extends Disposable implements IChatContentPart {
	...
	constructor(
		...
		this._selectedAction = review.actions.find(
      a => a.default,
    ) ?? review.actions[0];
		...
		const isResponseComplete = isResponseVM(
      context.element,
      ...
      new DomScrollableElement(this._messageEl, {
		...
		this._messageScrollable.getDomNode().classList.add(
      ...
      resizeObserver.observe(this._messageScrollable.getDomNode()),
		...
		if (review.canProvideFeedback) {
			...
			if (review.planUri) {
				dom.hide(
          elements.feedback,
        ); // Hidden until the user enters review mode or inline feedback exists.
			} else {
	...
	private renderFeedback(section: HTMLElement): void {
      ...
      new DomScrollableElement(this._commentsListEl, {
		...
		this._commentsListScrollable.getDomNode().classList.add(
      "chat-plan-review-comments-list-scrollable",
    );
	...
	private getInlineFeedbackItems(): readonly IPlanReviewFeedbackItem[] {
		const editingItems = this._editingUri ? this._planReviewFeedbackService.getFeedback(
      this._editingUri,
    ) : [];
		const planItems = this.review.planUri ? this._planReviewFeedbackService.getFeedback(
      ...
      String(!this._isCollapsed),
        ...
        isIconOnly,
			...
			if (isIconOnly) {
				...
				const fileName = this.review.planUri ? basename(
          URI.revive(this.review.planUri),
        ) : "";
	...
	private async mountInlineEditor(mountId: number): Promise<void> {
      ...
      toDisposable(() => editor.setModel(null)),
		...
		const editingRegistration = this._planReviewFeedbackService.registerPlanReview(
      editingUri,
      () => {
        void this.submitFeedback();
      },
      ...
      toDisposable(() => {
		...
		const preExistingComments = this._planReviewFeedbackService.getFeedback(
      planUri,
    );
	...
	private async unmountInlineEditor(options?: { writeBack?: boolean }): Promise<void> {
		...
		const model = editingUri ? this._modelService.getModel(
      editingUri,
    ) : undefined;
		const planUri = this.review.planUri ? URI.revive(
      this.review.planUri,
    ) : undefined;
