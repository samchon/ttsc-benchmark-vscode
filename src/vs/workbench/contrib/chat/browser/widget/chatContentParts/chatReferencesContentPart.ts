...
class CollapsibleListRenderer implements IListRenderer<IChatCollapsibleListItem, ICollapsibleListTemplate> {
	...
	renderElement(data: IChatCollapsibleListItem, index: number, templateData: ICollapsibleListTemplate): void {
		if (data.kind === "warning") {
			templateData.label.setResource({ name: data.content.value }, {
		...
		if (typeof reference === "object" && "variableName" in reference) {
			if (reference.value) {
				const uri = URI.isUri(
          reference.value,
        ) ? reference.value : reference.value.uri;
        ...
        title: data.options?.status?.description ?? data.title,
			...
			if (uri.scheme === "https" && isEqualAuthority(
        uri.authority,
        "github.com",
          ...
          extraClasses,
			...
			} else if (uri.scheme === this.productService.urlProtocol && isEqualAuthority(
        uri.authority,
        SETTINGS_AUTHORITY,
      )) {
		...
		if (data.state !== undefined) {
            ...
            localize(
              ...
              diffMeta.removed,
				...
				// eslint-disable-next-line no-restricted-syntax
				templateData.label.element.querySelector(".monaco-icon-name-container")?.classList.add(
          "modified",
        );
			}
			...
			if (templateData.contextKeyService) {
				if (data.state !== undefined) {
					chatEditingWidgetFileStateContextKey.bindTo(templateData.contextKeyService).set(
            data.state,
          );
				}
			}
