...
export class ChatTodoListWidget extends Disposable {
	...
	constructor(
		...
		this._inChatTodoListContextKey = ChatContextKeys.inChatTodoList.bindTo(
      contextKeyService,
    );
	...
	public clear(sessionResource: URI | undefined, force: boolean = false): void {
		...
		const shouldClear = force || (currentTodos.length > 0 && !currentTodos.some(
	...
	public hasTodos(): boolean {
		return this.domNode.classList.contains(
      "has-todos",
    ) && !!this._todoList && this._todoList.length > 0;
	}
	...
	private toggleExpanded(): void {
      ...
      !this._isExpanded,
		...
		const todoCount = this._currentSessionResource ? this.chatTodoListService.getTodos(
      this._currentSessionResource,
    ).length : 0;
		this.telemetryService.publicLog2<ChatTodoListWidgetEvent, ChatTodoListWidgetClassification>(
      "chatTodoListWidget",
	...
	private updateClearButtonState(): void {
      ...
      todo => todo.status === "in-progress",
		...
		const isRequestInProgress = ChatContextKeys.requestInProgress.getValue(
	...
	private updateTitleElement(titleElement: HTMLElement, todoList: IChatTodo[]): void {
      ...
      todo => todo.status === "not-started",
		...
		const currentTaskNumber = inProgressTodos.length > 0 ? completedCount + 1 : Math.max(
      1,
      completedCount,
    );
