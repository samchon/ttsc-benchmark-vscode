...
export const IChatOutputPartStateCache = createDecorator<IChatOutputPartStateCache>(
...
export class ChatOutputPartStateCache implements IChatOutputPartStateCache {
	...
	constructor(@IStorageService storageService: IStorageService) {
      ...
      storageService.get(LEGACY_CACHE_STORAGE_KEY, StorageScope.WORKSPACE, "{}"),
    );
		...
		try {
			...
			for (const key in data) {
				...
				if (typeof state.height === "number") {
					this._cache.set(key, {
            height: state.height,
            webviewState: typeof state.webviewState === "string" ? state.webviewState : undefined,
          });
				}
			}
		} catch {
