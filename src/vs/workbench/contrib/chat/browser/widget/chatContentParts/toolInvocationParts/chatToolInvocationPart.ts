...
export class ChatToolInvocationPart extends Disposable implements IChatContentPart {
	...
	constructor(
		...
		const render = () => {
			...
			if (toolInvocation.presentation === ToolInvocationPresentation.Hidden || (toolInvocation.presentation === ToolInvocationPresentation.HiddenAfterComplete && IChatToolInvocation.isComplete(
        toolInvocation,
      ))) {
	...
	private createToolInvocationSubPart(): BaseChatToolInvocationSubPart {
		if (this.toolInvocation.kind === "toolInvocation") {
          ...
          this.renderer,
			...
			if (state.type === IChatToolInvocation.StateKind.WaitingForConfirmation) {
				if (this.toolInvocation.toolSpecificData?.kind === "terminal" && !isLegacyChatTerminalToolInvocationData(
          this.toolInvocation.toolSpecificData,
        ) && this.toolInvocation.toolSpecificData.missingSandboxDependencies?.length) {
					return this.instantiationService.createInstance(
      ...
      this.toolInvocation,
		...
		if (this.toolInvocation.kind === "toolInvocation" && this.toolInvocation.toolSpecificData?.kind === "input" && !IChatToolInvocation.isComplete(
      this.toolInvocation,
    )) {
			return this.instantiationService.createInstance(
        ChatInputOutputMarkdownProgressPart,
