...
export class ChatMcpServersInteractionContentPart extends Disposable implements IChatContentPart {
	...
	private updateDetailedProgress(state: IAutostartResult): void {
      ...
      tooltip: localize("mcp.skip.tooltip", "Skip starting this MCP server"),
		...
		if (state.starting.length === 0) {
			content = new MarkdownString(undefined, { isTrusted: true }).appendText(localize("mcp.working.mcp", "Activating MCP extensions...") + " ").appendMarkdown(
        skipText,
      );
		} else {
			// Update to show specific server names as command links
			const serverLinks = this.createServerCommandLinks(state.starting);
			content = new MarkdownString(undefined, { isTrusted: true }).appendMarkdown(localize("mcp.starting.servers", "Starting MCP servers {0}...", serverLinks) + " ").appendMarkdown(
        skipText,
      );
		}
	...
	private async _start(startLink: HTMLElement) {
		...
		try {
          ...
          s => s.definition.id === serverInfo.id,
        );
				if (server) {
