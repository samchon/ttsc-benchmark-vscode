...
export class ChatProgressContentPart extends Disposable implements IChatContentPart {
	...
	constructor(
		...
		this.showSpinner = forceShowSpinner ?? shouldShowSpinner(
      followingContent,
      context.element,
    );
		this.isHidden = forceShowMessage !== true && followingContent.some(
      ...
      AccessibilityWorkbenchSettingId.VerboseChatProgressUpdates,
		...
		const isLoadingIcon = icon && ThemeIcon.isEqual(
      icon,
      ThemeIcon.modify(Codicon.loading, "spin"),
		...
		const codicon = useShimmer ? Codicon.check : (icon ?? (this.showSpinner ? ThemeIcon.modify(
      Codicon.loading,
      "spin",
    ) : Codicon.check));
...
function shouldShowSpinner(followingContent: IChatRendererContent[], element: ChatTreeItem): boolean {
	return isResponseVM(
    element,
  ) && !element.isComplete && followingContent.length === 0;
}
...
export class ChatWorkingProgressContentPart extends Disposable implements IChatContentPart {
	...
	constructor(
        ...
        ...ThemeIcon.asClassNameArray(ThemeIcon.modify(Codicon.loading, "spin")),
      );
		}
		append(this.domNode, iconElement);
