...
export class ModePickerActionItem extends ChatInputPickerActionViewItem {
	constructor(
      ...
      showHeader: true,
		...
		const agentModeDisabledViaPolicy = configurationService.inspect<boolean>(
      ChatConfiguration.AgentEnabled,
        ...
        return action;
			...
			const customActions = customModes.custom?.map(
        mode => makeActionFromCustomMode(mode, currentMode),
      ) ?? [];
      ...
      telemetryService,
		...
		assignmentService.getTreatment("chat.showOldAskMode").then(
      showOldAskMode => {
        assignments.set(
