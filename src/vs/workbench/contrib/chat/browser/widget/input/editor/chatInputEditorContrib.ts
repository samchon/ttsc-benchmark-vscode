...
class InputEditorDecorations extends Disposable {
	...
	private updateInputPlaceholderDecoration(): void {
      ...
      (p): p is ChatRequestAgentSubcommandPart => p instanceof ChatRequestAgentSubcommandPart,
    );
      ...
      p => p instanceof ChatRequestTextPart && !p.text.trim().length || p instanceof ChatRequestAgentPart,
        ...
        agentAndCommandToKey(agentPart.agent, undefined),
			...
			if (agentPart.agent.description && exactlyOneSpaceAfterPart(
        parsedRequest,
        agentPart,
      )) {
      ...
      p => p instanceof ChatRequestTextPart && !p.text.trim().length || p instanceof ChatRequestAgentPart || p instanceof ChatRequestAgentSubcommandPart,
        ...
        agentAndCommandToKey(agentPart.agent, agentSubcommandPart.command.name),
			...
			if (agentSubcommandPart?.command.description && exactlyOneSpaceAfterPart(
        parsedRequest,
        agentSubcommandPart,
      )) {
				placeholderDecoration = [{
	...
	private async updateAsyncInputEditorDecorations(token: CancellationToken): Promise<void> {
      ...
      (p): p is ChatRequestSlashCommandPart => p instanceof ChatRequestSlashCommandPart,
    );
		const slashPromptPart = parsedRequest.find(
      (p): p is ChatRequestSlashPromptPart => p instanceof ChatRequestSlashPromptPart,
    );

		// first, fetch all async context
		const promptSlashCommand = slashPromptPart ? await this.customizationHarnessService.resolvePromptSlashCommand(
      ...
      token,
		...
		if (slashPromptPart && promptSlashCommand) {
			const onlyPromptCommandAndWhitespace = slashPromptPart && parsedRequest.every(
        isWhitespaceOrPromptPart,
      );
			if (onlyPromptCommandAndWhitespace && exactlyOneSpaceAfterPart(
        parsedRequest,
        slashPromptPart,
      ) && promptSlashCommand) {
      ...
      (p): p is ChatRequestToolPart => p instanceof ChatRequestToolPart || p instanceof ChatRequestToolSetPart,
    );
      ...
      (p): p is ChatRequestDynamicVariablePart => p instanceof ChatRequestDynamicVariablePart,
    );
		...
		if (isEditingPreviousRequest) {
          ...
          hoverMessage: URI.isUri(variable.data) ? new MarkdownString(this.labelService.getUriLabel(variable.data, { relative: true })) : undefined,
        });
			}
		}
