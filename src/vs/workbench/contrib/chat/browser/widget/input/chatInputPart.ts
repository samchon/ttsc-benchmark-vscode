...
import {
  ...
  OpenAgentHostCustomAgentPickerAction,
...
export class ChatInputPart extends Disposable implements IHistoryNavigationWidget {
    ...
    new Emitter<{ removed?: IChatRequestVariableEntry[]; added?: IChatRequestVariableEntry[] }>(),
  );
    ...
    new Emitter<{ followup: IChatFollowup; response: IChatResponseViewModel | undefined }>(),
  );
	...
	private readonly overlayClickListener: MutableDisposable<IDisposable> = this._register(
    new MutableDisposable<IDisposable>(),
  );

	private attachedContextContainer!: HTMLElement;
	private readonly attachedContextDisposables: MutableDisposable<DisposableStore> = this._register(
	...
	private readonly _waitForPersistedLanguageModel: MutableDisposable<IDisposable> = this._register(
    new MutableDisposable<IDisposable>(),
  );
	private readonly _waitForSessionHistoryLanguageModel: MutableDisposable<IDisposable> = this._register(
    new MutableDisposable<IDisposable>(),
  );
	...
	private _currentLanguageModel = observableValue<ILanguageModelChatMetadataAndIdentifier | undefined>(
	...
	private readonly _chatEditsActionsDisposables: DisposableStore = this._register(
    new DisposableStore(),
  );
	...
	constructor(
      ...
      this.instantiationService.createInstance(
		...
		this.inputEditorHasText = ChatContextKeys.inputHasText.bindTo(
      contextKeyService,
    );
		this.chatCursorAtTop = ChatContextKeys.inputCursorAtTop.bindTo(
      contextKeyService,
    );
		this.inputEditorHasFocus = ChatContextKeys.inputHasFocus.bindTo(
      contextKeyService,
    );
		this._hasQuestionCarouselContextKey = ChatContextKeys.Editing.hasQuestionCarousel.bindTo(
      contextKeyService,
    );
		this.chatModeKindKey = ChatContextKeys.chatModeKind.bindTo(
      contextKeyService,
    );
		this.chatModeNameKey = ChatContextKeys.chatModeName.bindTo(
      contextKeyService,
    );
		this.chatModelIdKey = ChatContextKeys.chatModelId.bindTo(contextKeyService);
		this.permissionLevelKey = ChatContextKeys.chatPermissionLevel.bindTo(
      contextKeyService,
    );
		this.permissionLevelKey.set(this._currentPermissionLevel.get());
		this.withinEditSessionKey = ChatContextKeys.withinEditSessionDiff.bindTo(
      contextKeyService,
    );
		this.filePartOfEditSessionKey = ChatContextKeys.filePartOfEditSession.bindTo(
      contextKeyService,
    );
		this.chatSessionHasOptions = ChatContextKeys.chatSessionHasModels.bindTo(
      contextKeyService,
    );
		this.chatSessionOptionsValid = ChatContextKeys.chatSessionOptionsValid.bindTo(
      contextKeyService,
    );
		this.agentSessionTypeKey = ChatContextKeys.agentSessionType.bindTo(
      contextKeyService,
    );
		this.chatSessionSupportsDelegationKey = ChatContextKeys.chatSessionSupportsDelegation.bindTo(
      contextKeyService,
    );
		...
		this.chatSessionHasCustomAgentTarget = ChatContextKeys.chatSessionHasCustomAgentTarget.bindTo(
      contextKeyService,
    );
		this.chatSessionHasTargetedModels = ChatContextKeys.chatSessionHasTargetedModels.bindTo(
      contextKeyService,
    );
      ...
      this.instantiationService.createInstance(
		...
		this._hasFileAttachmentContextKey = ChatContextKeys.hasFileAttachments.bindTo(
      contextKeyService,
    );
		...
		const resetCurrentLanguageModelIfUnavailable = () => {
			...
			if (this._currentSessionType === SessionType.CopilotCLI) {
				if (shouldResetOnModelListChange(
          modelIdentifier,
          models,
        ) && !models.some(m => m.metadata.id === modelIdentifier)) {
					const ids = models.map(m => m.identifier).join(", ");
					const metadataIds = models.map(m => m.metadata.id).join(", ");
					logChangesToStateModel(
            this._inputModel,
            `resetting current language model due to model list change from ${modelIdentifier}, identifiers: ${ids}, metadataIds: ${metadataIds}`,
            undefined,
            undefined,
            this.logService,
          );
	...
	private setImplicitContextEnablement() {
		if (this.implicitContext && this.configurationService.getValue<boolean>(
      "chat.implicitContext.suggestedContext",
    )) {
	...
	public switchModelByQualifiedName(qualifiedModelNames: readonly string[]): boolean {
		...
		for (const qualifiedModelName of qualifiedModelNames) {
			const model = models.find(
        m => ILanguageModelChatMetadata.matchesQualifiedName(
          qualifiedModelName,
          m.metadata,
        ),
      );
	...
	public switchToNextModel(): void {
        ...
        model => model.identifier === this._currentLanguageModel.get()?.identifier,
      );
	...
	private getPermittedPermissionLevel(level: ChatPermissionLevel): ChatPermissionLevel {
		if (this.configurationService.inspect<boolean>(
      ChatConfiguration.GlobalAutoApprove,
    ).policyValue === false && level !== ChatPermissionLevel.Default) {
	...
	private createChatSessionPickerWidgets(action: MenuItemAction, pickerOptions?: IChatInputPickerOptions): ChatSessionPickerActionItem[] {
		...
		const sessionResource = this.getCurrentSessionResource();
		const visibleOptionGroups = this.getVisibleOptionGroupsModeAndUpdateContextKeys(
      sessionResource,
    );
	...
	*/
	...
	private getAllMergedModels(): ILanguageModelChatMetadataAndIdentifier[] {
		const cachedModels = this.storageService.getObject<ILanguageModelChatMetadataAndIdentifier[]>(
	...
	private checkModeInSessionPool(sessionType?: string): void {
		...
		const customAgentTarget = this.chatSessionsService.getCustomAgentTargetForSessionType(
      sessionType,
    );
	...
	private preselectModelFromSessionHistory(): void {
		...
		if (!requests || requests.length === 0 || getChatSessionType(
      sessionResource,
	...
	private validateCurrentChatMode() {
		const currentMode = this._currentModeObservable.get();
		const validMode = this._currentChatModesObservable.get().findModeById(
      currentMode.id,
    );
	...
	private getVisibleOptionGroupsModeAndUpdateContextKeys(sessionResource: URI | undefined): IChatSessionProviderOptionGroup[] {
		const customAgentTarget = sessionResource && this.chatSessionsService.getCustomAgentTargetForSessionType(
      ...
      customAgentTarget !== Target.Undefined,
		...
		const requiresCustomModels = sessionResource && this.chatSessionsService.requiresCustomModelsForSessionType(
      getChatSessionType(sessionResource),
    );
		...
		const allOptionsValid = sessionResource ? this.areAllOptionsValid(
      sessionResource,
      visibleOptionGroups,
    ) : true;
	...
	private areAllOptionsValid(sessionResource: URI, visibleOptionGroups: readonly IChatSessionProviderOptionGroup[]): boolean {
        ...
        optionGroup.id,
			...
			if (currentOption) {
				...
				if (!optionGroup.items.some(
          item => item.id === currentOptionId,
	...
	private getAllOptionsGroups(sessionResource: URI | undefined): IChatSessionProviderOptionGroup[] {
		...
		const delegateSessionType = this.options.sessionTypePickerDelegate?.getActiveSessionProvider?.();
		const effectiveSessionType = delegateSessionType ?? (sessionResource ? getChatSessionType(
      sessionResource,
    ) : undefined);
		if (!effectiveSessionType) {
			return [];
		...
		const allOptionGroups = this.chatSessionsService.getOptionGroupsForSessionType(
	...
	private getVisibleOptionGroups(sessionResource: URI | undefined): IChatSessionProviderOptionGroup[] {
		...
		for (const optionGroup of allOptionGroups) {
			...
			const sessionHasOption = !sessionResource || this.chatSessionsService.getSessionOption(
        sessionResource,
        optionGroup.id,
      ) !== undefined;
	...
	private getCurrentOptionForGroup(optionGroupId: string): IChatSessionProviderOptionItem | undefined {
		...
		if (this.chatSessionsService.getSessionOption(
      sessionResource,
      optionGroupId,
    ) === undefined) {
			return;
		...
		const effectiveSessionType = this.getEffectiveSessionType(sessionResource);
		const optionGroups = effectiveSessionType ? this.chatSessionsService.getOptionGroupsForSessionType(
      effectiveSessionType,
    ) : undefined;
	...
	private hasWorkspaceScmRepository(): boolean {
		...
		for (const repo of this.scmService.repositories) {
			if (repo.provider.rootUri && this.workspaceContextService.getWorkspaceFolder(
	...
	private updateAgentSessionTypeContextKey(): void {
		...
		const sessionType = delegateSessionType || (sessionResource ? getChatSessionType(
      sessionResource,
    ) : "");
	...
	render(container: HTMLElement, initialValue: string, widget: IChatWidget) {
      ...
      this.contextKeyService.createScoped(inputContainer),
		...
		this.currentlyEditingInputKey = ChatContextKeys.currentlyEditingInput.bindTo(
      ...
      ...EditorExtensionsRegistry.getSomeEditorContributions([ContentHoverController.ID, GlyphHoverController.ID, DropIntoEditorController.ID, CopyPasteController.ID, LinkDetector.ID]),
    );
      ...
      this.instantiationService.createInstance(
		...
		this.secondaryToolbar.getElement().classList.add(
      "chat-secondary-input-toolbar",
    );
      ...
      new dom.AnimationFrameScheduler(
        this.container,
	...
	public renderAttachedContext() {
      ...
      ([, a]) => isImageVariableEntry(a),
		...
		for (const [index, attachment] of attachments) {
			const resource = URI.isUri(
        attachment.value,
      ) ? attachment.value : isLocation(
        attachment.value,
      ) ? attachment.value.uri : undefined;
			const range = isLocation(
        attachment.value,
      ) ? attachment.value.range : undefined;
        ...
        this.attachmentModel.size - 1,
          ...
          this._contextResourceLabels,
				...
				attachmentWidget = this._chatAttachmentWidgetRegistry.createWidget(
          attachment,
          options,
          container,
        )
			...
			if (index === Math.min(
        this._indexOfLastOpenedContext,
        this.attachmentModel.size - 1,
      )) {
				attachmentWidget.element.focus();
			}
	...
	private handleAttachmentDeletion(e: KeyboardEvent | unknown, index: number, attachment: IChatRequestVariableEntry) {
      ...
      "chat.implicitContext.enableImplicitContext",
			...
			for (const implicitContext of (this._implicitContext?.values || [])) {
				const implicitValue = URI.isUri(implicitContext?.value) && URI.isUri(
          attachment.value,
	...
	private handleAttachmentNavigation(e: StandardKeyboardEvent): void {
      ...
      this.attachedContextContainer.querySelectorAll(
		...
		const activeElement = dom.getWindow(
      this.attachmentsContainer,
	...
	async renderChatTodoListWidget(chatSessionResource: URI) {
		...
		const isTodoWidgetEnabled = this.configurationService.getValue<boolean>(
	...
	hasToolInConfirmationCarousel(toolCallId: string): boolean {
		return this._currentToolConfirmationCarousel?.hasToolInvocation(
      toolCallId,
    ) ?? false;
	}
	...
	private renderChatEditingSessionWithEntries(
      ...
      ".working-set-title",
    ) as HTMLElement ?? dom.append(overviewRegion, $(".working-set-title"));
	...
	async renderFollowups(items: IChatFollowup[] | undefined, response: IChatResponseViewModel | undefined): Promise<void> {
        ...
        this.instantiationService.createInstance<typeof ChatFollowups<IChatFollowup>, ChatFollowups<IChatFollowup>>(
          ChatFollowups,
	...
	private _updateWorkingProgressAnimationDuration(width: number): void {
		...
		if (this._lastAnimDurationS !== undefined && Math.abs(
	...
	private getLayoutData() {
		...
		const inputSideToolbarWidth = this.inputSideToolbarContainer ? dom.getTotalWidth(
      this.inputSideToolbarContainer,
    ) : 0;
