...
export class ChatListItemRenderer extends Disposable implements ITreeRenderer<ChatTreeItem, FuzzyScore, IChatListItemTemplate> {
    ...
    new Emitter<{ readonly sessionResource: URI; readonly requestId: string }>(),
  );
	...
	private renderChatTreeItem(element: ChatTreeItem, index: number, templateData: IChatListItemTemplate): void {
      ...
      "chat-pending-dragging",
		...
		ChatContextKeys.isResponse.bindTo(templateData.contextKeyService).set(
      isResponseVM(element),
    );
		ChatContextKeys.itemId.bindTo(templateData.contextKeyService).set(
      element.id,
    );
		ChatContextKeys.isRequest.bindTo(templateData.contextKeyService).set(
      isRequestVM(element),
    );
      ...
      isRequestVM(element) && this.viewModel?.model.getRequests()[0]?.id === element.id,
		...
		ChatContextKeys.isPendingRequest.bindTo(templateData.contextKeyService).set(
      isRequestVM(element) && !!element.pendingKind,
    );
		ChatContextKeys.responseDetectedAgentCommand.bindTo(templateData.contextKeyService).set(
      isResponseVM(element) && element.agentOrSlashCommandDetected,
    );
		if (isResponseVM(element)) {
			ChatContextKeys.responseSupportsIssueReporting.bindTo(templateData.contextKeyService).set(
        !!element.agent?.metadata.supportIssueReporting,
      );
		...
		ChatContextKeys.responseHasError.bindTo(templateData.contextKeyService).set(
      isResponseVM(element) && !!element.errorDetails,
    );
		const isFiltered = !!(isResponseVM(
      element,
    ) && element.errorDetails?.responseIsFiltered);
		ChatContextKeys.responseIsFiltered.bindTo(templateData.contextKeyService).set(
      isFiltered,
    );
      ...
      "show-checkmarks",
      !!this.configService.getValue<boolean>(AccessibilityWorkbenchSettingId.ShowChatCheckmarks),
    );
		...
		const isSystemInitiatedRequest = isRequestVM(
      element,
      ...
      element.username === COPILOT_USERNAME || this.environmentService.isSessionsWindow || isSystemInitiatedRequest,
		...
		const checkpointEnabled = this.configService.getValue<boolean>(
      ...
      !(shouldShowRestore && checkpointEnabled),
		...
		const isInput = this.configService.getValue<string>(
      "chat.editRequests",
    ) === "input";
      ...
      isRequestVM(element) && !!element.confirmation,
		...
		const shouldShowHeader = (isResponseVM(
      element,
    ) && !this.rendererOptions.noHeader) && !isSystemInitiatedRequest;
	...
	private shouldShowWorkingProgress(element: IChatResponseViewModel, partsToRender: IChatRendererContent[], moreContentAvailable: boolean, templateData: IChatListItemTemplate): IChatWorkingProgress | undefined {
      ...
      part => part.kind === "planReview" && !part.isUsed,
    )) {
			return undefined;
		}

		const showProgressDetails = this.configService.getValue<boolean>(
      ChatConfiguration.ChatPersistentProgressEnabled,
    ) !== false
			&& (this.configService.getValue<boolean>(
        ChatConfiguration.ProgressBorder,
      ) !== true || this.accessibilityService.isMotionReduced());
		if (element.isComplete) {
			return undefined;
		}
      ...
      this.rendererOptions.progressMessageAtBottomOfResponse,
        ...
        element.sessionResource,
			...
			if (widget?.inputPart.hasActiveToolConfirmationCarousel) {
				const nonSubagentConfirmationCount = this.getPendingToolConfirmationCount(
          partsToRender,
          false,
        );
            ...
            content: new MarkdownString().appendText(this.getConfirmationPendingLabel(nonSubagentConfirmationCount)),
				...
				if (this.getPendingToolConfirmationCount(
          partsToRender,
          true,
        ) > 0 || this.getSubagentPart(templateData.renderedParts)) {
					return undefined;
				}
      ...
      part => part.kind === "toolInvocation" && !IChatToolInvocation.isComplete(part) && isMcpToolInvocation(part),
		...
		if (lastPart && (lastPart.kind === "toolInvocation" || lastPart.kind === "toolInvocationSerialized")) {
			...
			const isEffectivelyHiddenToolInvocation = IChatToolInvocation.isEffectivelyHidden(
        lastPart,
      );
			const collapsedToolsMode = this.configService.getValue<CollapsedToolsDisplayMode>(
        "chat.agent.thinking.collapsedTools",
      );
			if (!isEffectivelyHiddenToolInvocation && collapsedToolsMode !== CollapsedToolsDisplayMode.Off && this.shouldPinPart(
        lastPart,
      ...
      part => part.kind === "markdownContent" && this.hasEditCodeblockUri(part),
    );
		...
		if (
			...
			(lastPart.kind === "markdownContent" && !moreContentAvailable && this.hasBeenCaughtUpLongEnough(
        element,
      )) ||
			((lastPart.kind === "toolInvocation" || lastPart.kind === "toolInvocationSerialized") && (IChatToolInvocation.isComplete(
        lastPart,
	...
	private doUpdateWorkingProgressForPendingConfirmations(templateData: IChatListItemTemplate): void {
		...
		if (element.isComplete && this.configService.getValue<boolean>(
      ChatConfiguration.ChatPersistentProgressEnabled,
    ) !== false && (this.configService.getValue<boolean>(
      ChatConfiguration.ProgressBorder,
    ) !== true || this.accessibilityService.isMotionReduced())) {
			return;
		}
      ...
      false,
		...
		const workingProgressPart = this.getWorkingProgressContentPart(
      templateData,
    );
	...
	private getChatFileChangesSummaryPart(element: IChatResponseViewModel): IChatChangesSummaryPart | undefined {
		...
		if (!element.model.entireResponse.value.some(
      part => part.kind === "textEditGroup" || part.kind === "notebookEditGroup",
    )) {
			return undefined;
		}
	...
	private renderChatRequest(element: IChatRequestViewModel, index: number, templateData: IChatListItemTemplate) {
      ...
      !!element.isSystemInitiated,
		...
		if (element.pendingKind && this._pendingDragController) {
			...
			const sameKindCount = (this.viewModel?.model.getPendingRequests() ?? []).filter(
        p => p.kind === element.pendingKind,
      ).length;
			if (sameKindCount > 1) {
				const handle = dom.$(
          ".chat-pending-drag-handle" + ThemeIcon.asCSSSelector(Codicon.gripper),
        );
		...
		if (this.configService.getValue<string>(
      "chat.editRequests",
    ) !== "none" && this.rendererOptions.editable) {
	...
	private logIncrementalRenderingTelemetry(): void {
		...
		this.telemetryService.publicLog2<IncrementalRenderingSettingsEvent, IncrementalRenderingSettingsClassification>(
	...
	private getNextProgressiveRenderContent(element: IChatResponseViewModel, templateData: IChatListItemTemplate): { content: IChatRendererContent[]; moreContentAvailable: boolean } {
		...
		const renderImmediately = this.configService.getValue<boolean>(
          ...
          `  Chunk ${i}: Want to render ${numNeededWords} words and found ${wordCountResult.returnedWordCount} words. Total words in chunk: ${wordCountResult.totalWordCount}`,
            ...
            content: new MarkdownString(wordCountResult.value, part.content),
				...
				if (numNeededWords <= 0) {
					...
					if (renderableResponse.slice(i + 1).some(
            part => part.kind === "markdownContent",
          )) {
						moreContentAvailable = true;
					}
					break;
	...
	private shouldShowFileChangesSummary(element: IChatResponseViewModel): boolean {
		...
		const isLocalSession = sessionType === localChatSessionType || isAgentHostTarget(
      sessionType,
    );
		return element.isComplete && isLocalSession && this.configService.getValue<boolean>(
	...
	private isCodeblockComplete(part: IChatRendererContent, element: ChatTreeItem): boolean {
		...
		return !isResponseVM(
      element,
    ) || element.isComplete || codeblockHasClosingBackticks(part.content.value);
	}
	...
	private shouldPinPart(part: IChatRendererContent, element?: IChatResponseViewModel): boolean {
      ...
      "chat.agent.thinking.collapsedTools",
		...
		const isMcpTool = (part.kind === "toolInvocation" || part.kind === "toolInvocationSerialized") && isMcpToolInvocation(
      part,
    );
		if (isMcpTool) {
			return false;
		...
		const isMermaidTool = (part.kind === "toolInvocation" || part.kind === "toolInvocationSerialized") && part.toolId.toLowerCase().includes(
      "mermaid",
    );
		if (isMermaidTool) {
			return false;
		...
		if ((part.kind === "toolInvocation" || part.kind === "toolInvocationSerialized") && isSubagentToolInvocation(
      part,
    )) {
			return false;
		}
		...
		const isContributedTerminalToolInvocation = element
			&& (element.sessionResource.scheme !== Schemas.vscodeChatInput && getChatSessionType(
        element.sessionResource,
      ) !== localChatSessionType) // contributed sessions
		...
		if (isTerminalTool && !isContributedTerminalToolInvocation) {
			...
			if (part.kind === "toolInvocation" && IChatToolInvocation.getConfirmationMessages(
        part,
      )) {
				return false;
			}
	...
	private maybeRouteSubagentToolToCarousel(
		...
		if (!this.configService.getValue<boolean>(
      ChatConfiguration.ToolConfirmationCarousel,
    )) {
			return;
		}
		if (toolInvocation.kind !== "toolInvocation" || !isResponseVM(
      context.element,
    )) {
			return;
		}
      ...
      context.element.sessionResource,
        ...
        context.element.id,
			...
			const currentSubagentPart = this.getSubagentPart(
	...
	private renderChatContentPart(content: IChatRendererContent, templateData: IChatListItemTemplate, context: IChatContentPartRenderContext): IChatContentPart | undefined {
		try {
			...
			if (content.kind === "thinking" && (Array.isArray(
        content.value,
      ) ? content.value.length === 0 : content.value === "")) {
        ...
        isResponseElement ? context.element : undefined,
          ...
          this.chatContentMarkdownRenderer,
				...
				if (isResponseVM(
          context.element,
        ) && context.element.agent?.isDefault && !context.element.agent.modes.includes(
          ChatModeKind.Ask,
        )) {
        ...
        new MarkdownString(
          localize("renderFailMsg", "Failed to render content") + `: ${toErrorMessage(err, false)}`,
        ),
        content,
        this.chatContentMarkdownRenderer,
      );
      ...
      this._treePool,
        ...
        treePart.onDidFocus(() => {
          this.focusedFileTreesByResponseId.set(
			...
			const fileTrees = this.fileTreesByResponseId.get(
        context.element.id,
      ) ?? [];
	...
	private renderToolInvocation(toolInvocation: IChatToolInvocation | IChatToolInvocationSerialized, context: IChatContentPartRenderContext, templateData: IChatListItemTemplate): IChatContentPart | undefined {
		...
		if (IChatToolInvocation.isComplete(
      toolInvocation,
		...
		if (this.configService.getValue<CollapsedToolsDisplayMode>(
      "chat.agent.thinking.collapsedTools",
      ...
      context.element,
			...
			if (!lastThinking && !IChatToolInvocation.isEffectivelyHidden(
        toolInvocation,
      ) && this.shouldPinPart(
        toolInvocation,
        context.element,
      ...
      ChatConfiguration.ToolConfirmationCarousel,
			...
			toolInvocation.kind === "toolInvocation" && isResponseVM(
        context.element,
      ) &&
	...
	private renderHookPart(hookPart: IChatHookPart, context: IChatContentPartRenderContext, templateData: IChatListItemTemplate): IChatContentPart {
		if (!(hookPart.stopReason || hookPart.systemMessage)) {
			return this.renderNoContent(
        other => other.kind === "hook" && other.hookType === hookPart.hookType,
      );
		}
	...
	private renderQuestionCarousel(context: IChatContentPartRenderContext, carousel: IChatQuestionCarousel, templateData: IChatListItemTemplate): IChatContentPart {
        ...
        responseElement.sessionResource,
			...
			const request = model?.getRequests().find(
        r => r.id === responseElement.requestId,
      );
      ...
      context.element,
		...
		const handleSubmit = async (answers: Map<string, IChatQuestionAnswerValue> | undefined, part: ChatQuestionCarouselPart) => {
			...
			const answersRecord: IChatQuestionAnswers | undefined = answers ? Object.fromEntries(
        answers,
      ) : undefined;
		...
		const responseIsComplete = isResponseVM(
      context.element,
		...
		if (carousel.isUsed || responseIsComplete) {
			if (responseIsComplete && !carousel.isUsed && isResponseVM(
        context.element,
      ) && carousel.resolveId) {
				carousel.data = {};
				carousel.isUsed = true;
		...
		const part = isEditing ? undefined : widget?.input.renderQuestionCarousel(
      carousel,
      context,
      {
        shouldAutoFocus,
		...
		if (isResponseVM(
      context.element,
    ) && carousel.allowSkip && !carousel.isUsed) {
	...
	private renderPlanReview(context: IChatContentPartRenderContext, review: IChatPlanReview, templateData: IChatListItemTemplate): IChatContentPart {
      ...
      context.element,
		...
		const isEditing = !!this.viewModel?.editing;
		const dockedPart = isEditing ? undefined : widget?.input.renderPlanReview(
      review,
      context,
      {
	...
	private renderMarkdown(markdown: IChatMarkdownContent, templateData: IChatListItemTemplate, context: IChatContentPartRenderContext): IChatContentPart {
		...
		if (!this.hasEditCodeblockUri(
      markdown,
    ) && !isBlankMarkdown && !hasPendingEditCodeblock) {
			this.finalizeCurrentThinkingPart(context, templateData);
		}
      ...
      context.element,
        ...
        markdown.content.value,
          ...
          subAgentInvocationId,
			...
			// create thinking part if it doesn't exist yet
			const lastThinking = this.getLastThinkingPart(templateData.renderedParts);
			if (!lastThinking && markdownPart?.domNode && this.shouldPinPart(
        markdown,
        context.element,
      ) && collapsedToolsMode === CollapsedToolsDisplayMode.Always && isComplete) {
				const thinkingPart = this.renderThinkingPart(
          {
            kind: "thinking",
          },
      ...
      element,
		...
		if (Array.isArray(content.value)) {
          ...
          templateData.renderedParts,
			...
			return lastPart ?? this.renderNoContent(
        other => content.kind === other.kind,
      );
	...
	disposeElement(node: ITreeNode<ChatTreeItem, FuzzyScore>, index: number, templateData: IChatListItemTemplate, details?: IListElementRenderDetails): void {
		...
		if (isRequestVM(
      node.element,
    ) && node.element.id === this.viewModel?.editing?.id && details?.onScroll) {
