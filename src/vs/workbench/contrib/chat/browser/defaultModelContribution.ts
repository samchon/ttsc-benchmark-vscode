...
export abstract class DefaultModelContribution extends Disposable {
	...
	private _updateModelValues(): void {
        ...
        defaultEntryDescription ?? localize("defaultModelDescription", "Use the vendor's default model"),
      );
			...
			for (const modelId of allModelIds) {
				try {
					const metadata = this._languageModelsService.lookupLanguageModel(
            modelId,
          );
					if (metadata) {
        ...
        (a, b) => a.metadata.name.localeCompare(b.metadata.name),
      );
