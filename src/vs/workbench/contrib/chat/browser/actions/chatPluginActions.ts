...
class ManagePluginMarketplacesAction extends Action2 {
	...
	async run(accessor: ServicesAccessor): Promise<void> {
      ...
      IExtensionsWorkbenchService,
		...
		const configuredRefs = configurationService.getValue<unknown[]>(
      ChatConfiguration.PluginMarketplaces,
        ...
        placeHolder: localize("noMarketplaces", "No plugin marketplaces configured"),
      });
			return;
		}
		...
		switch (action.id) {
          ...
          `@agentPlugins ${ref.displayLabel}`,
			...
			case "removeMarketplace": {
				const currentValues = configurationService.getValue<unknown[]>(
          ChatConfiguration.PluginMarketplaces,
        ) ?? [];
