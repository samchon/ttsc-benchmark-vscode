...
export class ForkConversationAction extends Action2 {
	...
	async run(accessor: ServicesAccessor, ...args: unknown[]) {
		...
		if (URI.isUri(args[0])) {
			...
			if (contentProviderSchemes.includes(
			...
			: isChatTreeItem(arg?.element)
				...
				: isChatTreeItem(arg?.context)
					...
					: isChatTreeItem(arg?.item)
						? arg.item
						: undefined;
		const widget = (item && chatWidgetService.getWidgetBySessionResource(
      item.sessionResource,
		...
		const sessionResource = widget?.viewModel?.sessionResource ?? (isChatTreeItem(
      item,
    ) ? item.sessionResource : undefined);
		if (!sessionResource) {
			return;
		}
		...
		if (contentProviderSchemes.includes(getChatSessionType(sessionResource))) {
			const contributedSession = await chatSessionsService.getOrCreateChatSession(
        sessionResource,
        CancellationToken.None,
      );
			let request = contributedSession.history.find(
        (entry): entry is IChatSessionRequestHistoryItem => entry.type === "request" && entry.id === targetRequestId,
      );
			if (!request) {
		...
		if (targetIndex < 0) {
			const requestIndex = chatModel.getRequests().findIndex(
        r => r.id === targetRequestId,
      );
			targetIndex = isRequestItem ? Math.max(
        0,
        requestIndex - 1,
      ) : requestIndex;
		}
	...
	private async forkContributedChatSession(sourceSessionResource: URI, request: IChatSessionRequestHistoryItem | undefined, openForkedSessionImmediately: boolean, chatSessionsService: IChatSessionsService, chatWidgetService: IChatWidgetService) {
		...
		const forkPromise = forkContributedChatSession(
      sourceSessionResource,
      request,
      openForkedSessionImmediately,
      chatSessionsService,
      chatWidgetService,
    );
