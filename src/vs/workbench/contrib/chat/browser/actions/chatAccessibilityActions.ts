...
class AnnounceChatConfirmationAction extends Action2 {
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		const pendingWidget = chatWidgetService.getAllWidgets().find(
      widget => widget.viewModel?.model.requestNeedsInput.get(),
    );
		...
		if (isResponseVM(lastResponse)) {
			...
			const confirmationWidgets = pendingWidget.domNode.querySelectorAll<HTMLElement>(
		...
		if (firstConfirmationElement) {
			...
			if (firstConfirmationElement.contains(
...
class ToggleThinkingContentAccessibleViewAction extends Action2 {
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		const includeThinking = isThinkingContentIncludedInAccessibleView(
      storageService,
    );
