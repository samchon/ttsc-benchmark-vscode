          ...
          "chat.editing.confirmEditRequestRemoval",
              ...
              entriesModifiedInRequestsToRemove.length,
				...
				if (!confirmation.confirmed) {
					telemetryService.publicLog2<EditUndoEvent, EditUndoEventClassification>(
...
class ToggleChatModeAction extends Action2 {
	...
	async run(accessor: ServicesAccessor, ...args: unknown[]) {
		...
		const switchToMode = (arg && (modes.findModeById(
      arg.modeId,
    ) || modes.findModeByName(
      arg.modeId,
      ...
      widget.viewModel?.model,
		...
		telemetryService.publicLog2<ChatModeChangeEvent, ChatModeChangeClassification>(
      "chat.modeChange",
      {
...
export class CancelAction extends Action2 {
	...
	async run(accessor: ServicesAccessor, ...args: unknown[]) {
		...
		const widget = context?.widget ?? widgetService.lastFocusedWidget;
		if (!widget) {
			telemetryService.publicLog2<ChatStopCancellationNoopEvent, ChatStopCancellationNoopClassification>(
        ChatStopCancellationNoopEventName,
        {
          source: "cancelAction",
          reason: "noWidget",
