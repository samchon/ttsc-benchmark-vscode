...
function isCopilotExtension(id: ExtensionIdentifier): boolean {
    ...
    copilotId => ExtensionIdentifier.equals(id, copilotId),
  );
}

function getPluginUriFromCollectionId(collectionId: string | undefined): string | undefined {
	return collectionId?.startsWith(
    PLUGIN_COLLECTION_PREFIX,
  ) ? collectionId.slice(PLUGIN_COLLECTION_PREFIX.length) : undefined;
}
...
class McpServerItemRenderer implements IListRenderer<IMcpServerItemEntry | IMcpBuiltinItemEntry, IMcpServerItemTemplateData> {
	...
	renderElement(element: IMcpServerItemEntry | IMcpBuiltinItemEntry, index: number, templateData: IMcpServerItemTemplateData): void {
      ...
      s => s.definition.id === element.server.id,
    );
...
export class McpListWidget extends Disposable {
	...
	private create(): void {
      ...
      $(".empty-subtext"),
		...
		const groupHeaderRenderer = new CustomizationGroupHeaderRenderer<IMcpGroupHeaderEntry>(
      "mcpGroupHeader",
      this.hoverService,
    );
      ...
      this.list.onContextMenu(
        e => this.onContextMenu(e as IListContextMenuEvent<IMcpListEntry>),
      ),
    );
	...
	private filterServers(): void {
		...
		const groups: { scope: LocalMcpServerScope; label: string; icon: ThemeIcon; description: string; servers: IWorkbenchMcpServer[] }[] = [
      {
        ...
        icon: workspaceIcon,
        description: localize("workspaceGroupDescription", "MCP servers configured in your workspace settings, shared with your team via version control."),
        servers: [],
      },
      {
        ...
        icon: userIcon,
        description: localize("userGroupDescription", "MCP servers configured in your user settings. Private to you and available across all projects."),
        servers: [],
      },
    ];
      ...
      this.mcpRegistry.collections.get().map(c => [c.id, c.source]),
		...
		for (const server of builtinServers) {
			...
			} else if (source instanceof ExtensionIdentifier && !isCopilotExtension(
        source,
      )) {
	...
	private onContextMenu(e: IListContextMenuEvent<IMcpListEntry>): void {
		...
		if (e.element.type === "builtin-item") {
			...
			const plugin = this.agentPluginService.plugins.get().find(
        p => p.uri.toString() === pluginUriStr,
      );
			if (!plugin) {
				return;
			}
		...
		const disposables = new DisposableStore();
		const mcpServer = this.mcpWorkbenchService.local.find(
