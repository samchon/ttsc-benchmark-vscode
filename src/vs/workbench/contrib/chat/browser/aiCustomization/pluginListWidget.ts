...
class PluginMarketplaceItemRenderer implements IListRenderer<IPluginMarketplaceItemEntry, IPluginMarketplaceItemTemplateData> {
	...
	renderElement(element: IPluginMarketplaceItemEntry, _index: number, templateData: IPluginMarketplaceItemTemplateData): void {
      ...
      marketplaceType: element.item.marketplaceType,
		...
		const isAlreadyInstalled = this.agentPluginService.plugins.get().some(
      p => isEqual(p.uri, installUri),
    );
...
export class PluginListWidget extends Disposable {
	...
	private create(): void {
      ...
      this.list.onContextMenu(
        e => this.onContextMenu(e as IListContextMenuEvent<IPluginListEntry>),
      ),
    );
	...
	private updateAccessState(): void {
      ...
      ChatConfiguration.PluginsEnabled,
        ...
        ...ThemeIcon.asClassNameArray(policyLocked ? Codicon.shield : pluginIcon),
      );
	...
	private getAddDropdownActions(): Action[] {
		...
		return this.buildAddActions().slice(1).map(
      (action, index) => this.addDropdownActions.add(
        new Action(
          `plugin_add_${index}`,
	...
	private async queryMarketplace(): Promise<void> {
      ...
      "Loading marketplace...",
		...
		try {
			const plugins = await this.pluginMarketplaceService.fetchMarketplacePlugins(
        cts.token,
      );
	...
	private async getRemotePluginItems(query: string): Promise<readonly ICustomizationItem[]> {
		...
		try {
			const provided = await provider.provideChatSessionCustomizations(
        sessionResource,
	...
	private async filterPlugins(): Promise<void> {
		...
		const enabledPlugins = this.installedItems.filter(
