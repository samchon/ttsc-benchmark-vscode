...
async function addHookToFile(
	...
	const useCopilotCliNamingConvention = !isClaude && usesCopilotCliNaming(
    hooksContent.hooks,
  );
    ...
    e => isEqual(e.resource, hookFileUri),
  );
	...
	if (existingEditor) {
		...
		if (editor && editor.hasModel() && isEqual(
      editor.getModel().uri,
      hookFileUri,
    )) {
			...
			switch (step) {
              ...
              ([h]) => !vscodeTypes.has(h) && copilotTypes.has(h),
						...
						if (shared.length > 0) {
							pickerItems.push({
                type: "separator",
                label: localize("hookSection.default", "Local/Copilot CLI Agents"),
              });
                ...
                label: localize("hookSection.vscodeOnly", "Local Agents"),
						...
						if (copilotOnly.length > 0) {
							pickerItems.push({
                type: "separator",
                label: localize("hookSection.copilotCliOnly", "Copilot CLI Agents"),
              });
            ...
            PromptsType.hook,
              ...
              backButton,
				...
				case Step.EnterFilename: {
					...
					const hookFileName = fileNameResult.endsWith(
            ".json",
          ) ? fileNameResult : `${fileNameResult}.json`;
