	...
	private getActions(): IAction[] {
      ...
      this.createCapabilityAction(
		...
		if (configuredVendors.length > 1) {
			...
			actions.push(
        ...configuredVendors.map(vendor => this.createProviderAction(vendor.vendor.vendor, vendor.group.name)),
      );
		}
...
class Delegate implements ITableVirtualDelegate<IViewModelEntry> {
	...
	getHeight(element: IViewModelEntry): number {
		return isLanguageModelProviderEntry(element) || isLanguageModelGroupEntry(
      element,
    ) ? VENDOR_ROW_HEIGHT : MODEL_ROW_HEIGHT;
	}
}
...
class ActionsColumnRenderer extends ModelsTableColumnRenderer<IActionsColumnTemplateData> {
	...
	override renderVendorElement(entry: ILanguageModelProviderEntry, index: number, templateData: IActionsColumnTemplateData): void {
        ...
        ...createProviderGroupActions(this.viewModel, vendorEntry.vendor, vendorEntry.group.name, this.languageModelsService, this.dialogService),
      );
	...
	override renderModelElement(entry: ILanguageModelEntry, index: number, templateData: IActionsColumnTemplateData): void {
		...
		const configActions = this.languageModelsService.getModelConfigurationActions(
...
export class ChatModelsWidget extends Disposable {
	...
	private create(container: HTMLElement): void {
      ...
      this.viewModel.onDidChange(
        () => collapseAllAction.enabled = this.viewModel.viewModelEntries.some(e => isLanguageModelProviderEntry(e) || isLanguageModelGroupEntry(e)),
      ),
    );
	...
	private updateAddModelsButton(): void {
		const configurableVendors = this.languageModelsService.getVendors().filter(
      vendor => vendor.managementCommand || vendor.configuration,
    );
      ...
      !supportsAddingModels && isManagedEntitlement ? localize("models.managedByOrganization", "Adding models is managed by your organization") : "",
    );
