	...
	async update(cacheRoot: URI, plugin: IMarketplacePlugin, options?: IPullRepositoryOptions): Promise<boolean> {
		...
		try {
			const doUpdate = async (cts?: CancellationTokenSource) => {
				...
				let changed: boolean;
				if (git.sha) {
					const headBefore = await this._pluginGit.revParse(repoDir, "HEAD").catch(
            () => undefined,
          );
            ...
            cts?.token,
					...
					const headAfter = await this._pluginGit.revParse(repoDir, "HEAD").catch(
            () => undefined,
          );
...
export class GitUrlPluginSource extends AbstractGitPluginSource {
	...
	private _gitUrlCacheSegments(url: string, ref?: string, sha?: string): string[] {
		try {
			...
			const pathPart = parsed.path.replace(/^\/+/, "").replace(/\.git$/i, "").replace(
        /\/+$/g,
        "",
      );
			const segments = pathPart.split("/").map(
        s => s.replace(/[\\/:*?"<>|]/g, "_"),
      );
