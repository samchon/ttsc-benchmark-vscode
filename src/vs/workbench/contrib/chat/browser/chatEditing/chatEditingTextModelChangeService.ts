...
export class ChatEditingTextModelChangeService extends Disposable {
    ...
    new Emitter<ModifiedFileEntryState.Accepted | ModifiedFileEntryState.Rejected>(),
  );
	...
	async acceptAgentEdits(resource: URI, textEdits: (TextEdit | ICellEditOperation)[], isLastEdits: boolean, responseModel: IChatResponseModel | undefined): Promise<{ rewriteRatio: number; maxLineNumber: number }> {
      ...
      " INVALID args, can only edit THIS document",
		...
		if (isAtomicEdits) {
			// EDIT and DONE
			const minimalEdits = await this._editorWorkerService.computeMoreMinimalEdits(
        ...
        textEdits,
          ...
          ); // wait for animation to finish but also time-cap it
					listener.dispose();
				}
			}
	...
	public hasHunkAt(range: IRange) {
		...
		return this._diffInfo.get().changes.some(
      c => c.modified.intersectsStrict(LineRange.fromRangeInclusive(range)),
    );
	}
