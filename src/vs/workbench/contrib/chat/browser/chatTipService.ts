...
export const IChatTipService = createDecorator<IChatTipService>(
  "chatTipService",
);
...
export class ChatTipService extends Disposable implements IChatTipService {
	...
	constructor(
      ...
      false,
		...
		if (!this._thinkingPhrasesEverModified && this._isSettingModified(
	...
	private _getSlashCommandTrackingId(message: IParsedChatRequest): string | undefined {
		...
		const match = /^(?:@\S+\s+)?\/(init|create-(?:instructions|prompt|agent|skill)|fork)(?:\s|$)/.exec(
      trimmed,
    );
	...
	private _pickTip(sourceId: string, contextKeyService: IContextKeyService): IChatTip | undefined {
      ...
      tip => !dismissedIds.has(tip.id) && this._isEligible(tip, contextKeyService),
    );
	...
	private _isEligible(tip: ITipDefinition, contextKeyService: IContextKeyService): boolean {
		...
		if (tip.excludeWhenSettingsChanged?.some(
      setting => this._isSettingModified(setting),
    )) {
	...
	private _getCurrentChatModelId(contextKeyService: IContextKeyService): string {
		...
		const contextKeyModelId = normalize(
      ...
      ChatContextKeys.location.key,
		...
		const sessionType = contextKeyService.getContextKeyValue<string>(
	...
	private _isChatQuotaExceeded(contextKeyService: IContextKeyService): boolean {
		return contextKeyService.getContextKeyValue<boolean>(
