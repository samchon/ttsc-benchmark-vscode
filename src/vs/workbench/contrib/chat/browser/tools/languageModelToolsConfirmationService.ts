...
class ToolConfirmStore extends Disposable {
	private _autoConfirmTools: LRUCache<string, IAutoConfirmEntry> = new LRUCache<string, IAutoConfirmEntry>(
    100,
  );
...
export class LanguageModelToolsConfirmationService extends Disposable implements ILanguageModelToolsConfirmationService {
	...
	getPreConfirmAction(ref: ILanguageModelToolConfirmationRef): ConfirmedReason | undefined {
		...
		if (ref.combination) {
			const combinationResult = this._combinationConfirmStore.checkAutoConfirmation(
        ref.combination.key,
      );
      ...
      ref.toolId,
		...
		if (ref.source.type === "mcp") {
			const serverResult = this._preExecutionServerConfirmStore.checkAutoConfirmation(
	...
	getPostConfirmAction(ref: ILanguageModelToolConfirmationRef): ConfirmedReason | undefined {
      ...
      ref.toolId,
		...
		// Check server-level confirmation for MCP tools
		if (ref.source.type === "mcp") {
			const serverResult = this._postExecutionServerConfirmStore.checkAutoConfirmation(
        ref.source.definitionId,
      );
			if (serverResult) {
				return serverResult;
	...
	manageConfirmationPreferences(tools: readonly IToolData[], options?: { defaultScope?: "workspace" | "profile" | "session"; focusToolId?: string }): void {
      ...
      tooltip: localize("viewCombinationArguments", "View Arguments"),
		...
		for (const tool of tools) {
			if (tool.canRequestPreApproval || tool.canRequestPostApproval || this._contributions.has(
        tool.id,
      )) {
		...
		const buildTreeItems = (): IToolTreeItem[] => {
          ...
          return tool?.canRequestPostApproval;
				...
				const serverPreConfirmed = this._preExecutionServerConfirmStore.getAutoConfirmationIn(
          serverId,
          currentScope,
        );
				const serverPostConfirmed = this._postExecutionServerConfirmStore.getAutoConfirmationIn(
          serverId,
          currentScope,
        );
				...
				for (const toolId of serverInfo.tools) {
					...
					const hasPre = !serverPreConfirmed && (tool.canRequestPreApproval || this._preExecutionToolConfirmStore.getAutoConfirmationIn(
            tool.id,
            currentScope,
          ));
					const hasPost = !serverPostConfirmed && (tool.canRequestPostApproval || this._postExecutionToolConfirmStore.getAutoConfirmationIn(
            tool.id,
            currentScope,
          ));
            ...
            currentScope,
              ...
              buttons: args ? [viewArgsButton] : undefined,
					...
					const preApproval = this._preExecutionToolConfirmStore.getAutoConfirmationIn(
            tool.id,
            currentScope,
          );
					const postApproval = this._postExecutionToolConfirmStore.getAutoConfirmationIn(
            tool.id,
            currentScope,
          );
            ...
            checked: serverPreConfirmed,
				...
				const serverHasPre = this._preExecutionServerConfirmStore.getAutoConfirmationIn(
          serverId,
          currentScope,
        );
				const serverHasPost = this._postExecutionServerConfirmStore.getAutoConfirmationIn(
          serverId,
          currentScope,
        );
          ...
          pickable: false,
			...
			const sortedTools = tools.slice().sort(
        (a, b) => a.displayName.localeCompare(b.displayName),
			...
			for (const tool of sortedTools) {
				...
				if (contributed?.canUseDefaultApprovals !== false) {
					pickable = true;
					const hasPre = tool.canRequestPreApproval || this._preExecutionToolConfirmStore.getAutoConfirmationIn(
            tool.id,
            currentScope,
          );
					const hasPost = tool.canRequestPostApproval || this._postExecutionToolConfirmStore.getAutoConfirmationIn(
            tool.id,
            currentScope,
          );
            ...
            currentScope,
				...
				// Skip tools with no active approvals, no children, and no approval capabilities.
				// Tools that can request pre/post approval should always remain visible.
				if (checked === false && toolChildren.length === 0 && !tool.canRequestPreApproval && !tool.canRequestPostApproval && !this._contributions.has(
          tool.id,
        )) {
					continue;
				}
