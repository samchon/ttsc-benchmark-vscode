...
const jsonSchemaRegistry = Registry.as<JSONContributionRegistry.IJSONContributionRegistry>(
...
export class LanguageModelToolsService extends Disposable implements ILanguageModelToolsService {
	...
	constructor(
      ...
      this._configurationService,
		...
		this._ctxToolsCount = ChatContextKeys.Tools.toolsCount.bindTo(
      _contextKeyService,
    );
	...
	private isToolEnabledForModel(toolData: IToolData, model: ILanguageModelChatMetadata | undefined): boolean {
		...
		if (toolData.id === CopilotToolId.ReadFile && model?.family.startsWith(
      "gpt-5.5",
	...
	private isPermitted(toolOrToolSet: IToolData | ToolSet, reader?: IReader): boolean {
      ...
      SpecedToolAliases.web,
		...
		if (isToolSet(toolOrToolSet)) {
			const permitted = toolOrToolSet.source.type === "internal" && permittedInternalToolSetIds.includes(
        toolOrToolSet.referenceName,
      );
		...
		for (const toolSet of this._toolSets) {
			if (toolSet.source.type === "internal" && permittedInternalToolSetIds.includes(
		...
		if (toolOrToolSet.id === "vscode_fetchWebPage_internal" && permittedInternalToolSetIds.includes(
      SpecedToolAliases.web,
    )) {
			this._logService.trace(
	...
	override dispose(): void {
		...
		this._callsByRequestId.forEach(
      calls => calls.forEach(call => call.store.dispose()),
    );
	...
	private async _validateUpdatedInput(toolId: string, toolData: IToolData | undefined, updatedInput: object): Promise<string | undefined> {
		...
		try {
			...
			const diagnostics = await this._commandService.executeCommand<JsonDiagnostic[]>(
        "json.validate",
        schemaUri,
        inputJson,
      ) || [];
	...
	async invokeTool(dto: IToolInvocation, countTokens: CountTokensCallback, token: CancellationToken): Promise<IToolResult> {
      ...
      `[LanguageModelToolsService#invokeTool] Invoking tool ${dto.toolId} with parameters ${JSON.stringify(dto.parameters)}`,
		...
		} else if (dto.chatStreamToolCallId && this._pendingToolCalls.has(
      ...
      subagentInvocationId: dto.subAgentInvocationId,
		...
		try {
          ...
          token,
				...
				const { autoConfirmed, preparedInvocation: updatedPreparedInvocation } = await this.resolveAutoConfirmFromHook(
          preToolUseHookResult,
          tool,
          dto,
          preparedInvocation,
          dto.context?.sessionResource,
            ...
            dto.parameters,
				...
				if (preparedInvocation?.confirmationMessages?.title) {
					if (!IChatToolInvocation.executionConfirmedOrDenied(
            toolInvocation,
          ) && !autoConfirmed) {
          ...
          token,
				...
				const { autoConfirmed: fallbackAutoConfirmed, preparedInvocation: updatedPreparedInvocation } = await this.resolveAutoConfirmFromHook(
          preToolUseHookResult,
          tool,
          dto,
          preparedInvocation,
        ...
        "languageModelToolInvoked",
        {
          ...
          `[LanguageModelToolsService#invokeTool] Error from tool ${dto.toolId} with parameters ${JSON.stringify(dto.parameters)}:\n${toErrorMessage(err, true)}`,
			...
			toolResult ??= { content: [] };
			toolResult.toolResultError = err instanceof Error ? err.message : String(
        err,
      );
	...
	private _logToolApprovalTelemetry(tool: IToolEntry, dto: IToolInvocation, reason: ConfirmedReason): void {
      ...
      "inlineChat",
		...
		if (reason.type === ToolConfirmKind.ConfirmationNotNeeded && reason.reason) {
			...
			confirmationNotNeededReason = allowedConfirmationNotNeededReasons.has(
        raw,
      ) ? raw : "other";
		}
	...
	private async prepareToolInvocation(tool: IToolEntry, dto: IToolInvocation, forceConfirmationReason: string | undefined, token: CancellationToken): Promise<IPreparedToolInvocation | undefined> {
      ...
      tool.data,
		...
		if (!isEligibleForAutoApproval && prepared?.confirmationMessages?.title) {
			...
			prepared.confirmationMessages.disclaimer = toolIdsThatCannotBeAutoApproved.has(
        tool.data.id,
      ) ? undefined : new MarkdownString(localize(
        ...
        createMarkdownCommandLink(
          {
            ...
            tooltip: localize("openSettings.autoApproval.tooltip", "Open settings to configure auto-approval"),
          },
          false,
        ),
      ), {
	...
	private playAccessibilitySignal(toolInvocations: ChatToolInvocation[], chatSessionResource: URI | undefined): void {
      ...
      ChatConfiguration.GlobalAutoApprove,
		...
		if (chatSessionResource) {
			...
			if (isAutoApproveLevel(
        request?.modeInfo?.permissionLevel,
      ...
      inv => !IChatToolInvocation.executionConfirmedOrDenied(inv),
		...
		const setting: { sound?: "auto" | "on" | "off"; announcement?: "auto" | "off" } | undefined = this._configurationService.getValue(
      AccessibilitySignal.chatUserActionRequired.settingsKey,
        ...
        {
          customAlertMessage: this._instantiationService.invokeFunction(getToolConfirmationAlert, pendingInvocations),
          userGesture: true,
	...
	private ensureToolDetails(dto: IToolInvocation, toolResult: IToolResult, toolData: IToolData, toolInvocation: ChatToolInvocation | undefined): void {
		if (!toolResult.toolResultDetails && (toolData.alwaysDisplayInputOutput || (this.toolResultHasImages(
      toolResult,
	...
	private toolResultHasImages(toolResult: IToolResult): boolean {
      ...
      part => part.kind === "data" && part.value.mimeType?.startsWith("image/"),
    );
	}
	...
	private _isSessionLiveAutoApproveLevel(chatSessionResource: URI): boolean {
      ...
      chatSessionResource,
		...
		return !!widget && isAutoApproveLevel(
	...
	private isToolEligibleForAutoApproval(toolData: IToolData): boolean {
		const fullReferenceName = this.getEligibleForAutoApprovalSpecialCase(
      toolData,
		...
		const eligibilityConfig = this._configurationService.getValue<Record<string, boolean>>(
        ...
        fullReferenceName,
            ...
            legacyName,
					...
					if (legacyName.includes("/")) {
						...
						if (trimmedLegacyName && Object.prototype.hasOwnProperty.call(
              eligibilityConfig,
              trimmedLegacyName,
            )) {
	...
	private async shouldAutoConfirm(toolId: string, runsInWorkspace: boolean | undefined, source: ToolDataSource, parameters: unknown, chatSessionResource: URI | undefined, chatRequestId: string | undefined, combination?: { label: string; key: string }, workingDirectory?: URI): Promise<ConfirmedReason | undefined> {
		...
		if (chatSessionResource && !this._isAutoApprovePolicyRestricted() && this._isSessionInAutoApproveLevel(
      chatSessionResource,
    )) {
			// CLI sessions still need their multi-option dialogs (e.g. uncommitted changes).
			if (!(toolIdsThatCannotBeAutoApproved.has(
        tool.data.id,
      ...
      combination,
		...
		const config = this._configurationService.inspect<boolean | Record<string, boolean>>(
      ChatConfiguration.GlobalAutoApprove,
    );
		...
		const autoConfirm = value === true || (typeof value === "object" && value.hasOwnProperty(
      toolId,
    ) && value[toolId] === true);
		if (autoConfirm) {
	...
	private async shouldAutoConfirmPostExecution(toolId: string, runsInWorkspace: boolean | undefined, source: ToolDataSource, parameters: unknown, chatSessionResource: URI | undefined, chatRequestId: string | undefined, workingDirectory?: URI): Promise<ConfirmedReason | undefined> {
		...
		const sessionAutoApprove = chatSessionResource && !this._isAutoApprovePolicyRestricted() && this._isSessionInAutoApproveLevel(
      chatSessionResource,
    );
      ...
      ChatConfiguration.GlobalAutoApprove,
    ) && !sessionAutoApprove && await this._checkGlobalAutoApprove()) {
			return {
        type: ToolConfirmKind.Setting,
        id: ChatConfiguration.GlobalAutoApprove,
      };
	...
	private * getToolAliases(toolSet: IToolData, fullReferenceName: string): Iterable<string> {
		...
		if (toolSet.legacyToolReferenceFullNames) {
			for (const legacyName of toolSet.legacyToolReferenceFullNames) {
				...
				if (lastSlashIndex !== -1) {
					yield legacyName.substring(
            lastSlashIndex + 1,
          ); // it was also known under the simple name
				}
			}
		}
	...
	getToolSetsForModel(model: ILanguageModelChatMetadata | undefined, reader?: IReader): Iterable<IToolSet> {
      ...
      ts => new ToolSetForModel(
        ts,
        model,
        toolData => this.isToolEnabledForModel(toolData, model),
      ),
    );
	}
	...
	getSpecedToolSetName(referenceName: string): string {
		if (LanguageModelToolsService.githubMCPServerAliases.includes(
      referenceName,
    )) {
			return "github";
		}
		if (LanguageModelToolsService.playwrightMCPServerAliases.includes(
      referenceName,
    )) {
			return "playwright";
		}
	...
	* getFullReferenceNames(): Iterable<string> {
		...
		for (const [tool, fullReferenceName] of this.toolsWithFullReferenceName.get()) {
			if (isToolSet(tool)) {
				...
				if (tool.legacyToolReferenceFullNames) {
					...
					const slashIndex = fullReferenceName.lastIndexOf("/");
					const toolSetPrefix = slashIndex !== -1 ? fullReferenceName.substring(
            0,
            slashIndex + 1,
          ) : undefined;
	...
	getToolByFullReferenceName(fullReferenceName: string): IToolData | ToolSet | undefined {
		for (const [tool, toolFullReferenceName] of this.toolsWithFullReferenceName.get()) {
			...
			const aliases = isToolSet(tool) ? this.getToolSetAliases(
        tool,
