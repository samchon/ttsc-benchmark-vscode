...
export class ChatStatusDashboard extends DomWidget {
	...
	private renderUsageContent(container: HTMLElement, token: CancellationToken, headerAdditionalSpendButton: Button | undefined, headerUpgradeButton: Button | undefined, updatePromise: Promise<void>): void {
		...
		const planName = compact ? getChatPlanName(
      this.chatEntitlementService.entitlement,
    ) : undefined;
          ...
          compact ? planName : undefined,
			...
			if (premiumChatQuota && !premiumChatQuota.unlimited && premiumChatQuota.percentRemaining >= 0) {
				...
				const premiumChatLabel = isUBB
					? localize("creditsLabel", "Credits")
					: this.chatEntitlementService.quotas.additionalUsageEnabled ? localize(
              "includedPremiumChatsLabel",
              "Included premium requests",
            ) : localize("premiumChatsLabel", "Premium requests");
				const premiumChatResetLabel = isUBB ? this.formatResetAtLabel(
          premiumChatQuota.resetAt,
      ...
      $("div.collapsible-inner"),
		...
		if (disclosureHeader && chevron) {
          ...
          StorageTarget.USER,
			...
			this._store.add(
        addDisposableListener(disclosureHeader, EventType.CLICK, () => toggle()),
      );
		}
	...
	private renderContributedSections(contributedEntries: ChatStatusEntry[]): void {
        ...
        $("div.collapsible-header.non-collapsible"),
			...
			if (currentTooltip) {
				this._store.add(
          this.hoverService.setupDelayedHover(statusEl, () => ({
			...
			for (const provider of providers) {
              ...
              v => v.id === option.currentValueId,
                ...
                new SelectBox(
                  ...
                  defaultSelectBoxStyles,
                  {
                    ariaLabel: localize("selectOption", "Select {0}", option.label),
                    optionsAsChildren: true,
                  },
                ),
              );
	...
	private runCommandAndClose(commandOrFn: string | ((...args: unknown[]) => void), ...args: unknown[]): void {
		if (typeof commandOrFn === "function") {
			...
			this.telemetryService.publicLog2<WorkbenchActionExecutedEvent, WorkbenchActionExecutedClassification>(
        "workbenchActionExecuted",
        { id: commandOrFn, from: "chat-status" },
      );
	...
	private createQuotaIndicator(container: HTMLElement, quota: IQuotaSnapshot | string, label: string, resetLabel?: string, compactTitle?: string): (quota: IQuotaSnapshot | string) => void {
		...
		const quotaValueText = isCompact ? quotaValue.appendChild(
      $("span.quota-value-text"),
    ) : quotaValue;
	...
	private createGlobalQuotaCallout(container: HTMLElement): () => { calloutVisible: boolean; additionalUsageEnabled: boolean } {
      ...
      $("div.quota-callout", undefined, calloutIcon, calloutText),
		...
		const update = () => {
			...
			if (quotas.chat && !quotas.chat.unlimited) { allQuotas.push(
        quotas.chat,
      ); }
			if (quotas.premiumChat && !quotas.premiumChat.unlimited) { allQuotas.push(
        quotas.premiumChat,
      ); }
			if (quotas.completions && !quotas.completions.unlimited) { allQuotas.push(
        quotas.completions,
      ); }

			const maxUsedPercentage = allQuotas.length > 0 ? Math.max(
	...
	private createSettings(container: HTMLElement): void {
        ...
        $("span.setting-overridden"),
          ...
          defaultChat.completionsEnablementSetting,
				...
				const hasOverride = modeId && isObject(
          obj,
        ) && typeof obj[modeId] !== "undefined" && Boolean(
          obj[modeId],
        ) !== Boolean(obj["*"]);
          ...
          "(overridden)",
			...
			if (modeId) {
				...
				const languageName = this.languageService.getLanguageName(
          modeId,
        ) ?? modeId;
	...
	private createTriStateLanguageSetting(container: HTMLElement, label: string, modeId: string, onStateChange: () => void): void {
      ...
      $("span.setting-label", undefined, label),
		...
		const writeState = (state: boolean | "mixed") => {
			let result = this.configurationService.getValue<Record<string, boolean>>(
        settingId,
      );
			if (!isObject(result)) {
			...
			if (state === "mixed") {
				...
				this.telemetryService.publicLog2<ChatSettingChangedEvent, ChatSettingChangedClassification>(
