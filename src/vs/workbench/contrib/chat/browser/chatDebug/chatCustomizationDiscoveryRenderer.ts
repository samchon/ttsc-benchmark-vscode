...
function getFileLocationLabel(file: { uri: URI; storage?: string; extensionId?: string }, labelService: ILabelService, discoveryType?: string): string {
	...
	const parentDir = discoveryType === "skill" ? dirname(
    dirname(file.uri),
  ) : dirname(file.uri);
...
export function renderCustomizationDiscoveryContent(content: IChatDebugEventFileListContent, openerService: IOpenerService, modelService: IModelService, languageService: ILanguageService, hoverService: IHoverService, labelService: ILabelService, scrollable?: { scanDomNode(): void }): { element: HTMLElement; disposables: DisposableStore } {
	...
	const capitalizedType = content.discoveryType.charAt(0).toUpperCase() + content.discoveryType.slice(
    1,
  );
	DOM.append(
    container,
    ...
    $(
      ...
      undefined,
      localize("chatDebug.totalFiles", "Total files: {0}", content.files.length),
    ),
  );
      ...
      localize("chatDebug.loadedFilesList", "Loaded files"),
        ...
        $("div.chat-debug-file-list-group-header"),
          ...
          relative: true,
				...
				rows.push({
          element: row,
          activate: () => openerService.open(uri, { editorOptions: { preserveFocus: true } }),
        });
			}
		}
	...
	if (content.sourceFolders && content.sourceFolders.length > 0) {
      ...
      1,
		...
		const sourcesCaption = capitalizedType.endsWith(
      "s",
    ) ? capitalizedType : capitalizedType + "s";
		DOM.append(
      contentEl,
      $(
	...
	if (loaded.length > 0) {
      ...
      localize("chatDebug.plainText.loaded", "Loaded ({0})", loaded.length),
		...
		for (const f of loaded) {
			const parentDir = content.discoveryType === "skill" ? dirname(
        dirname(f.uri),
      ) : dirname(f.uri);
...
function getCategorySectionTitle(category: IChatDebugCustomizationLogEntry["category"], count: number): string {
	switch (category) {
		case "applying": return localize(
      "chatDebug.customization.instructions",
      "Instructions ({0})",
      count,
    );
		case "referenced": return localize(
      "chatDebug.customization.referenced",
      "Referenced ({0})",
      count,
    );
		case "skill": return localize(
      "chatDebug.customization.skill",
      "Skills ({0})",
      count,
    );
		case "custom-agent": return localize(
      "chatDebug.customization.customAgent",
      "Agents ({0})",
      count,
    );
		case "hook": return localize(
      "chatDebug.customization.hook",
      "Hooks ({0})",
      count,
    );
		case "skipped": return localize(
