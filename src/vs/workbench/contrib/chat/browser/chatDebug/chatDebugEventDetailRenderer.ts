...
export function formatEventDetail(event: IChatDebugEvent): string {
	switch (event.kind) {
        ...
        localize("chatDebug.detail.tool", "Tool: {0}", event.toolName),
			...
			if (event.toolCallId) { parts.push(
        ...
        localize("chatDebug.detail.result", "Result: {0}", event.result),
			...
			if (event.durationInMillis !== undefined) { parts.push(
        localize(
        ...
        event.model ?? localize("chatDebug.detail.modelTurn", "Model Turn"),
			...
			if (event.inputTokens !== undefined) { parts.push(
        localize(
          ...
          numberFormatter.value.format(event.inputTokens),
			...
			if (event.outputTokens !== undefined) { parts.push(
        localize(
          ...
          numberFormatter.value.format(event.outputTokens),
			...
			if (event.cachedTokens !== undefined) { parts.push(
        localize(
          ...
          numberFormatter.value.format(event.cachedTokens),
			...
			if (event.totalTokens !== undefined) { parts.push(
        localize(
        ...
        localize(
          ...
          numberFormatter.value.format(event.durationInMillis),
			...
			if (event.toolCallCount !== undefined) { parts.push(
        localize(
          ...
          numberFormatter.value.format(event.toolCallCount),
			...
			if (event.modelTurnCount !== undefined) { parts.push(
        localize(
