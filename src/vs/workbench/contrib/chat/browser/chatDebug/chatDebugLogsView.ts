...
export class ChatDebugLogsView extends Disposable {
	...
	constructor(
      ...
      new Button(this.headerContainer, {
        ...defaultButtonStyles,
        secondary: true,
        title: localize("chatDebug.toggleViewMode", "Toggle between list and tree view"),
      }),
    );
	...
	refreshList(): void {
		...
		const display = totalFiltered > this.visibleLimit ? this.filteredEvents.slice(
      0,
      this.visibleLimit,
    ) : this.filteredEvents;
	...
	private passesCurrentFilter(event: IChatDebugEvent): boolean {
		...
		if (this.cachedExcludeTerms.length > 0 && this.cachedExcludeTerms.some(
      term => debugEventMatchesText(event, term),
    )) {
			return false;
		}
		if (this.cachedIncludeTerms.length > 0 && !this.cachedIncludeTerms.some(
      term => debugEventMatchesText(event, term),
    )) {
			return false;
		}
	...
	private ensureCachedTerms(): void {
		...
		const terms = textOnly.split(",").map(t => t.trim()).filter(
      t => t.length > 0,
    );
		this.cachedIncludeTerms = terms.filter(t => !t.startsWith("!"));
		this.cachedExcludeTerms = terms.filter(t => t.startsWith("!")).map(t => t.slice(1).trim()).filter(
      t => t.length > 0,
    );
	}
