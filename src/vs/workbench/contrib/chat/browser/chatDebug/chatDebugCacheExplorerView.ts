...
export class ChatDebugCacheExplorerView extends Disposable {
	...
	updateBreadcrumb(): void {
		...
		const sessionTitle = this.chatService.getSessionTitle(
	...
	private async resolveSide(event: IChatDebugModelTurnEvent): Promise<ISideData> {
		...
		const inputMessages = stripFrom > 0 ? rawMessages.slice(
      stripFrom,
    ) : rawMessages;
		return {
      event,
	...
	private renderRail(groups: readonly ITurnGroup[]): void {
        ...
        DOM.addDisposableListener(header, DOM.EventType.CLICK, toggle),
          ...
          localize(
            ...
            "Turn {0}: {1}",
            i,
            evt.requestName ?? evt.model ?? localize("chatDebug.cache.modelTurn", "Model Turn"),
          ),
        );
				const select = () => {
	...
	private renderSummary(a: ISideData, b: ISideData, diff: ICacheDiffResult): void {
      ...
      $(
		...
		const expiration = !continuationComparison && isLikelyCacheExpiration(
      hit,
      diff,
      optionsDiff,
      systemChanged,
	...
	private renderSignature(a: ISideData, b: ISideData, diff: ICacheDiffResult, compareInputMessages: boolean): void {
      ...
      $("span.chat-debug-cache-sig-legend-entry"),
		...
		DOM.append(
      driftEntry,
      DOM.$("span", undefined, localize("chatDebug.cache.driftLegend", "drift")),
    );
	...
	private renderComponents(drift: readonly IComponentDrift[], a: ISideData, b: ISideData, compareInputMessages: boolean): void {
      ...
      $(
        ...
        formatTokens(c.bSize),
			...
			const aText = c.name === CURRENT_CONTINUATION_DELTA_COMPONENT ? "" : textForComponent(
        c,
        a,
      );
			const bText = c.name === CURRENT_CONTINUATION_DELTA_COMPONENT ? continuationDeltaText(
        b,
...
function badgeLabel(status: CacheDiffKind): string {
	switch (status) {
		case CacheDiffKind.Identical: return localize(
      "chatDebug.cache.badge.identical",
      "identical",
    );
		case CacheDiffKind.ContentDrift: return localize(
      "chatDebug.cache.badge.contentDrift",
      "content drift",
    );
		case CacheDiffKind.LengthChange: return localize(
      "chatDebug.cache.badge.lengthChange",
      "length change",
    );
		case CacheDiffKind.OnlyInA: return localize(
      "chatDebug.cache.badge.onlyA",
      "only in A",
    );
		case CacheDiffKind.OnlyInB: return localize(
...
function describeBreakKind(kind: Exclude<CacheDiffKind, CacheDiffKind.Identical>, diff: ICacheDiffResult, b: ISideData): string {
	...
	const charsB = bMsg ? numberFormatter.value.format(
    bMsg.charLength,
  ) : undefined;
	switch (kind) {
...
function describeRequestShape(inputMessages: readonly INormalizedMessage[], requestShapeJson: string | undefined): IRequestShapeInfo {
	...
	const hasToolSearchOutput = inputItemTypes.includes(
    "tool_search_output",
  ) || inputMessages.some(m => m.role === "tool_search");
	const hasOnlyToolOutput = inputMessages.length > 0 && inputMessages.every(
    m => m.role === "tool",
  );
