...
export function getEventNameText(element: IChatDebugEvent): string {
	switch (element.kind) {
      ...
      localize("chatDebug.unknownEvent", "(unknown)"),
		...
		case "generic": return safeStr(
      element.name,
      localize("chatDebug.unknownEvent", "(unknown)"),
    );
		case "subagentInvocation": return safeStr(
      element.agentName,
      localize("chatDebug.unknownEvent", "(unknown)"),
    );
		case "userMessage": return localize(
      "chatDebug.userMessage",
      "User Message",
    );
		case "agentResponse": return localize(
