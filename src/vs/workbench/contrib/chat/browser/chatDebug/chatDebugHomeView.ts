      ...
      AGENT_DEBUG_LOG_FILE_LOGGING_ENABLED_SETTING,
        ...
        localize("chatDebug.sessionList", "Chat sessions"),
			...
			for (const sessionResource of visibleSessions) {
				...
				const agentSession = this.agentSessionsService.model.getSession(
          sessionResource,
        );
				const rawTitle = agentSession?.label ?? this.chatService.getSessionTitle(
          sessionResource,
        );
				const importedTitle = this.chatDebugService.getImportedSessionTitle(
          sessionResource,
        );
				const historicalTitle = this.chatDebugService.getHistoricalSessionTitle(
          sessionResource,
        );
