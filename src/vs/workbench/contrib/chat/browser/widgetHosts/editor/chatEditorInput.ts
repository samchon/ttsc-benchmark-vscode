...
export class ChatEditorInput extends EditorInput implements IEditorCloseHandler {
	...
	showConfirm(): boolean {
		return !!(this.model && shouldShowClearEditingSessionConfirmation(
      this.model,
    ));
	}
	...
	override getName(): string {
		...
		if (this.model?.title) {
			...
			return this.model.hasCustomTitle ? this.model.title : truncate(
        this.model.title,
		...
		return this.options.title?.fallback ?? nls.localize(
      "chatEditorName",
      "Chat",
    );
	}
	...
	override async resolve(): Promise<ChatEditorModel | null> {
        ...
        "ChatEditorInput#resolve",
			...
			if (!this.model && LocalChatSessionUri.parseLocalSessionId(
        this._sessionResource,
      )) {
		...
		if (newIcon && (!this.cachedIcon || !this.iconsEqual(
      this.cachedIcon,
      newIcon,
    ))) {
