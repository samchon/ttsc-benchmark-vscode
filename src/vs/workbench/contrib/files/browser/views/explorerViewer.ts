...
export class ExplorerFindProvider implements IAsyncFindProvider<ExplorerItem> {
	...
	constructor(
		...
		this.explorerFindActiveContextKey = ExplorerFindProviderActive.bindTo(
      contextKeyService,
    );
	}
	...
	isVisible(element: ExplorerItem): boolean {
		...
		return this.filterSessionStartState.rootsWithProviders.has(
      element.root,
    ) ? element.isMarkedAsFiltered() : true;
	}
	...
	private async searchInWorkspace(patternLowercase: string, root: ExplorerItem, rootIndex: number, isFuzzyMatch: boolean, token: CancellationToken): Promise<{ explorerRoot: ExplorerItem; files: URI[]; directories: URI[]; hitMaxResults: boolean }> {
		const segmentMatchPattern = isFuzzyMatch ? fuzzyMatchingGlobPattern(
      patternLowercase,
...
export class FilesRenderer implements ICompressibleTreeRenderer<ExplorerItem, FuzzyScore, IFileTemplateData>, IListAccessibilityProvider<ExplorerItem>, IDisposable {
	...
	renderCompressedElements(node: ITreeNode<ICompressedTreeNode<ExplorerItem>, FuzzyScore>, index: number, templateData: IFileTemplateData): void {
      ...
      e => this.explorerService.isEditable(e),
		...
		const editableData = editable.length === 0 ? undefined : this.explorerService.getEditableData(
      editable[0],
    );
		...
		if (!editableData) {
			...
			if (fuzzyScore && fuzzyScore.length > 2) {
				const filterDataOffset = labels.join(
          "/",
        ).length - labels[labels.length - 1].length;
				fuzzyScore = [
          fuzzyScore[0],
        ...
        node.collapsed,
			...
			const nodeControllers = this.compressedNavigationControllers.get(
        stat,
      ) ?? [];
	...
	private renderStat(stat: ExplorerItem, label: string | string[], domId: string | undefined, filterData: FuzzyScore | undefined, templateData: IFileTemplateData): void {
      ...
      domId,
		...
		const highlightResults = stat.isDirectory ? this.highlightTree.get(
      stat,
    ) : 0;
	...
	getActiveDescendantId(stat: ExplorerItem): string | undefined {
		return this.compressedNavigationControllers.get(
      stat,
    )?.[0]?.currentId ?? undefined;
	}
...
export class FilesFilter implements ITreeFilter<ExplorerItem, FuzzyScore> {
	...
	filter(stat: ExplorerItem, parentVisibility: TreeVisibility): boolean {
		...
		if (stat.name === ".gitignore" && this.ignoreTreesPerRoot.has(
	...
	private isVisible(stat: ExplorerItem, parentVisibility: TreeVisibility): boolean {
      ...
      stat.isDirectory,
        ...
        e => e.resource && this.uriIdentityService.extUri.isEqualOrParent(e.resource, stat.resource),
			...
			if (editor && stat.root === this.explorerService.findClosestRoot(
        stat.resource,
      )) {
	...
	isIgnored(resource: URI, rootResource: URI, isDirectory: boolean): boolean {
		const ignoreFile = this.ignoreTreesPerRoot.get(rootResource.toString())?.findSubstr(
      resource,
    );
...
export class FileDragAndDrop implements ITreeDragAndDrop<ExplorerItem> {
	...
	onDragStart(data: IDragAndDropData, originalEvent: DragEvent): void {
      ...
      originalEvent,
        ...
        accessor => fillEditorsDragData(accessor, items, originalEvent),
			...
			const fileResources = items.filter(s => s.resource.scheme === Schemas.file).map(
        r => r.resource.fsPath,
      );
	...
	private async handleExplorerDrop(data: ElementsDragAndDropData<ExplorerItem, ExplorerItem[]>, target: ExplorerItem, targetIndex: number | undefined, targetSector: ListViewTargetSector | undefined, originalEvent: DragEvent): Promise<void> {
      ...
      elementsData.map(element => [element, this.isCollapsed(element)]),
		...
		const confirmDragAndDrop = !isCopy && this.configurationService.getValue<boolean>(
      FileDragAndDrop.CONFIRM_DND_SETTING_KEY,
        ...
        "Are you sure you want to change the order of multiple root folders in your workspace?",
			...
			const detail = items.length > 1 && !items.every(
        s => s.isRoot,
      ) ? getFileNamesMessage(items.map(i => i.resource)) : undefined;
		...
		try {
			...
			if ((<FileOperationError>error).fileOperationResult === FileOperationResult.FILE_MOVE_CONFLICT) {
				...
				for (const edit of resourceFileEdits) {
					if (edit.newResource && await this.fileService.exists(
            edit.newResource,
          )) {
            ...
            resourceFileEdits.map(
              re => new ResourceFileEdit(re.oldResource, re.newResource, {
                overwrite: true,
              }),
            ),
            options,
