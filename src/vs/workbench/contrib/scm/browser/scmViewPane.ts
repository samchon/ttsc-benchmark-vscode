	...
	onDragStart(data: IDragAndDropData, originalEvent: DragEvent): void {
      ...
      data as ElementsDragAndDropData<TreeElement, TreeElement[]>,
        ...
        accessor => fillEditorsDragData(accessor, items, originalEvent),
			...
			const fileResources = items.filter(s => s.scheme === Schemas.file).map(
        r => r.fsPath,
      );
...
class InputRenderer implements ICompressibleTreeRenderer<ISCMInput, FuzzyScore, InputTemplate> {
	...
	getHeight(input: ISCMInput): number {
		return (this.contentHeights.get(
      input,
    ) ?? InputRenderer.DEFAULT_HEIGHT) + 10;
	}
...
class ResourceRenderer implements ICompressibleTreeRenderer<ISCMResource | IResourceNode<ISCMResource, ISCMResourceGroup>, FuzzyScore | LabelFuzzyScore, ResourceTemplate> {
	...
	renderElement(node: ITreeNode<ISCMResource, FuzzyScore | LabelFuzzyScore> | ITreeNode<ISCMResource | IResourceNode<ISCMResource, ISCMResourceGroup>, FuzzyScore | LabelFuzzyScore>, index: number, template: ResourceTemplate): void {
		...
		const iconResource = ResourceTree.isResourceNode(
      resourceOrFolder,
    ) ? resourceOrFolder.element : resourceOrFolder;
		const uri = ResourceTree.isResourceNode(
      resourceOrFolder,
    ) ? resourceOrFolder.uri : resourceOrFolder.sourceUri;
		const fileKind = ResourceTree.isResourceNode(
      resourceOrFolder,
    ) ? FileKind.FOLDER : FileKind.FILE;
		const tooltip = !ResourceTree.isResourceNode(
      resourceOrFolder,
...
export class SCMTreeSorter implements ITreeSorter<TreeElement> {
	...
	compare(one: TreeElement, other: TreeElement): number {
		...
		const otherName = ResourceTree.isResourceNode(
      other,
    ) ? other.name : basename((other as ISCMResource).sourceUri);
...
export class SCMAccessibilityProvider implements IListAccessibilityProvider<TreeElement> {
	...
	getAriaLabel(element: TreeElement): string {
        ...
        noPrefix: true,
			...
			const verbosity = this.configurationService.getValue<boolean>(
...
class RepositoryVisibilityActionController {
	...
	constructor(
		...
		this.repositoryCountContextKey = ContextKeys.RepositoryCount.bindTo(
      contextKeyService,
    );
		this.repositoryVisibilityCountContextKey = ContextKeys.RepositoryVisibilityCount.bindTo(
      contextKeyService,
    );
...
export class SCMViewPane extends ViewPane {
	...
	constructor(
      ...
      hoverService,
		...
		this.viewSortKeyContextKey = ContextKeys.SCMViewSortKey.bindTo(
      contextKeyService,
    );
      ...
      contextKeyService,
		...
		this.scmProviderContextKey = ContextKeys.SCMProvider.bindTo(
      contextKeyService,
    );
		this.scmProviderRootUriContextKey = ContextKeys.SCMProviderRootUri.bindTo(
      contextKeyService,
    );
		this.scmProviderHasRootUriContextKey = ContextKeys.SCMProviderHasRootUri.bindTo(
      contextKeyService,
    );
	...
	protected override renderBody(container: HTMLElement): void {
      ...
      this.configurationService.getValue<boolean>("scm.alwaysShowActions"),
		...
		Event.filter(this.configurationService.onDidChangeConfiguration, e => e.affectsConfiguration("scm.alwaysShowActions"), this.disposables)(
      ...
      this.disposables,
		...
		const updateProviderCountVisibility = () => {
			const value = this.configurationService.getValue<"hidden" | "auto" | "visible">(
		...
		Event.filter(this.configurationService.onDidChangeConfiguration, e => e.affectsConfiguration("scm.providerCountBadge"), this.disposables)(
	...
	private createTree(container: HTMLElement, viewState?: IAsyncDataTreeViewState): void {
      ...
      ".scm-overflow-widgets-container.monaco-editor",
		...
		this.actionButtonRenderer = this.instantiationService.createInstance(
	...
	private onDidActiveEditorChange(): void {
      ...
      e => isSCMResource(e) && this.uriIdentityService.extUri.isEqual(e.sourceUri, uri),
    ) &&
	...
	private onDidChangeVisibleRepositories({ added, removed }: ISCMViewVisibleRepositoryChangeEvent): void {
        ...
        new DisposableMap<ISCMResourceGroup, IDisposable>(),
			...
			repositoryDisposables.add(
        repository.provider.onDidChangeResourceGroups(onDidChangeResourceGroups),
      );
	...
	private getSelectedRepositories(): ISCMRepository[] {
		const focusedRepositories = this.tree.getFocus().filter(
      r => !!r && isSCMRepository(r),
    )! as ISCMRepository[];
		const selectedRepositories = this.tree.getSelection().filter(
      r => !!r && isSCMRepository(r),
    )! as ISCMRepository[];

		return Array.from(
      new Set<ISCMRepository>([...focusedRepositories, ...selectedRepositories]),
    );
	}
	...
	private getViewMode(): ViewMode {
		let mode = this.configurationService.getValue<"tree" | "list">(
      "scm.defaultViewMode",
    ) === "list" ? ViewMode.List : ViewMode.Tree;
	...
	private getViewSortKey(): ViewSortKey {
		...
		const viewSortKeyString = this.configurationService.getValue<"path" | "name" | "status">(
	...
	private async focusResourceGroup(delta: number): Promise<void> {
		...
		const focusedResourceGroup = this.tree.getFocus().find(
      e => isSCMResourceGroup(e),
    );
		const focusedResourceGroupIndex = treeHasDomFocus && focusedResourceGroup ? resourceGroups.indexOf(
      focusedResourceGroup,
    ) : -1;
...
class SCMTreeDataSource extends Disposable implements IAsyncDataSource<ISCMViewService, TreeElement> {
	...
	async getChildren(inputOrElement: ISCMViewService | TreeElement): Promise<Iterable<TreeElement>> {
		...
		const showActionButton = this.configurationService.getValue<boolean>(
      ...
      inputOrElement,
    ) && repositoryCount === 1 && !alwaysShowRepositories) || isSCMRepository(
      inputOrElement,
    )) {
			const children: TreeElement[] = [];

			inputOrElement = isSCMRepository(
        inputOrElement,
      ) ? inputOrElement : this.scmViewService.visibleRepositories[0];
