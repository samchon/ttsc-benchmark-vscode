...
class SCMInputWidgetToolbar extends WorkbenchToolBar {
	...
	public setInput(input: ISCMInput): void {
      ...
      input.repository.provider.onDidChangeResources(() => updateToolbar()),
		...
		this._disposables.value.add(
      this.storageService.onDidChangeValue(StorageScope.PROFILE, SCMInputWidgetStorageKey.LastActionId, this._disposables.value)(
        () => updateToolbar(),
      ),
    );
...
class SCMInputWidgetEditorOptions {
	...
	getEditorOptions(): IEditorOptions {
		...
		const wordSegmenterLocales = this.configurationService.getValue<string | string[]>(
      ...
      "editor.cursorBlinking",
		...
		const cursorStyle = this.configurationService.getValue<IEditorOptions["cursorStyle"]>(
      "editor.cursorStyle",
    );
		const cursorWidth = this.configurationService.getValue<IEditorOptions["cursorWidth"]>(
      "editor.cursorWidth",
    ) ?? 1;
		const emptySelectionClipboard = this.configurationService.getValue<boolean>(
      "editor.emptySelectionClipboard",
    ) === true;
		const roundedSelection = this.configurationService.getValue<boolean>(
	...
	private _getEditorLanguageConfiguration(): IEditorOptions {
      ...
      overrideIdentifier: "scminput",
		...
		const rulers = rulersConfig.overrideIdentifiers?.includes(
      "scminput",
...
export class SCMInputWidget {
	...
	set input(input: ISCMInput | undefined) {
      ...
      this.configurationService,
		...
		const getPlaceholderText = (): string => {
			const binding = this.keybindingService.lookupKeybinding(
        "scm.acceptInput",
      );
	...
	constructor(
      ...
      undefined,
		...
		this.validationMessageContextKey = SCMInputContextKeys.SCMInputHasValidationMessage.bindTo(
	...
	getContentHeight(): number {
      ...
      "scm.inputMinLineCount",
		...
		const inputMinLines = typeof inputMinLinesConfig === "number" ? clamp(
      inputMinLinesConfig,
      1,
      50,
    ) : 1;
      ...
      "scm.inputMaxLineCount",
		...
		const inputMaxLines = typeof inputMaxLinesConfig === "number" ? clamp(
      inputMaxLinesConfig,
      1,
      50,
    ) : 10;
