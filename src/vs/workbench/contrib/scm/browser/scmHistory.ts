...
export function toISCMHistoryItemViewModelArray(
	...
	for (let index = 0; index < historyItems.length; index++) {
		...
		const outputSwimlanesFromPreviousItem = viewModels.at(
      -1,
    )?.outputSwimlanes ?? [];
      ...
      i => deepClone(i),
		...
		for (let i = firstParentAdded ? 1 : 0; i < historyItem.parentIds.length; i++) {
			...
			if (i === 0) {
				...
				colorIdentifier = historyItemParent ? getLabelColorIdentifier(
          historyItemParent,
          colorMap,
        ) : undefined;
			}
...
function addIncomingOutgoingChangesHistoryItems(
	...
	if (currentHistoryItemRef?.revision !== currentHistoryItemRemoteRef?.revision && mergeBase) {
        ...
        viewModels,
        vm => vm.outputSwimlanes.some(node => node.id === mergeBase),
      );
        ...
        vm => vm.historyItem.id === mergeBase,
            ...
            mergeBase,
				...
				if (!incomingChangeMerged) {
					...
					const inputSwimlanes = viewModels[beforeHistoryItemIndex].outputSwimlanes.map(
            i => deepClone(i),
          );
					const outputSwimlanes = viewModels[afterHistoryItemIndex].inputSwimlanes.map(
            i => deepClone(i),
          );
        ...
        vm => vm.kind === "HEAD" && vm.historyItem.id === currentHistoryItemRef.revision,
			...
			if (currentHistoryItemRefIndex !== -1) {
				...
				const inputSwimlanes = viewModels[currentHistoryItemRefIndex].inputSwimlanes.slice(
          0,
        );
	...
	if (includeReferences && historyItem.references?.length) {
      ...
      supportThemeIcons: true,
		...
		for (const reference of historyItem.references) {
			const labelIconId = ThemeIcon.isThemeIcon(
        reference.icon,
      ) ? reference.icon.id : "";

			const labelBackgroundColor = reference.color ? asCssVariable(
        reference.color,
      ) : asCssVariable(historyItemHoverDefaultLabelBackground);
			const labelForegroundColor = reference.color ? asCssVariable(
