	...
	private onDidChange(): void {
      ...
      change => change.providerId === primaryQuickDiff?.id,
        ...
        quickDiff.id,
			...
			if (quickDiff.kind !== "primary" && primaryQuickDiffChanges.some(
        c => c.change2.modified.intersectsOrTouches(change.change2.modified),
      )) {
		...
		if (!this.decorationsCollection) {
			this.decorationsCollection = this.codeEditor.createDecorationsCollection(
        decorations,
      );
		} else {
...
export class QuickDiffWorkbenchController extends Disposable implements IWorkbenchContribution {
	...
	constructor(
      ...
      this._store,
		...
		this.quickDiffDecorationCount = quickDiffDecorationCount.bindTo(
      contextKeyService,
    );
	...
	private onDidChangeConfiguration(): void {
		const enabled = this.configurationService.getValue<string>(
      "scm.diffDecorations",
    ) !== "none";
	...
	private enable(): void {
      ...
      Event.any(this.editorService.onDidCloseEditor, this.editorService.onDidVisibleEditorsChange)(
