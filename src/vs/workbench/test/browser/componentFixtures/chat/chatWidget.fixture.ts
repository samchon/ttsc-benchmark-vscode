...
export async function renderChatWidget(context: ComponentFixtureContext, options: IChatWidgetFixtureOptions): Promise<void> {
	...
	const hasRiskAssessment = options.messages.some(
    m => m.assistant?.some(
      p => (p.kind === "terminalConfirmation" || p.kind === "elicitation") && p.riskAssessment,
    ),
  );
	const hasRiskLoading = options.messages.some(
    m => m.assistant?.some(
      p => (p.kind === "terminalConfirmation" || p.kind === "elicitation") && p.riskLoading,
    ),
  );
	...
	menuService.addItem(MenuId.ChatInput, {
    command: { id: "workbench.action.chat.openModelPicker", title: "GPT-5.3-Codex" },
    group: "navigation",
    order: 3,
  });
	menuService.addItem(MenuId.ChatInput, {
    command: { id: "workbench.action.chat.configureTools", title: "", icon: Codicon.settingsGear },
    group: "navigation",
    order: 100,
  });
    ...
    command: { id: "workbench.action.chat.openSessionTargetPicker", title: "Local" },
    group: "navigation",
    order: 0,
  });
	menuService.addItem(MenuId.ChatInputSecondary, {
    command: { id: "workbench.action.chat.openPermissionPicker", title: "Default Approvals" },
    group: "navigation",
    order: 10,
  });
