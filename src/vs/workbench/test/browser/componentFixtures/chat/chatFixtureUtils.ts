...
export function registerChatFixtureServices(reg: ServiceRegistration, options: IChatFixtureServicesOptions = {}): void {
	...
	reg.defineInstance(
    IDecorationsService,
    new class extends mock<IDecorationsService>() { override onDidChangeDecorations = Event.None; }(),
  );
	reg.defineInstance(
    ITextFileService,
    new class extends mock<ITextFileService>() { override readonly untitled = new class extends mock<ITextFileService["untitled"]>() { override readonly onDidChangeLabel = Event.None; }(); }(),
  );
	reg.defineInstance(
    IFileService,
    new class extends mock<IFileService>() { override onDidFilesChange = Event.None; override onDidRunOperation = Event.None; override hasProvider() { return false; } }(),
  );
	reg.defineInstance(
    IEditorService,
    new class extends mock<IEditorService>() { override onDidActiveEditorChange = Event.None; }(),
  );
	reg.defineInstance(
    IExtensionService,
    new class extends mock<IExtensionService>() { override readonly onDidChangeExtensions = Event.None; }(),
  );
	reg.defineInstance(
    IPathService,
    ...
    new class extends mock<IWorkbenchAssignmentService>() { override async getCurrentExperiments() { return []; } override async getTreatment() { return undefined; } override onDidRefetchAssignments = Event.None; }(),
  );
	reg.defineInstance(
    ...
    new class extends mock<IWorkbenchLayoutService>() { override onDidChangePartVisibility = Event.None; override onDidChangeWindowMaximized = Event.None; override isVisible() { return true; } }(),
  );
	reg.defineInstance(
    IViewDescriptorService,
    new class extends mock<IViewDescriptorService>() { override onDidChangeLocation = Event.None; }(),
  );
	reg.defineInstance(
	...
	reg.defineInstance(
    IUpdateService,
    new class extends mock<IUpdateService>() { override onStateChange = Event.None; override get state() { return { type: StateType.Uninitialized as const }; } }(),
  );
	reg.defineInstance(
    IUriIdentityService,
    ...
    new class extends mock<IActionWidgetService>() { override show() { } override hide() { } override get isVisible() { return false; } }(),
  );
	reg.defineInstance(
    ...
    new class extends mock<IAccessibleViewService>() { override getOpenAriaHint() { return null; } }(),
  );
    ...
    new class extends mock<IChatSessionsService>() { override getAllChatSessionContributions() { return []; } override readonly onDidChangeSessionOptions = Event.None; override readonly onDidChangeOptionGroups = Event.None; override readonly onDidChangeAvailability = Event.None; override getCustomAgentTargetForSessionType() { return Target.Undefined; } override requiresCustomModelsForSessionType() { return false; } override getOptionGroupsForSessionType() { return []; } }(),
  );
	reg.defineInstance(
    IChatEntitlementService,
    new class extends mock<IChatEntitlementService>() { override readonly quotas = {}; override readonly onDidChangeQuotaRemaining = Event.None; override readonly onDidChangeUsageBasedBilling = Event.None; }(),
  );
	reg.defineInstance(IChatModeService, new MockChatModeService());
	reg.defineInstance(
    ILanguageModelsService,
    new class extends mock<ILanguageModelsService>() { override onDidChangeLanguageModels = Event.None; override onDidChangeModelVisibility = Event.None; override getLanguageModelIds() { return []; } override getVendors() { return []; } override hasResolvedVendor() { return false; } }(),
  );
	reg.defineInstance(
    ILanguageModelToolsService,
    new class extends mock<ILanguageModelToolsService>() { override onDidChangeTools = Event.None; override onDidPrepareToolCallBecomeUnresponsive = Event.None; override getTools() { return []; } }(),
  );
    ...
    new class extends mock<IChatWidgetHistoryService>() { override getHistory() { return []; } override readonly onDidChangeHistory = Event.None; }(),
  );
	reg.defineInstance(
    ...
    new class extends mock<IChatMarkdownAnchorService>() { override register() { return { dispose() { } }; } }(),
  );
    ...
    new class extends mock<IAgentSessionsService>() { override readonly model = new class extends mock<IAgentSessionsService["model"]>() { override readonly onDidChangeSessions = Event.None; }(); }(),
  );
	reg.defineInstance(
    IAgentHostService,
	...
	const artifactGroups = options.artifactGroups ?? observableValue<readonly IArtifactSourceGroup[]>(
    "artifactGroups",
    [],
  );
