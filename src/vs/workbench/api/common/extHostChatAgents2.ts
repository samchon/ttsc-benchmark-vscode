...
export class ExtHostChatAgents2 extends Disposable implements ExtHostChatAgentsShape2 {
	...
	private readonly _sessionDisposables: DisposableResourceMap<DisposableStore> = this._register(
    new DisposableResourceMap(),
  );
	private readonly _completionDisposables: DisposableMap<number, DisposableStore> = this._register(
    new DisposableMap(),
  );
    ...
    () => this._proxy.$provideCustomAgents(CancellationToken.None).then(
      agents => agents.map(agent => this.toCustomAgent(agent)),
    ),
  );
	private readonly _instructions = new CachedPromise(
    () => this._proxy.$provideInstructions(CancellationToken.None).then(
      instructions => instructions.map(
        instruction => this.toInstruction(instruction),
      ),
    ),
  );
	private readonly _skills = new CachedPromise(
    () => this._proxy.$provideSkills(CancellationToken.None).then(
      skills => skills.map(skill => this.toSkill(skill)),
    ),
  );
	private readonly _slashCommands = new CachedPromise(
    () => this._proxy.$provideSlashCommands(CancellationToken.None).then(
      slashCommands => slashCommands.map(
        slashCommand => this.toSlashCommand(slashCommand),
      ),
    ),
  );
	private readonly _hooks = new CachedPromise(
    () => this._proxy.$provideHooks(CancellationToken.None).then(
      hooks => hooks.map(hook => this.toHook(hook)),
    ),
  );
	private readonly _plugins = new CachedPromise(
    () => this._proxy.$providePlugins(CancellationToken.None).then(
      plugins => plugins.map(plugin => this.toPlugin(plugin)),
    ),
  );
	...
	async $providePromptFiles(handle: number, type: PromptsType, context: IPromptFileContext, token: CancellationToken): Promise<IPromptFileResource[] | undefined> {
		...
		switch (type) {
			case PromptsType.agent:
				resources = await (provider as vscode.ChatCustomAgentProvider).provideCustomAgents(
          context,
          token,
        ) ?? undefined;
				break;
			case PromptsType.instructions:
				resources = await (provider as vscode.ChatInstructionsProvider).provideInstructions(
          context,
          token,
        ) ?? undefined;
				break;
			case PromptsType.prompt:
				resources = await (provider as vscode.ChatPromptFileProvider).providePromptFiles(
          context,
          token,
        ) ?? undefined;
				break;
			case PromptsType.skill:
				resources = await (provider as vscode.ChatSkillProvider).provideSkills(
          context,
          token,
        ) ?? undefined;
				break;
			case PromptsType.hook:
				resources = await (provider as vscode.ChatHookProvider).provideHooks(
          context,
          token,
        ) ?? undefined;
				break;
	...
	async $provideChatSessionCustomizations(handle: number, sessionResource: UriComponents | undefined, token: CancellationToken): Promise<IChatSessionCustomizationItemDto[] | undefined> {
		...
		try {
			const items = await providerData.provider.provideChatSessionCustomizations(
	...
	async $invokeAgent(handle: number, requestDto: Dto<IChatAgentRequest>, context: { history: IChatAgentHistoryEntryDto[]; chatSessionContext?: IChatSessionContextDto }, token: CancellationToken): Promise<IChatAgentInvokeResult | undefined> {
        ...
        token,
			...
			return {
        errorDetails: { message: toErrorMessage(e), responseIsIncomplete: true, isQuotaExceeded, isRateLimited, isExpectedError },
        errorCallstack,
        errorName,
      };
	...
	private async prepareHistoryTurns(extension: Readonly<IRelaxedExtensionDescription>, agentId: string, context: { history: IChatAgentHistoryEntryDto[] }): Promise<(vscode.ChatRequestTurn | vscode.ChatResponseTurn)[]> {
		...
		for (const h of context.history) {
			...
			const result: vscode.ChatResult = agentId === h.request.agentId || (isBuiltinParticipant(
        h.request.agentId,
      ...
      ) ? h.request.editedFileEvents : undefined;
			const modeInstructions2 = isProposedApiEnabled(
        extension,
        "chatParticipantPrivate",
      ) && h.request.modeInstructions ? typeConvert.ChatRequestModeInstructions.to(
	...
	$acceptActiveChatSession(sessionResourceDto: UriComponents | undefined): void {
		const sessionResource = sessionResourceDto ? URI.revive(
      sessionResourceDto,
    ) : undefined;
	...
	async $provideChatTitle(handle: number, context: IChatAgentHistoryEntryDto[], token: CancellationToken): Promise<string | undefined> {
      ...
      history: context,
		...
		const sessionResource = context[0]?.request.sessionResource ? URI.revive(
      context[0].request.sessionResource,
    ) : undefined;
...
class ExtHostChatAgent {
	...
	async provideTitle(context: vscode.ChatContext, token: CancellationToken): Promise<string | undefined> {
		...
		return await this._titleProvider.provideChatTitle(
      context,
      token,
    ) ?? undefined;
	}
	...
	async provideSummary(context: vscode.ChatContext, token: CancellationToken): Promise<string | undefined> {
		...
		return await this._summarizer.provideChatSummary(
      context,
      token,
    ) ?? undefined;
	}
