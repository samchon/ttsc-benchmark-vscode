...
export class DynamicAuthProvider implements vscode.AuthenticationProvider {
	...
	constructor(
      ...
      this._tokenStore.onDidChangeSessions(
        e => this._onDidChangeSessions.fire(e),
      ),
    );
	...
	async getSessions(scopes: readonly string[] | undefined, _options: vscode.AuthenticationProviderSessionOptions): Promise<vscode.AuthenticationSession[]> {
      ...
      `Getting sessions for scopes: ${scopes?.join(" ") ?? "all"}`,
		...
		let sessions = this._tokenStore.sessions.filter(
      session => arraysEqual([...session.scopes].sort(), sortedScopes),
    );
		this._logger.info(
      `Found ${sessions.length} sessions for scopes: ${scopeStr}`,
        ...
        this._tokenStore.tokens.map(token => [token.access_token, token]),
			...
			if (newTokens.length || removedTokens.length) {
				...
				sessions = this._tokenStore.sessions.filter(
          session => arraysEqual([...session.scopes].sort(), sortedScopes),
        );
			}
			this._logger.info(
	...
	protected async exchangeCodeForToken(code: string, codeVerifier: string, redirectUri: string): Promise<IAuthorizationTokenResponse> {
		...
		if (isAuthorizationTokenResponse(result)) {
			...
			return result;
		} else if (isAuthorizationErrorResponse(
      result,
...
class TokenStore implements Disposable {
	...
	constructor(
      ...
      (reader) => this._tokensObservable.read(reader).map(
        t => this._getSessionFromToken(t),
      ),
    );
		this._disposable.add(this._registerChangeEventAutorun());
		this._disposable.add(
      this._persistence.onDidChange(
        (tokens) => this._tokensObservable.set(tokens, undefined),
      ),
    );
	}
