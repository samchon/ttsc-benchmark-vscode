...
export class ExtHostMcpService extends Disposable implements IExtHostMpcService {
	...
	$onDidChangeMcpServerDefinitions(servers: McpServerDefinition.Serialized[]): void {
		this._mcpServerDefinitions = servers.map(
      dto => Convert.McpServerDefinition.to(dto),
    );
	...
	async $substituteVariables<T>(_workspaceFolder: UriComponents | undefined, value: T): Promise<T> {
		...
		const folder = folderURI && await this._workspaceService.resolveWorkspaceFolder(
      folderURI,
    );
      ...
      configTarget: this._extHostInitData.remote.isRemote ? ConfigurationTarget.USER_REMOTE : ConfigurationTarget.USER,
        ...
        CancellationToken.None,
			...
			for (const item of list ?? []) {
				let id = ExtensionIdentifier.toKey(
	...
	private _parseScopesFromResponse(responseHeaders: Headers): string[] | undefined {
		...
		for (const challenge of challenges) {
			if (challenge.scheme === "Bearer" && challenge.params["scope"]) {
				const scopes = challenge.params["scope"].split(AUTH_SCOPE_SEPARATOR).filter(
          s => s.trim().length,
        );
				if (scopes.length) {
					this._log(
...
export async function createAuthMetadata(
	...
	const { resourceMetadataChallenge, scopesChallenge: scopesChallengeFromHeader } = parseWWWAuthenticateHeaderForChallenges(
	...
	try {
      ...
      `Fetching auth server metadata for: ${serverMetadataUrl} ...`,
		...
		const { metadata, discoveryUrl, errors } = await fetchAuthorizationServerMetadata(
      serverMetadataUrl,
      {
        additionalHeaders,
        fetch: (url, init) => fetch(url, init as MinimalRequestInit),
