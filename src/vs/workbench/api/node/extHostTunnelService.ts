...
export function loadConnectionTable(stdout: string): Record<string, string>[] {
    ...
    line => line.trim().split(/\s+/).reduce(
      (obj: Record<string, string>, value, i) => {
        obj[names[i] || i] = value;
        return obj;
      },
      {},
...
export class NodeExtHostTunnelService extends ExtHostTunnelService {
	...
	private async findCandidatePorts(): Promise<CandidatePort[]> {
		...
		const connections: { socket: number; ip: string; port: number }[] = loadListeningPorts(
      tcp,
      tcp6,
    );
	...
	private async defaultTunnelHost(): Promise<string> {
		const settingValue = (await this.configurationService.getConfigProvider()).getConfiguration("remote").get(
      "localPortHost",
    );
