...
export class NativeExtHostSearch extends ExtHostSearch implements IDisposable {
	...
	constructor(
		...
		this.handleConfigurationChanged = this.handleConfigurationChanged.bind(
      this,
    );
	...
	async getNumThreads(): Promise<number | undefined> {
		...
		const numThreads = configProvider.getConfiguration("search").get<number>(
      "ripgrep.maxThreads",
    );
		return numThreads;
	}
	...
	override async doInternalFileSearchWithCustomCallback(rawQuery: IFileQuery, token: vscode.CancellationToken, handleFileMatch: (data: URI[]) => void): Promise<ISearchCompleteStats> {
		...
		return <Promise<ISearchCompleteStats>>this._internalFileSearchProvider.doFileSearch(
      rawQuery,
      numThreads,
      onResult,
      token,
