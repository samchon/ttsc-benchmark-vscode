...
export class CLIServerBase {
	...
	private async open(data: OpenCommandPipeArgs): Promise<undefined> {
		...
		const waitMarkerFileURI = waitMarkerFilePath ? URI.file(
      waitMarkerFilePath,
    ) : undefined;
	...
	private async openExternal(data: OpenExternalCommandPipeArgs): Promise<undefined> {
        ...
        uriString,
      ); // always send the string, workaround for #112577
		}
	}

	private async manageExtensions(data: ExtensionManagementPipeArgs): Promise<string | undefined> {
		const toExtOrVSIX = (inputs: string[] | undefined) => inputs?.map(
	...
	dispose(): void {
		...
		if (this._ipcHandlePath && process.platform !== "win32" && fs.existsSync(
      this._ipcHandlePath,
    )) {
