...
export class MainThreadTreeViews extends Disposable implements MainThreadTreeViewsShape {
	...
	private readonly _dataProviders: DisposableMap<string, { dataProvider: TreeViewDataProvider; dispose: () => void }> = this._register(
    new DisposableMap<string, { dataProvider: TreeViewDataProvider; dispose: () => void }>(),
  );
	...
	private getTreeView(treeViewId: string): ITreeView | null {
		const viewDescriptor: ITreeViewDescriptor = <ITreeViewDescriptor>Registry.as<IViewsRegistry>(Extensions.ViewsRegistry).getView(
      treeViewId,
    );
...
class TreeViewDragAndDropController implements ITreeViewDragAndDropController {
	...
	async handleDrop(dataTransfer: VSDataTransfer, targetTreeItem: ITreeItem | undefined, token: CancellationToken,
		...
		try {
			const dataTransferDto = await typeConvert.DataTransfer.fromList(
        dataTransfer,
      );
			...
			for (const newTreeItemHandle of Object.keys(itemsToRefreshByHandle)) {
				...
				if (currentTreeItem) { // Refresh only if the item exists
					...
					if (newTreeItemHandle === newTreeItem.handle) {
						...
						const parent = newTreeItem.parentHandle ? this.itemsMap.get(
              newTreeItem.parentHandle,
            ) : null;
						if (parent) {
