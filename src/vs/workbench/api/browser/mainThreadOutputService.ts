...
export class MainThreadOutputService extends Disposable implements MainThreadOutputServiceShape {
	...
	constructor(
        ...
        OUTPUT_VIEW_ID,
      ) ? this._outputService.getActiveChannel() : undefined;
			this._proxy.$setVisibleChannel(visibleChannel ? visibleChannel.id : null);
			this._outputStatusItem.value = undefined;
		};
		this._register(
      Event.any<unknown>(this._outputService.onActiveOutputChannel, Event.filter(this._viewsService.onDidChangeViewVisibility, ({ id }) => id === OUTPUT_VIEW_ID))(
        () => setVisibleChannel(),
      ),
    );
		setVisibleChannel();
	...
	public async $register(label: string, file: UriComponents, languageId: string | undefined, extensionId: string): Promise<string> {
		const idCounter = (MainThreadOutputService._extensionIdPool.get(
      extensionId,
    ) || 0) + 1;
      ...
      { id, label, source: { resource }, log: false, languageId, extensionId },
    );
	...
	public async $reveal(channelId: string, preserveFocus: boolean): Promise<void> {
		...
		const viewsToShowQuietly = this._configurationService.getValue<Record<string, boolean> | undefined>(
      "workbench.view.showQuietly",
    ) ?? {};
		if (!this._viewsService.isViewVisible(
      OUTPUT_VIEW_ID,
