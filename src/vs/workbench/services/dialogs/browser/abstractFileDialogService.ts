...
export abstract class AbstractFileDialogService implements IFileDialogService {
	...
	async preferredHome(schemeFilter = this.getSchemeFilterForWindow()): Promise<URI> {
      ...
      `[FileDialogService] Preferred home: preferLocal=${preferLocal}, userLocalValue=${preferredHomeConfig.userLocalValue}, userRemoteValue=${preferredHomeConfig.userRemoteValue}`,
		...
		if (preferredHomeCandidate) {
			const isPreferredHomeCandidateAbsolute = preferLocal ? localPathIsAbsolute(
        preferredHomeCandidate,
      ) : (await this.pathService.path).isAbsolute(preferredHomeCandidate);
			if (isPreferredHomeCandidateAbsolute) {
				const preferredHomeNormalized = preferLocal ? localPathNormalize(
          preferredHomeCandidate,
	...
	async defaultWorkspacePath(schemeFilter = this.getSchemeFilterForWindow()): Promise<URI> {
		...
		if (this.contextService.getWorkbenchState() === WorkbenchState.WORKSPACE) {
			...
			if (configuration?.scheme === schemeFilter && isSavedWorkspace(
        configuration,
	...
	protected addFileToRecentlyOpened(uri: URI): void {
      ...
      {
        fileUri: uri,
        label: this.labelService.getUriLabel(uri, { appendWorkspaceSuffix: true }),
      },
    ]);
	}
	...
	protected getFileSystemSchema(options: { availableFileSystems?: readonly string[]; defaultUri?: URI }): string {
		return options.availableFileSystems?.[0] || this.getSchemeFilterForWindow(
	...
	protected getPickFileToSaveDialogOptions(defaultUri: URI, availableFileSystems?: string[]): ISaveDialogOptions {
      ...
      availableFileSystems,
		...
		const ext: string | undefined = defaultUri ? resources.extname(
      defaultUri,
    ) : undefined;
