...
export class SimpleFileDialog extends Disposable implements ISimpleFileDialog {
	...
	constructor(
		...
		disposableStore.add(
      this.storageService.onDidChangeValue(StorageScope.WORKSPACE, "remoteFileDialog.showDotFiles", disposableStore)(
        async _ => {
          this.getShowDotFiles();
          this.setButtons();
	...
	public async showOpenDialog(options: IOpenDialogOptions = {}): Promise<URI[] | undefined> {
      ...
      options.defaultUri,
		...
		this.scopedPathPrefix = options.defaultUri && this.scopedAuthority ? this.computeScopedPathPrefix(
      options.defaultUri,
    ) : "";
	...
	private getOptions(options: ISaveDialogOptions | IOpenDialogOptions, isSave: boolean = false): IOpenDialogOptions | undefined {
		...
		if ((this.scheme !== Schemas.file) && !this.fileService.hasProvider(
      defaultUri,
    )) {
	...
	private remoteUriFrom(path: string, hintUri?: URI): URI {
		...
		const uri: URI = this.scheme === Schemas.file ? URI.file(path) : URI.from({
      scheme: this.scheme,
      path,
      query: hintUri?.query,
      fragment: hintUri?.fragment,
	...
	private async pickResource(isSave: boolean = false): Promise<URI[] | URI | undefined> {
		...
		this.separator = this.scopedAuthority ? "/" : this.labelService.getSeparator(
      this.scheme,
      ...
      currentPath.substr(0, 2),
		...
		if (relativePath && isSameRoot) {
          ...
          this.currentFolder,
			...
			return resources.hasTrailingPathSeparator(
        directUri,
      ) ? resources.addTrailingPathSeparator(path) : path;
		} else {
			return directUri;
		}
	}
		...
		if (!this.updatingPromise && this.filePickBox.activeItems.length === 1) {
            ...
            this.filePickBox.value,
              ...
              this.filePickBox.value.length,
					...
					} else if ((item.label === "..") && startsWithIgnoreCase(
            this.filePickBox.value,
            newPath,
          )) {
						this.filePickBox.valueSelection = [
	...
	private async tryUpdateItems(value: string, valueUri: URI, reset: boolean = false): Promise<UpdateResult> {
		if ((value.length > 0) && ((value[0] === "~") || this.canTildaEscapeHatch(
      value,
    ))) {
        ...
        newDir,
        true,
      ) ? UpdateResult.UpdatedWithTrailing : UpdateResult.Updated;
		} else if (value === "\\") {
        ...
        resources.dirname(valueUri),
			...
			if ((!newFolderIsOldFolder && (this.endsWithSlash(
        value,
      ) || newFolderIsParent || newFolderIsUnrelated)) || reset) {
				let stat: IFileStatWithPartialMetadata | undefined;
				try {
				...
				if (stat?.isDirectory && (resources.basename(
          valueUri,
            ...
            inputUriDirnameWithoutSep,
                ...
                this.filePickBox.value,
						...
						try {
							statWithoutTrailing = await this.fileService.stat(
                inputUriDirname,
              );
						} catch (e) {
							// do nothing
	...
	private setActiveItems(value: string) {
		...
		const pathsEqual = equalsIgnoreCase(
      userPath,
      value.substring(0, userPath.length),
    ) ||
	...
	private addPostfix(uri: URI): URI {
		let result = uri;
		if (this.requiresTrailing && this.options.filters && this.options.filters.length > 0 && !resources.hasTrailingPathSeparator(
      uri,
    )) {
	...
	private async createItem(stat: IFileStat, parent: URI, token: CancellationToken): Promise<FileQuickPickItem | undefined> {
		...
		} else if (!stat.isDirectory && this.allowFileSelection && this.filterFile(
      fullPath,
    )) {
			return {
        label: stat.name,
