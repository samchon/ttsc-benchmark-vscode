...
class AccountMcpServerQuery extends BaseQuery implements IAccountMcpServerQuery {
	...
	getUsage(): {
		...
		const allUsages = this.queryService.authenticationMcpUsageService.readAccountUsages(
      this.providerId,
      this.accountName,
    );
		return allUsages
	...
	isPreferred(): boolean {
		const preferredAccount = this.queryService.authenticationMcpService.getAccountPreference(
      this.mcpServerId,
      this.providerId,
    );
		return preferredAccount === this.accountName;
	...
	isTrusted(): boolean {
		const allowedMcpServers = this.queryService.authenticationMcpAccessService.readAllowedMcpServers(
      this.providerId,
      this.accountName,
    );
...
class AccountExtensionsQuery extends BaseQuery implements IAccountExtensionsQuery {
	...
	getAllowedExtensions(): { id: string; name: string; allowed?: boolean; lastUsed?: number; trusted?: boolean }[] {
      ...
      this.accountName,
		...
		const usages = this.queryService.authenticationUsageService.readAccountUsages(
      this.providerId,
      this.accountName,
    );
...
class AccountMcpServersQuery extends BaseQuery implements IAccountMcpServersQuery {
	...
	forEach(callback: (mcpServerQuery: IAccountMcpServerQuery) => void): void {
		const usages = this.queryService.authenticationMcpUsageService.readAccountUsages(
      this.providerId,
      this.accountName,
    );
		const allowedMcpServers = this.queryService.authenticationMcpAccessService.readAllowedMcpServers(
...
class AccountEntitiesQuery extends BaseQuery implements IAccountEntitiesQuery {
	...
	hasAnyUsage(): boolean {
      ...
      this.accountName,
		...
		const mcpUsages = this.queryService.authenticationMcpUsageService.readAccountUsages(
      this.providerId,
      this.accountName,
    );
	...
	getEntityCount(): { extensions: number; mcpServers: number; total: number } {
      ...
      this.accountName,
		...
		const allowedExtensions = this.queryService.authenticationAccessService.readAllowedExtensions(this.providerId, this.accountName).filter(
      ext => ext.allowed,
    );
		const extensionIds = new Set<string>();
      ...
      this.accountName,
		...
		const allowedMcpServers = this.queryService.authenticationMcpAccessService.readAllowedMcpServers(this.providerId, this.accountName).filter(
...
class ProviderMcpServerQuery extends BaseQuery implements IProviderMcpServerQuery {
	...
	async getLastUsedAccount(): Promise<string | undefined> {
		try {
			const accounts = await this.queryService.authenticationService.getAccounts(
        this.providerId,
      );
...
class ExtensionQuery implements IExtensionQuery {
	...
	async getProvidersWithAccess(includeInternal?: boolean): Promise<string[]> {
		...
		for (const providerId of providerIds) {
			...
			if (!includeInternal && providerId.startsWith(
        INTERNAL_AUTH_PROVIDER_PREFIX,
      )) {
				continue;
			}
...
export class AuthenticationQueryService extends Disposable implements IAuthenticationQueryService {
	...
	async clearAllData(confirmation: "CLEAR_ALL_AUTH_DATA", includeInternal: boolean = true): Promise<void> {
		...
		for (const providerId of providerIds) {
			try {
				const accounts = await this.authenticationService.getAccounts(
          providerId,
        );
