...
export class AuthenticationMcpService extends Disposable implements IAuthenticationMcpService {
	...
	private _onDidAccountPreferenceChange: Emitter<{ providerId: string; mcpServerIds: string[] }> = this._register(
	...
	constructor(
		...
		this._inheritAuthAccountPreferenceChildToParent = Object.entries(this._inheritAuthAccountPreferenceParentToChildren).reduce<{ [mcpServerId: string]: string }>(
      (acc, [parent, children]) => {
	...
	private removeAccessRequest(providerId: string, mcpServerId: string): void {
		const providerRequests = this._sessionAccessRequestItems.get(
      providerId,
    ) || {};
	...
	getAccountPreference(mcpServerId: string, providerId: string): string | undefined {
      ...
      this._inheritAuthAccountPreferenceChildToParent[mcpServerId] ?? mcpServerId,
      providerId,
    );
	...
	async selectSession(providerId: string, mcpServerId: string, mcpServerName: string, scopes: string[], availableSessions: AuthenticationSession[]): Promise<AuthenticationSession> {
		const allAccounts = await this._authenticationService.getAccounts(
      providerId,
    );
      ...
      this.quickInputService.createQuickPick<{ label: string; session?: AuthenticationSession; account?: AuthenticationSessionAccount }>(),
    );
	...
	async requestNewSession(providerId: string, scopes: string[], mcpServerId: string, mcpServerName: string): Promise<void> {
      ...
      providerId,
		...
		const mcpServerHasExistingRequest = providerRequests
			...
			&& providerRequests[scopesList].requestingMcpServerIds.includes(
        mcpServerId,
      );
