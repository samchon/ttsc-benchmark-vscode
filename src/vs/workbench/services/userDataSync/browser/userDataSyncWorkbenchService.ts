...
export function isMergeEditorInput(editor: unknown): editor is MergeEditorInput {
	...
	return URI.isUri(candidate?.base) && URI.isUri(
    candidate?.input1?.uri,
  ) && URI.isUri(candidate?.input2?.uri) && URI.isUri(candidate?.result);
}
...
export class UserDataSyncWorkbenchService extends Disposable implements IUserDataSyncWorkbenchService {
	...
	constructor(
		...
		super();
		this.syncEnablementContext = CONTEXT_SYNC_ENABLEMENT.bindTo(
      contextKeyService,
    );
		this.syncStatusContext = CONTEXT_SYNC_STATE.bindTo(contextKeyService);
		this.accountStatusContext = CONTEXT_ACCOUNT_STATE.bindTo(contextKeyService);
		this.activityViewsEnablementContext = CONTEXT_ENABLE_ACTIVITY_VIEWS.bindTo(
      contextKeyService,
    );
		this.hasConflicts = CONTEXT_HAS_CONFLICTS.bindTo(contextKeyService);
		this.enableConflictsViewContext = CONTEXT_ENABLE_SYNC_CONFLICTS_VIEW.bindTo(
      contextKeyService,
    );
	...
	private async initialize(): Promise<void> {
      ...
      Event.filter(this.userDataSyncAccountService.onTokenFailed, isSuccessive => !isSuccessive)(
		...
		this._register(
      Event.filter(this.authenticationService.onDidChangeSessions, e => this.isSupportedAuthenticationProviderId(e.providerId))(
        ({ event }) => this.onDidChangeSessions(event),
      ),
    );
		this._register(
      this.storageService.onDidChangeValue(StorageScope.APPLICATION, UserDataSyncWorkbenchService.CACHED_SESSION_STORAGE_KEY, this._store)(
        () => this.onDidChangeStorage(),
      ),
    );
		this._register(
      Event.filter(this.userDataSyncAccountService.onTokenFailed, bailout => bailout)(
	...
	async synchroniseUserDataSyncStoreType(): Promise<void> {
      ...
      this.userDataSyncAccountService.account.authenticationProviderId,
		...
		await this.instantiationService.createInstance(UserDataSyncStoreTypeSynchronizer, userDataSyncStoreClient).sync(
	...
	private async handleConflictsWhileTurningOn(token: CancellationToken): Promise<void> {
		...
		const singleConflictResource = conflicts.length === 1 ? getSyncAreaLabel(
      conflicts[0].syncResource,
    ) : undefined;
	...
	async getAllLogResources(): Promise<URI[]> {
      ...
      this.uriIdentityService.extUri.dirname(this.environmentService.logsHome),
        ...
        stat => this.uriIdentityService.extUri.basename(stat.resource).startsWith(
          `${USER_DATA_SYNC_LOG_ID}.`,
        ),
      );
			if (childStat) {
	...
	private async waitForActiveSyncViews(): Promise<void> {
      ...
      SYNC_VIEW_CONTAINER_ID,
		...
		if (viewContainer) {
			const model = this.viewDescriptorService.getViewContainerModel(
        viewContainer,
      );
	...
	private async doPick(): Promise<UserDataSyncAccount | IAuthenticationProvider | undefined> {
      ...
      this.quickInputService.createQuickPick<AccountQuickPickItem>({
		...
		const promise = new Promise<UserDataSyncAccount | IAuthenticationProvider | undefined>(
      c => {
        disposables.add(
          quickPick.onDidHide(() => {
            disposables.dispose();
	...
	private onDidChangeSessions(e: AuthenticationSessionsChangeEvent): void {
		if (this.currentSessionId && e.removed?.find(
