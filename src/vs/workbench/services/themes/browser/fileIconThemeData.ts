	...
	private processIconThemeDocument(id: string, iconThemeDocumentLocation: URI, iconThemeDocument: IconThemeDocument): { content: string; hasFileIcons: boolean; hasFolderIcons: boolean; hidesExplorerArrows: boolean } {
      ...
      iconThemeDocumentLocation,
		...
		if (Array.isArray(fonts)) {
			const defaultFontSize = this.tryNormalizeFontSize(
        fonts[0].size,
      ) || "150%";
			fonts.forEach(font => {
		...
		for (const defId in selectorByDefinitionId) {
            ...
            css.inline`${selectors.join(", ")} { content: ${emQuad}; background-image: ${css.asCSSUrl(resolvePath(definition.iconPath))}; }`,
					...
					if (definition.fontColor && definition.fontColor.match(
            fontColorRegex,
          )) {
						body.push(
              ...
              css.inline`content: ${css.stringValue(definition.fontCharacter)};`,
					...
					const fontSize = definition.fontSize ?? (definition.fontId ? fontSizes.get(
            definition.fontId,
          ) : undefined);
              ...
              css.inline`font-family: ${css.stringValue(definition.fontId)};`,
					...
					if (showLanguageModeIcons) {
						body.push(
              css.inline`background-image: unset;`,
            ); // potentially set by the language default
					}
					cssRules.push(
