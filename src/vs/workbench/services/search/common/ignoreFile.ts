...
export class IgnoreFile {
	...
	private parseIgnoreFile(ignoreContents: string, dirPath: string, parent: IgnoreFile | undefined): (path: string, isDir: boolean) => boolean {
      ...
      true,
		...
		const fileIncludeLines = fileLines.filter(line => line.includes("!")).map(
      line => line.replace(/!/g, ""),
    );
      ...
      true,
		...
		// Same hack.
		const dirIncludeLines = contentLines.filter(line => line.includes("!")).map(
      line => line.replace(/!/g, ""),
    );
      ...
      false,
		...
		const isPathIgnored = (path: string, isDir: boolean) => {
			if (!(this.ignoreCase ? startsWithIgnoreCase(
        path,
        dirPath,
      ) : path.startsWith(dirPath))) { return false; }
