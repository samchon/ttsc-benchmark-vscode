...
export const IBrowserWorkbenchEnvironmentService = refineServiceDecorator<IEnvironmentService, IBrowserWorkbenchEnvironmentService>(
  IEnvironmentService,
);
...
export class BrowserWorkbenchEnvironmentService implements IBrowserWorkbenchEnvironmentService {
	...
	get logLevel(): string | undefined {
		...
		return this.options.developmentOptions?.logLevel !== undefined ? LogLevelToString(
      this.options.developmentOptions?.logLevel,
    ) : undefined;
	}
	...
	get extensionLogLevel(): [string, string][] | undefined {
		...
		return this.options.developmentOptions?.extensionLogLevel !== undefined ? this.options.developmentOptions?.extensionLogLevel.map(
	...
	@memoize
	get userRoamingDataHome(): URI { return URI.file("/User").with({
    scheme: Schemas.vscodeUserData,
  }); }

	@memoize
	get argvResource(): URI { return joinPath(
	...
	@memoize
	get workspaceStorageHome(): URI { return joinPath(
    this.userRoamingDataHome,
    "workspaceStorage",
  ); }

	@memoize
	get appSharedDataHome(): URI { return joinPath(
    this.userRoamingDataHome,
    "sharedData",
  ); }

	@memoize
	get localHistoryHome(): URI { return joinPath(
    this.userRoamingDataHome,
    "History",
  ); }

	@memoize
	get stateResource(): URI { return joinPath(
    this.userRoamingDataHome,
    "State",
    "storage.json",
  ); }
	 ...
	 */
	@memoize
	get userDataSyncHome(): URI { return joinPath(
    this.userRoamingDataHome,
    "sync",
    this.workspaceId,
  ); }
	...
	@memoize
	get keyboardLayoutResource(): URI { return joinPath(
    this.userRoamingDataHome,
    "keyboardLayout.json",
  ); }

	@memoize
	get untitledWorkspacesHome(): URI { return joinPath(
    this.userRoamingDataHome,
    "Workspaces",
  ); }

	@memoize
	get serviceMachineIdResource(): URI { return joinPath(
    this.userRoamingDataHome,
    "machineid",
  ); }
	...
	@memoize
	get disableExtensions() { return this.payload?.get(
    "disableExtensions",
  ) === "true"; }
	...
	@memoize
	get extensionTelemetryLogResource(): URI { return joinPath(
    this.logsHome,
	...
	@memoize
	get logExtensionHostCommunication(): boolean { return this.payload?.get(
    "logExtensionHostCommunication",
  ) === "true"; }

	@memoize
	get skipReleaseNotes(): boolean { return this.payload?.get(
    "skipReleaseNotes",
  ) === "true"; }

	@memoize
	get skipWelcome(): boolean { return this.payload?.get(
    "skipWelcome",
  ) === "true"; }
	...
	@memoize
	get isSessionsWindow(): boolean { return this.payload?.get(
    "isSessionsWindow",
  ) === "true"; }
	...
	constructor(
		...
		if (options.workspaceProvider && Array.isArray(
	...
	private resolveExtensionHostDebugEnvironment(): IExtensionHostDebugEnvironment {
			...
			for (const [key, value] of this.payload) {
              ...
              <ExtensionKind>value,
					...
					case "extensionTestsPath":
						extensionHostDebugEnvironment.extensionTestsLocationURI = URI.parse(
              value,
            );
						break;
					case "debugRenderer":
		...
		if (developmentOptions && !extensionHostDebugEnvironment.isExtensionDevelopment) {
			if (developmentOptions.extensions?.length) {
				extensionHostDebugEnvironment.extensionDevelopmentLocationURI = developmentOptions.extensions.map(
          e => URI.revive(e),
        );
				extensionHostDebugEnvironment.isExtensionDevelopment = true;
			}
