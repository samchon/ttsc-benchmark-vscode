...
export const ChatAccountPolicyGateActiveContext = new RawContextKey<boolean>(
  ...
  {
    type: "boolean",
    description: localize("chatAccountPolicyGateActive", "True when the 'Require Approved Account' policy is in effect and the user is not yet signed into an approved GitHub organization, so all AI features are disabled until they sign in."),
  },
);
 ...
 * gate decision.
 */
export const IAccountPolicyGateService = createDecorator<IAccountPolicyGateService>(
	...
	private computeGateInfo(): IAccountPolicyGateInfo {
      ...
      APPROVED_ACCOUNT_ORGANIZATIONS_POLICY_NAME,
		...
		if (!approvedOrgs.includes("*")) {
			const accountOrgs = (account.entitlementsData?.organization_login_list ?? []).map(
        o => o.toLowerCase(),
      );
