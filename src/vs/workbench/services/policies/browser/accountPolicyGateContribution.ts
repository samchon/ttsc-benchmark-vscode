...
export class AccountPolicyGateContribution extends Disposable implements IWorkbenchContribution {
	...
	constructor(
		...
		super();
		this.contextKey = ChatAccountPolicyGateActiveContext.bindTo(
      contextKeyService,
    );
	...
	private apply(info: IAccountPolicyGateInfo, forceTelemetry: boolean, showNotification: boolean): void {
      ...
      `[AccountPolicyGate] apply: state=${info.state}, reason=${info.reason}, isRestricted=${isRestricted}`,
		...
		if (stateChanged) {
			this.telemetryService.publicLog2<AccountPolicyGateStateEvent, AccountPolicyGateStateClassification>(
