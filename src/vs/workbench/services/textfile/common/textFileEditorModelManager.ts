...
export class TextFileEditorModelManager extends Disposable implements ITextFileEditorModelManager {
	...
	private joinPendingResolves(resource: URI): Promise<void> | undefined {
		const pendingModelResolve = this.mapResourceToPendingModelResolvers.get(
      resource,
    );
	...
	private async doJoinPendingResolves(resource: URI): Promise<void> {
		...
		while (this.mapResourceToPendingModelResolvers.has(resource)) {
			const nextPendingModelResolve = this.mapResourceToPendingModelResolvers.get(
        resource,
      );
	...
	private registerModel(model: TextFileEditorModel): void {
		...
		modelListeners.add(
      model.onDidResolve(reason => this._onDidResolve.fire({ model, reason })),
    );
		modelListeners.add(
	...
	canDispose(model: TextFileEditorModel): true | Promise<true> {
		...
		if (
			model.isDisposed() ||
			(!this.mapResourceToPendingModelResolvers.has(
        model.resource,
      ) && !model.isDirty())
		) {
			return true;
