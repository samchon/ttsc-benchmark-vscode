...
class ViewDescriptorsState extends Disposable {
	...
	constructor(
      ...
      viewContainerStorageId,
		...
		this._register(
      this.storageService.onDidChangeValue(StorageScope.PROFILE, this.globalViewsStateStorageId, this._store)(
	...
	private initialize(): Map<string, IViewDescriptorState> {
      ...
      "[]",
		...
		const { state: workspaceVisibilityStates } = this.parseStoredGlobalState(
      value,
    );
	...
	private parseStoredGlobalState(value: string): { state: Map<string, IStoredGlobalViewState>; hasDuplicates: boolean } {
		const storedValue: Array<string | IStoredGlobalViewState> = JSON.parse(
      value,
    );
...
export class ViewContainerModel extends Disposable implements IViewContainerModel {
	...
	get allViewDescriptors(): ReadonlyArray<IViewDescriptor> { return this.viewDescriptorItems.map(
    item => item.viewDescriptor,
  ); }
	...
	get activeViewDescriptors(): ReadonlyArray<IViewDescriptor> { return this.viewDescriptorItems.filter(item => item.state.active).map(
    item => item.viewDescriptor,
  ); }
	...
	get visibleViewDescriptors(): ReadonlyArray<IViewDescriptor> { return this.viewDescriptorItems.filter(item => this.isViewDescriptorVisible(item)).map(
    item => item.viewDescriptor,
  ); }
	...
	private updateViewDescriptorItemVisibility(viewDescriptorItem: IViewDescriptorItem, visible: boolean): boolean {
		...
		if (this.isViewDescriptorVisibleWhenActive(
      viewDescriptorItem,
    ) === visible) {
			return false;
		}
	...
	add(addedViewDescriptorStates: IAddedViewDescriptorState[]): void {
		...
		for (const addedViewDescriptorState of addedViewDescriptorStates) {
          ...
          ) ? !viewDescriptor.hideByDefault : state.visibleWorkspace) : addedViewDescriptorState.visible;
				} else {
            ...
            state.visibleGlobal,
          ) ? !viewDescriptor.hideByDefault : state.visibleGlobal) : addedViewDescriptorState.visible;
          ...
          state.collapsed,
        ) ? !!viewDescriptor.collapsed : state.collapsed) : addedViewDescriptorState.collapsed;
			} else {
				state = {
          active: false,
	...
	remove(viewDescriptors: IViewDescriptor[]): void {
      ...
      item => this.viewDescriptorItems.splice(
		...
		if (removedActiveDescriptors.length) {
			this._onDidChangeActiveViewDescriptors.fire(
        ({ added: [], removed: removedActiveDescriptors }),
      );
		}
		if (removed.length) {
	...
	private compareViewDescriptors(a: IViewDescriptorItem, b: IViewDescriptorItem): number {
		...
		return (this.getViewOrder(a) - this.getViewOrder(
      b,
