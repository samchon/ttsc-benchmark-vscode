...
export class WorkspaceTrustManagementService extends Disposable implements IWorkspaceTrustManagementService {
	...
	private async getCanonicalUri(uri: URI): Promise<URI> {
		...
		if (this.environmentService.remoteAuthority && uri.scheme === Schemas.vscodeRemote) {
			canonicalUri = await this.remoteAuthorityResolverService.getCanonicalURI(
        uri,
      );
	...
	private async resolveCanonicalUris(): Promise<void> {
		...
		if (filesToOpen.length) {
			const filesToOpenOrCreateUris = filesToOpen.filter(f => !!f.fileUri).map(
        f => f.fileUri!,
      );
        ...
        ...canonicalFilesToOpen.filter(uri => this._canonicalStartupFiles.every(u => !this.uriIdentityService.extUri.isEqual(uri, u))),
      );
		}

		// Workspace
		const workspaceUris = this.workspaceService.getWorkspace().folders.map(
      f => f.uri,
    );
	...
	private getWorkspaceUris(): URI[] {
		...
		if (workspaceConfiguration && isSavedWorkspace(
	...
	async setParentFolderTrust(trusted: boolean): Promise<void> {
      ...
      ) as ISingleFolderWorkspaceIdentifier).uri;
...
export class WorkspaceTrustRequestService extends Disposable implements IWorkspaceTrustRequestService {
	...
	async requestOpenFilesTrust(uris: URI[]): Promise<WorkspaceTrustUriResponse> {
      ...
      uris.map(uri => this.workspaceTrustManagementService.getUriTrustInfo(uri)),
		...
		if (openFilesTrustInfo.map(info => info.trusted).every(
      trusted => trusted,
    )) {
			return WorkspaceTrustUriResponse.Open;
		}
		...
		if (!this._openFilesTrustRequestPromise) {
			this._openFilesTrustRequestPromise = new Promise<WorkspaceTrustUriResponse>(
        resolve => {
	...
	async requestResourcesTrust(options: ResourceTrustRequestOptions): Promise<boolean | undefined> {
		...
		const resourcesTrustInfo = await this.workspaceTrustManagementService.getUriTrustInfo(
      options.uri,
    );
