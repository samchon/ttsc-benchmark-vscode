...
export class PreferencesService extends Disposable implements IPreferencesService {
	...
	get workspaceSettingsResource(): URI | null {
		...
		return workspace.configuration || workspace.folders[0].toResource(
      FOLDER_SETTINGS_PATH,
    );
	}
	...
	hasDefaultSettingsContent(uri: URI): boolean {
		return this.isDefaultSettingsResource(uri) || isEqual(
      uri,
      this.defaultSettingsRawResource,
          ...
          this.editorService.openEditor(
            {
              ...
              options: { pinned: true, preserveFocus: true, revealIfOpened: true, override: DEFAULT_EDITOR_ASSOCIATION.id },
	...
	private isDefaultSettingsResource(uri: URI): boolean {
		return this.isDefaultUserSettingsResource(
      uri,
    ) || this.isDefaultWorkspaceSettingsResource(
      uri,
    ) || this.isDefaultFolderSettingsResource(uri);
	}

	private isDefaultUserSettingsResource(uri: URI): boolean {
		return uri.authority === "defaultsettings" && uri.scheme === network.Schemas.vscode && !!uri.path.match(
      /\/(\d+\/)?settings\.json$/,
    );
	}
	...
	private async getPositionToReveal(settingKey: string, edit: boolean, settingsModel: IPreferencesEditorModel<ISetting>, codeEditor: ICodeEditor): Promise<IPosition | null> {
		...
		if (!setting && edit) {
			let defaultValue = (type === "object" || type === "array") ? this.configurationService.inspect(
        settingKey,
	...
	getSetting(settingId: string): ISetting | undefined {
		if (!this._settingsGroups) {
			...
			const defaultsChangedDisposable: MutableDisposable<IDisposable> = this._register(
