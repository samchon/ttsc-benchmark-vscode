...
export class WorkingCopyHistoryTracker extends Disposable implements IWorkbenchContribution {
	...
	private readonly pendingAddHistoryEntryOperations = new ResourceMap<CancellationTokenSource>(
	...
	private onDidSave(e: IWorkingCopySaveEvent): void {
		...
		if (this.historyEntryContentVersion.get(
      e.workingCopy.resource,
		...
		this.pendingAddHistoryEntryOperations.get(e.workingCopy.resource)?.dispose(
      true,
    );
	...
	private shouldTrackHistory(resource: URI, stat: IFileStatWithMetadata): boolean {
		...
		const configuredMaxFileSizeInBytes = 1024 * this.configurationService.getValue<number>(
