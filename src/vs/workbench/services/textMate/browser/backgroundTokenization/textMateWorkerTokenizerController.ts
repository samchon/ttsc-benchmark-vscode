...
export class TextMateWorkerTokenizerController extends Disposable {
	...
	public async setTokensAndStates(controllerId: number, versionId: number, rawTokens: Uint8Array, fontTokens: ISerializedAnnotation<IFontTokenOption>[], stateDeltas: StateDeltas[]): Promise<void> {
      ...
      deserializeFontTokenOptions(),
		...
		if (this._shouldLog) {
			const changes = this._pendingChanges.filter(c => c.versionId <= versionId).map(c => c.changes).map(c => changesToString(c)).join(
        " then ",
      );
		...
		if (this._pendingChanges.length > 0) {
			if (this._shouldLog) {
				const changes = this._pendingChanges.map(c => c.changes).map(c => changesToString(c)).join(
          " then ",
        );
        ...
        this._pendingChanges.map(
          (c) => linesLengthEditFromModelContentChange(c.changes),
        ),
      );
      ...
      this._pendingChanges.map(
		...
		for (const d of stateDeltas) {
			let prevState = d.startLineNumber <= 1 ? this._initialState : this._states.getEndState(
        d.startLineNumber - 1,
      );
