...
export class BrowserAuxiliaryWindowService extends Disposable implements IAuxiliaryWindowService {
	...
	private static WINDOW_IDS = getWindowId(
    mainWindow,
  ) + 1; // start from the main window ID + 1
	...
	async open(options?: IAuxiliaryWindowOpenOptions): Promise<IAuxiliaryWindow> {
      ...
      disposables: eventDisposables,
		...
		this.telemetryService.publicLog2<AuxiliaryWindowOpenEvent, AuxiliaryWindowClassification>(
      "auxiliaryWindowOpen",
	...
	private applyMeta(auxiliaryWindow: CodeWindow): void {
      ...
      'meta[name="theme-color"]',
		...
		// eslint-disable-next-line no-restricted-syntax
		const originalIconLinkTag = mainWindow.document.querySelector(
      'link[rel="icon"]',
    );
	...
	private applyCSS(auxiliaryWindow: CodeWindow, disposables: DisposableStore) {
		...
		stylesLoaded.wait().then(
      () => mark("code/auxiliaryWindow/didLoadCSSStyles"),
    );
