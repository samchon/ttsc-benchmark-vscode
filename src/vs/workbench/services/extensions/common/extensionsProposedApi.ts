...
export class ExtensionsProposedApi {
	...
	constructor(
      ...
      (_environmentService.extensionEnabledProposedApi ?? []).map(
		...
		this._envEnablesProposedApiForAll =
			...
			(this._envEnabledExtensions.size === 0 && Array.isArray(
        _environmentService.extensionEnabledProposedApi,
      )); // always allow proposed API if --enable-proposed-api is provided without extension ID
	...
	private doUpdateEnabledApiProposals(extension: Mutable<IExtensionDescription>): void {
		...
		if (this._envEnablesProposedApiForAll || this._envEnabledExtensions.has(
      key,
    )) {
		...
		if (!extension.isBuiltin && isNonEmptyArray(
