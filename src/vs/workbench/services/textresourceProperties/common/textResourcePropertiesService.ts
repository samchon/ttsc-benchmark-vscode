...
export class TextResourcePropertiesService implements ITextResourcePropertiesService {
	...
	constructor(
		...
		remoteAgentService.getEnvironment().then(
      remoteEnv => this.remoteEnvironment = remoteEnv,
    );
	}
	...
	private getOS(resource?: URI): OperatingSystem {
		...
		if (remoteAuthority) {
			if (resource && resource.scheme !== Schemas.file) {
				...
				os = this.remoteEnvironment ? this.remoteEnvironment.os : /* Get it from cache */ this.storageService.getNumber(
          osCacheKey,
