...
export class KeybindingsResource implements IProfileResource {
	...
	async getContent(profile: IUserDataProfile): Promise<string> {
		const keybindingsContent = await this.getKeybindingsResourceContent(
      profile,
    );
...
export class KeybindingsResourceTreeItem implements IProfileResourceTreeItem {
	...
	async hasContent(): Promise<boolean> {
		const keybindingsContent = await this.instantiationService.createInstance(KeybindingsResource).getKeybindingsResourceContent(
      this.profile,
    );
	...
	async getContent(): Promise<string> {
		return this.instantiationService.createInstance(KeybindingsResource).getContent(
      this.profile,
    );
	}
