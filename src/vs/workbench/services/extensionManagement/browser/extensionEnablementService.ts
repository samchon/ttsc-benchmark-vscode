...
export class ExtensionEnablementService extends Disposable implements IWorkbenchExtensionEnablementService {
	...
	constructor(
      ...
      Event.filter(extensionManagementService.onDidUninstallExtension, e => !e.error)(
      ...
      (productService.sessionsWindowAllowedExtensions ?? []).map(
        id => id.toLowerCase(),
      ),
    );
		const unificationExtensions = [this._completionsExtensionId, this._chatExtensionId].filter(
      id => !!id,
    );
		...
		if (isWeb && this.environmentService.remoteAuthority === undefined) {
			...
			this._extensionUnificationEnabled = this.configurationService.getValue<boolean>(
	...
	private ensureChatExtensionInitialDisabledState(): void {
		if (!this._chatExtensionId || this.environmentService.isSessionsWindow || this.environmentService.skipBuiltinExtensions?.some(
      id => id.toLowerCase() === this._chatExtensionId,
		...
		const builtinChatExtensionEnablementMigration = this.storageService.getBoolean(
	...
	getDependenciesEnablementStates(extension: IExtension): [IExtension, EnablementState][] {
		return getExtensionDependencies(this.extensionsManager.extensions, extension).map(
      e => [e, this.getEnablementState(e)],
    );
	}
	...
	async setEnablement(extensions: IExtension[], newState: EnablementState): Promise<boolean[]> {
		...
		if (newState === EnablementState.EnabledGlobally || newState === EnablementState.EnabledWorkspace) {
			extensions.push(
        ...this.getExtensionsToEnableRecursively(extensions, this.extensionsManager.extensions, newState, { dependencies: true, pack: true }),
      );
		}
	...
	private _computeEnablementState(extension: IExtension, extensions: ReadonlyArray<IExtension>, workspaceType: WorkspaceType, computedEnablementStates?: Map<IExtension, EnablementState>): EnablementState {
      ...
      this.getMaliciousExtensions().map(e => ({ extensionOrPublisher: e })),
		...
		else if (isEnabled && extension.type === ExtensionType.User && this.allowedExtensionsService.isAllowed(
      extension,
    ) !== true) {
		...
		else if (isEnabled && this._isDisabledByWorkspaceTrust(
      extension,
      workspaceType,
    )) {
        ...
        extension.manifest,
				...
				if (extensionKind === "web") {
					if (this.extensionManagementServerService.webExtensionManagementServer /* web */) {
						...
						const enableLocalWebWorker = this.configurationService.getValue<WebWorkerExtHostConfigValue>(
	...
	private _isDisabledByExtensionDependency(extension: IExtension, extensions: ReadonlyArray<IExtension>, workspaceType: WorkspaceType, computedEnablementStates: Map<IExtension, EnablementState>): boolean {
		...
		try {
          ...
          computedEnablementStates,
				...
				if (!this.isEnabledEnablementState(
          enablementState,
        ) && enablementState !== EnablementState.DisabledByExtensionKind) {
					return true;
				}
	...
	private _isDisabledGlobally(identifier: IExtensionIdentifier): boolean {
		return this.globalExtensionEnablementService.getDisabledExtensions().some(
      e => areSameExtensions(e, identifier),
    );
	}
	...
	private _isDisabledBySessionsWindow(extension: IExtension): boolean {
		...
		if (this._sessionsWindowAllowedExtensions.has(
	...
	public async updateExtensionsEnablementsWhenWorkspaceTrustChanges(): Promise<void> {
      ...
      trusted: false,
		...
		const enablementChangedExtensionsBecauseOfTrust = enablementStatesWithTrustedWorkspace.filter(([, enablementState], index) => enablementState !== enablementStatesWithUntrustedWorkspace[index][1]).map(
      ([extension]) => extension,
    );
...
class ExtensionsManager extends Disposable {
	...
	private updateExtensions(added: IExtension[], identifiers: IExtensionIdentifier[], server: IExtensionManagementServer | undefined, isProfileSwitch: boolean): void {
		if (added.length) {
          ...
          e => areSameExtensions(e.identifier, extension.identifier) && this.extensionManagementServerService.getExtensionManagementServer(e) === extensionServer,
        );
				if (index !== -1) {
		...
		for (const identifier of identifiers) {
			const index = this._extensions.findIndex(
        e => areSameExtensions(e.identifier, identifier) && this.extensionManagementServerService.getExtensionManagementServer(e) === server,
      );
			if (index !== -1) {
