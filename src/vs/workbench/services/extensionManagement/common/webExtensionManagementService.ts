...
export class WebExtensionManagementService extends AbstractExtensionManagementService implements IProfileAwareExtensionManagementService {
	...
	override get onInstallExtension() { return Event.filter(
	...
	override get onUninstallExtension() { return Event.filter(
	...
	override get onDidUninstallExtension() { return Event.filter(
	...
	zip(extension: ILocalExtension): Promise<URI> { throw new Error(
    "unsupported",
  ); }
	getManifest(vsix: URI): Promise<IExtensionManifest> { throw new Error(
    "unsupported",
  ); }
...
class InstallExtensionTask extends AbstractExtensionTask<ILocalExtension> implements IInstallExtensionTask {
  ...
  ) ? this._operation : this.options.operation; }
	...
	protected async doRun(token: CancellationToken): Promise<ILocalExtension> {
      ...
      e => areSameExtensions(e.identifier, this.identifier),
		...
		const scannedExtension = URI.isUri(
      this.extension,
