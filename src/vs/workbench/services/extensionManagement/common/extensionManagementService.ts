...
export class ExtensionManagementService extends CommontExtensionManagementService implements IWorkbenchExtensionManagementService {
	...
	async uninstallExtensions(extensions: UninstallExtensionInfo[]): Promise<void> {
		...
		for (const { extension, options } of extensions) {
			...
			if (this.servers.length > 1 && isLanguagePackExtension(
        extension.manifest,
      )) {
		...
		const errors = result.filter(r => r.status === "rejected").map(
      r => r.reason,
    );
		if (errors.length) {
	...
	async updateFromGallery(gallery: IGalleryExtension, extension: ILocalExtension, installOptions?: InstallOptions): Promise<ILocalExtension> {
      ...
      isApplicationScoped: extension.isApplicationScoped,
		...
		return Promises.settled(servers.map(server => server.extensionManagementService.installFromGallery(gallery, installOptions))).then(
      ([local]) => local,
    );
	}
	...
	async installFromGallery(gallery: IGalleryExtension, installOptions?: InstallOptions, servers?: IExtensionManagementServer[]): Promise<ILocalExtension> {
      ...
      CancellationToken.None,
		...
		servers = servers?.length ? this.validServers(
      gallery,
      manifest,
      servers,
	...
	private validServers(gallery: IGalleryExtension, manifest: IExtensionManifest, servers: IExtensionManagementServer[]): IExtensionManagementServer[] {
		const installableServers = this.getInstallableExtensionManagementServers(
      manifest,
    );
		for (const server of servers) {
		...
		if (verifiedPublishers.length || unverfiiedPublishers.length === 1) {
          ...
          `$(${verifiedPublisherIcon.id})&nbsp;${publisherVerifiedMessage}`,
            ...
            `$(${Codicon.unverified.id})&nbsp;${localize("unverifiedPublisherWithName", "{0} is [**not** verified]({1}).", getPublisherLink(unverfiiedPublishers[0]), unverifiedLink)}`,
					...
					customMessage.appendMarkdown(
            `$(${Codicon.unverified.id})&nbsp;${localize("unverifiedPublishers", "{0} and {1} are [**not** verified]({2}).", unverfiiedPublishers.slice(0, unverfiiedPublishers.length - 1).map(p => getPublisherLink(p)).join(", "), getPublisherLink(unverfiiedPublishers[unverfiiedPublishers.length - 1]), unverifiedLink)}`,
          );
				}
			}
		} else {
	...
	installExtensionsFromProfile(extensions: IExtensionIdentifier[], fromProfileLocation: URI, toProfileLocation: URI): Promise<ILocalExtension[]> { throw new Error(
    "Not Supported",
  ); }

	isPublisherTrusted(extension: IGalleryExtension): boolean {
		const publisher = extension.publisher.toLowerCase();
		if (this.defaultTrustedPublishers.includes(
      publisher,
      ...
      extension.publisherDisplayName.toLowerCase(),
		...
		if (this.allowedExtensionsService.allowedExtensionsConfigValue && this.allowedExtensionsService.isAllowed(
      extension,
    )) {
			return true;
		}
	...
	private getTrustedPublishersFromStorage(): IStringDictionary<IPublisherInfo> {
      ...
      {},
		...
		return Object.keys(trustedPublishers).reduce<IStringDictionary<IPublisherInfo>>(
      (result, publisher) => {
...
class WorkspaceExtensionsManagementService extends Disposable {
				...
				for (const location of parsed) {
					if (isString(location)) {
						if (this.workspaceService.getWorkbenchState() === WorkbenchState.FOLDER) {
							locations.push(
                this.workspaceService.getWorkspace().folders[0].toResource(
                  location,
                ),
              );
						} else {
	...
	async scanWorkspaceExtension(location: URI): Promise<ILocalExtension | null> {
      ...
      { includeInvalid: true },
		...
		return scannedExtension ? this.toLocalWorkspaceExtension(
      scannedExtension,
    ) : null;
	}
