...
namespace schema {
	...
	export function isValidCommand(command: IUserFriendlyCommand, collector: ExtensionMessageCollector): boolean {
		...
		if (command.shortTitle && !isValidLocalizedString(
      command.shortTitle,
      collector,
      "shortTitle",
    )) {
		...
		if (command.category && !isValidLocalizedString(
      command.category,
      collector,
      "category",
    )) {
	...
	function isValidLocalizedString(localized: string | ILocalizedString, collector: ExtensionMessageCollector, propertyName: string): boolean {
        ...
        localize(
          ...
          propertyName,
			...
			return false;
		} else if (typeof localized === "string" && isFalsyOrWhitespace(
      localized,
    )) {
			collector.error(
        localize(
...
class CommandsTableRenderer extends Disposable implements IExtensionFeatureTableRenderer {
	...
	render(manifest: IExtensionManifest): IRenderedData<ITableData> {
      ...
      menus: [] as string[],
		...
		const rawKeybindings = manifest.contributes?.keybindings ? (Array.isArray(
