...
export class ResourcesDropHandler {
	...
	async handleDrop(event: DragEvent, targetWindow: Window, resolveTargetGroup?: () => IEditorGroup | undefined, afterDrop?: (targetGroup: IEditorGroup | undefined) => void, options?: IEditorOptions): Promise<void> {
      ...
      Extensions.DragAndDropContribution,
        ...
        editors.filter(editor => editor.allowWorkspaceOpen && editor.resource?.scheme === Schemas.file).map(
      ...
      editors.filter(editor => editor.isExternal && editor.resource?.scheme === Schemas.file).map(
        editor => editor.resource,
      ),
    );
...
export function fillEditorsDragData(accessor: ServicesAccessor, resourcesOrEditors: Array<URI | IResourceStat | IEditorIdentifier>, event: DragMouseEvent | DragEvent, options?: { disableStandardTransfer: boolean }): void {
	...
	if (!options?.disableStandardTransfer) {
      ...
      DataTransfers.TEXT,
      fileSystemResources.map(({ resource }) => labelService.getUriLabel(resource, { noPrefix: true })).join(
        lineDelimiter,
      ),
    );
      ...
      ({ isDirectory }) => !isDirectory,
		...
		if (firstFile) {
			const firstFileUri = FileAccess.uriToFileUri(
        firstFile.resource,
      ); // enforce `file:` URIs
	...
	const draggedDirectories: URI[] = fileSystemResources.filter(({ isDirectory }) => isDirectory).map(
    ({ resource }) => resource,
  );
...
export class ResourceListDnDHandler<T> implements IListDragAndDrop<T> {
	...
	getDragLabel(elements: T[]): string | undefined {
		...
		return resources.length === 1 ? basename(
      resources[0],
    ) : resources.length > 1 ? String(resources.length) : undefined;
	}
...
class GlobalWindowDraggedOverTracker extends Disposable {
	...
	private registerListeners(): void {
		this._register(
      Event.runAndSubscribe(onDidRegisterWindow, ({ window, disposables }) => {
        disposables.add(
          addDisposableListener(
            window,
            EventType.DRAG_OVER,
