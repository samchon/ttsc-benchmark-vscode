...
export class ToggleSidebarPositionAction extends Action2 {
	...
	static getLabel(layoutService: IWorkbenchLayoutService): string {
		return layoutService.getSideBarPosition() === Position.LEFT ? localize(
      "moveSidebarRight",
...
class MoveFocusedViewAction extends Action2 {
	...
	run(accessor: ServicesAccessor, viewId?: string): void {
		...
		const focusedViewId = viewId || FocusedViewContext.getValue(
      contextKeyService,
    );
		...
		const viewDescriptor = viewDescriptorService.getViewDescriptorById(
      focusedViewId,
    );
		if (!viewDescriptor?.canMoveView) {
			dialogService.error(
      ...
      viewDescriptor.name.value,
		...
		const currentContainer = viewDescriptorService.getViewContainerByViewId(
      focusedViewId,
    )!;
		const currentLocation = viewDescriptorService.getViewLocationById(
      focusedViewId,
    )!;
		const isViewSolo = viewDescriptorService.getViewContainerModel(
      currentContainer,
