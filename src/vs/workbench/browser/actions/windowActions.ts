...
abstract class BaseOpenRecentAction extends Action2 {
	...
	override async run(accessor: ServicesAccessor): Promise<void> {
      ...
      [
		...
		for (const recent of recentlyOpened.workspaces) {
			const isDirty = isRecentFolder(recent) ? dirtyFolders.has(
        recent.folderUri,
      ) : dirtyWorkspaces.has(recent.workspace.configPath);
			const windowState = isRecentFolder(recent) ? openedInWindows.get(
        recent.folderUri,
      ...
      p => this.toQuickPick(modelService, languageService, labelService, p, {
        isDirty: false,
		...
		const autoFocusSecondEntry: boolean = firstEntry && contextService.isCurrentWorkspace(
	...
	private toQuickPick(modelService: IModelService, languageService: ILanguageService, labelService: ILabelService, recent: IRecent, kind: { isDirty: boolean; windowState?: { isActive: boolean } }): IRecentlyOpenedPick {
        ...
        FileKind.ROOT_FOLDER,
			...
			fullLabel = recent.label || labelService.getWorkspaceLabel(
        recent.workspace,
        { verbose: Verbosity.LONG },
      );
			isWorkspace = true;
