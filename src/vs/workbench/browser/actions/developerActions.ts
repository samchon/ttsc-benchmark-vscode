...
class ToggleScreencastModeAction extends Action2 {
	...
	run(accessor: ServicesAccessor): void {
		...
		function registerContainerListeners(container: HTMLElement, windowDisposables: DisposableStore): void {
			...
			listeners.add(
        listeners.add(new DomEmitter(container, "mousedown", true)).event(
          e => onMouseDown.fire(e),
        ),
      );
			listeners.add(
        listeners.add(new DomEmitter(container, "mouseup", true)).event(
          e => onMouseUp.fire(e),
        ),
      );
			listeners.add(
        listeners.add(new DomEmitter(container, "mousemove", true)).event(
          e => onMouseMove.fire(e),
        ),
      );
		...
		disposables.add(
      onDidRegisterWindow(
        ({ window, disposables }) => registerContainerListeners(
          layoutService.getContainer(window),
          disposables,
        ),
      ),
      ...
      new Emitter<CompositionEvent>(),
		...
		function registerWindowListeners(window: Window, windowDisposables: DisposableStore): void {
			...
			listeners.add(
        listeners.add(new DomEmitter(window, "keydown", true)).event(
          e => onKeyDown.fire(e),
        ),
      );
			listeners.add(
        listeners.add(new DomEmitter(window, "compositionstart", true)).event(
          e => onCompositionStart.fire(e),
        ),
      );
			listeners.add(
        listeners.add(new DomEmitter(window, "compositionupdate", true)).event(
          e => onCompositionUpdate.fire(e),
        ),
      );
			listeners.add(
        listeners.add(new DomEmitter(window, "compositionend", true)).event(
          e => onCompositionEnd.fire(e),
        ),
      );
...
class SnapshotTrackedDisposables extends Action2 {
	...
	run(accessor: ServicesAccessor): void {
      ...
      tracker?.computeLeakingDisposables(1000)?.leaks.map(
					...
					if (policyServiceConstructorName === "MultiplexPolicyService") {
						...
						if (multiplexService.policyServices) {
							...
							for (const service of componentServices) {
								if (service.getPolicyValue && service.getPolicyValue(
                  policyName,
                ) !== undefined) {
...
class SyncAccountPolicyAction extends Action2 {
	...
	async run(accessor: ServicesAccessor): Promise<void> {
        ...
        localize("syncAccountPolicy.success", "Account policy has been synced."),
      );
		} catch (error) {
