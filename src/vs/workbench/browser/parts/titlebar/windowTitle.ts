...
export class WindowTitle extends Disposable {
	...
	private static readonly NLS_USER_IS_ADMIN = isWindows ? localize(
    "userIsAdmin",
    "[Administrator]",
	...
	get workspaceName() { return this.labelService.getWorkspaceLabel(
	...
	getWindowTitle(): string {
      ...
      supportSideBySide: SideBySideEditor.PRIMARY,
		...
		let editorFolderResource = editorResource ? dirname(
      editorResource,
    ) : undefined;
      ...
      Verbosity.LONG,
		...
		const activeFolderShort = editorFolderResource ? basename(
      editorFolderResource,
    ) : "";
		const activeFolderMedium = editorFolderResource ? this.labelService.getUriLabel(
      editorFolderResource,
      { relative: true },
    ) : "";
		const activeFolderLong = editorFolderResource ? this.labelService.getUriLabel(
      editorFolderResource,
    ) : "";
      ...
      verbose: LabelVerbosity.SHORT,
		...
		const activeEditorState = editorResource ? this.decorationsService.getDecoration(
      editorResource,
      false,
    )?.tooltip : undefined;
		...
		for (const [contextKey, name] of this.variables) {
			variables[name] = this.contextKeyService.getContextKeyValue(
        contextKey,
      ) ?? "";
		}
