...
export class EditorAutoSave extends Disposable implements IWorkbenchContribution {
	...
	private readonly waitingOnConditionAutoSaveWorkingCopies = new ResourceMap<{ readonly workingCopy: IWorkingCopy; readonly reason: SaveReason; condition: AutoSaveDisabledReason }>(
    resource => this.uriIdentityService.extUri.getComparisonKey(resource),
  );
	private readonly waitingOnConditionAutoSaveEditors = new ResourceMap<{ readonly editor: IEditorIdentifier; readonly reason: SaveReason; condition: AutoSaveDisabledReason }>(
    resource => this.uriIdentityService.extUri.getComparisonKey(resource),
  );
	...
	private onConditionChanged(resources: readonly URI[], condition: AutoSaveDisabledReason.ERRORS | AutoSaveDisabledReason.DISABLED): void {
		for (const resource of resources) {
			...
			const workingCopyResult = this.waitingOnConditionAutoSaveWorkingCopies.get(
        resource,
      );
			...
			// Waiting editors
				...
				const editorResult = this.waitingOnConditionAutoSaveEditors.get(
          resource,
        );
				if (
