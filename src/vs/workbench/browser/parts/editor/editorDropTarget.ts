...
class DropOverlay extends Themable {
	...
	constructor(
      ...
      new RunOnceScheduler(() => this.dispose(), 300),
		...
		this.enableDropIntoEditor = isDropIntoEditorEnabledGlobally(
	...
	override updateStyles(): void {
		...
		overlay.style.backgroundColor = this.getColor(
      EDITOR_DRAG_AND_DROP_BACKGROUND,
    ) || "";
		...
		if (this.dropIntoPromptElement) {
			this.dropIntoPromptElement.style.backgroundColor = this.getColor(
        EDITOR_DROP_INTO_PROMPT_BACKGROUND,
      ) ?? "";
			this.dropIntoPromptElement.style.color = this.getColor(
        EDITOR_DROP_INTO_PROMPT_FOREGROUND,
      ) ?? "";
        ...
        DraggedTreeItemsIdentifier.prototype,
            ...
            id.identifier,
					...
					if (dataTransferItem) {
						...
						editors.push(
              ...treeDropData.map(editor => ({ ...editor, options: { ...editor.options, pinned: true } })),
            );
					}
				}
				if (editors.length) {
