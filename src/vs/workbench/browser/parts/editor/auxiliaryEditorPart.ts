...
export class AuxiliaryEditorPart {
	...
	async create(label: string, options?: IAuxiliaryEditorPartOpenOptions): Promise<ICreateAuxiliaryEditorPartResult> {
		...
		function updateCompact(newCompact: boolean): void {
			...
			statusbarVisible = !compact && that.configurationService.getValue<boolean>(
    ...
    ); // custom title in aux windows only enabled in native
		if (useCustomTitle) {
      ...
      this.statusbarService.createAuxiliaryStatusbarPart(
		...
		let statusbarVisible = !compact && this.configurationService.getValue<boolean>(
      AuxiliaryEditorPart.STATUS_BAR_VISIBILITY,
    ) !== false;
	...
	private doRemoveLastGroup(preserveFocus?: boolean): void {
      ...
      GroupsOrder.MOST_RECENTLY_ACTIVE,
		...
		if (nextActiveGroup && restoreFocus) {
			const nextGroupInHiddenMainPart = nextActiveGroup.groupsView === this.editorPartsView.mainPart && !this.layoutService.isVisible(
        Parts.EDITOR_PART,
        mainWindow,
      );
