...
export class CompositeBarActionViewItem extends BaseActionViewItem {
	...
	constructor(
      ...
      action.onDidChangeCompositeBarActionItem(() => this.update()),
		...
		this._register(
      Event.filter(keybindingService.onDidUpdateKeybindings, () => this.keybindingLabel !== this.computeKeybindingLabel())(
        () => this.updateTitle(),
      ),
    );
	...
	protected updateStyles(): void {
		...
		if (this.badgeContent) {
			...
			const badgeFg = badgeStyles?.badgeForeground ?? colors.badgeForeground ?? theme.getColor(
        badgeForeground,
      );
			const badgeBg = badgeStyles?.badgeBackground ?? colors.badgeBackground ?? theme.getColor(
        badgeBackground,
      );
			const contrastBorderColor = badgeStyles?.badgeBorder ?? theme.getColor(
        contrastBorder,
      );
	...
	private getVisibleBadges(activities: IActivity[]): { badges: IBadge[]; type: "progress" | "icon" | "number" | undefined } {
		const progressBadges = activities.filter(activity => activity.badge instanceof ProgressBadge).map(
      activity => activity.badge,
		...
		const iconBadges = activities.filter(activity => activity.badge instanceof IconBadge).map(
      activity => activity.badge,
    );
		...
		const numberBadges = activities.filter(activity => activity.badge instanceof NumberBadge).map(
      activity => activity.badge,
    );
...
export class ToggleCompositePinnedAction extends Action {
	...
	constructor(
      ...
      activity ? activity.name : localize("toggle", "Toggle View Pinned"),
		...
		this.checked = !!this.activity && this.compositeBar.isPinned(
      this.activity.id,
    );
	}
