...
export class ViewPaneContainer<MementoType extends object = object> extends Component<MementoType> implements IViewPaneContainer {
	...
	constructor(
      ...
      undefined,
		...
		this.viewContainerModel = this.viewDescriptorService.getViewContainerModel(
      container,
    );
	}
	...
	create(parent: HTMLElement): void {
      ...
      this.paneview.onDidDrop(
        ({ from, to }) => this.movePane(from as ViewPane, to as ViewPane),
      ),
    );
      ...
      addDisposableListener(
        ...
        (e: MouseEvent) => this.showContextMenu(
          ...
          e => {
            ...
            this.onContextMenu(
              ...
              pane,
				...
				const collapseDisposable = Event.latch(Event.map(pane.onDidChange, () => !pane.isExpanded()))(
          collapsed => {
...
export abstract class ViewPaneContainerAction<T extends IViewPaneContainer> extends Action2 {
	...
	run(accessor: ServicesAccessor, ...args: unknown[]): unknown {
		const viewPaneContainer = accessor.get(IViewsService).getActiveViewPaneContainerWithId(
      this.desc.viewPaneContainerId,
    );
...
class MoveViewPosition extends Action2 {
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		const viewContainer = viewDescriptorService.getViewContainerByViewId(
      viewId,
    )!;
