...
export class TreeViewPane extends ViewPane {
	...
	constructor(
      ...
      accessibleViewService,
		...
		const { treeView } = (<ITreeViewDescriptor>Registry.as<IViewsRegistry>(Extensions.ViewsRegistry).getView(
      options.id,
    ));
...
abstract class AbstractTreeView extends Disposable implements ITreeView {
	...
	private _onDidChangeSelectionAndFocus: Emitter<{ selection: readonly ITreeItem[]; focus: ITreeItem }> = this._register(
	...
	focus(reveal: boolean = true, revealItem?: ITreeItem): void {
		if (this.tree && this.root.children && this.root.children.length > 0) {
			...
			this.tree.domFocus();
		} else if (this.tree && this.treeContainer && !this.treeContainer.classList.contains(
      "hide",
    )) {
			this.tree.domFocus();
		} else {
	...
	protected createTree() {
      ...
      TreeDataSource,
      this,
      <T>(task: Promise<T>) => this.progressService.withProgress(
        { location: this.id },
        () => task,
      ),
    );
      ...
      renderer.onDidChangeMenuContext(
        e => e.forEach(e => this.tree?.rerender(e)),
      ),
    );
	...
	private showMessage(message: string | IMarkdownString): void {
		...
		if (typeof this._messageValue === "string" && !isFalsyOrWhitespace(
      this._messageValue,
    )) {
...
async function doGetChildrenOrBatch(dataProvider: ITreeViewDataProvider, nodes: ITreeItem[] | undefined): Promise<(readonly ITreeItem[])[] | undefined> {
	if (dataProvider.getChildrenBatch) {
		...
		if (nodes) {
			return Promise.all(
        nodes.map(
          node => dataProvider.getChildren(node).then(
            children => children ?? [],
          ),
        ),
      );
...
class TreeRenderer extends Disposable implements ITreeRenderer<ITreeItem, FuzzyScore, ITreeExplorerTemplateData> {
	...
	private _onDidChangeMenuContext: Emitter<readonly ITreeItem[]> = this._register(
    new Emitter<readonly ITreeItem[]>(),
  );
	...
	constructor(
      ...
      checkboxStateHandler.onDidChangeCheckboxState(items => {
		...
		this._register(
      this.contextKeyService.onDidChangeContext(e => this.onDidChangeContext(e)),
    );
	}
	...
	renderElement(element: ITreeNode<ITreeItem, FuzzyScore>, index: number, templateData: ITreeExplorerTemplateData): void {
		...
		const treeItemLabel: ITreeItemLabel | undefined = node.label ? node.label : (resource ? {
      label: basename(resource),
    } : undefined);
		const description = isString(node.description) ? node.description : resource && node.description === true ? this.labelService.getUriLabel(
      dirname(resource),
      { relative: true },
    ) : undefined;
		const labelStr = treeItemLabel ? isMarkdownString(
      treeItemLabel.label,
    ) ? treeItemLabel.label.value : treeItemLabel.label : undefined;
		...
		const { label, bold, italic, strikethrough, supportIcons } = this.processLabel(
      treeItemLabel?.label,
      matches,
    );
		const icon = !isDark(
      this.themeService.getColorTheme().type,
    ) ? node.icon : node.iconDark;
        ...
        "explorer.decorations",
			...
			const labelResource = resource ? resource : URI.parse(
      ...
      $treeItemHandle: node.handle,
		...
		const renderedItems = this._renderedElements.get(
	...
	private onDidChangeContext(e: IContextKeyChangeEvent) {
      ...
      this.menus.getEntireMenuContexts(),
		...
		for (const [_, elements] of this._renderedElements) {
			for (const element of elements) {
				if (affectsEntireMenuContexts || e.affectsSome(
	...
	disposeElement(resource: ITreeNode<ITreeItem, FuzzyScore>, index: number, templateData: ITreeExplorerTemplateData): void {
		const itemRenders = this._renderedElements.get(
...
export class CustomTreeView extends AbstractTreeView {
	...
	protected activate() {
		if (!this.activated) {
			...
			this.telemetryService.publicLog2<ExtensionViewTelemetry, ExtensionViewTelemetryMeta>(
...
export class CustomTreeViewDragAndDrop implements ITreeDragAndDrop<ITreeItem> {
	...
	private addResourceInfoToTransfer(originalEvent: DragEvent, resources: URI[]) {
        ...
        accessor => fillEditorsDragData(accessor, resources, originalEvent),
			...
			const fileResources = resources.filter(s => s.scheme === Schemas.file).map(
        r => r.fsPath,
      );
	...
	getDragLabel?(elements: ITreeItem[]): string | undefined {
        ...
        element.label.label,
      ) ? element.label.label.value : element.label.label;
		}
		return element.resourceUri ? this.labelService.getUriLabel(
      URI.revive(element.resourceUri),
    ) : undefined;
	}
	...
	async drop(data: IDragAndDropData, targetNode: ITreeItem | undefined, targetIndex: number | undefined, targetSector: ListViewTargetSector | undefined, originalEvent: DragEvent): Promise<void> {
      ...
      true,
		...
		for (const [type, item] of originalDataTransfer) {
			if (type === this.treeMimeType || dndController.dropMimeTypes.includes(
        type,
      ) || (item.asFile() && dndController.dropMimeTypes.includes(
