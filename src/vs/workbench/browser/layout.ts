...
export abstract class Layout extends Disposable implements IWorkbenchLayoutService {
	...
	private computeContainerOffset(targetWindow: Window) {
		...
		const isCommandCenterVisible = titlebarVisible && this.configurationService.getValue<boolean>(
      LayoutSettings.COMMAND_CENTER,
    ) !== false;
	...
	private registerLayoutListeners(): void {
      ...
      addDisposableListener(
		...
		const showingCustomMenu = (isWindows || isLinux || isWeb) && !hasNativeTitlebar(
	...
	private updateWindowBorder(skipLayout = false) {
		if (
			...
			(
				...
				useWindowControlsOverlay(
          this.configurationService,
        )	// Windows/Linux: not working with WCO (border cannot draw over the overlay)
			) ||
	...
	private shouldRestoreEditors(contextService: IWorkspaceContextService, initialEditorsState: IInitialEditorsState | undefined): boolean {
      ...
      WorkbenchLayoutSettings.EDITOR_RESTORE_EDITORS,
		...
		const forceRestoreEditors = this.configurationService.getValue<string>(
	...
	private async resolveEditorsToOpen(fileService: IFileService, initialEditorsState: IInitialEditorsState | undefined): Promise<IEditorToOpen[]> {
        ...
        await pathsToEditors(initialEditorsState.filesToMerge, fileService, this.logService),
			...
			if (filesToMerge.length === 4 && isResourceEditorInput(
        filesToMerge[0],
      ) && isResourceEditorInput(filesToMerge[1]) && isResourceEditorInput(
        filesToMerge[2],
		...
		else if (this.contextService.getWorkbenchState() === WorkbenchState.EMPTY && this.configurationService.getValue(
		...
		switch (part) {
          ...
          LayoutStateKeys.ACTIVITYBAR_HIDDEN,
			...
			case Parts.BANNER_PART:
				return this.initialized ? this.workbenchGrid.isViewVisible(
          this.bannerPartView,
        ) : false;
			default:
	...
	getMaximumEditorDimensions(container: HTMLElement): IDimension {
		...
		if (container === this.mainContainer) {
          ...
          Parts.ACTIVITYBAR_PART,
				...
				(this.isVisible(
          Parts.SIDEBAR_PART,
        ) ? this.sideBarPartView.minimumWidth : 0) +
				(this.isVisible(
          Parts.PANEL_PART,
        ) && !isPanelHorizontal ? this.panelPartView.minimumWidth : 0) +
				(this.isVisible(
          ...
          Parts.TITLEBAR_PART,
          targetWindow,
        ) ? this.titleBarPartView.minimumHeight : 0) +
				(this.isVisible(
          Parts.STATUSBAR_PART,
          targetWindow,
        ) ? this.statusBarPartView.minimumHeight : 0) +
				(this.isVisible(
          Parts.PANEL_PART,
	...
	resizePart(part: Parts, sizeChangeWidth: number, sizeChangeHeight: number): void {
		const sizeChangePxWidth = Math.sign(
      sizeChangeWidth,
    ) * computeScreenAwareSize(getActiveWindow(), Math.abs(sizeChangeWidth));
		const sizeChangePxHeight = Math.sign(
      sizeChangeHeight,
		...
		switch (part) {
          ...
          height: viewSize.height,
            ...
            height: viewSize.height + sizeChangePxHeight,
					...
					const { width, height } = this.editorGroupService.mainPart.getSize(
            activeGroup,
          );
            ...
            height: height + sizeChangePxHeight,
					...
					const { width: newWidth, height: newHeight } = this.editorGroupService.mainPart.getSize(
            activeGroup,
          );
	...
	private adjustPartPositions(sideBarPosition: Position, panelAlignment: PanelAlignment, panelPosition: Position): void {
      ...
      this.workbenchGrid.getViewCachedVisibleSize(this.auxiliaryBarPartView) ?? this.auxiliaryBarPartView.minimumWidth,
		...
		const focusedPart = [Parts.PANEL_PART, Parts.SIDEBAR_PART, Parts.AUXILIARYBAR_PART].find(
      part => this.hasFocus(part),
	...
	setAuxiliaryBarMaximized(maximized: boolean): boolean {
        ...
        false,
          ...
          !state?.editorVisible,
        );	// this order of updating view visibility
				this.setPanelHidden(
          !state?.panelVisible,
        );		// helps in restoring the previous view
	...
	isPanelMaximized(): boolean {
		return (
			...
			!isHorizontal(
        this.getPanelPosition(),
      )		// maximization with non-center panel alignment
		) && !this.isVisible(
      Parts.EDITOR_PART,
      mainWindow,
    ) && !this.isAuxiliaryBarMaximized();
	...
	private panelOpensMaximized(): boolean {
		if (this.getPanelAlignment() !== "center" && isHorizontal(
      this.getPanelPosition(),
    )) {
	...
	updateMenubarVisibility(skipLayout: boolean): void {
      ...
      this.state.runtime.menuBar.toggled,
		...
		if (!skipLayout && this.workbenchGrid && shouldShowTitleBar !== this.isVisible(
      Parts.TITLEBAR_PART,
      mainWindow,
    )) {
			this.workbenchGrid.setViewVisible(
	...
	toggleMenuBar(): void {
      ...
      this.configurationService,
		...
		if (currentVisibilityValue === "visible" || currentVisibilityValue === "classic") {
			newVisibilityValue = hasNativeMenu(
        this.configurationService,
      ) ? "toggle" : "compact";
		} else {
	...
	setPanelPosition(position: Position): void {
      ...
      this.auxiliaryBarPartView,
		...
		if (isHorizontal(
      position,
    ) && this.getPanelAlignment() !== "center" && editorHidden) {
			this.toggleMaximizedPanel();
			editorHidden = false;
		}
	...
	private arrangeMiddleSectionNodes(nodes: { editor: ISerializedNode; panel: ISerializedNode; activityBar: ISerializedNode; sideBar: ISerializedNode; auxiliaryBar: ISerializedNode }, availableWidth: number, availableHeight: number): ISerializedNode[] {
      ...
      LayoutStateKeys.SIDEBAR_POSITON,
        ...
        LayoutStateKeys.PANEL_ALIGNMENT,
			...
			const data = panelPostion === Position.BOTTOM ? [
        editorNodes,
        nodes.panel,
...
class LayoutStateModel extends Disposable {
	...
	constructor(
      ...
      this.configurationService.onDidChangeConfiguration(
        configurationChange => this.updateStateFromLegacySettings(
          configurationChange,
        ),
      ),
    );
	...
	load(configuration: ILayoutStateLoadConfiguration): void {
      ...
      mainContainerDimension.width / 4,
		...
		LayoutStateKeys.AUXILIARYBAR_SIZE.defaultValue = auxiliaryBarForceMaximized ? Math.max(
      300,
      mainContainerDimension.width / 2,
		...
		LayoutStateKeys.PANEL_SIZE.defaultValue = (this.stateCache.get(
      LayoutStateKeys.PANEL_POSITION.name,
    ) ?? isHorizontal(
      LayoutStateKeys.PANEL_POSITION.defaultValue,
    )) ? mainContainerDimension.height / 3 : mainContainerDimension.width / 4;
	...
	private applyOverrides(configuration: ILayoutStateLoadConfiguration): void {
        ...
        WorkbenchLayoutSettings.AUXILIARYBAR_DEFAULT_VISIBILITY,
			...
			const startupEditor = this.configurationService.getValue<"none" | "welcomePage" | "readme" | "newUntitledFile" | "welcomePageInEmptyWorkbench" | "terminal" | "agentSessionsWelcomePage">(
        "workbench.startupEditor",
      );
