...
class WordHighlighter {
	...
	public restore(delay: number): void {
		...
		this.runDelayer.trigger(() => { this._run(false, delay); }).catch(
      onUnexpectedError,
    );
	}
	...
	public moveNext() {
      ...
      (range) => range.containsPosition(this.editor.getPosition()),
		...
		try {
			...
			if (word) {
				const lineContent = this.editor.getModel().getLineContent(
          dest.startLineNumber,
        );
				alert(
	...
	private renderDecorations(): void {
		...
		for (const editor of currentEditors) {
			...
			if (uri && this.workerRequestValue.has(uri)) {
				const oldDecorationIDs: string[] | undefined = WordHighlighter.storedDecorationIDs.get(
          uri,
        );
...
export class WordHighlighterContribution extends Disposable implements IEditorContribution {
	...
	public restoreViewState(state: boolean | undefined): void {
        ...
        250,
      ); // 250 ms delay to restoring view state, since only exts call this
		}
	}
