...
export class AutoIndentOnPasteCommand implements ICommand {
	...
	public getEditOperations(model: ITextModel, builder: IEditOperationBuilder): void {
		...
		if (Array.isArray(
      this._edits,
    ) && this._edits.length === 1 && this._initialSelection.isEmpty()) {
      ...
      range.getStartPosition().lineNumber,
        ...
        this._languageConfigurationService,
            ...
            text: newIndent,
					...
					firstLineText = newIndent + firstLineText.substring(
        ...
        range.endColumn - 1,
			...
			for (let i = range.startLineNumber; i <= range.endLineNumber; i++) {
				...
				if (i === range.startLineNumber) {
					...
					containsOnlyWhitespace = model.getLineFirstNonWhitespaceColumn(
            i,
          ) === 0;
				}
				if (!containsOnlyWhitespace) {
