...
export function provideInlineCompletions(
	...
	const inlineCompletionLists = AsyncIterableProducer.fromPromisesResolveOrder(providers.map(p => queryProvider.get(p))).filter(
    isDefined,
  );
...
function toInlineSuggestData(
	...
	const uri = inlineCompletion.uri ? URI.revive(
	...
	if (inlineCompletion.jumpToPosition !== undefined) {
      ...
      uri,
		...
		let range = inlineCompletion.range ? Range.lift(
