...
export class InlineEditsWordReplacementView extends Disposable implements IInlineEditsView {
	...
	constructor(
      ...
      return this._userInteractionService.createHoverTracker(elem.element, reader.store).read(
        reader,
      );
    });
      ...
      this._viewData.edit.range.getStartPosition(),
		...
		const altCount = observableFromPromise(this._viewData.alternativeAction?.count ?? new Promise<undefined>(resolve => resolve(undefined))).map(
      c => c.value,
    );
