...
export class InlineEditsView extends Disposable {
	...
	constructor(
      ...
      this._instantiationService.createInstance(
		...
		this._showLongDistanceHint = this._editorObs.getOption(EditorOption.inlineSuggest).map(
      this,
      s => s.edits.showLongDistanceHint,
    );
      ...
      (viewData, store) => {
		...
		this._useCodeShifting = this._editorObs.getOption(EditorOption.inlineSuggest).map(
      s => s.edits.allowCodeShifting,
    );
		this._renderSideBySide = this._editorObs.getOption(EditorOption.inlineSuggest).map(
      s => s.edits.renderSideBySide,
    );
	...
	private _determineView(model: ModelPerInlineEdit, reader: IReader, diff: DetailedLineRangeMapping[], newText: AbstractText): InlineCompletionViewKind {
		...
		const reconsiderViewEditorWidthChange = this._previousView?.editorWidth !== this._editorObs.layoutInfoWidth.read(
      reader,
    ) &&
			(
		...
		if (model.displayLocation && !model.inlineEdit.inlineCompletion.identity.jumpedTo.read(
      reader,
    )) {
			return InlineCompletionViewKind.Custom;
		}
		...
		if (model.editorType !== InlineCompletionEditorType.DiffEditor) {
			if (
				...
				if (isSingleLineInsertionAfterPosition(
          diff,
          inlineEdit.cursorPosition,
        )) {
        ...
        m => TextLength.ofRange(m.originalRange).columnCount < InlineEditsWordReplacementView.MAX_LENGTH && TextLength.ofRange(m.modifiedRange).columnCount < InlineEditsWordReplacementView.MAX_LENGTH,
			...
			if (allInnerChangesNotTooLong && isSingleInnerEdit && numOriginalLines === 1 && numModifiedLines === 1) {
				...
				const modifiedText = inner.map(
          m => newText.getValueOfRange(m.modifiedRange),
        );
				const originalText = inner.map(
		...
		if (numOriginalLines > 0 && numModifiedLines > 0) {
			...
			if (this._renderSideBySide.read(
        reader,
      ) !== "never" && InlineEditsSideBySideView.fitsInsideViewport(
        this._editor,
        this._previewTextModel,
        inlineEdit,
        reader,
	...
	private _determineRenderState(model: ModelPerInlineEdit, reader: IReader, diff: DetailedLineRangeMapping[], newText: AbstractText) {
      ...
      modified: newText.getValueOfRange(m.modifiedRange),
		...
		switch (view) {
			case InlineCompletionViewKind.InsertionInline: return {
        kind: InlineCompletionViewKind.InsertionInline as const,
        viewData,
      };
			case InlineCompletionViewKind.SideBySide: return {
        kind: InlineCompletionViewKind.SideBySide as const,
        viewData,
      };
			case InlineCompletionViewKind.Collapsed: return {
        kind: InlineCompletionViewKind.Collapsed as const,
        viewData,
      };
			case InlineCompletionViewKind.Custom: return {
...
function getViewData(inlineEdit: InlineEditWithChanges, stringChanges: { originalRange: Range; modifiedRange: Range; original: string; modified: string }[], textModel: ITextModel) {
    ...
    stringChanges.every(
      r => r.original === stringChanges[0].original && r.modified === stringChanges[0].modified,
    ),
  );
	return viewData;
}
