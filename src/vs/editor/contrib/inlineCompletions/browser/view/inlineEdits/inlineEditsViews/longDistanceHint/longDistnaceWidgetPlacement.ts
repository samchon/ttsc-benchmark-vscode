...
export class WidgetPlacementContext {
	...
	constructor(
      ...
      s => s.height,
		...
		this.availableSpaceSizesTransposed = this.availableSpaceSizes.map(
      s => s.transpose(),
    );
	}
	...
	public getWidgetVerticalOutline(
		...
		const verticalWidgetRange = editorRange.withMargin(previewEditorMargin + widgetPadding + widgetBorder).withMargin(
      0,
      lowerBarHeight,
    );
...
export function splitIntoContinuousLineRanges(
	...
	for (let i = 0; i < sizes.length; i++) {
		...
		const expectedTop = currentRangeTop + currentSizes.reduce(
      (p, c) => p + c.height,
      0,
    );
