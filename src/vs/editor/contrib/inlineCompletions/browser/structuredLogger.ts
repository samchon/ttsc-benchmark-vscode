...
export class StructuredLogger<T extends IRecordableLogEntry> extends Disposable {
	...
	constructor(
		...
		super();
		this._isEnabledContextKeyValue = observableContextKey<boolean>("structuredLogger.enabled:" + this._key, this._contextKeyService).recomputeInitiallyAndOnChange(
      this._store,
    );
	...
	public log(data: T): boolean {
		...
		this._dataChannelService.getDataChannel<T>("structuredLogger:" + this._key).sendData(
      data,
    );
		return true;
	}
