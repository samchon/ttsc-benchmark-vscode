...
setInlineCompletionsControllerGetter(
  (editor) => InlineCompletionsController.get(editor),
);
...
export class InlineCompletionsController extends Disposable {
	...
	protected readonly _view = derived(
	...
	constructor(
      ...
      { min: 50, max: 50 },
		...
		this._hideInlineEditOnSelectionChange = this._editorObs.getOption(EditorOption.inlineSuggest).map(
      val => true,
    );
      ...
      contextKeySvcObs.bind(
        ...
        this.model.map(
          (m, reader) => m?.inlineEditState?.read(reader)?.cursorAtInlineEdit.read(
            reader,
          ),
        ),
      ),
	...
	private _provideScreenReaderUpdate(content: string): void {
		const accessibleViewShowing = this._contextKeyService.getContextKeyValue<boolean>(
