...
export class RenameWidget implements IRenameWidget, IContentWidget, IDisposable {
	...
	private _getSelection(where: IRange, currentName: string): { start: number; end: number } {
		...
		if (!Range.isEmpty(selection) && !Range.spansMultipleLines(
      selection,
	...
	private async _updateRenameCandidates(candidates: ProviderResult<NewSymbolName[]>[], currentName: string, token: CancellationToken) {
		const trace = (...args: unknown[]) => this._trace(
      "_updateRenameCandidates",
      ...args,
    );
	...
	private _getTopForPosition(): number {
        ...
        "RenameWidget#_getTopForPosition: this should not happen - visibleRanges is empty",
			...
			firstLineInViewport = Math.max(
        1,
        this._position!.lineNumber - 5,
      ); // @ulugbekna: fallback to current line minus 5
		}
		return this._editor.getTopForLineNumber(
      this._position!.lineNumber,
	...
	public focusNext(): boolean {
		...
		if (focusedIxs.length === 0) {
			...
			if (focusedIxs[0] === this._listWidget.length - 1) {
				this._listWidget.setFocus([]);
				this._listWidget.reveal(
          0,
        ); // @ulugbekna: without this, it seems like focused element is obstructed
				return false;
			} else {
