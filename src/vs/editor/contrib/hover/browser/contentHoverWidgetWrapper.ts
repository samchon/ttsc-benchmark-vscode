...
export class ContentHoverWidgetWrapper extends Disposable implements IHoverWidget {
	...
	private _initializeHoverParticipants(): IEditorHoverParticipant[] {
      ...
      this._contentHoverWidget.onContentsChanged(() => {
        this._participants.forEach(
          participant => participant.handleContentsChanged?.(),
        );
      }),
    );
		return participants;
	...
	private _startShowingOrUpdateHover(
      ...
      mouseEvent.event.posy,
		...
		const currentAnchorEqualsPreviousAnchor = this._currentResult && this._currentResult.options.anchor.equals(
      anchor,
    );
	...
	private _startHoverOperationIfNecessary(anchor: HoverAnchor, mode: HoverStartMode, source: HoverStartSource, shouldFocus: boolean, insistOnKeepingHoverVisible: boolean): void {
		const currentAnchorEqualToPreviousHover = this._hoverOperation.options && this._hoverOperation.options.anchor.equals(
      anchor,
    );
	...
	public showsOrWillShow(mouseEvent: IEditorMouseEvent): boolean {
		...
		const anchorCandidates: HoverAnchor[] = this._findHoverAnchorCandidates(
      mouseEvent,
    );
	...
	private _onMouseLeave(e: MouseEvent): void {
		...
		const isMousePositionOutsideOfEditor = !editorDomNode || !isMousePositionWithinElement(
      editorDomNode,
      e.x,
      e.y,
    );
	...
	public containsNode(node: Node | null | undefined): boolean {
		return (node ? this._contentHoverWidget.getDomNode().contains(
      node,
    ) : false);
	}
