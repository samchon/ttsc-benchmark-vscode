...
export class MoveLinesCommand implements ICommand {
	...
	public getEditOperations(model: ITextModel, builder: IEditOperationBuilder): void {
      ...
      insertSpaces,
		...
		if (s.startLineNumber === s.endLineNumber && model.getLineMaxColumn(
      s.startLineNumber,
    ) === 1) {
          ...
          null,
            ...
            s.startLineNumber - 1,
              ...
              model.getLineContent(movingLineNumber),
						...
						const newSpaceCnt = movingLineMatchResult + indentUtils.getSpaceCnt(
              oldIndentation,
              tabSize,
            );
	...
	private parseEnterResult(model: ITextModel, indentConverter: IIndentConverter, tabSize: number, line: number, enter: CompleteEnterAction | null) {
		if (enter) {
			...
			if (enter.indentAction === IndentAction.None) {
				...
				enterPrefix = indentConverter.unshiftIndent(
          enter.indentation,
        ) + enter.appendText;
			}
