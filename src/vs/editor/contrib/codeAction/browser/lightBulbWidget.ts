...
export class LightBulbWidget extends Disposable implements IContentWidget {
    ...
    new Emitter<{ readonly x: number; readonly y: number; readonly actions: CodeActionSet; readonly trigger: CodeActionTrigger }>(),
  );
	...
	public update(actions: CodeActionSet, trigger: CodeActionTrigger, atPosition: IPosition) {
      ...
      EditorOption.lightbulb,
		...
		const isFolded = (lineNumber: number) => {
			return lineNumber > 2 && this._editor.getTopForLineNumber(
        lineNumber,
		...
		if (!lineHasSpace) {
          ...
          lineContent,
			...
			if (lineNumber > 1 && !isFolded(lineNumber - 1)) {
				...
				const prevLineEmptyOrIndented = lineNumber > 1 && isLineEmptyOrIndented(
          lineNumber - 1,
        );
				const nextLineEmptyOrIndented = !endLine && isLineEmptyOrIndented(
          lineNumber + 1,
        );
			...
			} else if (lineNumber === 1 && (lineNumber === model.getLineCount() || !isLineEmptyOrIndented(
        lineNumber + 1,
			...
			} else if ((lineNumber < model.getLineCount()) && !isFolded(
        lineNumber + 1,
      )) {
				effectiveLineNumber += 1;
