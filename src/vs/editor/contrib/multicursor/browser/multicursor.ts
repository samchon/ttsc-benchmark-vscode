	...
	public run(accessor: ServicesAccessor, editor: ICodeEditor): void {
		...
		if (viewModel) {
			...
			if (findController) {
				...
				const newFindController = accessor.get(IInstantiationService).createInstance(
...
class SelectionHighlighterState {
	...
	public findMatches(): Range[] {
		if (this._cachedFindMatches === null) {
			this._cachedFindMatches = this._model.findMatches(this._searchText, true, false, this._matchCase, this._wordSeparators, false).map(
        m => m.range,
      );
	...
	private _setState(newState: SelectionHighlighterState | null): void {
		...
		for (let i = 0, j = 0, len = allMatches.length, lenJ = selections.length; i < len;) {
			...
			if (j >= lenJ) {
				...
				if (cmp < 0) {
					// match is before sel
					if (selections[j].isEmpty() || !Range.areIntersecting(
            match,
            selections[j],
          )) {
						matches.push(match);
		...
		const occurrenceHighlighting: boolean = this.editor.getOption(
