...
export class RichScreenReaderContent extends Disposable implements IScreenReaderContent {
	...
	private _renderLine(viewLineNumber: number): RichRenderedScreenReaderLine {
      ...
      true,
		...
		const lineHeight = viewModel.viewLayout.getLineHeightForLineNumber(
      viewLineNumber,
    ) + "px";
	...
	private _setSelectionOnScreenReaderContent(context: ViewContext, renderedLines: Map<number, RichRenderedScreenReaderLine>, viewSelection: Selection): void {
      ...
      viewSelection.selectionStartColumn,
		...
		const modelStartRange = coordinatesConverter.convertViewRangeToModelRange(
      startRange,
    );
		const characterCountForStart = model.getCharacterCountInRange(
      modelStartRange,
    );
      ...
      viewSelection.positionColumn,
		...
		const modelEndRange = coordinatesConverter.convertViewRangeToModelRange(
      endRange,
    );
      ...
      characterCountForStart,
		...
		const endDomPosition = endRenderedLine.characterMapping.getDomPosition(
      characterCountForEnd,
    );
	...
	private _getEditorSelectionFromDomRange(): Selection | undefined {
		...
		if (!startSpanElement || !isHTMLElement(
      startSpanElement,
    ) || !endSpanElement || !isHTMLElement(endSpanElement)) {
			return;
		}
      ...
      LINE_NUMBER_ATTRIBUTE,
		...
		const startMapping = this._renderedLines.get(
      startLineNumber,
    )?.characterMapping;
