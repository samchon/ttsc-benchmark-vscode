...
export class LineNumbersOverlay extends DynamicViewOverlay {
	...
	public prepareRender(ctx: RenderingContext): void {
		...
		const lineNoDecorations = this._context.viewModel.getDecorationsInViewport(ctx.visibleRange).filter(
      d => !!d.options.lineNumberClassName,
    );
      ...
      (a, b) => Range.compareRangesUsingEnds(a.range, b.range),
        ...
        new Position(lineNumber, 1),
			...
			if (lineNumber === lineCount && this._context.viewModel.getLineLength(
        lineNumber,
      ) === 0) {
