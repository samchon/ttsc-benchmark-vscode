...
function transformAndRemoveOverflowing(lineContent: string, lineContainsRTL: boolean, tokens: IViewLineTokens, fauxIndentLength: number, len: number): LinePart[] {
	...
	for (let tokenIndex = 0, tokensLen = tokens.getCount(); tokenIndex < tokensLen; tokenIndex++) {
		...
		if (endIndex >= len) {
			const tokenContainsRTL = (lineContainsRTL ? strings.containsRTL(
        lineContent.substring(startOffset, len),
      ) : false);
			result[resultLen++] = new LinePart(len, type, 0, tokenContainsRTL);
			break;
		}
		const tokenContainsRTL = (lineContainsRTL ? strings.containsRTL(
      lineContent.substring(startOffset, endIndex),
    ) : false);
...
function _applyRenderWhitespace(input: RenderLineInput, lineContent: string, len: number, tokens: LinePart[]): LinePart[] {
	...
	if (wasInWhitespace) {
		...
		if (continuesWithWrappedLine && onlyBoundary) {
			const lastCharCode = (len > 0 ? lineContent.charCodeAt(
        len - 1,
      ) : CharCode.Null);
			const prevCharCode = (len > 1 ? lineContent.charCodeAt(
        len - 2,
      ) : CharCode.Null);
		...
		if (partRendersWhitespace) {
          ...
          charHorizontalOffset,
				...
				if (charCode === CharCode.Tab) {
					...
					sb.appendCharCode(
            renderSpaceCharCode,
          ); // &middot; or word separator middle dot
          ...
          charHorizontalOffset,
				...
				switch (charCode) {
					...
					default:
						...
						} else if (renderControlCharacters && isControlCharacter(
              charCode,
            )) {
							sb.appendString("[U+");
