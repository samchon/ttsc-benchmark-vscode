...
export class ViewModel extends Disposable implements IViewModel {
	...
	public getFontSizeAtPosition(position: IPosition): string | null {
      ...
      this._editorId,
		...
		let fontSize: string = this._configuration.options.get(
	...
	*/
	...
	public saveState(): IViewState {
		...
		const firstViewLineNumber = this.viewLayout.getLineNumberAtVerticalOffset(
      scrollTop,
    );
	...
	public getDecorationsInViewport(visibleRange: Range): ViewModelDecoration[] {
		return this._decorations.getDecorationsViewportData(
      visibleRange,
    ).decorations;
	}
	...
	public getTextDirection(lineNumber: number): TextDirection {
		const decorationsCollection = this._decorations.getDecorationsOnLine(
      lineNumber,
    );
	...
	public getViewportViewLineRenderingData(visibleRange: Range, lineNumber: number): ViewLineRenderingData {
		const viewportDecorationsCollection = this._decorations.getDecorationsViewportData(
      visibleRange,
    );
	...
	public getValueInRange(range: Range, eol: EndOfLinePreference): string {
		const modelRange = this.coordinatesConverter.convertViewRangeToModelRange(
      range,
    );
	...
	public getPlainTextToCopy(modelRanges: Range[], emptySelectionClipboard: boolean, forceCRLF: boolean): { sourceRanges: Range[]; sourceText: string | string[] } {
		...
		const pushRange = (modelRange: Range, append: string = "") => {
			...
			result.push(
        this.model.getValueInRange(modelRange, forceCRLF ? EndOfLinePreference.CRLF : EndOfLinePreference.TextDefined) + append,
      );
		};
	...
	public setCursorStates(source: string | null | undefined, reason: CursorChangeReason, states: PartialCursorState[] | null): boolean {
      ...
      eventsCollector => this._cursor.setStates(
        eventsCollector,
        source,
        reason,
        states,
	...
	public executeCommand(command: ICommand, source?: string | null | undefined): void {
		this._executeCursorEdit(
      eventsCollector => this._cursor.executeCommand(
        eventsCollector,
        command,
        source,
      ),
	...
	public executeCommands(commands: ICommand[], source?: string | null | undefined): void {
		this._executeCursorEdit(
      eventsCollector => this._cursor.executeCommands(
        eventsCollector,
        commands,
        source,
      ),
...
class OverviewRulerDecorations {
	...
	private readonly _asMap: { [color: string]: OverviewRulerDecorationsGroup } = Object.create(
    null,
  );
