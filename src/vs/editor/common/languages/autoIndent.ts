...
function getPrecedingValidLine(model: IVirtualModel, lineNumber: number, processedIndentRulesSupport: ProcessedIndentRulesSupport) {
	...
	if (lineNumber > 1) {
        ...
        0,
			...
			if (processedIndentRulesSupport.shouldIgnore(
        lastLineNumber,
      ) || /^\s+$/.test(text) || text === "") {
...
export function getInheritIndentForLine(
	...
	if (processedIndentRulesSupport.shouldIncrease(
	...
	} else if (processedIndentRulesSupport.shouldDecrease(
    precedingUnIgnoredLine,
  )) {
		const precedingUnIgnoredLineContent = model.getLineContent(
      precedingUnIgnoredLine,
...
export function getIndentForEnter(
	...
	const processedContextTokens = indentationContextProcessor.getProcessedTokenContextAroundRange(
    range,
  );
...
export function getIndentActionForType(
	...
	if (previousLineNumber > 0) {
		...
		if (indentRulesSupport.shouldIndentNextLine(
      previousLine,
        ...
        languageConfigurationService,
			...
			if (inheritedIndentation !== undefined) {
				const currentLine = model.getLineContent(range.startLineNumber);
				const actualCurrentIndentation = strings.getLeadingWhitespace(
          currentLine,
        );
				const inferredCurrentIndentation = indentConverter.shiftIndent(
