...
function loadExternalComputer(useWasm: boolean): Promise<IExternalDiffComputer> {
	if (useWasm) {
		if (!externalWasmDiffComputerPromise) {
			externalWasmDiffComputerPromise = loadExternalModule().then(
        m => m.createDiffComputer({ useWasm: true }),
      );
		}
		return externalWasmDiffComputerPromise;
	}
	if (!externalDiffComputerPromise) {
		externalDiffComputerPromise = loadExternalModule().then(
      m => m.createDiffComputer({ useWasm: false }),
    );
	}
