...
export class LineSequence implements ISequence {
	...
	getBoundaryScore(length: number): number {
		const indentationBefore = length === 0 ? 0 : getIndentation(
      this.lines[length - 1],
    );
		const indentationAfter = length === this.lines.length ? 0 : getIndentation(
      this.lines[length],
    );
...
function getIndentation(str: string): number {
	let i = 0;
	while (i < str.length && (str.charCodeAt(
    i,
  ) === CharCode.Space || str.charCodeAt(i) === CharCode.Tab)) {
		i++;
	}
	return i;
}
