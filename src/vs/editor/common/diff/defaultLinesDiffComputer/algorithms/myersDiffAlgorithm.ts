...
export class MyersDiffAlgorithm implements IDiffAlgorithm {
	compute(seq1: ISequence, seq2: ISequence, timeout: ITimeout = InfiniteTimeout.instance): DiffAlgorithmResult {
		...
		function getXAfterSnake(x: number, y: number): number {
			while (x < seqX.length && y < seqY.length && seqX.getElement(
        x,
      ) === seqY.getElement(y)) {
				x++;
				y++;
		...
		loop: while (true) {
			...
			for (k = lowerBound; k <= upperBound; k += 2) {
				...
				const maxXofDLineTop = k === upperBound ? -1 : V.get(
          k + 1,
        ); // We take a vertical non-diagonal (add a symbol in seqX)
				const maxXofDLineLeft = k === lowerBound ? -1 : V.get(
          k - 1,
        ) + 1; // We take a horizontal non-diagonal (+1 x) (delete a symbol in seqX)
				step++;
				const x = Math.min(
