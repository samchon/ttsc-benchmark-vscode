...
export class ModelService extends Disposable implements IModelService {
	...
	private static _readModelOptions(config: IRawConfig, isForSimpleWidget: boolean): ITextModelCreationOptions {
		...
		if (config.editor && typeof config.editor.insertSpaces !== "undefined") {
			insertSpaces = (config.editor.insertSpaces === "false" ? false : Boolean(
        config.editor.insertSpaces,
      ));
		}
		...
		if (config.editor && typeof config.editor.trimAutoWhitespace !== "undefined") {
			trimAutoWhitespace = (config.editor.trimAutoWhitespace === "false" ? false : Boolean(
        config.editor.trimAutoWhitespace,
      ));
		}

		let detectIndentation = EDITOR_MODEL_DEFAULTS.detectIndentation;
		if (config.editor && typeof config.editor.detectIndentation !== "undefined") {
			detectIndentation = (config.editor.detectIndentation === "false" ? false : Boolean(
        config.editor.detectIndentation,
      ));
		}

		let largeFileOptimizations = EDITOR_MODEL_DEFAULTS.largeFileOptimizations;
		if (config.editor && typeof config.editor.largeFileOptimizations !== "undefined") {
			largeFileOptimizations = (config.editor.largeFileOptimizations === "false" ? false : Boolean(
        config.editor.largeFileOptimizations,
      ));
		}
	...
	public updateModel(model: ITextModel, value: string | ITextBufferFactory, reason: TextModelEditSource = EditSources.unknown(
    { name: "updateModel" },
  )): void {
		const options = this.getCreationOptions(
      model.getLanguageId(),
	...
	private static _commonPrefix(a: ITextModel, aLen: number, aDelta: number, b: ITextBuffer, bLen: number, bDelta: number): number {
		...
		let result = 0;
		for (let i = 0; i < maxResult && a.getLineContent(
      aDelta + i,
    ) === b.getLineContent(bDelta + i); i++) {
			result++;
		}
		return result;
	}
	...
	private _onWillDispose(model: ITextModel): void {
		...
		const sharesUndoRedoStack = (this._undoRedoService.getUriComparisonKey(
      model.uri,
    ) !== model.uri.toString());
		let maintainUndoRedoStack = false;
		let heapSize = 0;
		if (sharesUndoRedoStack || (this._shouldRestoreUndoStack() && this._schemaShouldMaintainUndoRedoElements(
      model.uri,
    ))) {
			...
			if (elements.past.length > 0 || elements.future.length > 0) {
				for (const element of elements.past) {
					if (isEditStackElement(element) && element.matchesResource(model.uri)) {
						...
						element.setModel(
              model.uri,
            ); // remove reference from text buffer instance
					}
				}
				for (const element of elements.future) {
					if (isEditStackElement(element) && element.matchesResource(model.uri)) {
		...
		} else if (!sharesUndoRedoStack && (heapSize > maxMemory || !sha1Computer.canComputeSHA1(
      model,
    ))) {
