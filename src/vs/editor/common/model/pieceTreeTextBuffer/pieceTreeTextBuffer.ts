...
export class PieceTreeTextBuffer extends Disposable implements ITextBuffer {
	...
	public getCharacterCountInRange(range: Range, eol: EndOfLinePreference = EndOfLinePreference.TextDefined): number {
		if (this._mightContainNonBasicASCII) {
			...
			result += this._getEndOfLine(
        eol,
      ).length * (toLineNumber - fromLineNumber);
	...
	public applyEdits(rawOperations: ValidAnnotatedEditOperation[], recordTrimAutoWhitespace: boolean, computeUndoEdits: boolean): ApplyEditsResult {
		...
		for (let i = 0; i < rawOperations.length; i++) {
			...
			if (op.text) {
				...
				if (!mightContainUnusualLineTerminators && textMightContainNonBasicASCII) {
					...
					mightContainUnusualLineTerminators = strings.containsUnusualLineTerminators(
            op.text,
          );
				}
			}
		...
		const reverseRanges = (computeUndoEdits || recordTrimAutoWhitespace ? PieceTreeTextBuffer._getInverseEditRanges(
      operations,
    ) : []);
        ...
        (a, b) => b.lineNumber - a.lineNumber,
			...
			for (let i = 0, len = newTrimAutoWhitespaceCandidates.length; i < len; i++) {
				...
				if (lineContent.length === 0 || lineContent === prevContent || strings.firstNonWhitespaceIndex(
          lineContent,
        ) !== -1) {
					continue;
				}
