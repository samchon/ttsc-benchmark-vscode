...
class PieceTreeSnapshot implements ITextSnapshot {
	...
	read(): string | null {
		...
		if (this._index === 0) {
			return this._BOM + this._tree.getPieceContent(
        this._pieces[this._index++],
      );
		}
...
export class PieceTreeBase {
	...
	private createNewPieces(text: string): Piece[] {
		...
		if (this._buffers[0].lineStarts[this._buffers[0].lineStarts.length - 1] === startOffset
			...
			&& this.endWithCR(
        this._buffers[0].buffer,
      ) // todo, we can check this._lastChangeBufferPos's column as it's the last one
		) {
        ...
        column: this._lastChangeBufferPos.column + 1,
			...
			this._buffers[0].lineStarts = (<number[]>this._buffers[0].lineStarts).concat(
        <number[]>lineStarts.slice(1),
      );
	...
	private getIndexOf(node: TreeNode, accumulatedValue: number): { index: number; remainder: number } {
		...
		if (this.offsetInBuffer(piece.bufferIndex, piece.end) - this.offsetInBuffer(
      piece.bufferIndex,
      piece.start,
    ) === accumulatedValue) {
	...
	private shrinkNode(node: TreeNode, start: BufferCursor, end: BufferCursor) {
      ...
      newEnd,
		...
		const newLength = this.offsetInBuffer(
      piece.bufferIndex,
      start,
	...
	private appendToNode(node: TreeNode, value: string): void {
		...
		const hitCRLF = this.shouldCheckCRLF() && this.startWithLF(
      value,
    ) && this.endWithCR(node);
	...
	private nodeCharCodeAt(node: TreeNode, offset: number): number {
		...
		const buffer = this._buffers[node.piece.bufferIndex];
		const newOffset = this.offsetInBuffer(
      node.piece.bufferIndex,
      node.piece.start,
    ) + offset;
	...
	private startWithLF(val: string | TreeNode): boolean {
		...
		return this._buffers[piece.bufferIndex].buffer.charCodeAt(
      startOffset,
    ) === 10;
	}
