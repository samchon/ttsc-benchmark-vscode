...
export class TextModel extends Disposable implements model.ITextModel, IDecorationsTreesHost {
    ...
    new DidChangeDecorationsEmitter(
      (affectedInjectedTextLines, affectedLineHeights, affectedFontLines) => this.handleBeforeFireDecorationsChangedEvent(
	...
	public onDidChangeContent(listener: (e: IModelContentChangedEvent) => void): IDisposable {
      ...
      (e: InternalModelContentChangeEvent) => listener(e.contentChangedEvent),
    );
	}
	//#endregion
	...
	private _isValidRange(range: Range, validationType: StringOffsetValidationType): boolean {
      ...
      StringOffsetValidationType.Relaxed,
        ...
        startColumn - 2,
			...
			const charCodeBeforeEnd = (endColumn > 1 && endColumn <= this._buffer.getLineLength(
        endLineNumber,
      ) ? this._buffer.getLineCharCode(endLineNumber, endColumn - 2) : 0);
	...
	public findMatches(searchString: string, rawSearchScope: boolean | IRange | IRange[] | null, isRegex: boolean, matchCase: boolean, wordSeparators: string | null, captureMatches: boolean, limitResultCount: number = LIMIT_FIND_COUNT): model.FindMatch[] {
        ...
        (searchScope: IRange) => Range.isIRange(searchScope),
      )) {
				searchRanges = rawSearchScope.map(
          (searchScope: IRange) => this.validateRange(searchScope),
        );
			}
		}
      ...
      (d1, d2) => d1.startLineNumber - d2.startLineNumber || d1.startColumn - d2.startColumn,
        ...
        limitResultCount,
			...
			matchMapper = (searchRange: Range) => TextModelSearch.findMatches(
        this,
	...
	private _validateEditOperation(rawOperation: model.IIdentifiedSingleEditOperation): model.ValidAnnotatedEditOperation {
		...
		if (opText) {
			const endsWithLoneCR = (
				opText.length > 0 && opText.charCodeAt(
          opText.length - 1,
        ) === CharCode.CarriageReturn
			);
			const removeTrailingCR = (
				this.getEOL() === "\r\n" && endsWithLoneCR && validatedRange.endColumn === this.getLineMaxColumn(
				...
				for (let i = 0, len = this._trimAutoWhitespaceLines.length; i < len; i++) {
					...
					for (let j = 0, lenJ = incomingEdits.length; j < lenJ; j++) {
						...
						if (
							...
							&& editRange.isEmpty() && editText && editText.length > 0 && editText.charAt(
                0,
              ) === "\n"
						) {
	...
	private _fireOnDidChangeLineHeight(affectedLineHeights: Set<LineHeightChangingDecoration> | null): void {
        ...
        specialLineHeightChange => new ModelLineHeightChanged(
	...
	private _fireOnDidChangeFont(affectedFontLines: Set<LineFontChangingDecoration> | null): void {
        ...
        fontChange => new ModelFontChanged(
          fontChange.ownerId,
          fontChange.lineNumber,
        ),
      );
	...
	private _changeDecorationOptionsImpl(ownerId: number, decorationId: string, options: ModelDecorationOptions): void {
		...
		const changedWhetherInjectedText = isOptionsInjectedText(
      options,
    ) !== isNodeInjectedText(node);
...
export class ModelDecorationOptions implements model.IModelDecorationOptions {
	...
	private constructor(options: model.IModelDecorationOptions) {
		...
		this.blockClassName = options.blockClassName ? cleanClassName(
      options.blockClassName,
		...
		this.className = options.className ? cleanClassName(
      options.className,
    ) : null;
		...
		this.lineHeight = options.lineHeight ? Math.min(
      options.lineHeight,
      LINE_HEIGHT_CEILING,
    ) : null;
		...
		this.overviewRuler = options.overviewRuler ? new ModelDecorationOverviewRulerOptions(
      options.overviewRuler,
    ) : null;
		this.minimap = options.minimap ? new ModelDecorationMinimapOptions(
      options.minimap,
    ) : null;
		this.glyphMargin = options.glyphMarginClassName ? new ModelDecorationGlyphMarginOptions(
      options.glyphMargin,
    ) : null;
		this.glyphMarginClassName = options.glyphMarginClassName ? cleanClassName(
      options.glyphMarginClassName,
    ) : null;
		this.linesDecorationsClassName = options.linesDecorationsClassName ? cleanClassName(
      options.linesDecorationsClassName,
    ) : null;
		this.lineNumberClassName = options.lineNumberClassName ? cleanClassName(
      options.lineNumberClassName,
    ) : null;
		this.linesDecorationsTooltip = options.linesDecorationsTooltip ? strings.htmlAttributeEncodeValue(
      options.linesDecorationsTooltip,
    ) : null;
		this.firstLineDecorationClassName = options.firstLineDecorationClassName ? cleanClassName(
      options.firstLineDecorationClassName,
    ) : null;
		this.marginClassName = options.marginClassName ? cleanClassName(
      options.marginClassName,
    ) : null;
		this.inlineClassName = options.inlineClassName ? cleanClassName(
      options.inlineClassName,
    ) : null;
		this.inlineClassNameAffectsLetterSpacing = options.inlineClassNameAffectsLetterSpacing || false;
		this.beforeContentClassName = options.beforeContentClassName ? cleanClassName(
      options.beforeContentClassName,
    ) : null;
		this.afterContentClassName = options.afterContentClassName ? cleanClassName(
      options.afterContentClassName,
    ) : null;
		this.after = options.after ? ModelDecorationInjectedTextOptions.from(
      options.after,
    ) : null;
		this.before = options.before ? ModelDecorationInjectedTextOptions.from(
      options.before,
    ) : null;
