	...
	public read(): Token | null {
		...
		while (true) {
			...
			if (this.lineTokenOffset < tokenCount) {
				...
				while (this.lineTokenOffset + 1 < tokenCount && tokenMetadata === lineTokens.getMetadata(
          this.lineTokenOffset + 1,
        )) {
				...
				const isOther = TokenMetadata.getTokenType(
          tokenMetadata,
        ) === StandardTokenType.Other;
				const containsBracketType = TokenMetadata.containsBalancedBrackets(
          tokenMetadata,
        );
