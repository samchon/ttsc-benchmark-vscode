...
class MockChatAgentContribution extends Disposable implements IWorkbenchContribution {
	...
	private addSessionItem(resource: URI, message: string, responseText: string, fileEdits?: MockFileEdit[]): void {
      ...
      {
        type: "response",
        parts: [{ kind: "markdownContent", content: { value: responseText, isTrusted: false, supportThemeIcons: false, supportHtml: false } }],
        participant: "copilot",
      },
    );
      ...
      edit => ({
		...
		const existingIndex = this._sessionItems.findIndex(
      s => isEqual(s.resource, resource),
    );
		let addedOrUpdated = existingIndex !== -1 ? {
