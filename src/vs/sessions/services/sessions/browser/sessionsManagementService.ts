  ...
  implements ISessionsManagementService
{
  ...
  constructor(
    ...
    this._isNewChatInSessionContext =
      IsNewChatInSessionContext.bindTo(contextKeyService);
  ...
  async sendNewChatRequest(
    ...
    try {
      ...
      if (
        ...
        this._activeSession.get()?.sessionId === session.sessionId
      ) {
        ...
        this._activeSessionDisposables.add(
          autorun((reader) => {
            ...
            if (
              ...
              const fallback =
                chats[chats.length - 1] ?? session.mainChat.read(reader);
              if (fallback) {
