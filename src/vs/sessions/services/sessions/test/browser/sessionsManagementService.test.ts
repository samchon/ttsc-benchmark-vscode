...
suite("SessionsManagementService", () => {
  ...
  test("openSession waits for a loading session before opening chat content", async () => {
    ...
    const { service, chatWidgetService, agentSessionsService } =
      createSessionsManagementService(session, disposables);
    ...
    assert.deepStrictEqual(
      {
        opened: chatWidgetService.opened.map((uri) => uri.toString()),
  ...
  test("restoreLastActiveSession waits for session to appear via onDidChangeSessions", async () => {
    ...
    await Promise.resolve();
    assert.deepStrictEqual(
      chatWidgetService.opened.map((uri) => uri.toString()),
      [],
    );
