  ...
  implements IWorkbenchContribution
{
  ...
  private _onDidChangeSessions(e: ISessionsChangeEvent): void {
    ...
    for (const session of e.removed) {
      if (
        this._sessionDisposables.get(session.sessionId) &&
    ...
    const replacement =
      e.added.length === 0 &&
      e.changed.length === 1 &&
      removedTrackedSessions.length === 1
        ? removedTrackedSessions[0]
        : undefined;
    for (const session of e.changed) {
      this._trackSession(
        session,
        replacement?.providerId === session.providerId &&
          replacement.sessionType === session.sessionType,
      );
    }
  }
  ...
  private _hasWorktree(session: ISession): boolean {
    return (
      session.workspace
        .get()
        ?.folders.some((folder) => !!folder.gitRepository?.workTreeUri) ?? false
    );
  }
