...
suite("WorktreeCreatedTaskDispatcher", () => {
  ...
  test("per-session task lists do not cross-contaminate", async () => {
    createDispatcher();
    const { session: sessionA, workspace: workspaceA } = makeSession({
      id: "a",
      hasWorktree: false,
    });
    const { session: sessionB, workspace: workspaceB } = makeSession({
      id: "b",
      hasWorktree: false,
    });
  ...
  test("runs for committed replacement of tracked pending session", async () => {
    createDispatcher();
    const { session: pending } = makeSession({
      id: "pending",
      hasWorktree: false,
    });
    const { session: committed } = makeSession({
      id: "committed",
      hasWorktree: true,
    });
    ...
    assert.deepStrictEqual(tasks.ranTasks, [
      { label: "setup", sessionId: "committed" },
    ]);
  });
  ...
  test("does not treat mixed changed sessions as pending replacements", async () => {
    ...
    tasks.setTasks(committed.sessionId, [
      entry("setup-committed", "worktreeCreated"),
    ]);
    tasks.setTasks(restored.sessionId, [
      entry("setup-restored", "worktreeCreated"),
    ]);
  ...
  test("does not treat dispatched sessions as pending replacements", async () => {
    createDispatcher();
    const { session: dispatched, workspace } = makeSession({
      id: "dispatched",
      hasWorktree: false,
    });
    ...
    tasks.setTasks(dispatched.sessionId, [
      entry("setup-dispatched", "worktreeCreated"),
    ]);
    tasks.setTasks(changed.sessionId, [
      entry("setup-changed", "worktreeCreated"),
    ]);
    ...
    assert.deepStrictEqual(tasks.ranTasks, [
      { label: "setup-dispatched", sessionId: "dispatched" },
    ]);
  });
