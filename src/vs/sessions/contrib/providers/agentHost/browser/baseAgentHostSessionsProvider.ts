...
} from "../../../../../platform/agentHost/common/changesetUri.js";
import {
...
export class AgentHostSessionAdapter implements ISession {
  ...
  readonly changes = observableValueOpts<
  ...
  update(metadata: IAgentSessionMetadata): boolean {
    ...
    transaction((tx) => {
      ...
      if (metadata.changesets !== undefined && !this._branchChangesPopulated) {
        const nextChanges = synthesizeChangesFromCatalogue(
          metadata.changesets,
          this.resource,
        );
 ...
 */
function synthesizeChangesFromCatalogue(
  catalogue: readonly ChangesetSummary[] | undefined,
  sessionResource: URI,
  ...
  implements IAgentHostSessionsProvider
{
  ...
  constructor(
    ...
    this._register(
      autorun((reader) => {
        ...
        reader.store.add(
          this.connection.getSubscription(
    ...
    const branchChangesetTarget = derived((reader) => {
      ...
      return {
        adapter,
        uri: URI.parse(buildSessionChangesetUri(backendUri.toString())),
      };
    });
  ...
  protected _initialNewSessionConfig(): Record<string, unknown> | undefined {
    ...
    const policyRestricted =
      this._baseConfigurationService.inspect<boolean>(
  ...
  private _handleSessionSummaryChanged(
    ...
    if (
      ...
      cached.applyCatalogueCounts(changes.changesets)
    ) {
      didChange = true;
    }
