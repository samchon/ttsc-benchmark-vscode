      ...
      customizations: [
        ...
        },
        {
          customization: {
            ...
            displayName: "second session plugin",
          },
          enabled: true,
          status: CustomizationStatus.Loaded,
          agents: [
...
suite(
  "LocalAgentHostSessionsProvider - active-session uncommitted refresh rotation",
  () => {
    const disposables = new DisposableStore();
    let agentHost: MockAgentHostService;
    ...
    test("rotates the subscription when active session changes (refresh fires for the new one)", () => {
      ...
      activeSession.set(makeActive("sess-A", provider.id), undefined);
      const subsAfterA =
        agentHost.sessionSubscribeCounts.get(uncommittedKeyFor("sess-A")) ?? 0;
      assert.strictEqual(
        subsAfterA,
        1,
        "first activation should subscribe once",
      );

      activeSession.set(makeActive("sess-B", provider.id), undefined);
      const subsAfterB =
        agentHost.sessionSubscribeCounts.get(uncommittedKeyFor("sess-B")) ?? 0;
      const unsubsForA =
        agentHost.sessionUnsubscribeCounts.get(uncommittedKeyFor("sess-A")) ??
        0;
      assert.strictEqual(
        subsAfterB,
        1,
        "B should be subscribed exactly once on activation",
      );
      assert.strictEqual(
        unsubsForA,
        1,
    ...
    test("switching back to a previously-active session re-subscribes (triggers a refresh)", () => {
      ...
      assert.strictEqual(
        subsForA,
        2,
        "switching back to A must open a fresh subscription so the 0→1 refresh fires",
      );
    });
    ...
    test("does NOT subscribe when the active session belongs to a different provider", () => {
      ...
      activeSession.set(
        makeActive("sess-other", "some-other-provider-id"),
        undefined,
      );

      const subKeys = [...agentHost.sessionSubscribeCounts.keys()].filter((k) =>
        k.endsWith("/changeset/uncommitted"),
      );
      assert.deepStrictEqual(
        subKeys,
        [],
        "no uncommitted subscription should open for a foreign provider",
      );
