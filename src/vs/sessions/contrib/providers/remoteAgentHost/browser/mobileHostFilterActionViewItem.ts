...
export class MobileHostFilterActionViewItem extends HostFilterActionViewItem {
	...
	private _showSheet(): void {
		...
		const workbenchContainer = dom.findParentWithClass(
      this.element!,
      "monaco-workbench",
    )
			?? targetDocument.body;
	...
	private _renderHostItem(disposables: DisposableStore, body: HTMLElement, host: IAgentHostFilterEntry, checked: boolean, finish: () => void): HTMLButtonElement {
      ...
      $("span.host-picker-sheet-item-status"),
		...
		dom.append(
      text,
      $("span.host-picker-sheet-item-name"),
    ).textContent = host.label;
		dom.append(
      text,
      $(
        "span.host-picker-sheet-item-sub",
      ),
    ).textContent = this._statusLabel(host.status);
