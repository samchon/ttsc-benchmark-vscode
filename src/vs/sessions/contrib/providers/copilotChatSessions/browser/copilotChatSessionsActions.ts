...
export function shouldShowSessionManageModelsAction(
  ...
  return (
    session?.providerId === COPILOT_PROVIDER_ID &&
 ...
 */
class CopilotSessionContextMenuBridge
  extends Disposable
  implements IWorkbenchContribution
{
  ...
  constructor(
    ...
    this._bridgeItems();
    this._register(
      MenuRegistry.onDidChangeMenu((menuIds) => {
    ...
    for (const item of items) {
      ...
      this._register(
        CommandsRegistry.registerCommand(
          wrapperId,
          (accessor, context?: ISession | ISession[]) => {
            if (!context) {
              return;
            }
            const sessions = Array.isArray(context) ? context : [context];
            const agentSessions = coalesce(
              sessions.map((s) =>
                this.agentSessionsService.getSession(s.resource),
              ),
            );
      ...
      this._register(
        MenuRegistry.appendMenuItem(SessionItemContextMenuId, {
          ...
          order: item.order,
          when: item.when
            ? ContextKeyExpr.and(providerWhen, item.when)
            : providerWhen,
        }),
      );
    }
