...
export class ClaudePermissionModePicker extends Disposable {
	...
	private _showPicker(): void {
		...
		const availableModes = autoAvailable ? [
      ...permissionModes,
      autoPermissionMode,
    ] : permissionModes;
		const items: IActionListItem<IClaudePermissionModeItem>[] = availableModes.map(
      mode => ({
	...
	private _selectMode(mode: IClaudePermissionModeItem): void {
		...
		const beforeLabel = [...permissionModes, autoPermissionMode].find(
      m => m.id === beforeId,
    )?.label;
	...
	private _updateTriggerLabel(trigger: HTMLElement | undefined): void {
		...
		const currentMode = [...permissionModes, autoPermissionMode].find(
