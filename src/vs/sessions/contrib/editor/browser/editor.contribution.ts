...
class RestoreMainEditorPartAction extends Action2 {
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		const layoutService = accessor.get(IAgentWorkbenchLayoutService);
		const shouldRestoreTerminalPanel = terminalPanelHiddenForMaximizedEditor.has(
      layoutService,
    );

		layoutService.setEditorMaximized(false);

		if (shouldRestoreTerminalPanel && !layoutService.isVisible(
      Parts.PANEL_PART,
    )) {
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		if (navigation) {
			...
			if (changes && navigation.current < changes.length) {
				...
				await view?.openChanges(
          changes[navigation.current].modifiedUri ?? changes[navigation.current].originalUri,
        );
