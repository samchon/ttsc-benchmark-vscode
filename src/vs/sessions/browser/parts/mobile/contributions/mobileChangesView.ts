...
export class MobileChangesView extends Disposable {
	...
	constructor(
      ...
      info,
      $(
        "div.mobile-overlay-header-title",
      ),
    ).textContent = localize("changesView.title", "Session Changes");
	...
	private renderRow(row: IMobileChangesRow, siblings: readonly IFileDiffViewData[], index: number): void {
      ...
      textHost,
      $(
        "span.mobile-changes-row-filename",
      ),
    ).textContent = basename(row.displayUri);
        ...
        counts,
        $("span.mobile-changes-row-added"),
      ).textContent = `+${row.added}`;
		}
		if (row.removed > 0) {
			DOM.append(
        counts,
        $("span.mobile-changes-row-removed"),
      ).textContent = `-${row.removed}`;
		}
