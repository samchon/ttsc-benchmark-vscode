...
export class ChatBarPart extends AbstractPaneCompositePart {
  ...
  override create(parent: HTMLElement): void {
    ...
    this._register(
      this._sessionCompositeBar.onDidChangeVisibility(() => {
        if (this._lastLayout) {
          this.layout(
            this._lastLayout.width,
            this._lastLayout.height,
            this._lastLayout.top,
            this._lastLayout.left,
          );
        }
      }),
    );
  ...
  override layout(
    ...
    const sessionBarHeight = this._sessionCompositeBar?.visible
      ? ChatBarPart.SESSION_BAR_HEIGHT
      : 0;
  ...
  protected getCompositeBarOptions(): IPaneCompositeBarOptions {
    return {
      ...
      viewContainersWorkspaceStateKey:
        ChatBarPart.viewContainersWorkspaceStateKey,
      icon: false,
